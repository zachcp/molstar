 ...
 */
async function parseSlowTypeErrors(
  outputFile: string,
): Promise<SlowTypeError[]> {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Look for error lines with file paths
    // Format:   --> /path/to/file.ts:123:45 (note: may have leading spaces)
    ...
    if (pathMatch) {
      ...
      for (let j = i + 1; j < Math.min(i + 10, lines.length); j++) {
        ...
        if (codeMatch) {
          context = codeMatch[1].trim();

          // Extract function name from context
          // Patterns: get name(), set name(), functionName(), async functionName(), export function name()
          const nameMatch = context.match(
            /(?:export\s+)?(?:get|set|async|function)?\s*(\w+)\s*\(/,
          );
          if (nameMatch) {
...
function inferReturnType(
  ...
  if (firstReturn.includes("new Array") || firstReturn.startsWith("["))
    return "any[]";
...
function groupErrorsByFile(
  errors: SlowTypeError[],
): Map<string, SlowTypeError[]> {
...
async function main() {
  ...
  for (const [file, fileErrors] of grouped) {
    ...
    for (const error of fileErrors) {
      ...
      if (inferredType) {
        console.log(
          `  Line ${error.line}: ${error.functionName}() -> ${inferredType}`,
        );
        ...
        skipped++;
        console.log(
          `  Line ${error.line}: ${error.functionName}() -> ❌ Cannot infer`,
        );
