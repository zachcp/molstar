...
export const HighlightLoci = PluginBehavior.create({
  ...
  ctor: class extends PluginBehavior.Handler<HighlightLociProps> {
    private lociMarkProvider = (
      interactionLoci: Representation.Loci,
      action: MarkerAction,
    ) => {
      if (!this.ctx.canvas3d || !this.params.mark) return;
      this.ctx.canvas3d.mark(interactionLoci, action);
    };
    private getLoci(loci: Loci): Loci {
      return this.params.preferAtoms &&
        Bond.isLoci(loci) &&
        loci.bonds.length === 2
    ...
    register(): void {
      ...
      this.ctx.managers.interactivity.lociHighlights.addProvider(
        this.lociMarkProvider,
      );
    }
    unregister() {
      this.ctx.managers.interactivity.lociHighlights.removeProvider(
        this.lociMarkProvider,
      );
    }
  },
...
export const SelectLoci = PluginBehavior.create({
  ...
  ctor: class extends PluginBehavior.Handler<SelectLociProps> {
    ...
    private applySelectMark(ref: string, clear?: boolean): void {
      const cell = this.ctx.state.data.cells.get(ref);
      if (cell && SO.isRepresentation3D(cell.obj)) {
        this.spine.current = cell;
        const so: SO.Molecule.Structure | undefined = this.spine.getRootOfType(
    ...
    register() {
      const lociIsEmpty: (loci: Loci) => boolean = (loci: Loci): boolean =>
        Loci.isEmpty(loci);
      const lociIsNotEmpty: (loci: Loci) => boolean = (loci: Loci): boolean =>
        !Loci.isEmpty(loci);
      ...
      const actions: [
        ...
        [
          "clickToggle",
          (current) =>
            this.ctx.managers.interactivity.lociSelects.toggle(current),
          lociIsNotEmpty,
        ],
        [
          "clickToggleExtend",
          (current) =>
            this.ctx.managers.interactivity.lociSelects.toggleExtend(current),
          lociIsNotEmpty,
        ],
        [
          "clickSelectOnly",
          (current) =>
            this.ctx.managers.interactivity.lociSelects.selectOnly(current),
          lociIsNotEmpty,
        ],
        [
          "clickDeselect",
          (current) =>
            this.ctx.managers.interactivity.lociSelects.deselect(current),
          lociIsNotEmpty,
        ],
        [
          "clickDeselectAllOnEmpty",
          (): void => this.ctx.managers.interactivity.lociSelects.deselectAll(),
          lociIsEmpty,
        ],
      ];
      ...
      actions.sort((a, b): number => {
        const x = this.params.bindings[a[0]],
          y = this.params.bindings[b[0]];
        const k: number =
          x.triggers.length === 0
            ? 0
      ...
      this.ctx.managers.interactivity.lociSelects.addProvider(
        this.lociMarkProvider,
      );

      this.subscribeObservable(
        this.ctx.state.events.object.created,
        ({ ref }): void => this.applySelectMark(ref),
      );
      ...
      this.subscribeObservable(
        ...
        ({ ref, obj, oldObj, oldData, action }) => {
          ...
          if (cell && SO.Molecule.Structure.is(cell.obj)) {
            ...
            const oldStructure: Structure | undefined =
              action === "recreate"
                ? oldObj?.data
                : action === "in-place"
                  ? oldData
    ...
    unregister() {
      this.ctx.managers.interactivity.lociSelects.removeProvider(
        this.lociMarkProvider,
      );
    }
...
export const DefaultLociLabelProvider = PluginBehavior.create({
  ...
  ctor: class implements PluginBehavior<undefined> {
    private f = {
      label: (loci: Loci): string => {
        ...
        if (StructureElement.Loci.is(loci)) {
          ...
          for (const { unit: u } of loci.elements) {
            ...
            const name: string = StructureProperties.entity
              .pdbx_description(l)
              .join(", ");
      ...
      group: (label: LociLabel) =>
        label.toString().replace(/Model [0-9]+/g, "Models"),
      priority: 100,
    };
    register(): void {
      ...
      this.subscribeObservable(
        ...
        ({ current, button, modifiers }) => {
          ...
          const binding = this.ctx.selectionMode
            ? clickFocusSelectMode
            : clickFocus;
          const matched: boolean = Binding.match(binding, button, modifiers);

          const bindingAdd = this.ctx.selectionMode
            ? clickFocusAddSelectMode
            : clickFocusAdd;
          const matchedAdd: boolean = Binding.match(
            bindingAdd,
            button,
            modifiers,
          );
          ...
          const bindingExtend = this.ctx.selectionMode
            ? clickFocusExtendSelectMode
            : clickFocusExtend;
          const matchedExtend: boolean = Binding.match(
            bindingExtend,
            button,
            modifiers,
          );
          ...
          if (
            !this.ctx.selectionMode &&
            matched &&
            typeof markdownCommands === "object"
          ) {
            if (Object.keys(markdownCommands).length > 0) {
              this.ctx.managers.markdownExtensions.tryExecute(
                "click",
                markdownCommands,
              );
              return;
          ...
          const entry: FocusEntry | undefined =
            this.ctx.managers.structure.focus.current;
          if (entry && Loci.areEqual(entry.loci, loci)) {
            ...
            if (matched) {
              ...
              const current: StructureElement.Loci | undefined =
                this.ctx.managers.structure.focus.current?.loci;
