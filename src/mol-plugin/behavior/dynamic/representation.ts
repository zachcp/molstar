...
export const SelectLoci = PluginBehavior.create({
    ...
    ctor: class extends PluginBehavior.Handler<SelectLociProps> {
        ...
        register() {
            const lociIsEmpty: (loci: Loci) => boolean = (loci: Loci): boolean => Loci.isEmpty(loci);
            const lociIsNotEmpty: (loci: Loci) => boolean = (loci: Loci): boolean => !Loci.isEmpty(loci);
            ...
            actions.sort((a, b): number => {
                ...
                const k: number = x.triggers.length === 0 ? 0 : arrayMax(x.triggers.map((t) => M.size(t.modifiers)));
                const l: number = y.triggers.length === 0 ? 0 : arrayMax(y.triggers.map((t) => M.size(t.modifiers)));
                return l - k;
            });
        ...
        register(): void {
            this.subscribeObservable(
                ...
                ({ current, button, modifiers }) => {
                    ...
                    const entry: FocusEntry | undefined = this.ctx.managers.structure.focus.current;
                    if (entry && Loci.areEqual(entry.loci, loci)) {
                        ...
                        if (matched) {
                            ...
                            const current: StructureElement.Loci | undefined = this.ctx.managers.structure.focus.current
                                ?.loci;
