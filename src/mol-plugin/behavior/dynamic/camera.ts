...
const FocusLociParams = {
    ...
    durationMs: PD.Numeric(250, { min: 0, max: 1000, step: 1 }, {
        ...
        register(): void {
            this.subscribeObservable(
                ...
                ({ current, button, modifiers }) => {
                    ...
                    if (
                        Loci.isEmpty(current.loci) && Binding.match(resetBinding, button, modifiers)
                    ) {
...
export const CameraControls = PluginBehavior.create<CameraControlsProps>({
    ...
    ctor: class extends PluginBehavior.Handler<CameraControlsProps> {
        register(): void {
            this.subscribeObservable(
                this.ctx.behaviors.interaction.key,
                ({ code, key, modifiers }) => {
                    if (!this.ctx.canvas3d) return;
                    ...
                    if (Binding.matchKey(b.keyToggleFlyMode, code, modifiers, key)) {
                        ...
                        if (this.ctx.canvas3dContext?.canvas) {
                            this.ctx.canvas3dContext.canvas.style.cursor = flyMode
                                ? 'crosshair'
                                : 'unset';
                        }
                    }
