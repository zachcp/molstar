    ...
    ctor: class extends PluginBehavior.Handler<{ autoAttach: boolean; showTooltip: boolean }> {
        ...
        register(): void {
            ...
            this.ctx.representation.structure.themes.colorThemeRegistry.add(
                AccessibleSurfaceAreaColorThemeProvider,
            );
    ...
    if (loci.kind === 'element-loci') {
        ...
        if (
            ...
            loci.structure.customPropertyDescriptors.hasReference(
                AccessibleSurfaceAreaProvider.descriptor,
            )
        ) return;
        ...
        const residueCount = `<small>(${seen.size} ${
            seen.size > 1 ? 'Residues sum' : 'Residue'
        })</small>`;

        return `Accessible Surface Area ${residueCount}: ${
            cummulativeArea.toFixed(2)
        } \u212B<sup>2</sup>`;
