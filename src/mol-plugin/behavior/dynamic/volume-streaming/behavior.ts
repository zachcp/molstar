...
export namespace VolumeStreaming {
  ...
  export function createParams(
    ...
    const names = data
      ? data.entries.map((d) => [d.dataId, d.dataId] as [string, string])
      : [];
    const defaultKey = data ? data.entries[0].dataId : "";
    return {
      entry: PD.Mapped<EntryParams>(defaultKey, names, (name) =>
        PD.Group(
          createEntryParams({
  ...
  export function createEntryParams(options: {
    ...
    return {
      view: PD.MappedStatic(
        defaultView || (info.kind === "em" ? "auto" : "selection-box"),
        {
          off: PD.Group<{}>({}),
          box: PD.Group(
            {
          ...
          "selection-box": PD.Group(
            {
              radius: PD.Numeric(
                ...
                {
                  description:
                    "Radius in \u212B within which the volume is shown.",
                },
              ),
          ...
          "camera-target": PD.Group(
            {
              ...
              dynamicDetailLevel: createDetailParams(
                info.header.availablePrecisions,
                0,
          ...
          auto: PD.Group(
            {
              ...
              selectionDetailLevel: createDetailParams(
                info.header.availablePrecisions,
                6,
  ...
  function createDetailParams(
    ...
    return PD.Select<number>(
      ...
      availablePrecisions.map(
        (p, i) =>
          [i, `${i + 1} [ ${Math.pow(p.maxVoxels, 1 / 3) | 0}^3 cells ]`] as [
            number,
            string,
          ],
      ),
  ...
  export class Behavior extends PluginBehavior.WithSubscribers<Params> {
    ...
    private async parseCif(
      data: Uint8Array,
    ): Promise<ChannelsData | undefined> {
      const parsed = await this.plugin.runTask(CIF.parseBinary(data));
      if (parsed.isError) {
        this.plugin.log.error(
          "VolumeStreaming, parsing CIF: " + parsed.toString(),
        );
        return;
      }
    ...
    private getBoxFromLoci(loci: StructureElement.Loci | EmptyLoci): Box3D {
      ...
      const parent = this.plugin.helpers.substructureParent.get(
        loci.structure,
        true,
      );
      ...
      const box = StructureElement.Loci.getBoundary(
        extendedLoci,
        transform && !Number.isNaN(this._invTransform[0])
          ? this._invTransform
          : void 0,
      ).box;
    ...
    private boxFromCameraTarget(
      ...
      const relativeRadius =
        this.params.entry.params.view.name === "camera-target"
    ...
    constructor(
      public override plugin: PluginContext,
      public data: VolumeServerInfo.Data,
    ) {
      super(plugin, {} as any);
