...
class StructureFocusRepresentationBehavior extends PluginBehavior.WithSubscribers<StructureFocusRepresentationProps> {
    ...
    private ensureShape(cell: StateObjectCell<PluginStateObject.Molecule.Structure>) {
        ...
        if (
            components.indexOf('interactions') >= 0 && !refs[StructureFocusRepresentationTags.SurrNciRepr] &&
            cell.obj && InteractionsRepresentationProvider.isApplicable(cell.obj?.data)
        ) {
    ...
    private clear(root: StateTransform.Ref) {
        ...
        const foci = state.select(
            StateSelection.Generators.byRef(root).subtree().withTag(StructureFocusRepresentationTags.TargetSel),
        );
        const surrs = state.select(
            StateSelection.Generators.byRef(root).subtree().withTag(StructureFocusRepresentationTags.SurrSel),
        );
