...
type StructureFocusRepresentationProps = PD.ValuesFor<
    ReturnType<typeof StructureFocusRepresentationParams>
>;
...
const TagSet: Set<StructureFocusRepresentationTags> = new Set([
    StructureFocusRepresentationTags.TargetSel,
    StructureFocusRepresentationTags.TargetRepr,
    StructureFocusRepresentationTags.SurrSel,
    StructureFocusRepresentationTags.SurrRepr,
    StructureFocusRepresentationTags.SurrNciRepr,
]);

class StructureFocusRepresentationBehavior
    extends PluginBehavior.WithSubscribers<StructureFocusRepresentationProps> {
    private get surrLabel() {
    ...
    private getReprParams(reprParams: PD.Values<PD.Params>) {
        return {
            ...
            type: {
                ...
                params: {
                    ...reprParams.type.params,
                    ignoreHydrogens: this.params.ignoreHydrogens,
                    ignoreHydrogensVariant: this.params.ignoreHydrogensVariant,
                    ignoreLight: this.params.ignoreLight,
                    material: this.params.material,
                    clip: this.params.clip,
                },
            },
        };
    }
    ...
    private ensureShape(cell: StateObjectCell<PluginStateObject.Molecule.Structure>) {
        ...
        if (!refs[StructureFocusRepresentationTags.TargetSel]) {
            refs[StructureFocusRepresentationTags.TargetSel] = builder
                ...
                .apply(StateTransforms.Model.StructureSelectionFromBundle, {
                    ...
                    label: '[Focus] Target',
                }, { tags: StructureFocusRepresentationTags.TargetSel }).ref;
        }
        ...
        if (!refs[StructureFocusRepresentationTags.SurrSel]) {
            refs[StructureFocusRepresentationTags.SurrSel] = builder
                ...
                }, { tags: StructureFocusRepresentationTags.SurrSel }).ref;
        }
        ...
        // Representations
        if (
            components.indexOf('target') >= 0 && !refs[StructureFocusRepresentationTags.TargetRepr]
        ) {
            refs[StructureFocusRepresentationTags.TargetRepr] = builder
                .to(refs[StructureFocusRepresentationTags.TargetSel]!)
                .apply(
                    StateTransforms.Representation.StructureRepresentation3D,
                    this.getReprParams(this.params.targetParams),
                    { tags: StructureFocusRepresentationTags.TargetRepr },
                ).ref;
        }

        if (
            components.indexOf('surroundings') >= 0 &&
            !refs[StructureFocusRepresentationTags.SurrRepr]
        ) {
        ...
        if (
            ...
            !refs[StructureFocusRepresentationTags.SurrNciRepr] && cell.obj &&
            ...
            refs[StructureFocusRepresentationTags.SurrNciRepr] = builder
                ...
                .apply(
                    StateTransforms.Representation.StructureRepresentation3D,
                    this.getReprParams(this.params.nciParams),
    ...
    private clear(root: StateTransform.Ref) {
        ...
        const foci = state.select(
            StateSelection.Generators.byRef(root).subtree().withTag(
                StructureFocusRepresentationTags.TargetSel,
            ),
        );
        const surrs = state.select(
            StateSelection.Generators.byRef(root).subtree().withTag(
                StructureFocusRepresentationTags.SurrSel,
            ),
        );
    ...
    private async focus(sourceLoci: StructureElement.Loci) {
        ...
        builder.to(refs[StructureFocusRepresentationTags.TargetSel]!).update(
            StateTransforms.Model.StructureSelectionFromBundle,
            (old) => ({ ...old, bundle: residueBundle }),
        );
        builder.to(refs[StructureFocusRepresentationTags.SurrSel]!).update(
            StateTransforms.Model.StructureSelectionFromExpression,
            (old) => ({ ...old, expression: surroundings, label: this.surrLabel }),
        );
    ...
    async update(params: StructureFocusRepresentationProps) {
        ...
        for (
            const repr of state.select(all.withTag(StructureFocusRepresentationTags.SurrNciRepr))
        ) {
            if (!hasComponent) builder.delete(repr.transform.ref);
