...
import {
    ...
    HeadlessScreenshotHelperOptions,
    RawImageData,
} from './util/headless-screenshot.ts';
...
export class HeadlessPluginContext extends PluginContext {
    ...
    constructor(
        ...
        spec: PluginSpec,
        canvasSize: { width: number; height: number } = { width: 640, height: 480 },
        rendererOptions?: HeadlessScreenshotHelperOptions,
    ) {
        super(spec);
        this.renderer = new HeadlessScreenshotHelper(
            externalModules,
            canvasSize,
            undefined,
            rendererOptions,
    ...
    async getAnimation(
        ...
        const task = Task.create('Export Animation', async (ctx) => {
            ...
            const movie = await encodeMp4Animation(this, ctx, {
                ...
                fps: options?.fps,
                pass: {
                    getImageData: (runtime: RuntimeContext, width: number, height: number) =>
                        this.renderer.getImageRaw(
                            runtime,
                            { width, height },
