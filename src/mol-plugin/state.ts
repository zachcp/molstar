...
import { State, type StateBuilder, type StateTransform, type StateTransformer } from '../mol-state/index.ts';
        ...
        return {
            ...
            camera: p.camera
                ? {
                    ...
                    transitionDurationInMs: p?.cameraTransition?.name === 'animate'
    ...
    async setAnimationSnapshot(
        ...
        if (frame.camera?.current) {
            PluginCommands.Camera.Reset(this.plugin, {
                ...
                durationMs: frame.camera.transitionStyle === 'animate'
