...
class PluginState extends PluginComponent {
  ...
  readonly events = {
    cell: {
      stateUpdated: merge(
        ...
        this.behaviors.events.cell.stateUpdated,
      ),
      created: merge(
        this.data.events.cell.created,
        this.behaviors.events.cell.created,
      ),
      removed: merge(
        this.data.events.cell.removed,
        this.behaviors.events.cell.removed,
      ),
    },
    object: {
      created: merge(
        this.data.events.object.created,
        this.behaviors.events.object.created,
      ),
      removed: merge(
        this.data.events.object.removed,
        this.behaviors.events.object.removed,
      ),
      updated: merge(
        this.data.events.object.updated,
  ...
  getSnapshot(params?: PluginState.SnapshotParams): PluginState.Snapshot {
    ...
    return {
      ...
      camera: p.camera
        ? {
            ...
            transitionDurationInMs:
              p?.cameraTransition?.name === "animate"
                ? p.cameraTransition.params.durationInMs
                : void 0,
          }
        : void 0,
      canvas3dContext: p.canvas3dContext
        ? { props: this.plugin.canvas3dContext?.props }
        : void 0,
      ...
      structureSelection: p.structureSelection
        ? this.plugin.managers.structure.selection.getSnapshot()
        : void 0,
  ...
  async setSnapshot(snapshot: PluginState.Snapshot) {
    ...
    if (snapshot.structureComponentManager?.options)
      this.plugin.managers.structure.component._setSnapshotState(
        snapshot.structureComponentManager?.options,
      );
    ...
    if (snapshot.structureSelection) {
      this.plugin.managers.structure.selection.setSnapshot(
        snapshot.structureSelection,
      );
    }
    ...
    if (
      typeof snapshot?.onLoadMarkdownCommands === "object" &&
      Object.keys(snapshot.onLoadMarkdownCommands).length > 0
    ) {
      this.plugin.managers.markdownExtensions.tryExecute(
        "click",
        snapshot.onLoadMarkdownCommands,
      );
    }
  ...
  async setAnimationSnapshot(
    ...
    if (frame.camera?.current) {
      PluginCommands.Camera.Reset(this.plugin, {
        ...
        durationMs:
          frame.camera.transitionStyle === "animate"
            ? frame.camera.transitionDurationInMs
            : undefined,
      });
    }
  ...
  updateBehavior<T extends StateTransformer>(
    ...
    if (!this.behaviors.tree.transforms.has(behavior.id)) {
      ...
      tree
        ...
        .apply(behavior, produce(defaultParams, params) as any, {
          ref: behavior.id,
        });
    } else {
  ...
  constructor(private plugin: PluginContext) {
    super();
    this.data = State.create(new SO.Root({}), {
      runTask: this.plugin.runTask,
      globalContext: this.plugin,
      historyCapacity: this.plugin.config.get(
        PluginConfig.State.HistoryCapacity,
      ),
    });
    this.behaviors = State.create(new PluginBehavior.Root({}), {
      runTask: this.plugin.runTask,
      globalContext: this.plugin,
      rootState: { isLocked: true },
    });
  }
}
...
namespace PluginState {
  ...
  export function getStateTransitionFrameIndex(
    snapshot: Snapshot,
    timestamp: number,
  ): number | undefined {
    const { transition } = snapshot;
