...
class PluginState extends PluginComponent {
    ...
    readonly data = State.create(new SO.Root({}), {
        runTask: this.plugin.runTask,
        globalContext: this.plugin,
        historyCapacity: this.plugin.config.get(PluginConfig.State.HistoryCapacity),
    });
    readonly behaviors = State.create(new PluginBehavior.Root({}), {
        runTask: this.plugin.runTask,
        globalContext: this.plugin,
    ...
    updateBehavior<T extends StateTransformer>(
        ...
        if (!this.behaviors.tree.transforms.has(behavior.id)) {
            ...
            tree.to(PluginBehavior.getCategoryId(behavior)).apply(
                behavior,
                produce(defaultParams, params) as any,
                { ref: behavior.id },
            );
        } else {
...
namespace PluginState {
    ...
    export const SnapshotParams = {
        durationInMs: PD.Numeric(1500, { min: 100, max: 15000, step: 100 }, {
        ...
        cameraTransition: PD.MappedStatic('animate', {
            animate: PD.Group({
                durationInMs: PD.Numeric(250, { min: 100, max: 5000, step: 500 }, {
                    label: 'Duration in ms',
                }),
            }),
            instant: PD.Group({}),
