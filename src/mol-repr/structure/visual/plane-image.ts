...
import { ComplexImageParams, ComplexImageVisual, ComplexVisual } from '../complex-visual.ts';
...
export const PlaneImageParams = {
    ...
    defaultColor: PD.Color(Color(0xCCCCCC), {
        description:
            'Default color for parts of the image that are not covered by the color theme.',
    }),
    includeParent: PD.Boolean(false, {
        description: 'Show parent structure (but within extent of this structure).',
    }),
};
...
function getFrame(structure: Structure, props: PlaneImageProps) {
    ...
    if (extent === 'sphere' || rotation.angle !== 0) {
        ...
        const s = Vec3.magnitude(
            Box3D.size(Vec3(), Box3D.fromSphere3D(Box3D(), structure.boundary.sphere)),
        );
        Vec3.set(size, s, s, r);
        if (extent === 'sphere') {
...
export function createPlaneImage(
    ...
    const plane = Plane3D.fromNormalAndCoplanarPoint(
        Plane3D(),
        Vec3.normalize(Vec3(), normal),
        anchor,
    );
                    ...
                    if (distSq < distArray[k]) {
                        ...
                        if (isVertex) {
                            if (f === 1) {
                                ...
                                if (cutout) {
                                    if (
                                        groupArray[k4] !== 0 || groupArray[k4 + 1] !== 0 ||
                                        groupArray[k4 + 2] !== 0
                                    ) {
                                        f = 1;
                                    }
