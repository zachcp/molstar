...
import { AtomicConformation } from "../../../../../mol-model/structure/model/properties/atomic.ts";
 ...
 */
export function PolymerTraceIterator(
  unit: Unit,
  structure: Structure,
  options: PolymerTraceIteratorOptions = {},
): Iterator<PolymerTraceElement> {
  switch (unit.kind) {
...
const SecStrucTypeNA = SecondaryStructureType.create(
  SecondaryStructureType.Flag.NA,
);

function createPolymerTraceElement(
  structure: Structure,
  unit: Unit,
): PolymerTraceElement {
  return {
...
const enum AtomicPolymerTraceIteratorState {
  nextPolymer,
  nextResidue,
}
...
export class AtomicPolymerTraceIterator
  implements Iterator<PolymerTraceElement>
{
  ...
  private state: AtomicPolymerTraceIteratorState =
    AtomicPolymerTraceIteratorState.nextPolymer;
  ...
  private pos(
    ...
    ss: SecondaryStructureType.Flag,
  ) {
  ...
  private updateResidueSegmentRange(
    polymerSegment: Segmentation.Segment<number>,
  ) {
    const { index } = this.residueAtomSegments;
    this.residueSegmentMin =
      index[this.polymerRanges[polymerSegment.index * 2]];
    this.residueSegmentMax =
      index[this.polymerRanges[polymerSegment.index * 2 + 1]];
  }
  ...
  private getResidueIndex(residueIndex: number) {
    if (residueIndex < this.residueSegmentMin) {
      ...
      if (cyclicIndex !== undefined) {
        residueIndex =
          cyclicIndex - (this.residueSegmentMin - residueIndex - 1);
      } else {
      ...
      if (cyclicIndex !== undefined) {
        residueIndex =
          cyclicIndex + (residueIndex - this.residueSegmentMax - 1);
      } else {
  ...
  private setControlPoint(
    ...
    if (isSheetSS(ss) || (this.helixOrientationCenters && isHelixSS(ss))) {
      Vec3.scale(
        out,
        Vec3.add(out, p1, Vec3.add(out, p3, Vec3.add(out, p2, p2))),
        1 / 4,
      );
    } else {
      Vec3.copy(out, p2);
  ...
  private setDirection(out: Vec3, v1: Vec3, v2: Vec3, v3: Vec3) {
    ...
    Vec3.scale(
      out,
      Vec3.add(out, tmpVecA, Vec3.add(out, tmpVecB, Vec3.add(out, v2, v2))),
      1 / 4,
    );
  }
  ...
  move() {
    ...
    if (this.state === AtomicPolymerTraceIteratorState.nextPolymer) {
      while (polymerIt.hasNext) {
        ...
        if (residueIt.hasNext) {
          ...
          const residueIndexBeg =
            this.residueAtomSegments.index[
              this.unit.elements[this.polymerSegment.start]
            ];
          const residueIndexBegPrev = this.getResidueIndex(residueIndexBeg - 1);
          this.currSecStrucType =
            residueIndexBeg === residueIndexBegPrev
              ? SecStrucTypeNA
              : this.getSecStruc(residueIndexBegPrev);
          this.nextSecStrucType = this.getSecStruc(residueIndexBeg);
          this.currCoarseBackbone =
            this.directionFromElementIndex[residueIndexBegPrev] === -1 ||
            this.directionToElementIndex[residueIndexBegPrev] === -1;
          this.nextCoarseBackbone =
            this.directionFromElementIndex[residueIndexBeg] === -1 ||
            this.directionToElementIndex[residueIndexBeg] === -1;
          break;
        }
      }
    }
    ...
    if (this.state === AtomicPolymerTraceIteratorState.nextResidue) {
      ...
      this.nextSecStrucType =
        residueIndex === residueIndexNext1
          ? SecStrucTypeNA
          : this.getSecStruc(residueIndexNext1);

      this.prevCoarseBackbone = this.currCoarseBackbone;
      this.currCoarseBackbone = this.nextCoarseBackbone;
      this.nextCoarseBackbone =
        this.directionFromElementIndex[residueIndexNext1] === -1 ||
        this.directionToElementIndex[residueIndexNext1] === -1;
      ...
      if (
        this.helixOrientationCenters &&
        !(isHelix && value.secStrucFirst && value.secStrucLast)
      ) {
      ...
      if (residueIndex === residueIndexPrev1 || (ss !== ssPrev1 && helixFlag)) {
        ...
        residueIndexPrev1 === residueIndexPrev2 ||
        (ss !== ssPrev2 && helixFlag)
      ) {
        Vec3.setMagnitude(tmpDir, Vec3.sub(tmpDir, this.p2, this.p3), f);
        Vec3.add(this.p1, this.p2, tmpDir);
        Vec3.add(this.p0, this.p1, tmpDir);
      } else if (
        residueIndexPrev2 === residueIndexPrev3 ||
        (ss !== ssPrev3 && helixFlag)
      ) {
      ...
      if (residueIndex === residueIndexNext1 || (ss !== ssNext1 && helixFlag)) {
        ...
        residueIndexNext1 === residueIndexNext2 ||
        (ss !== ssNext2 && helixFlag)
      ) {
        Vec3.setMagnitude(tmpDir, Vec3.sub(tmpDir, this.p4, this.p3), f);
        Vec3.add(this.p5, this.p4, tmpDir);
        Vec3.add(this.p6, this.p5, tmpDir);
      } else if (
        residueIndexNext2 === residueIndexNext3 ||
        (ss !== ssNext3 && helixFlag)
      ) {
  ...
  constructor(
    ...
    this.traceElementIndex = unit.model.atomicHierarchy.derived.residue
      .traceElementIndex as ArrayLike<ElementIndex>; // can assume it won't be -1 for polymer residues
    this.directionFromElementIndex =
      unit.model.atomicHierarchy.derived.residue.directionFromElementIndex;
    this.directionToElementIndex =
      unit.model.atomicHierarchy.derived.residue.directionToElementIndex;
    ...
    this.polymerIt = SortedRanges.transientSegments(
      this.polymerRanges,
      unit.elements,
    );
    this.residueIt = Segmentation.transientSegments(
      this.residueAtomSegments,
      unit.elements,
    );
    ...
    if (!options.ignoreSecondaryStructure) {
      this.secondaryStructure = SecondaryStructureProvider.get(
        structure,
      ).value?.get(unit.invariantId);
    }
...
const enum CoarsePolymerTraceIteratorState {
  nextPolymer,
  nextElement,
}
  ...
  implements Iterator<PolymerTraceElement>
{
  ...
  private state: CoarsePolymerTraceIteratorState =
    CoarsePolymerTraceIteratorState.nextPolymer;
  ...
  private getElementIndex(elementIndex: number) {
    return Math.min(
      Math.max(this.polymerSegment.start, elementIndex),
      this.polymerSegment.end - 1,
    ) as ElementIndex;
  }
  ...
  move() {
    ...
    if (this.state === CoarsePolymerTraceIteratorState.nextElement) {
      ...
      this.value.centerPrev.element =
        this.value.center.unit.elements[elementIndexPrev1];
      this.value.center.element =
        this.value.center.unit.elements[this.elementIndex];
      this.value.centerNext.element =
        this.value.center.unit.elements[elementIndexNext1];
    ...
    this.hasNext =
      this.elementIndex + 1 < this.polymerSegment.end || this.polymerIt.hasNext;
    this.elementIndex += 1;
    return this.value;
  }

  constructor(
    private unit: Unit.Spheres | Unit.Gaussians,
    structure: Structure,
  ) {
    this.polymerIt = SortedRanges.transientSegments(
      getPolymerRanges(unit),
      unit.elements,
    );
    this.value = createPolymerTraceElement(structure, unit);
