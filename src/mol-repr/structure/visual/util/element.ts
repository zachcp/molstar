...
import { isHydrogen, isTrace, StructureGroup } from './common.ts';
...
export function makeElementIgnoreTest(
    ...
    return (element: ElementIndex) => {
        return (
            ...
            (!isCoarse && ignoreHydrogens &&
                isHydrogen(structure, unit, element, ignoreHydrogensVariant)) ||
...
export function createStructureElementSphereImpostor(
    ...
    const { getSerialIndex } = structure.serialMapping;
    const structureElementCount = structure.elementCount;
    const builder = SpheresBuilder.create(
...
export function eachSerialElement(
    loci: Loci,
    structure: Structure,
    apply: (interval: Interval) => boolean,
) {
