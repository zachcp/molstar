...
import { EmptyLoci, isEmptyLoci, Loci as ModelLoci } from '../mol-model/loci.ts';
...
import { getQualityProps, LocationCallback } from './util.ts';
...
export type RepresentationParamsGetter<D, P extends PD.Params> = (
...
export namespace RepresentationProvider {
    export type ParamValues<R extends RepresentationProvider<any, any, any>> = R extends
        RepresentationProvider<any, infer P, any> ? PD.Values<P> : never;
...
interface Representation<
    D,
    P extends PD.Params = PD.Params,
    S extends Representation.State = Representation.State,
> {
    readonly label: string;
...
namespace Representation {
    ...
    export type Any<P extends PD.Params = PD.Params, S extends State = State> = Representation<
        any,
        P,
        S
    >;
    ...
    export function createMulti<D, P extends PD.Params = PD.Params, S extends State = State>(
        ...
        return {
            ...
            createOrUpdate: (props: Partial<P> = {}, data?: D) => {
                ...
                const qualityProps = getQualityProps(
                ...
                const { visuals } = currentProps;
                return Task.create(
                    `Creating or updating '${label}' representation`,
                    async (runtime) => {
                        for (let i = 0, il = reprList.length; i < il; ++i) {
                            if (!visuals || visuals.includes(reprMap[i])) {
                                await reprList[i].createOrUpdate(currentProps, currentData)
