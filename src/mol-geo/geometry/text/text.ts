...
import { TextValues } from '../../../mol-gl/renderable/text.ts';
...
import { createRenderObject as _createRenderObject } from '../../../mol-gl/render-object.ts';
...
export namespace Text {
    ...
    function createValues(
        ...
        const padding = getPadding(
            text.mappingBuffer.ref.value,
            text.depthBuffer.ref.value,
            text.charCount,
            getMaxSize(size),
        );
        ...
        return {
            ...
            uBorderColor: ValueCell.create(
                Color.toArrayNormalized(props.borderColor, Vec3.zero(), 0),
            ),
            uOffsetX: ValueCell.create(props.offsetX),
            uOffsetY: ValueCell.create(props.offsetY),
            uOffsetZ: ValueCell.create(props.offsetZ),
            uBackgroundColor: ValueCell.create(
                Color.toArrayNormalized(props.backgroundColor, Vec3.zero(), 0),
            ),
    ...
    function updateValues(values: TextValues, props: PD.Values<Params>) {
        ...
        if (
            Color.fromNormalizedArray(values.uBackgroundColor.ref.value, 0) !==
    ...
    function updateBoundingSphere(values: TextValues, text: Text) {
        const padding = getPadding(
            values.aMapping.ref.value,
            values.aDepth.ref.value,
            text.charCount,
            getMaxSize(values),
        );
        const invariantBoundingSphere = Sphere3D.expand(Sphere3D(), text.boundingSphere, padding);
