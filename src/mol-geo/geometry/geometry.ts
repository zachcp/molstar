...
export type Geometry<T extends GeometryKind = GeometryKind> = T extends "mesh"
  ? Mesh
  : T extends "points"
    ? Points
    : T extends "spheres"
      ...
      : T extends "cylinders"
        ...
        : T extends "text"
          ...
          : T extends "lines"
            ? Lines
            : T extends "direct-volume"
              ? DirectVolume
              : T extends "image"
                ? Image
                : T extends "texture-mesh"
                  ? TextureMesh
                  : never;

type GeometryParams<T extends GeometryKind> = T extends "mesh"
  ? Mesh.Params
  : T extends "points"
    ? Points.Params
    : T extends "spheres"
...
export namespace Geometry {
  ...
  export function getGroupCount(geometry: Geometry): number {
    switch (geometry.kind) {
      ...
      case "text":
      case "lines":
        return getDrawCount(geometry) === 0
          ? 0
          : arrayMax(geometry.groupBuffer.ref.value) + 1;
      case "direct-volume":
        return 1;
      case "image":
        return (
  ...
  export function getGranularity<T extends ColorType | SizeType>(
    ...
    return granularity === "instance" && locationIt.nonInstanceable
      ? "group"
      : granularity;
  }
}
