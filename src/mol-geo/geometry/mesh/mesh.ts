...
import {
  LocationIterator,
  PositionLocation,
} from "../../util/location-iterator.ts";
...
export namespace Mesh {
  ...
  export function computeNormals(mesh: Mesh): void {
    ...
    const normals =
      mesh.normalBuffer.ref.value.length >= vertexCount * 3
  ...
  export function checkForDuplicateVertices(
    mesh: Mesh,
    fractionDigits = 3,
  ): number {
    ...
    const hash = (v: Vec3, d: number) =>
      `${v[0].toFixed(d)}|${v[1].toFixed(d)}|${v[2].toFixed(d)}`;
    let duplicates = 0;
  ...
  const tmpMat3 = Mat3();
  export function transform(mesh: Mesh, t: Mat4): void {
    const v = mesh.vertexBuffer.ref.value;
  ...
  export function getOriginalData(
    ...
    const { originalData } =
      "kind" in x ? x.meta : (x.meta.ref.value as Mesh["meta"]);
   ...
   */
  export function uniformTriangleGroup(
    mesh: Mesh,
    splitTriangles = true,
  ): Mesh {
    const {
    ...
    (mesh.meta.originalData as OriginalData) = {
      indexBuffer: ib,
      vertexCount,
      triangleCount,
    };
  ...
  function getBorderNeighboursMap(
    ...
    borderVertices.forEach((v) => {
      ...
      for (const nb of neighbours) {
        if (
          borderVertices.has(nb) &&
          edgeCounts.get(sortedCantorPairing(v, nb)) === 1
        ) {
          add(v, nb);
        }
      }
  ...
  function laplacianEdgeSmoothing(
    ...
    let dst: Float32Array<ArrayBufferLike> = new Float32Array(
      mesh.vertexBuffer.ref.value.length,
    );
  ...
  export function smoothEdges(
    ...
    for (let k = 0; k < 10; ++k) {
      ...
      const borderNeighboursMap = getBorderNeighboursMap(
        neighboursMap,
        borderVertices,
        edgeCounts,
      );
      fillEdges(
        ...
        neighboursMap,
        borderNeighboursMap,
        options.maxNewEdgeLength * options.maxNewEdgeLength,
      );
  ...
  function createPositionIterator(
    mesh: Mesh,
  ...
  function createValues(
    ...
    return {
      ...
      uInvariantBoundingSphere: ValueCell.create(
        Vec4.ofSphere(invariantBoundingSphere),
      ),
      ...color,
      ...marker,
      ...overpaint,
      ...
      dXrayShaded: ValueCell.create(
        props.xrayShaded === "inverted"
          ? "inverted"
          : props.xrayShaded === true
            ? "on"
            : "off",
      ),
      ...
      uInteriorColor: ValueCell.create(
        getInteriorColor(props.interior, Vec4()),
      ),
      uInteriorSubstance: ValueCell.create(
        getInteriorSubstance(props.interior, Vec4()),
      ),
  ...
  function updateValues(values: MeshValues, props: PD.Values<Params>) {
    ...
    ValueCell.updateIfChanged(
      values.dTransparentBackfaces,
      props.transparentBackfaces,
    );
    ValueCell.updateIfChanged(values.uBumpFrequency, props.bumpFrequency);
    ValueCell.updateIfChanged(values.uBumpAmplitude, props.bumpAmplitude);
    ValueCell.update(
      values.uInteriorColor,
      getInteriorColor(props.interior, values.uInteriorColor.ref.value),
    );
    ValueCell.update(
      values.uInteriorSubstance,
      getInteriorSubstance(props.interior, values.uInteriorSubstance.ref.value),
    );
  }
  ...
  function updateBoundingSphere(values: MeshValues, mesh: Mesh) {
    ...
    if (
      ...
      ValueCell.update(
        ...
        Vec4.fromSphere(
          values.uInvariantBoundingSphere.ref.value,
          invariantBoundingSphere,
        ),
      );
    }
