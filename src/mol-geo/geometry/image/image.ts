...
namespace Image {
    ...
    function update(
        ...
        ValueCell.update(
            image.imageTextureDim,
            Vec2.set(image.imageTextureDim.ref.value, width, height),
        );
        ...
        ValueCell.update(
            image.trimRotation,
            Quat.copy(image.trimRotation.ref.value, trim.rotation),
        );
        ValueCell.update(image.trimScale, Vec3.copy(image.trimScale.ref.value, trim.scale));
        ValueCell.update(
            image.trimTransform,
            Mat4.copy(image.trimTransform.ref.value, trim.transform),
        );
