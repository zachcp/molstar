    ...
    switch (loci.kind) {
        ...
        case 'cell-loci':
            ...
            if (size === 1) {
                ...
                label.push(
                    `${Volume.IsoValue.toString(absVal)} (${Volume.IsoValue.toString(relVal)})`,
                );
            }
            return label.join(' | ');
        case 'segment-loci':
...
export function _bundleLabel(bundle: Loci.Bundle<any>, options: LabelOptions) {
    ...
    if (isSingleElements) {
        ...
        const labels = locations.map((l) =>
            _elementLabel(l, granularity, hidePrefix, reverse || condensed)
        );
        ...
        if (offset > 0) {
            ...
            return labels.map((l) => l.filter((l) => !!l).join(' | ')).filter((l) => !!l).join(
                '</br>',
            );
        }
    } else {
...
export function elementLabel(
    ...
    const label = o.condensed
        ? _label[0]?.replace(/\[.*\]/g, '').trim() ?? ''
        : _label.filter((l) => !!l).join(' | ');
    return o.htmlStyling ? label : stripTags(label);
}
...
function _elementLabel(
    ...
    if (Unit.isAtomic(location.unit)) {
        ...
        label.push(
            ..._coarseElementLabel(
                location as StructureElement.Location<Unit.Spheres | Unit.Gaussians>,
                granularity,
            ),
        );
    } else {
...
function _atomicElementLabel(
    ...
    switch (granularity) {
        ...
        case 'residue':
            const seq_id = label_seq_id === auth_seq_id || !has_label_seq_id
                ? auth_seq_id
                : label_seq_id;
            label.push(
                `<b>${compId} ${seq_id}</b>${
        ...
        case 'chain':
            if (label_asym_id === auth_asym_id) {
                ...
                if (
                    granularity === 'chain' &&
                    Unit.Traits.is(location.unit.traits, Unit.Trait.MultiChain)
                ) {
                    label.push(`<small>[auth</small> <b>${auth_asym_id}</b><small>]</small>`);
                } else {
                    label.push(
                        `<b>${label_asym_id}</b> <small>[auth</small> <b>${auth_asym_id}</b><small>]</small>`,
                    );
                }
            }
    }
    ...
    if (label.length > 0 && occupancy !== 1 && !hideOccupancy) {
        label[0] = `${label[0]} <small>[occupancy</small> <b>${
...
export function angleLabel(
    triple: Loci.Bundle<3>,
    options: Partial<LabelOptions & { measureOnly: boolean }> = {},
) {
