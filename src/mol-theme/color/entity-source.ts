...
import { Bond, Model, StructureElement, StructureProperties } from '../../mol-model/structure.ts';
...
export function getEntitySourceColorThemeParams(ctx: ThemeDataContext) {
    ...
    if (ctx.structure) {
        if (
            getMaps(ctx.structure.root.models).srcKeySerialMap.size >
                ColorLists[DefaultList].list.length
        ) {
...
function addSrc(
    ...
    for (let j = 0, jl = entity_src._rowCount; j < jl; ++j) {
        ...
        const beg = pdbx_beg_seq_num.valueKind(j) === Column.ValueKinds.Present
            ? pdbx_beg_seq_num.value(j)
            : 1;
...
export const EntitySourceColorThemeProvider: ColorTheme.Provider<
