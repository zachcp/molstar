...
export const PhysicalSizeThemeParams: PD.Params = {
...
export function getPhysicalRadius(unit: Unit, element: ElementIndex): number {
  if (Unit.isAtomic(unit)) {
    return VdwRadius(
      unit.model.atomicHierarchy.atoms.type_symbol.value(element),
    );
    ...
    if (StructureElement.Location.is(location)) {
      ...
      size =
        ...
        Math.min(
          ...
          getPhysicalRadius(
            location.bUnit,
            location.bUnit.elements[location.bIndex],
          ),
        );
    } else {
...
export const PhysicalSizeThemeProvider: SizeTheme.Provider<
