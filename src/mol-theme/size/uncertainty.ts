...
export const UncertaintySizeThemeParams: PD.Params = {
...
export function getUncertainty(
    ...
    if (Unit.isAtomic(unit)) {
        ...
        return (
            unit.model.coarseConformation.spheres.rmsf[element] * props.rmsfFactor
        );
    } else {
        return 0;
    }
...
> = {
    ...
    isApplicable: (ctx: ThemeDataContext) =>
        ...
        ctx.structure.models.some(
            (m) =>
                m.atomicConformation.B_iso_or_equiv.isDefined ||
                m.coarseHierarchy.isDefined,
        ),
};
