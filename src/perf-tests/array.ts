...
const Copies = {
    ...
    get<T>(copies: Copies<T>): T {
        return (copies.offset < copies.copies.length)
            ? copies.copies[copies.offset++]
            : copies.init();
    },
};
...
export function runBenchmarks(arrayLength: number) {
    ...
    suite
        ...
        .add(
            `sortIfNeeded (${arrayLength}, pre-sorted)`,
            () => sortIfNeeded(Copies.get(sortedData), le),
        )
        .add(`native sort (${arrayLength}, not sorted)`, () => Copies.get(randomData).sort(le))
        .add(
            `sortIfNeeded (${arrayLength}, not sorted)`,
            () => sortIfNeeded(Copies.get(randomData), le),
        )
        .add(`native sort (${arrayLength}, worst case)`, () => Copies.get(worstData).sort(le))
        .add(
            `sortIfNeeded (${arrayLength}, worst case)`,
            () => sortIfNeeded(Copies.get(worstData), le),
        )
        .on('cycle', (e: any) => {
