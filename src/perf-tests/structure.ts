...
export namespace PropertyAccess {
    ...
    export async function testSymmetry(id: string, s: Structure) {
        ...
        const a = await StructureSymmetry.buildSymmetryRange(
            s,
            Vec3.create(-1, -1, -1),
            Vec3.create(1, 1, 1),
        ).run();
    ...
    export async function run() {
        ...
        const { structures, models /* , mmcif */ } = await readCIF(
            'e:/test/quick/1cbs_updated.cif',
        );
        ...
        console.log(
            ...
            sumProperty(
                structures[0],
                (l) => l.unit.model.atomicConformation.atomId.value(l.element),
            ),
        );
        ...
        const q = Q.generators.atoms({
            atomTest: Q.pred.eq((l) => SP.atom.auth_comp_id(l.element), 'ALA'),
        });
        const P = SP;
