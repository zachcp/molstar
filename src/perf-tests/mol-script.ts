...
import {
    ...
    QuerySymbolRuntime,
} from '../mol-script/runtime/query/compiler.ts';
import { QueryContext, Structure, StructureQuery } from '../mol-model/structure.ts';
...
export async function testQ() {
    ...
    let expr = MolScriptBuilder.struct.generator.atomGroups({
        ...
        'residue-test': MolScriptBuilder.core.rel.inRange([
            CustomProp.symbols.residueIndex.symbol(),
            1,
            5,
        ]),
    });
