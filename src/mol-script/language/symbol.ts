...
export function Argument<T extends Type>(
    ...
    const { description = void 0, isOptional = false, isRest = false, defaultValue = void 0 } =
        params || {};
...
export namespace Arguments {
    ...
    export type PropTypes<Map extends { [key: string]: Argument<any> }> = {
        [P in keyof Map]: Map[P]['type']['@type'];
    };
    export function Dictionary<Map extends { [key: string]: Argument<any> }>(
        map: Map,
    ): Arguments<PropTypes<Map>> {
...
export type ExpressionArguments<T> = { [P in keyof T]?: Expression } | {
...
export function MSymbol<A extends Arguments, T extends Type>(
    name: string,
    args: A,
    type: T,
    description?: string,
...
export function CustomPropSymbol<T extends Type>(
    namespace: string,
    name: string,
    type: T,
    description?: string,
) {
    const symbol: MSymbol<Arguments<{}>, T> = function (
        args: ExpressionArguments<Arguments<{}>['@type']>,
    ) {
        return Expression.Apply(Expression.Symbol(symbol.id), args as any);
    } as any;
    symbol.info = { namespace, name, description };
...
export function isSymbol(x: any): x is MSymbol {
    ...
    return typeof s === 'function' && !!s.info && !!s.args &&
        typeof s.info.namespace === 'string' && !!s.type;
}
