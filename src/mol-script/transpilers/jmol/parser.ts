...
const lang = P.MonadicParser.createLanguage({
    ...
    Expression: function (r: any) {
        return P.MonadicParser.alt(
            ...
            r.Within.map((x: [number, Expression]) =>
                B.struct.modifier.includeSurroundings({ 0: x[1], radius: x[0] })
            ),
            r.ValueQuery,
    ...
    Atomname: () =>
        P.MonadicParser.regexp(/\.([a-zA-Z0-9]{1,4})/, 1).map(B.atomName).desc('atomname'),
    ...
    BracketedResname: () =>
        P.MonadicParser.regexp(/\[([a-zA-Z0-9]{1,4})\]/, 1).desc('bracketed-resname'),
    ResnoRange: (r: any) => {
        return P.MonadicParser.seq(
    ...
    String: function () {
        ...
        return P.MonadicParser.alt(
            ...
            P.MonadicParser.regexp(/"((?:[^"\\]|\\.)*)"/, 1).map((x) =>
                B.core.type.regex([`^${x}$`, 'i'])
            ),
        ).desc('string');
    },
    ...
    ValuePropertyNames: function () {
        return P.MonadicParser.alt(
            ...h.getPropertyNameRules(properties, /LIKE|>=|<=|=|!=|>|<|\)|\s/i),
        );
    },

    ValueOperator: function (r: any) {
        return h.combineOperators(
            valueOperators,
            P.MonadicParser.alt(r.ValueParens, r.ValueExpressions),
        );
    },
