...
import { BondType, ElementSymbol, SecondaryStructureType } from '../../../mol-model/structure/model/types.ts';
import { SetUtils } from '../../../mol-util/set.ts';
import { upperCaseAny } from '../../../mol-util/string.ts';
import { AtomNumber, AtomWeight, VdwRadius } from '../../../mol-model/structure/model/properties/atomic.ts';
...
const symbols = [
    ...
    C(MolScript.core.type.compositeKey, function core_type_compositeKey(ctx, xs) {
        return QueryRuntimeArguments.forEachEval(xs, ctx, (v, i, list) => list[i] = '' + v, [] as string[]).join('-');
    }),
    ...
    D(
        ...
        (ctx, xs) => Queries.filters.withSameAtomProperties(xs[0] as any, xs['source'] as any, xs['property'])(ctx),
    ),
    D(
    ...
    D(MolScript.structureQuery.generator.queryInSelection, function structureQuery_generator_queryInSelection(ctx, xs) {
        return Queries.generators.querySelection(xs[0] as any, xs['query'] as any, xs['in-complement']?.(ctx) as any)(
            ctx,
        );
    }),
    ...
    D(MolScript.structureQuery.modifier.intersectBy, function structureQuery_modifier_intersectBy(ctx, xs) {
    ...
    D(
        ...
        (ctx, xs) => VdwRadius(StructureProperties.atom.type_symbol((xs && xs[0] && xs[0](ctx) as any) || ctx.element)),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.mass,
        (ctx, xs) =>
            AtomWeight(StructureProperties.atom.type_symbol((xs && xs[0] && xs[0](ctx) as any) || ctx.element)),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.atomicNumber,
        (ctx, xs) =>
            AtomNumber(StructureProperties.atom.type_symbol((xs && xs[0] && xs[0](ctx) as any) || ctx.element)),
    ),
    ...
    D(
        ...
        (ctx, xs) => StructureElement.residueIndex((xs && xs[0] && xs[0](ctx) as any) || ctx.element),
    ),
    D(
    ...
    D(
        ...
        (ctx, xs) => StructureElement.entityIndex((xs && xs[0] && xs[0](ctx) as any) || ctx.element),
    ),
    ...
    D(
        ...
        (ctx, xs) =>
            StructureProperties.residue.group_PDB((xs && xs[0] && xs[0](ctx) as any) || ctx.element) !== 'ATOM',
    ),
    ...
    D(
        MolScript.structureQuery.atomProperty.macromolecular.residueSourceIndex,
        atomProp(StructureProperties.residue.residueSourceIndex),
    ),
...
function secondaryStructureFlag(current: SecondaryStructureType, f: string): SecondaryStructureType {
    switch (f.toLowerCase()) {
        ...
        case 'alpha':
            return current | SecondaryStructureType.Flag.Helix | SecondaryStructureType.Flag.HelixAlpha;
        case 'pi':
            return current | SecondaryStructureType.Flag.Helix | SecondaryStructureType.Flag.HelixPi;
        case '310':
            return current | SecondaryStructureType.Flag.Helix | SecondaryStructureType.Flag.Helix3Ten;
        case 'beta':
            return current | SecondaryStructureType.Flag.Beta;
        case 'strand':
            return current | SecondaryStructureType.Flag.Beta | SecondaryStructureType.Flag.BetaStrand;
        case 'sheet':
            return current | SecondaryStructureType.Flag.Beta | SecondaryStructureType.Flag.BetaSheet;
        case 'turn':
