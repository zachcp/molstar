...
import { DefaultQueryRuntimeTable, QueryRuntimeArguments, QuerySymbolRuntime } from './base.ts';
import {
    Queries,
    QueryContext,
    StructureElement,
...
import {
    ...
    AtomWeight,
    VdwRadius,
} from '../../../mol-model/structure/model/properties/atomic.ts';
import { cantorPairing, invertCantorPairing, sortedCantorPairing } from '../../../mol-data/util.ts';
import {
    bundleElementImpl,
    bundleGenerator,
} from '../../../mol-model/structure/query/queries/internal.ts';
...
const symbols = [
    ...
    C(MolScript.core.type.list, function core_type_list(ctx, xs) {
        return QueryRuntimeArguments.forEachEval(xs, ctx, (v, i, list) => list[i] = v, [] as any[]);
    }),
    C(MolScript.core.type.set, function core_type_set(ctx, xs) {
        return QueryRuntimeArguments.forEachEval(
            xs,
            ctx,
            function core_type_set_argEval(v, i, set) {
                return set.add(v);
            },
            new Set<any>(),
        );
    ...
    C(
        MolScript.core.math.sortedCantorPairing,
        (ctx, v) => sortedCantorPairing(v[0](ctx), v[1](ctx)),
    ),
    ...
    C(MolScript.core.set.isSubset, function core_set_isSubset(ctx, v) {
        return SetUtils.isSuperset(v[1](ctx) as Set<any>, v[0](ctx) as Set<any>);
    }),
    ...
    C(
        MolScript.structureQuery.type.ringFingerprint,
        (ctx, xs) => UnitRing.elementFingerprint(getArray(ctx, xs)),
    ),
    ...
    // ============= FILTERS ================
    D(
        MolScript.structureQuery.filter.pick,
        (ctx, xs) => Queries.filters.pick(xs[0] as any, xs['test'])(ctx),
    ),
    ...
    D(
        ...
        (ctx, xs) =>
            Queries.filters.withSameAtomProperties(
                xs[0] as any,
                xs['source'] as any,
                xs['property'],
            )(ctx),
    ...
    D(
        MolScript.structureQuery.filter.intersectedBy,
        (ctx, xs) => Queries.filters.areIntersectedBy(xs[0] as any, xs['by'] as any)(ctx),
    ),
    D(MolScript.structureQuery.filter.within, (ctx, xs) =>
        Queries.filters.within({
            query: xs[0] as any,
    ...
    // ============= GENERATORS ================
    D(
        MolScript.structureQuery.generator.atomGroups,
        function structureQuery_generator_atomGroups(ctx, xs) {
    ...
    D(
        MolScript.structureQuery.generator.bondedAtomicPairs,
        function structureQuery_generator_bondedAtomicPairs(ctx, xs) {
    ...
    D(MolScript.structureQuery.generator.rings, function structureQuery_generator_rings(ctx, xs) {
        return Queries.generators.rings(
            xs?.['fingerprint']?.(ctx) as any,
            xs?.['only-aromatic']?.(ctx),
        )(ctx);
    }),
    D(
        MolScript.structureQuery.generator.queryInSelection,
        function structureQuery_generator_queryInSelection(ctx, xs) {
            return Queries.generators.querySelection(
                xs[0] as any,
                xs['query'] as any,
                xs['in-complement']?.(ctx) as any,
            )(ctx);
        },
    ),
    ...
    D(
        ...
        function structureQuery_modifier_includeSurroundings(ctx, xs) {
    ...
    D(
        ...
        function structureQuery_modifier_includeSurroundingLigands(ctx, xs) {
    ...
    D(
        MolScript.structureQuery.modifier.wholeResidues,
        function structureQuery_modifier_wholeResidues(ctx, xs) {
            return Queries.modifiers.wholeResidues(xs[0] as any)(ctx);
        },
    ),
    D(MolScript.structureQuery.modifier.union, function structureQuery_modifier_union(ctx, xs) {
        return Queries.modifiers.union(xs[0] as any)(ctx);
    }),
    D(
        MolScript.structureQuery.modifier.expandProperty,
        function structureQuery_modifier_expandProperty(ctx, xs) {
            return Queries.modifiers.expandProperty(xs[0] as any, xs['property'])(ctx);
        },
    ),
    D(
        MolScript.structureQuery.modifier.exceptBy,
        function structureQuery_modifier_exceptBy(ctx, xs) {
            return Queries.modifiers.exceptBy(xs[0] as any, xs['by'] as any)(ctx);
        },
    ),
    D(
        MolScript.structureQuery.modifier.includeConnected,
        function structureQuery_modifier_includeConnected(ctx, xs) {
    ...
    D(
        MolScript.structureQuery.modifier.intersectBy,
        function structureQuery_modifier_intersectBy(ctx, xs) {
            return Queries.modifiers.intersectBy(xs[0] as any, xs['by'] as any)(ctx);
        },
    ),
    ...
    D(
        MolScript.structureQuery.combinator.merge,
        (ctx, xs) => Queries.combinators.merge(xs as any)(ctx),
    ),
    ...
    // ~~~ CORE ~~~
    D(
        MolScript.structureQuery.atomProperty.core.elementSymbol,
        atomProp(StructureProperties.atom.type_symbol),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.vdw,
        (ctx, xs) =>
            VdwRadius(
                StructureProperties.atom.type_symbol(
                    (xs && xs[0] && xs[0](ctx) as any) || ctx.element,
                ),
            ),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.mass,
        (ctx, xs) =>
            AtomWeight(
                StructureProperties.atom.type_symbol(
                    (xs && xs[0] && xs[0](ctx) as any) || ctx.element,
    ...
    D(
        MolScript.structureQuery.atomProperty.core.atomicNumber,
        (ctx, xs) =>
            AtomNumber(
                StructureProperties.atom.type_symbol(
                    (xs && xs[0] && xs[0](ctx) as any) || ctx.element,
    ...
    D(
        MolScript.structureQuery.atomProperty.core.sourceIndex,
        atomProp(StructureProperties.atom.sourceIndex),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.operatorName,
        atomProp(StructureProperties.unit.operator_name),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.instanceId,
        atomProp(StructureProperties.unit.instance_id),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.operatorKey,
        atomProp(StructureProperties.unit.operator_key),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.modelIndex,
        atomProp(StructureProperties.unit.model_index),
    ),
    D(
        MolScript.structureQuery.atomProperty.core.modelLabel,
        atomProp(StructureProperties.unit.model_label),
    ),
    ...
    // keys
    D(
        MolScript.structureQuery.atomProperty.macromolecular.residueKey,
        (ctx, xs) =>
            StructureElement.residueIndex((xs && xs[0] && xs[0](ctx) as any) || ctx.element),
    ),
    D(
        MolScript.structureQuery.atomProperty.macromolecular.chainKey,
        (ctx, xs) => StructureElement.chainIndex((xs && xs[0] && xs[0](ctx) as any) || ctx.element),
    ),
    D(
        MolScript.structureQuery.atomProperty.macromolecular.entityKey,
        (ctx, xs) =>
            StructureElement.entityIndex((xs && xs[0] && xs[0](ctx) as any) || ctx.element),
    ),

    // mmCIF
    D(
        MolScript.structureQuery.atomProperty.macromolecular.id,
        atomProp(StructureProperties.atom.id),
    ),
    D(
        MolScript.structureQuery.atomProperty.macromolecular.isHet,
        (ctx, xs) =>
            StructureProperties.residue.group_PDB(
                (xs && xs[0] && xs[0](ctx) as any) || ctx.element,
            ) !== 'ATOM',
    ),
    ...
    D(
        ...
        function structureQuery_atomProperty_ihm_hasSeqId(ctx, xs) {
            ...
            return seqId >= StructureProperties.coarse.seq_id_begin(current) &&
                seqId <= StructureProperties.coarse.seq_id_end(current);
        },
    ),
    ...
    D(
        MolScript.structureQuery.atomSet.atomCount,
    ...
    D(
        MolScript.structureQuery.atomSet.countQuery,
    ...
    D(
        MolScript.structureQuery.atomSet.propertySet,
    ...
    // Internal
    D(
        MolScript.internal.generator.bundleElement,
        function internal_generator_bundleElement(ctx, xs) {
...
function secondaryStructureFlag(
    current: SecondaryStructureType,
    f: string,
): SecondaryStructureType {
    switch (f.toLowerCase()) {
        case 'helix':
            return current | SecondaryStructureType.Flag.Helix;
        case 'alpha':
            return current | SecondaryStructureType.Flag.Helix |
                SecondaryStructureType.Flag.HelixAlpha;
        case 'pi':
            return current | SecondaryStructureType.Flag.Helix |
        ...
        case 'beta':
            return current | SecondaryStructureType.Flag.Beta;
        case 'strand':
            return current | SecondaryStructureType.Flag.Beta |
                SecondaryStructureType.Flag.BetaStrand;
        case 'sheet':
            return current | SecondaryStructureType.Flag.Beta |
                SecondaryStructureType.Flag.BetaSheet;
        case 'turn':
            return current | SecondaryStructureType.Flag.Turn;
        case 'bend':
            return current | SecondaryStructureType.Flag.Bend;
        case 'coil':
            return current | SecondaryStructureType.Flag.NA;
        default:
            return current;
    }
}
