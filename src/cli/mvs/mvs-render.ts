...
function parseArguments(): Args {
    ...
    parser.add_argument('-i', '--input', {
        ...
        nargs: '+',
        help:
            'Input file(s) in .mvsj or .mvsx format. File format is inferred from the file extension.',
    });
    ...
    parser.add_argument('-n', '--no-extensions', {
        action: 'store_true',
        help:
            `Do not apply builtin MVS-loading extensions (not a part of standard MVS specification)`,
    });
...
async function main(args: Args): Promise<void> {
    ...
    for (let i = 0; i < args.input.length; i++) {
        ...
        if (input.toLowerCase().endsWith('.mvsj')) {
            ...
            const mvsx = await plugin.runTask(
                Task.create('Load MVSX', async (ctx) => loadMVSX(plugin, ctx, data)),
            );
