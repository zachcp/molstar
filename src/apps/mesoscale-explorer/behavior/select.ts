        ...
        register(): void {
                ...
                ({ current, button, modifiers }) => {
                    ...
                    if (Binding.match(clickToggleSelect, button, modifiers)) {
                        ...
                        if (StructureElement.Loci.is(current.loci)) {
                            const cell = this.ctx.helpers.substructureParent.get(
                    ...
                    if (Binding.match(click, button, modifiers)) {
                        ...
                        if (snapshotKey) {
                            ...
                            if (StructureElement.Loci.is(current.loci)) {
                                ...
                                const d = getCellDescription(cell!);
                                MesoscaleState.set(this.ctx, {
                                    focusInfo: `${d}`,
                                    filter: `${cell?.obj?.label}`,
                                });
                    ...
                    if (Loci.isEmpty(current.loci)) {
                        ...
                        if (StructureElement.Loci.is(current.loci)) {
                            ...
                            if (loci.kind === 'group-loci') {
                                if (
                                    'shape' in current.loci &&
                                    current.loci.shape.geometry.kind === 'text'
                                ) {
            ...
            this.subscribeObservable(
                ...
                ({ code, modifiers }) => {
                    ...
                    if (
                        (code.startsWith('Shift') && !modifiers.control) ||
                        (code.startsWith('Control') && !modifiers.shift)
                    ) {
                        if (dimDisabled) {
                            dimDisabled = false;
