...
import { StateBuilder, StateObjectRef, StateObjectSelector } from '../../../../mol-state/index.ts';
...
export async function createCellpackHierarchy(
    ...
    const compRoot = await state.build()
        ...
        }, { tags: 'group:comp:', state: { isCollapsed: false, isHidden: groupParams.hidden } })
        .commit();
    ...
    const funcRoot = await state.build()
        ...
        }, { tags: 'group:func:', state: { isCollapsed: false, isHidden: groupParams.hidden } })
        .commit();
    ...
    if (entities._rowCount > 1) {
        ...
        await state.transaction(async () => {
            try {
                ...
                for (let i = 0; i < entities._rowCount; i++) {
                    ...
                    build = build
                        .toRoot()
                        .apply(CellpackStructure, {
                            structureRef: base.ref,
                            entityId: entities.id.value(i),
                        }, { dependsOn })
                        .apply(
                            StructureRepresentation3D,
                            getSpacefillParams(color, sizeFactor, graphicsMode),
                            { tags: [`comp:${n}`, `func:${f}`] },
                        );
                }
                await build.commit();
        ...
        await state.build()
            .toRoot()
            .apply(CellpackStructure, { structureRef: base.ref, entityId: entities.id.value(0) }, {
                dependsOn,
            })
            .apply(
                StructureRepresentation3D,
                getSpacefillParams(ColorNames.lightgray, 1, graphicsMode, merge),
                { tags: [`comp:`, `func:`] },
            )
            .commit();
    }
