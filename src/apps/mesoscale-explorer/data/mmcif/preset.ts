            ...
            params: {
                ...
                lodLevels: gmp.lodLevels.map((l) => {
                    return {
                        ...
                        stride: Math.max(
                            1,
                            Math.round(l.stride / Math.pow(scaleFactor, l.scaleBias)),
                        ),
                    };
                }),
...
export async function createMmcifHierarchy(
    ...
    for (let i = 0; i < entities._rowCount; i++) {
        ...
        if (!entGroups.has(t)) {
            ...
            const group = await state.build()
                .to(entRoot)
                .applyOrUpdateTagged(`group:ent:${t}`, MesoscaleGroup, {
                    ...groupParams,
                    index: colorIdx,
                    tag: `ent:${t}`,
                    label: t,
                ...
                }, { tags: `ent:`, state: { isCollapsed: true, isHidden: groupParams.hidden } })
    ...
    await state.transaction(async () => {
        try {
            ...
            for (let i = 0; i < entities._rowCount; i++) {
                ...
                build = build
                    .toRoot()
                    .apply(MmcifStructure, {
                        structureRef: base.ref,
                        entityId: entities.id.value(i),
                    }, { dependsOn })
                    .apply(
                        StructureRepresentation3D,
                        getSpacefillParams(color, scaleFactor, graphicsMode, clipVariant),
                        { tags: [`ent:${t}`] },
                    );
            }
            await build.commit();
