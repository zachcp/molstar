...
export class SelectionInfo extends PluginUIComponent<{}, { isDisabled: boolean }> {
    ...
    get selection() {
        ...
        const help_selection = (
            <>
                <div>
                    Use <i>ctrl+left</i> to select entities, either on the 3D canvas or in the tree below
                </div>
                <div>
...
export function MesoMarkdownAnchor({ href, children, element }: { href?: string; children?: any; element?: any }) {
    ...
    if (decodedHref[0] === 'i' || decodedHref[0] === 'g') {
        return (
            <a href={decodedHref[0]} onMouseLeave={handleLeave} onMouseOver={handleHover} onClick={handleClick}>
                {children}
            </a>
        );
    }
    ...
    render() {
        ...
        const expand = (
            <IconButton
                ...
                style={{ visibility: groups.length > 0 || entities.length > 0 ? 'visible' : 'hidden' }}
            />
        );
        const color = (entities.length > 0 && !isRoot) && this.renderColor();
        const root = (isRoot && this.allGroups.length > 1) && (
            <IconButton svg={BrushSvg} toggleState={false} disabled={disabled} small onClick={this.toggleRoot} />
        );
        const clip = (
            <IconButton svg={ContentCutSvg} toggleState={false} disabled={disabled} small onClick={this.toggleClip} />
        );
        const visibility = (
            <IconButton
...
export class EntityNode extends Node<{}, { action?: 'color' | 'clip'; isDisabled: boolean }> {
    ...
    updateLod = (values: PD.Values) => {
        ...
        if (
            ...
            params.type.params.cellSize !== values.cellSize || params.type.params.batchSize !== values.batchSize ||
