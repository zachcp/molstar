...
import { getMVSStoriesContext, MVSStoriesContext } from '../context.ts';
...
export class MVSStoriesViewerModel extends PluginComponent {
                ...
                if (cmd.kind === 'load-mvs') {
                    ...
                    if (cmd.url) {
                        const data = await this.plugin.runTask(
                            this.plugin.fetch({
                                url: cmd.url,
                                type: cmd.format === 'mvsx' ? 'binary' : 'string',
                            }),
                        );
                        loadedData = await loadMVSData(this.plugin, data, cmd.format ?? 'mvsj', {
                            sourceUrl: cmd.url,
                        });
                    } else if (cmd.data) {
    ...
    constructor(
        private options?: { context?: { name?: string; container?: object }; name?: string },
    ) {
        super();
