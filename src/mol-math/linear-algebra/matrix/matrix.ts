...
namespace Matrix {
    ...
    export function set<N extends number, M extends number>(
        m: Matrix<N, M>,
        i: number,
        j: number,
        value: number,
    ...
    export function add<N extends number, M extends number>(
        m: Matrix<N, M>,
        i: number,
        j: number,
        value: number,
    ...
    export function fromArray<N extends number, M extends number>(
        data: NumberArray,
        cols: N,
        rows: M,
    ): Matrix<N, M> {
    ...
    export function transpose<N extends number, M extends number>(
        out: Matrix<M, N>,
        mat: Matrix<N, M>,
    ): Matrix<M, N> {
    ...
    export function meanRows<N extends number, M extends number, V extends Vec<N>>(
        mat: Matrix<N, M>,
    ): V {
        const nrows = mat.rows, ncols = mat.cols;
        const md = mat.data;
    ...
    export function subRows<N extends number, M extends number>(
        mat: Matrix<N, M>,
        row: NumberArray,
    ) {
        const nrows = mat.rows, ncols = mat.cols;
