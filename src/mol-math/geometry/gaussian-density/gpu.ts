...
export function GaussianDensityTexture2d(
    ...
    const data = calcGaussianDensityTexture2d(
        webgl,
        position,
        box,
        radius,
...
function calcGaussianDensityTexture2d(
    ...
    const { drawCount, positions, radii, groups, scale, expandedBox, dim, maxRadius } =
        prepareGaussianDensityData(position, box, radius, props);
    const [dx, dy, dz] = dim;
