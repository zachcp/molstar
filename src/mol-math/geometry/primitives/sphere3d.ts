...
import { NumberArray, PickRequired } from "../../../mol-util/type-helpers.ts";
...
import { EPSILON } from "../../linear-algebra/3d/common.ts";
...
namespace Sphere3D {
  export function hasExtrema(
    sphere: Sphere3D,
  ): sphere is PickRequired<Sphere3D, "extrema"> {
  ...
  export function scale(out: Sphere3D, sphere: Sphere3D, s: number): Sphere3D {
    ...
    if (hasExtrema(sphere)) {
      setExtrema(
        out,
        sphere.extrema.map((e) => Vec3.scale(Vec3(), e, s)),
      );
    }
    return out;
  }
  ...
  export function fromAxes3D(out: Sphere3D, axes: Axes3D): Sphere3D {
    ...
    out.radius = Math.max(
      Vec3.magnitude(axes.dirA),
      Vec3.magnitude(axes.dirB),
      Vec3.magnitude(axes.dirC),
    );
    return out;
  }
  ...
  export function fromDimensionsAndTransform(
    out: Sphere3D,
    dimensions: Vec3,
    transform: Mat4,
  ): Sphere3D {
  ...
  export function expandBySphere(
    out: Sphere3D,
    sphere: Sphere3D,
    by: Sphere3D,
  ): Sphere3D {
    ...
    out.radius = Math.max(
      sphere.radius,
      Vec3.distance(sphere.center, by.center) + by.radius,
    );
  ...
  export function equals(a: Sphere3D, b: Sphere3D): boolean {
    ...
    return (
      Math.abs(ar - br) <=
        EPSILON * Math.max(1.0, Math.abs(ar), Math.abs(br)) &&
