...
import { CIF, CifFrame } from '../../mol-io/reader/cif.ts';
...
import { CartoonRepresentationProvider } from '../../mol-repr/structure/representation/cartoon.ts';
import { trajectoryFromMmCIF } from '../../mol-model-formats/structure/mmcif.ts';
import { MolecularSurfaceRepresentationProvider } from '../../mol-repr/structure/representation/molecular-surface.ts';
import { BallAndStickRepresentationProvider } from '../../mol-repr/structure/representation/ball-and-stick.ts';
import { GaussianSurfaceRepresentationProvider } from '../../mol-repr/structure/representation/gaussian-surface.ts';
...
import { MembraneOrientationRepresentationProvider } from '../../extensions/anvil/representation.ts';
...
function getInteractionRepr() {
    return InteractionsRepresentationProvider.factory(
        reprCtx,
        InteractionsRepresentationProvider.getParams,
    );
}

function getBallAndStickRepr() {
    return BallAndStickRepresentationProvider.factory(
        reprCtx,
        BallAndStickRepresentationProvider.getParams,
    );
}

function getMolecularSurfaceRepr() {
    return MolecularSurfaceRepresentationProvider.factory(
        reprCtx,
        MolecularSurfaceRepresentationProvider.getParams,
    );
}

function getGaussianSurfaceRepr() {
    return GaussianSurfaceRepresentationProvider.factory(
        reprCtx,
        GaussianSurfaceRepresentationProvider.getParams,
    );
}
...
async function init() {
    ...
    if (show.cartoon) {
        ...
        await cartoonRepr.createOrUpdate({
    ...
    if (show.interaction) {
        ...
        await interactionRepr.createOrUpdate({
    ...
    if (show.ballAndStick) {
        ...
        await ballAndStickRepr.createOrUpdate({
    ...
    if (show.molecularSurface) {
        ...
        await molecularSurfaceRepr.createOrUpdate({
    ...
    if (show.membrane) {
        await membraneOrientationRepr.createOrUpdate({
            ...MembraneOrientationRepresentationProvider.defaultValues,
            quality: 'auto',
        }, structure).run();
    }
