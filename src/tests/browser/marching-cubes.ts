...
import { Canvas3D, Canvas3DContext, Canvas3DParams } from '../../mol-canvas3d/canvas3d.ts';
import { ColorNames } from '../../mol-util/color/names.ts';
import { Box3D, PositionData, Sphere3D } from '../../mol-math/geometry.ts';
...
async function init() {
    ...
    const activeVoxelsTex = calcActiveVoxels(
        webgl,
    ...
    const compacted = createHistogramPyramid(
        webgl,
        activeVoxelsTex,
        densityTextureData.gridTexScale,
        densityTextureData.gridTexDim,
    ...
    console.log({
        ...webgl.stats,
        programCount: webgl.stats.resourceCounts.program,
        shaderCount: webgl.stats.resourceCounts.shader,
    });

    const mcBoundingSphere = Sphere3D.fromBox3D(Sphere3D(), densityTextureData.bbox);
    const mcIsosurface = TextureMesh.create(
        gv.vertexCount,
        1,
        gv.vertexTexture,
        gv.groupTexture,
    ...
    const mcIsoSurfaceValues = TextureMesh.Utils.createValuesSimple(
        mcIsosurface,
        mcIsoSurfaceProps,
        Color(0x112299),
        1,
    ...
    const mcIsoSurfaceRenderObject = createRenderObject(
        'texture-mesh',
        mcIsoSurfaceValues,
        mcIsoSurfaceState,
        -1,
    );
    const mcIsoSurfaceRepr = Representation.fromRenderObject(
        'texture-mesh',
        mcIsoSurfaceRenderObject,
    );
