...
export const StereoCameraParams: PD.Params = {
    eyeSeparation: PD.Numeric(
        ...
        { description: 'Distance between left and right camera.' },
    ),
    focus: PD.Numeric(
        10,
...
export const DefaultStereoCameraProps: StereoCameraProps = PD.getDefaultValues(StereoCameraParams);
...
class StereoCamera {
    ...
    constructor(
        ...
        props: Partial<StereoCameraProps> = {},
    ) {
    ...
    update(xr?: { pose: XRViewerPose; layer: XRWebGLLayer }) {
        ...
        if (xr) {
            xrUpdate(
                this.parent,
                this.left as EyeCamera,
                this.right as EyeCamera,
                xr,
            );
        } else {
            update(
...
function xrUpdate(
    ...
    left: EyeCamera,
    right: EyeCamera,
    xr: { pose: XRViewerPose; layer: XRWebGLLayer },
) {
