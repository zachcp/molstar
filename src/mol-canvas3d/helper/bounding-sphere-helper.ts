...
export class BoundingSphereHelper {
  ...
  constructor(
    ctx: WebGLContext,
    parent: Scene,
    props: Partial<DebugHelperProps>,
  ) {
  ...
  syncVisibility() {
    if (this.sceneData) {
      this.sceneData.renderObject.state.visible =
    ...
    this.parent.forEach((_, ro) => {
      const objectData = this.objectsData.get(ro);
      if (objectData)
        objectData.renderObject.state.visible =
          ro.state.visible && this._props.objectBoundingSpheres;

      const instanceData = this.instancesData.get(ro);
      if (instanceData)
        instanceData.renderObject.state.visible =
          ro.state.visible && this._props.instanceBoundingSpheres;
    });
  }
