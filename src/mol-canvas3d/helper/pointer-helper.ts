...
export class PointerHelper {
  ...
  update(pointers: Ray3D[], points: Vec3[], hit: Vec3 | undefined) {
    ...
    ValueCell.updateIfChanged(
      this.renderObject.values.drawCount,
      Geometry.getDrawCount(this.shape.geometry),
    );
    ValueCell.updateIfChanged(
      this.renderObject.values.uVertexCount,
      Geometry.getVertexCount(this.shape.geometry),
    );
    ValueCell.updateIfChanged(this.renderObject.values.uGroupCount, 2);
    Mesh.Utils.updateBoundingSphere(
      this.renderObject.values,
      this.shape.geometry,
    );
...
function getPointerMeshShape(
  data: PointerData,
  props: PointerHelperProps,
  shape?: Shape<Mesh>,
) {
