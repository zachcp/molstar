...
// Conditionally declare types for environments that have setImmediate/clearImmediate
declare const setImmediate:
  | ((handler: (...args: any[]) => void, ...args: any[]) => number)
  | undefined;
...
import { Loci, EmptyLoci, isEmptyLoci } from "../mol-model/loci.ts";
...
import {
  ...
  Canvas3dInteractionHelperParams,
} from "./helper/interaction-events.ts";
import { PostprocessingParams } from "./passes/postprocessing.ts";
import {
  MultiSampleHelper,
  MultiSampleParams,
  MultiSamplePass,
} from "./passes/multi-sample.ts";
import { AsyncPickData, DefaultPickOptions, PickData } from "./passes/pick.ts";
import { PickHelper } from "./helper/pick-helper.ts";
import { ImagePass, ImageProps } from "./passes/image.ts";
...
import {
  DefaultXRManagerAttribs,
  XRManager,
  XRManagerParams,
} from "./helper/xr-manager.ts";
...
const _Canvas3DParams = {
  ...
  userInteractionReleaseMs: PD.Numeric(
    ...
    {
      description:
        "The time before the user is not considered interacting anymore.",
    },
  ),
...
namespace Canvas3DContext {
  export const DefaultAttribs = {
    powerPreference:
      "high-performance" as WebGLContextAttributes["powerPreference"],
...
const requestAnimationFrame =
  typeof window !== "undefined"
    ...
    : (f: (time: number) => void) =>
        setImmediate!(() => f(Date.now())) as unknown as number;
...
function syncCanvasBackground(
  canvas: HTMLCanvasElement,
  canvasProps: Canvas3DProps,
) {
  if (
    canvasProps.transparentBackground &&
    canvasProps.checkeredTransparentBackground
  ) {
...
namespace Canvas3D {
  ...
  export function create(
    ...
    const xrSubs = [
      ...
      xrManager.sessionChanged.subscribe(() => {
        ...
        if (xrManager.session) {
          saveNonXRProps();
          xrPassthrough =
            xrManager.session?.environmentBlendMode === "alpha-blend";
          setXRProps();
        } else {
          loadNonXRProps();
        }
    ...
    function render(force: boolean, xrFrame?: XRFrame) {
      ...
      const markingUpdated =
        resolveMarking() && (renderer.props.colorMarker || p.marking.enabled);
      ...
      const shouldRender =
        force || cameraChanged || resized || forceNextRender || xrChanged;
      ...
      if (passes.illumination.supported && p.illumination.enabled && !xrFrame) {
        ...
        const multiSampleChanged =
          multiSampleHelper.update(
    ...
    function resolveCameraReset() {
      ...
      if (autoAdjustControls.name === "on") {
        const minDistance =
          autoAdjustControls.params.minDistanceFactor * radius +
      ...
      if (radius > 0) {
        const duration =
          nextCameraResetDuration === undefined
            ? p.cameraResetDurationMs
            : nextCameraResetDuration;
        const focus = camera.getFocus(center, radius);
        const next =
          typeof nextCameraResetSnapshot === "function"
    ...
    if (isDebugMode && canvas) {
      ...
      const printOcclusion = (loci: Loci | undefined) => {
        const s =
          loci && Loci.getBoundingSphere(Loci.normalize(loci, "residue"));
    ...
    return {
      ...
      setProps: (properties, doNotRequestDraw = false) => {
        ...
        if (props.cameraFog !== undefined && props.cameraFog.params) {
          const newFog =
            props.cameraFog.name === "on"
              ? props.cameraFog.params.intensity
              : 0;
        ...
        if (props.cameraClipping !== undefined) {
          ...
          if (props.cameraClipping.radius !== undefined) {
            const radius =
              (getSceneRadius() / 100) * (100 - props.cameraClipping.radius);
        ...
        if (props.checkeredTransparentBackground !== undefined)
          p.checkeredTransparentBackground =
            props.checkeredTransparentBackground;
        ...
        if (props.renderer) {
          scene.setGlobals({
            dColorMarker:
              props.renderer.colorMarker ?? renderer.props.colorMarker,
            dLightCount:
              props.renderer.light?.length ?? renderer.props.light.length,
          });
