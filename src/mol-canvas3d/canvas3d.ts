...
import { BehaviorSubject, debounceTime, merge, Subject, type Subscription } from 'rxjs';
import { now } from '../mol-util/now.ts';
import { Vec2, Vec3 } from '../mol-math/linear-algebra.ts';
import { type ButtonsType, InputObserver, type ModifiersKeys } from '../mol-util/input/input-observer.ts';
import { Renderer, RendererParams, type RendererStats } from '../mol-gl/renderer.ts';
import type { GraphicsRenderObject } from '../mol-gl/render-object.ts';
import { DefaultTrackballControlsAttribs, TrackballControls, TrackballControlsParams } from './controls/trackball.ts';
import { Viewport } from './camera/util.ts';
import { createContext, getGLContext, type WebGLContext } from '../mol-gl/webgl/context.ts';
...
import { DebugHelperParams } from './helper/bounding-sphere-helper.ts';
import { SetUtils } from '../mol-util/set.ts';
import { Canvas3dInteractionHelper, Canvas3dInteractionHelperParams } from './helper/interaction-events.ts';
import { PostprocessingParams } from './passes/postprocessing.ts';
import { MultiSampleHelper, MultiSampleParams, MultiSamplePass } from './passes/multi-sample.ts';
import { type AsyncPickData, DefaultPickOptions, type PickData } from './passes/pick.ts';
import { PickHelper } from './helper/pick-helper.ts';
import { ImagePass, type ImageProps } from './passes/image.ts';
import { Sphere3D } from '../mol-math/geometry.ts';
import { addConsoleStatsProvider, isDebugMode, isTimingMode, removeConsoleStatsProvider } from '../mol-util/debug.ts';
import { CameraHelperParams } from './helper/camera-helper.ts';
import { HandleHelperParams } from './helper/handle-helper.ts';
...
import { HiZParams, HiZPass } from './passes/hi-z.ts';
import { IlluminationParams } from './passes/illumination.ts';
import { isMobileBrowser } from '../mol-util/browser.ts';
import { PointerHelperParams } from './helper/pointer-helper.ts';
import { DefaultXRManagerAttribs, XRManager, XRManagerParams } from './helper/xr-manager.ts';
import type { Ray3D } from '../mol-math/geometry/primitives/ray3d.ts';
import { RayHelper } from './helper/ray-helper.ts';
    ...
    camera: PD.Group(
        {
            mode: PD.Select(
                'perspective',
                PD.arrayToOptions(['perspective', 'orthographic'] as const),
                { label: 'Camera' },
            ),
    ...
    cameraClipping: PD.Group(
        {
            ...
            minNear: PD.Numeric(
                ...
                {
                    description:
                        'Note, may cause performance issues rendering impostors when set too small and cause issues with outline rendering when too close to 0.',
                },
            ),
        },
        { pivot: 'radius' },
...
namespace Canvas3DContext {
    ...
    export function fromCanvas(
        ...
        const getPixelScale = () => {
            const scaled = p.pixelScale /
                (typeof window !== 'undefined' ? globalThis?.devicePixelRatio || 1 : 1);
        ...
        return {
            ...
            setProps: (props?: Partial<Props>) => {
                ...
                if (
                    props.resolutionMode !== undefined &&
                    props.resolutionMode !== p.resolutionMode
                ) {
...
interface Canvas3D {
    ...
    requestAnimationFrame(
        callback: FrameRequestCallback | XRFrameRequestCallback,
    ): number;
...
namespace Canvas3D {
    ...
    export function create(
        ...
        const p: Canvas3DProps = {
            ...deepClone(DefaultCanvas3DParams),
            ...deepClone(props),
        };
        ...
        const controls = TrackballControls.create(
            input,
            camera,
            scene,
            p.trackball,
        ...
        passes.draw.postprocessing.background.update(
            camera,
            p.postprocessing.background,
            (changed) => {
        ...
        function render(force: boolean, xrFrame?: XRFrame): boolean {
            ...
            if (x > drs.width || x + width < 0 || y > drs.height || y + height < 0) {
                return false;
            }
            ...
            if (passes.illumination.supported && p.illumination.enabled && !xrFrame) {
                ...
                const multiSampleChanged = multiSampleHelper.update(
                    markingUpdated || shouldRender,
                    p.multiSample,
                ) && !xrFrame;
                ...
                if (shouldRender || multiSampleChanged || markingUpdated) {
                    ...
                    if (MultiSamplePass.isEnabled(p.multiSample) && !xrFrame) {
                        const forceOn = p.multiSample.reduceFlicker &&
                            !cameraChanged &&
                            markingUpdated &&
        ...
        function asyncIdentify(target: Vec2 | Ray3D): AsyncPickData | undefined {
            ...
            if ('origin' in target) {
                ...
                const cam: StereoCamera | Camera = p.camera.stereo.name === 'on' ? stereoCamera : camera;
        ...
        function resolveCameraReset(): void {
            ...
            if (autoAdjustControls.name === 'on') {
                const minDistance = autoAdjustControls.params.minDistanceFactor * radius +
                    autoAdjustControls.params.minDistancePadding;
            ...
            if (radius > 0) {
                const duration = nextCameraResetDuration === undefined
                    ? p.cameraResetDurationMs
                    : nextCameraResetDuration;
                const focus = camera.getFocus(center, radius);
                const next = typeof nextCameraResetSnapshot === 'function'
                    ? nextCameraResetSnapshot(scene, camera)
                    : nextCameraResetSnapshot;
        ...
        function shouldResetCamera(): boolean {
            ...
            for (const r of scene.renderables) {
                ...
                if (
                    (cameraDist > cameraSphere.radius ||
                        cameraDist > b.radius ||
                        b.radius > camera.state.radiusMax) &&
            ...
            return (
                cameraSphereOverlapsNone || (!isEmpty && cameraSphere.radius <= 0.1)
            );
        }
        ...
        function commitScene(isSynchronous: boolean): boolean {
            ...
            if (helper.debug.isEnabled) helper.debug.update();
            if (
                !p.camera.manualReset &&
                (reprCount.value === 0 || shouldResetCamera())
            ) {
        ...
        function consoleStats(): void {
            ...
            console.log(
                JSON.stringify(
                    {
                        renderables: `${
                            (scene.renderables.reduce((sum, r) => sum + r.getByteCount(), 0) / 1024 / 1024).toFixed(3)
                        } MiB`,
                        passes: {
        ...
        function getProps(): Canvas3DProps {
            const radius = scene.boundingSphere.radius > 0
                ? 100 -
                    Math.round(
                        (camera.transition.target.radius / getSceneRadius()) * 100,
                    )
                : 0;
        ...
        const interactionSubs = [
            ...
            interactionEvent
                .pipe(debounceTime(p.userInteractionReleaseMs))
                .subscribe(() => {
                    isActivelyInteracting = isDragging;
                    if (
                        !isDragging &&
                        passes.illumination.supported &&
                        p.illumination.enabled
                    ) {
                        requestDraw();
                    }
                }),
        ...
        return {
            ...
            setProps: (properties, doNotRequestDraw = false) => {
                ...
                if (
                    props.camera &&
                    props.camera.mode !== undefined &&
                    props.camera.mode !== camera.state.mode
                ) {
                ...
                if (
                    props.camera &&
                    props.camera.fov !== undefined &&
                    props.camera.fov !== oldFov
                ) {
                ...
                if (props.cameraClipping !== undefined) {
                    if (
                        props.cameraClipping.far !== undefined &&
                        props.cameraClipping.far !== camera.state.clipFar
                    ) {
                        cameraState.clipFar = props.cameraClipping.far;
                    }
                    if (
                        props.cameraClipping.minNear !== undefined &&
                        props.cameraClipping.minNear !== camera.state.minNear
                    ) {
                ...
                if (props.camera?.manualReset !== undefined) {
                ...
                if (props.viewport !== undefined) {
                    const doNotUpdate = p.viewport === props.viewport ||
                        (p.viewport.name === props.viewport.name &&
                            shallowEqual(p.viewport.params, props.viewport.params));
