...
import { createComputeRenderItem, type RenderItem } from '../../mol-gl/webgl/render-item.ts';
import { createNullTexture, type CubeFaces, type Texture } from '../../mol-gl/webgl/texture.ts';
...
const SkyboxParams: PD.Params = {
    ...
    blur: PD.Numeric(
        ...
        {
            description: 'Note, this only works in WebGL2 or when "EXT_shader_texture_lod" is available.',
        },
    ),
    rotation: PD.Group({
...
export const BackgroundParams: PD.Params = {
    variant: PD.MappedStatic(
        'off',
        {
            ...
            horizontalGradient: PD.Group(HorizontalGradientParams, {
                isExpanded: true,
            }),
...
export class BackgroundPass {
    ...
    private updateSkybox(
        ...
        if (
            !this.skybox ||
            !tf ||
            !areSkyboxTexturePropsEqual(props.faces, this.skybox.props.faces)
        ) {
            this.clearSkybox();
        ...
        ValueCell.updateIfChanged(
            this.renderable.values.uSaturation,
            props.saturation,
        );
        ValueCell.updateIfChanged(
            this.renderable.values.uLightness,
            props.lightness,
        );
    ...
    private updateImage(
        ...
        if (
            !this.image ||
            !this.image.props.source.params ||
    ...
    private updateImageScaling(): void {
        ...
        if (r < ir) {
            ValueCell.update(
                v.uImageScale,
                Vec2.set(v.uImageScale.ref.value, (iw * h) / ih, h),
            );
        } else {
            ValueCell.update(
                v.uImageScale,
                Vec2.set(v.uImageScale.ref.value, w, (ih * w) / iw),
            );
        }
    ...
    private updateGradient(
        ...
        ValueCell.update(
            ...
            Color.toVec3Normalized(
                this.renderable.values.uGradientColorA.ref.value,
                colorA,
            ),
        );
        ValueCell.update(
            ...
            Color.toVec3Normalized(
                this.renderable.values.uGradientColorB.ref.value,
                colorB,
            ),
        );
...
function getCubeFaces(
    ...
    const resolve = (asset: Asset) => {
        return assetManager
            .resolve(asset, 'binary')
            .run()
            .then((a) => new Blob([a.data]));
    };
...
function areSkyboxTexturePropsEqual(
    facesA: SkyboxProps['faces'],
    facesB: SkyboxProps['faces'],
) {
...
function getImageHash(source: ImageProps['source']): string {
    if (source.name === 'url') {
...
function areImageTexturePropsEqual(
    sourceA: ImageProps['source'],
    sourceB: ImageProps['source'],
): boolean {
...
function getImageTexture(
    ...
    const asset = source.name === 'url'
        ? (assetManager.tryFindFilename(source.params) ??
    ...
    const texture = ctx.resources.texture(
        'image-uint8',
        'rgba',
        'ubyte',
        'linear',
...
const SkyboxShaderCode = ShaderCode(
    'background',
    background_vert,
    background_frag,
    {
...
function getBackgroundRenderable(
    ...
    const renderItem: RenderItem<'compute'> = createComputeRenderItem(
        ctx,
        'triangles',
        SkyboxShaderCode,
        schema,
