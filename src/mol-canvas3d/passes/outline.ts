...
export class OutlinePass {
  ...
  update(
    ...
    depthTextureTransparent: Texture,
    depthTextureOpaque: Texture,
  ): { transparentOutline: boolean; outlineScale: number } {
    ...
    const outlineScale =
      Math.max(1, Math.round(props.scale * this.webgl.pixelRatio)) - 1;
    ...
    if (
      this.renderable.values.dTransparentOutline.ref.value !==
      transparentOutline
    ) {
      needsUpdate = true;
      ValueCell.update(
        this.renderable.values.dTransparentOutline,
        transparentOutline,
      );
    }
    ...
    ValueCell.updateIfChanged(
      this.renderable.values.uOutlineThreshold,
      outlineThreshold,
    );

    if (
      this.renderable.values.tDepthTransparent.ref.value !==
      depthTextureTransparent
    ) {
      needsUpdate = true;
      ValueCell.update(
        this.renderable.values.tDepthTransparent,
        depthTextureTransparent,
      );
    }
...
export type OutlinesRenderable = ComputeRenderable<
  Values<typeof OutlinesSchema>
>;
