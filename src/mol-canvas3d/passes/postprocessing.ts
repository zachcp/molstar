...
import { Light } from '../../mol-gl/renderer.ts';
import { CasParams, CasPass } from './cas.ts';
import { DofParams, DofPass } from './dof.ts';
import { BloomParams } from './bloom.ts';
import { OutlineParams, OutlinePass, OutlineProps } from './outline.ts';
import { ShadowParams, ShadowPass, ShadowProps } from './shadow.ts';
import { SsaoParams, SsaoPass, SsaoProps } from './ssao.ts';
...
export class PostprocessingPass {
    static isEnabled(props: PostprocessingProps) {
        return props.enabled &&
            (SsaoPass.isEnabled(props) || ShadowPass.isEnabled(props) ||
                OutlinePass.isEnabled(props) || props.background.variant.name !== 'off');
    }

    static isTransparentDepthRequired(scene: Scene, props: PostprocessingProps) {
        return props.enabled &&
            (DofPass.isEnabled(props) ||
                OutlinePass.isEnabled(props) &&
                    PostprocessingPass.isTransparentOutlineEnabled(props) ||
                SsaoPass.isEnabled(props) &&
                    PostprocessingPass.isTransparentSsaoEnabled(scene, props));
    }
    ...
    updateState(
        ...
        if (occlusionEnabled) {
            ...
            if (
                this.renderable.values.dOcclusionIncludeTransparency.ref.value !==
                    includeTransparency
            ) {
                needsUpdateMain = true;
                ValueCell.update(
                    this.renderable.values.dOcclusionIncludeTransparency,
                    includeTransparency,
                );
            }
            ValueCell.update(
                ...
                Color.toVec3Normalized(
                    this.renderable.values.uOcclusionColor.ref.value,
                    params.color,
                ),
            );
        }
        ...
        if (outlinesEnabled) {
            ...
            ValueCell.update(
                ...
                Color.toVec3Normalized(
                    this.renderable.values.uOutlineColor.ref.value,
                    outlineProps.color,
                ),
            );
        ...
        ValueCell.updateIfChanged(
            this.renderable.values.uTransparentBackground,
    ...
    render(
        ...
        if (
            props.occlusion.name === 'on' && this.occlusionOffset[0] === 0 &&
            this.occlusionOffset[1] === 0
        ) {
...
export class AntialiasingPass {
    static isEnabled(props: PostprocessingProps) {
        return props.enabled &&
            (props.antialiasing.name !== 'off' || props.sharpening.name !== 'off');
    }
    ...
    render(
        camera: ICamera,
        input: Texture,
        toDrawingBuffer: boolean | RenderTarget,
        props: PostprocessingProps,
    ) {
