...
import type { MarkingProps } from "./marking.ts";
...
function getComposeRenderable(
  ...
  const values: Values<typeof ComposeSchema> = {
    ...
    uTexSize: ValueCell.create(
      Vec2.create(colorTexture.getWidth(), colorTexture.getHeight()),
    ),
...
export const MultiSampleParams: PD.Params = {
  mode: PD.Select("temporal", [
    ["off", "Off"],
    ["on", "On"],
    ["temporal", "Temporal"],
  ]),
  ...
  reduceFlicker: PD.Boolean(true, {
    description: 'Reduce flicker in "temporal" mode.',
  }),
...
export class MultiSamplePass {
  ...
  constructor(
    ...
    const type =
      colorBufferHalfFloat && textureHalfFloat
        ? "fp16"
  ...
  render(
    ...
    if (props.multiSample.mode === "temporal" && !forceOn) {
      return this.renderTemporalMultiSample(
        sampleIndex,
        ctx,
        props,
        toDrawingBuffer,
  ...
  private renderMultiSample(
    ...
    const offsetList =
      JitterVectors[Math.max(0, Math.min(props.multiSample.sampleLevel, 5))];
    ...
    for (let i = 0; i < offsetList.length; ++i) {
      ...
      const sampleWeight =
        baseSampleWeight + roundingRange * uniformCenteredDistribution;
  ...
  private renderTemporalMultiSample(
    ...
    if (sampleIndex === -1) {
      ...
      // render the scene multiple times, each slightly jitter offset
      // from the last and accumulate the results.
      const numSamplesPerFrame = Math.pow(
        2,
        Math.max(0, props.multiSample.sampleLevel - 2),
      );
