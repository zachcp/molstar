...
export function getAtomRangesForRow(
    row: MVSAnnotationRow,
    model: Model,
    instanceId: string,
    indices: IndicesAndSortings,
): AtomRanges {
    ...
    const hasResidueFilter = isAnyDefined(
        row.label_seq_id,
        row.auth_seq_id,
        row.pdbx_PDB_ins_code,
        row.beg_label_seq_id,
        row.end_label_seq_id,
        row.beg_auth_seq_id,
        row.end_auth_seq_id,
        row.label_comp_id,
        row.auth_comp_id,
    ...
    if (hasAtomIds) {
        ...
        return theAtom !== undefined
            ? AtomRanges.single(theAtom, theAtom + 1 as ElementIndex)
            : AtomRanges.empty();
    }
...
export function getAtomRangesForRows(
    rows: MVSAnnotationRow[],
    model: Model,
    instanceId: string,
    indices: IndicesAndSortings,
): AtomRanges {
    return AtomRanges.union(
        rows.map((row) => getAtomRangesForRow(row, model, instanceId, indices)),
    );
}

/** Return an array of chain indexes which satisfy criteria given by `row` */
function getQualifyingChains(
    model: Model,
    row: MVSAnnotationRow,
    indices: IndicesAndSortings,
): readonly ChainIndex[] {
...
function getQualifyingResidues(
    model: Model,
    row: MVSAnnotationRow,
    indices: IndicesAndSortings,
    fromChains: readonly ChainIndex[],
): ResidueIndex[] {
    ...
    for (const iChain of fromChains) {
        ...
        if (isDefined(row.auth_seq_id)) {
            if (residuesHere) {
                residuesHere = residuesHere.filter((i) =>
                    auth_seq_id.valueKind(i) === Present && auth_seq_id.value(i) === row.auth_seq_id
                );
            } else {
        ...
        if (isDefined(row.pdbx_PDB_ins_code)) {
            if (residuesHere) {
                residuesHere = residuesHere.filter((i) =>
                    pdbx_PDB_ins_code.value(i) === row.pdbx_PDB_ins_code
                );
            } else {
                residuesHere = indices.residuesByInsCode.get(iChain)!.get(row.pdbx_PDB_ins_code) ??
                    EmptyArray;
            }
        }
        if (isDefined(row.beg_label_seq_id) || isDefined(row.end_label_seq_id)) {
            if (residuesHere) {
                if (isDefined(row.beg_label_seq_id)) {
                    residuesHere = residuesHere.filter((i) =>
                        label_seq_id.valueKind(i) === Present &&
                        label_seq_id.value(i) >= row.beg_label_seq_id!
                    );
                }
                if (isDefined(row.end_label_seq_id)) {
                    residuesHere = residuesHere.filter((i) =>
                        label_seq_id.valueKind(i) === Present &&
                        label_seq_id.value(i) <= row.end_label_seq_id!
                    );
                }
            } else {
                const sorting = indices.residuesSortedByLabelSeqId.get(iChain)!;
                residuesHere = Sorting.getKeysWithValueInRange(
                    sorting,
                    row.beg_label_seq_id,
                    row.end_label_seq_id,
                );
        ...
        if (isDefined(row.beg_auth_seq_id) || isDefined(row.end_auth_seq_id)) {
            if (residuesHere) {
                if (isDefined(row.beg_auth_seq_id)) {
                    residuesHere = residuesHere.filter((i) =>
                        auth_seq_id.valueKind(i) === Present &&
                        auth_seq_id.value(i) >= row.beg_auth_seq_id!
                    );
                }
                if (isDefined(row.end_auth_seq_id)) {
                    residuesHere = residuesHere.filter((i) =>
                        auth_seq_id.valueKind(i) === Present &&
                        auth_seq_id.value(i) <= row.end_auth_seq_id!
                    );
                }
            } else {
                const sorting = indices.residuesSortedByAuthSeqId.get(iChain)!;
        ...
        if (isDefined(row.label_comp_id)) {
            if (residuesHere) {
                if (indices.residuesByLabelCompIdIsPure) {
                    residuesHere = residuesHere.filter((i) =>
                        label_comp_id.value(residueAtomSegments.offsets[i]) === row.label_comp_id
                    );
                } else {
                    residuesHere = residuesHere.filter((i) => {
                        for (
                            let iAtom = residueAtomSegments.offsets[i],
                                stop = residueAtomSegments.offsets[i + 1];
                            iAtom < stop;
                            iAtom++
                        ) {
                ...
                residuesHere = indices.residuesByLabelCompId.get(iChain)!.get(row.label_comp_id) ??
                    EmptyArray;
            }
        }
        if (isDefined(row.auth_comp_id)) {
            if (residuesHere) {
                if (indices.residuesByAuthCompIdIsPure) {
                    residuesHere = residuesHere.filter((i) =>
                        auth_comp_id.value(residueAtomSegments.offsets[i]) === row.auth_comp_id
                    );
                } else {
                    residuesHere = residuesHere.filter((i) => {
                        for (
                ...
                residuesHere = indices.residuesByAuthCompId.get(iChain)!.get(row.auth_comp_id) ??
                    EmptyArray;
            }
        }
        if (!residuesHere) {
            const firstResidueForChain =
                residueAtomSegments.index[chainAtomSegments.offsets[iChain]];
            const firstResidueAfterChain =
                residueAtomSegments.index[chainAtomSegments.offsets[iChain + 1] - 1] + 1;
...
function getQualifyingAtoms(
    model: Model,
    row: MVSAnnotationRow,
    indices: IndicesAndSortings,
    fromResidues: readonly ResidueIndex[],
): ElementIndex[] {
    const { label_atom_id, auth_atom_id, type_symbol, label_comp_id, auth_comp_id } =
    ...
    for (const iRes of fromResidues) {
        const atomIdcs = range(
            residueAtomSegments_offsets[iRes],
            residueAtomSegments_offsets[iRes + 1],
        ) as ElementIndex[];
        ...
        if (isDefined(row.type_symbol)) {
            filterInPlace(
                atomIdcs,
                (iAtom) => type_symbol.value(iAtom) === row.type_symbol?.toUpperCase(),
            );
        }
...
function getTheAtomForRow(
    ...
    if (!isDefined(row.atom_id) && !isDefined(row.atom_index)) {
        throw new Error(
            'ArgumentError: at least one of row.atom_id, row.atom_index must be defined.',
        );
    }
    if (isDefined(row.atom_id) && isDefined(row.atom_index)) {
...
export function atomQualifies(model: Model, iAtom: ElementIndex, row: MVSAnnotationRow): boolean {
    ...
    const label_seq_id = (h.residues.label_seq_id.valueKind(iRes) === Column.ValueKind.Present)
        ? h.residues.label_seq_id.value(iRes)
        : undefined;
    const auth_seq_id = (h.residues.auth_seq_id.valueKind(iRes) === Column.ValueKind.Present)
        ? h.residues.auth_seq_id.value(iRes)
        : undefined;
...
function matchesRange<T>(
    requiredMin: T | undefined | null,
    requiredMax: T | undefined | null,
    value: T | undefined,
): boolean {
