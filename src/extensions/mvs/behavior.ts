...
import type { CustomModelProperty } from "../../mol-model-props/common/custom-model-property.ts";
import type { CustomStructureProperty } from "../../mol-model-props/common/custom-structure-property.ts";
import type { DataFormatProvider } from "../../mol-plugin-state/formats/provider.ts";
import type { PluginDragAndDropHandler } from "../../mol-plugin-state/manager/drag-and-drop.ts";
...
} from "./components/custom-tooltips-prop.ts";
import {
  LoadMvsData,
  MVSJFormatProvider,
  MVSXFormatProvider,
  loadMVSX,
} from "./components/formats.ts";
...
export const MolViewSpec = PluginBehavior.create<{ autoAttach: boolean }>({
  ...
  ctor: class extends PluginBehavior.Handler<{ autoAttach: boolean }> {
    private readonly registrables: Registrables = {
      ...
      colorThemes: [
        ...
        makeMultilayerColorThemeProvider(
          this.ctx.representation.structure.themes.colorThemeRegistry,
        ),
      ],
      lociLabels: [
        CustomTooltipsLabelProvider,
    ...
    register(): void {
      ...
      this.ctx.managers.markdownExtensions.registerRefResolver(
        "mvs",
        (plugin, refs) => {
      ...
      this.ctx.managers.markdownExtensions.registerUriResolver(
        "mvs",
        (plugin, uri) => {
          ...
          try {
            ...
            console.error(
              `MVS: Failed to convert asset file to data URI for '${uri}'`,
              e,
            );
            return undefined;
          }
