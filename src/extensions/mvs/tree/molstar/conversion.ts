...
import { FullMVSTree, MVSTree, MVSTreeSchema } from '../mvs/mvs-tree.ts';
import { MolstarParseFormatT, ParseFormatT } from '../mvs/param-types.ts';
import { MolstarKind, MolstarNode, MolstarTree } from './molstar-tree.ts';
...
const mvsToMolstarConversionRules: ConversionRules<FullMVSTree, MolstarTree> = {
    ...
    'parse': (node, parent) => {
        ...
        const convertedNode: MolstarNode<'parse'> = {
            kind: 'parse',
            params: { ...node.params, format },
            custom: node.custom,
            ref: node.ref,
        };
    ...
    'structure': (node, parent) => {
        if (parent?.kind !== 'parse') {
            throw new Error(`Parent of "structure" must be "parse", not "${parent?.kind}".`);
        }
        ...
        return [
            ...
            {
                kind: 'structure',
                params: omitObjectKeys(node.params, ['block_header', 'block_index', 'model_index']),
                custom: node.custom,
                ref: node.ref,
            },
        ] satisfies MolstarNode[];
...
export function convertMvsToMolstar(mvsTree: MVSTree, sourceUrl: string | undefined): MolstarTree {
    ...
    if (converted.kind !== 'root') {
        throw new Error(
            "Root's type is not 'root' after conversion from MVS tree to Molstar tree.",
        );
    }
