...
export function SimpleParamsSchema<TFields extends Fields>(
    fields: TFields,
...
type ValuesForSimpleParamsSchema<TSchema extends SimpleParamsSchema> = ValuesForFields<
    TSchema['fields']
>;

type AllRequiredFields<F extends Fields> = {
    [key in keyof F]: F[key] extends Field<infer V> ? RequiredField<V>
        : never;
};

type AllRequiredSimple<TSchema extends SimpleParamsSchema> = SimpleParamsSchema<
    AllRequiredFields<TSchema['fields']>
>;
...
export interface UnionParamsSchema<
...
export type ValuesFor<P extends ParamsSchema> = P extends SimpleParamsSchema
...
export type AllRequired<P extends ParamsSchema> = P extends SimpleParamsSchema
    ? AllRequiredSimple<P>
    : P extends UnionParamsSchema ? AllRequiredUnion<P>
    : never;

function AllRequiredSimple<TSchema extends SimpleParamsSchema>(
    schema: TSchema,
): AllRequired<TSchema> {
    const newFields = mapObjectMap(
        schema.fields,
        (field) => RequiredField(field.type, field.description),
    );
    return SimpleParamsSchema(newFields) as AllRequired<TSchema>;
}
function AllRequiredUnion<TSchema extends UnionParamsSchema>(
    schema: TSchema,
): AllRequired<TSchema> {
    const newCases = mapObjectMap(schema.cases, AllRequired);
    return UnionParamsSchema(
        schema.discriminator,
        schema.discriminatorDescription,
        newCases,
    ) as AllRequired<TSchema>;
}
...
function unionParamsValidationIssues(
    ...
    const issues = paramsValidationIssues(
        subschema,
        omitObjectKeys(values, [schema.discriminator]),
        options,
    );
    if (issues) {
...
export function addParamDefaults<P extends ParamsSchema>(
    schema: P,
    values: ValuesFor<P>,
): FullValuesFor<P> {
    if (schema.type === 'simple') {
