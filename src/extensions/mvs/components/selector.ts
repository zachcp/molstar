...
import { ElementIndex, Structure, StructureElement } from '../../../mol-model/structure.ts';
import {
...
export const SelectorParams = PD.MappedStatic('static', {
    ...
    annotation: PD.Group(
        pickObjectKeys(MVSAnnotationStructureComponentParams, [
            'annotationId',
            'fieldName',
            'fieldValues',
        ]),
...
export function substructureFromSelector(structure: Structure, selector: Selector): Structure {
    const pso = (selector.name === 'annotation')
        ? createMVSAnnotationStructureComponent(structure, {
            ...selector.params,
            label: '',
            nullIfEmpty: false,
        })
        : createStructureComponent(structure, { type: selector, label: '', nullIfEmpty: false }, {
            source: structure,
        });
