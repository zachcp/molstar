...
import { getMVSAnnotationForStructure, type MVSAnnotation } from './annotation-prop.ts';
...
export const MVSCategoricalPaletteParams = {
    ...
    repeatColorList: PD.Boolean(false, {
        ...
        description: 'Repeat color list once all colors are depleted (only applies if `colors` is a list).',
    }),
    sort: PD.Select('none', [['none', 'None'], ['lexical', 'Lexical'], ['numeric', 'Numeric']] as const, {
        hideIf: (g) => g.colors.name !== 'list',
        description:
            'Sort actual annotation values before assigning colors from a list (none = take values in order of their first occurrence).',
    }),
    sortDirection: PD.Select('ascending', [['ascending', 'Ascending'], ['descending', 'Descending']] as const, {
        hideIf: (g) => g.colors.name !== 'list',
...
export const MVSDiscretePaletteParams = {
    ...
    mode: PD.Select('normalized', [['normalized', 'Normalized'], ['absolute', 'Absolute']] as const, {
        description:
...
function makePaletteFunction(
    ...
    if (props.name === 'discrete') {
        return makePaletteFunctionDiscrete(props.params as MVSDiscretePaletteProps, annotation, fieldName);
    }
    if (props.name === 'continuous') {
        return makePaletteFunctionContinuous(props.params as MVSContinuousPaletteProps, annotation, fieldName);
    }
...
function makeNumericPaletteScale(
    ...
    if (props.mode === 'normalized') {
        ...
        if (xMin === null || xMax === null) {
            const values = annotation.getDistinctValuesInField(fieldName, false).map(parseFloat).filter((x) =>
                !isNaN(x)
            );
            if (values.length > 0) {
