...
export const MVSCategoricalPaletteParams = {
    colors: PD.MappedStatic('list', {
        ...
        dictionary: PD.ObjectList(
            ...
            (e) => `${e.value}: ${Color.toHexStyle(e.color)}`,
            { description: 'Mapping of annotation values to colors.' },
        ),
    }),
    repeatColorList: PD.Boolean(false, {
        hideIf: (g) => g.colors.name !== 'list',
        description:
            'Repeat color list once all colors are depleted (only applies if `colors` is a list).',
    }),
    sort: PD.Select(
        'none',
        [['none', 'None'], ['lexical', 'Lexical'], ['numeric', 'Numeric']] as const,
        {
            hideIf: (g) => g.colors.name !== 'list',
            description:
    ...
    sortDirection: PD.Select(
        'ascending',
        [['ascending', 'Ascending'], ['descending', 'Descending']] as const,
        { hideIf: (g) => g.colors.name !== 'list', description: 'Sort direction.' },
    ),
    caseInsensitive: PD.Boolean(false, {
        description: 'Treat annotation values as case-insensitive strings.',
    }),
    setMissingColor: PD.Boolean(false, {
        description: 'Allow setting a color for missing values.',
    }),
    missingColor: PD.Color(ColorNames.white, {
        hideIf: (g) => !g.setMissingColor,
        description:
            'Color to use when (a) `colors` is a dictionary and given key is not present, or (b) `color` is a list and there are more actual annotation values than listed colors and `repeat_color_list` is not true.',
    }),
};
...
export const MVSDiscretePaletteParams = {
    colors: PD.ObjectList(
        ...
        (e) => `${Color.toHexStyle(e.color)} [${e.fromValue}, ${e.toValue}]`,
        { description: 'Mapping of annotation value ranges to colors.' },
    ),
    mode: PD.Select(
        'normalized',
        [['normalized', 'Normalized'], ['absolute', 'Absolute']] as const,
        {
            description:
                'Defines whether the annotation values should be normalized before assigning color based on checkpoints in `colors` (`x_normalized = (x - x_min) / (x_max - x_min)`, where `[x_min, x_max]` are either `value_domain` if provided, or the lowest and the highest value encountered in the annotation).',
        },
    ),
    xMin: MaybeFloatParamDefinition({
        hideIf: (g) => g.mode !== 'normalized',
        placeholder: 'auto',
        description:
            'Defines `x_min` for normalization of annotation values. If not provided, minimum of the actual values will be used. Only used when `mode` is `"normalized"',
    }),
    xMax: MaybeFloatParamDefinition({
        hideIf: (g) => g.mode !== 'normalized',
        placeholder: 'auto',
        description:
            'Defines `x_max` for normalization of annotation values. If not provided, maximum of the actual values will be used. Only used when `mode` is `"normalized"',
    }),
};
...
export const MVSContinuousPaletteParams = {
    colors: PD.ColorList('yellow-green', {
        description: 'List of colors, with optional checkpoints.',
        presetKind: 'scale',
        offsets: true,
    }),
    mode: PD.Select(
    ...
    setUnderflowColor: PD.Boolean(false, {
        description: 'Allow setting a color for values below the lowest checkpoint.',
    }),
    underflowColor: PD.Color(ColorNames.white, {
        hideIf: (g) => !g.setUnderflowColor,
        description: 'Color for values below the lowest checkpoint.',
    }),
    setOverflowColor: PD.Boolean(false, {
        description: 'Allow setting a color for values above the highest checkpoint.',
    }),
    overflowColor: PD.Color(ColorNames.white, {
        hideIf: (g) => !g.setOverflowColor,
        description: 'Color for values above the highest checkpoint.',
    }),
};
...
export const MVSAnnotationColorThemeParams = {
    ...
    fieldName: PD.Text('color', {
        description: 'Annotation field (column) from which to take color values',
    }),
    background: PD.Color(ColorNames.gainsboro, {
        description: 'Color for elements without annotation',
    }),
...
export function MVSAnnotationColorTheme(
    ctx: ThemeDataContext,
    props: MVSAnnotationColorThemeProps,
): ColorTheme<MVSAnnotationColorThemeParams> {
...
export const MVSAnnotationColorThemeProvider: ColorTheme.Provider<
...
function makePaletteFunction(
    props: MVSAnnotationColorThemeProps['palette'],
    ...
    if (props.name === 'discrete') {
        return makePaletteFunctionDiscrete(
            props.params as MVSDiscretePaletteProps,
            annotation,
            fieldName,
        );
    }
    if (props.name === 'continuous') {
        return makePaletteFunctionContinuous(
            props.params as MVSContinuousPaletteProps,
            annotation,
            fieldName,
        );
    }
...
function makePaletteFunctionCategorical(
    props: MVSCategoricalPaletteProps,
    annotation: MVSAnnotation,
    fieldName: string,
): (value: string) => Color | undefined {
...
function makePaletteFunctionContinuous(
    props: MVSContinuousPaletteProps,
    annotation: MVSAnnotation,
    fieldName: string,
): (value: string) => Color | undefined {
...
function makeNumericPaletteScale(
    props: MVSContinuousPaletteProps | MVSDiscretePaletteProps,
    annotation: MVSAnnotation,
    fieldName: string,
): (x: number) => number {
    if (props.mode === 'normalized') {
        ...
        if (xMin === null || xMax === null) {
            const values = annotation.getDistinctValuesInField(fieldName, false).map(parseFloat)
                .filter((x) => !isNaN(x));
            if (values.length > 0) {
