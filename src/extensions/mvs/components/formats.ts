...
import { murmurHash3_128_fromBytes } from "../../../mol-data/util.ts";
...
import { loadMVS, type MVSLoadOptions } from "../load.ts";
...
export class Mvs extends MvsBase {}
...
export async function loadMVSX(
  ...
  mainFilePathOrOptions?:
    | string
    | { mainFilePath?: string; doNotClearAssets?: boolean },
  ...
  if (format === "mvsj") {
    ...
    await plugin.runTask(
      Task.create("Load MVSX file", async (ctx) => {
        const parsed = await loadMVSX(
          plugin,
          ctx,
          data as Uint8Array<ArrayBuffer>,
          { doNotClearAssets: options?.appendSnapshots },
        );
...
function tryGetDownloadUrl(
  pso: SO.Data.String,
  plugin: PluginContext,
): string | undefined {
  const theCell = plugin.state.data
    .selectQ((q) => q.ofTransformer(Download))
    .find((cell) => cell.obj === pso);
