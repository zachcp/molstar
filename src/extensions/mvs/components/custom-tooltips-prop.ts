...
import { ElementSet, Selector, SelectorParams } from './selector.ts';
...
export const CustomTooltipsProvider: CustomStructureProperty.Provider<
    CustomTooltipsParams,
    CustomTooltipsData
> = CustomStructureProperty.createProvider({
    ...
    obtain: async (
        ...
        data: Structure,
        props: Partial<CustomTooltipsProps>,
    ) => {
            ...
            case 'element-loci':
                if (
                    !loci.structure.customPropertyDescriptors.hasReference(
                        CustomTooltipsProvider.descriptor,
                    )
                ) return undefined;
                ...
                for (const tooltip of tooltipData) {
                    const elements = tooltip.elementSet ??= ElementSet.fromSelector(
                        location.structure,
                        tooltip.selector,
                    );
