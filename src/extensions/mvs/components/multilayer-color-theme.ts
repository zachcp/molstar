...
import { ColorTypeLocation } from '../../../mol-geo/geometry/color-data.ts';
...
import { ElementSet, isSelectorAll, SelectorParams, substructureFromSelector } from './selector.ts';
...
export function makeMultilayerColorThemeParams(
    ...
    const nestedThemeTypes = colorThemeRegistry.types.filter(([name, label, category]) =>
        name !== MultilayerColorThemeName && colorThemeRegistry.get(name).isApplicable(ctx)
    ); // Adding 'multilayer' theme itself would cause infinite recursion
    return {
        layers: PD.ObjectList(
            {
                theme: PD.Mapped<any>(
                    ...
                    (name) =>
                        PD.Group<any>(
                            colorThemeRegistry.get(name).getParams({ structure: Structure.Empty }),
                        ),
                    colorThemeInfo,
                ),
            ...
            {
                description:
                    'A list of layers, each defining a color theme. The last listed layer is the top layer (applies first). If the top layer does not provide color for a location or its selection does not cover the location, the underneath layers will apply.',
            },
        ),
        background: PD.Color(DefaultBackgroundColor, {
            description: 'Color for elements where no layer applies',
        }),
    };
}
/** Parameter definition for color theme "Multilayer" */
...
export const DefaultMultilayerColorThemeProps: MultilayerColorThemeProps = {
    layers: [],
...
function makeMultilayerColorTheme(
    ctx: ThemeDataContext,
    props: MultilayerColorThemeProps,
    colorThemeRegistry: ColorTheme.Registry,
): ColorTheme<MultilayerColorThemeParams> {
...
function makeLayers(
    ...
    for (let i = props.layers.length - 1; i >= 0; i--) { // iterate from the end to get top layer first, bottom layer last
        ...
        if (themeProvider.ensureCustomProperties?.attach) {
            console.warn(
                `Multilayer color theme: layer "${themeProvider.name}" has ensureCustomProperties.attach method, but Multilayer color theme does not call it. If the layer does not work, make sure you call ensureCustomProperties.attach somewhere.`,
            );
        }
        ...
        switch (theme.granularity) {
            ...
            default:
                console.warn(
                    `Skipping color theme '${layer.theme.name}', cannot process granularity '${theme.granularity}'`,
                );
        }
    }
...
function getSubstructureGranularity(parent: Structure, substructure: Structure) {
    ...
    const elementsPerInstance: {
        [instance: string]: { [invariantId: number]: StructureElement.Set };
    } = {};
    ...
    const groupGranularity = !childInstances.every((inst) =>
        childCounts[inst] === parentCounts[inst]
    );
    ...
    if (childInstances.length === 0) {
        ...
        for (let i = 1; i < childInstances.length; i++) {
            if (
                !deepEqual(
                    elementsPerInstance[childInstances[0]],
                    elementsPerInstance[childInstances[i]],
                )
            ) {
                instanceGranularity = true;
                break;
...
export function makeMultilayerColorThemeProvider(
    colorThemeRegistry: ColorTheme.Registry,
): ColorTheme.Provider<MultilayerColorThemeParams, typeof MultilayerColorThemeName> {
    return {
        ...
        getParams: (ctx: ThemeDataContext) =>
            makeMultilayerColorThemeParams(colorThemeRegistry, ctx),
