    ...
    tooltips: PD.ObjectList(
        {
            annotationId: PD.Text('', {
                description: 'Reference to "MVS Annotation" custom model property',
            }),
            fieldName: PD.Text('tooltip', {
...
export const MVSAnnotationTooltipsProvider: CustomStructureProperty.Provider<
    MVSAnnotationTooltipsParams,
...
> = CustomStructureProperty.createProvider({
    ...
    obtain: async (
        ...
        data: Structure,
        props: Partial<MVSAnnotationTooltipsProps>,
    ) => {
...
export const MVSAnnotationTooltipsLabelProvider = {
    label: (loci: Loci): string | undefined => {
        switch (loci.kind) {
            case 'element-loci':
                if (
                    !loci.structure.customPropertyDescriptors.hasReference(
                        MVSAnnotationTooltipsProvider.descriptor,
                    )
                ) return undefined;
                ...
                const texts = tooltipProps.tooltips.map((p) =>
                    annotations?.getAnnotation(p.annotationId)?.getValueForLocation(
                        location,
                        p.fieldName,
                    )
                );
