...
export const CustomLabelTextParams = {
  ...
  borderColor: {
    ...Original.LabelTextParams.borderColor,
    defaultValue: ColorNames.black,
  },
};
...
export function CustomLabelTextVisual(
  materialId: number,
): ComplexVisual<CustomLabelTextParams> {
  ...
  for (const item of props.items) {
    switch (item.position.name) {
      case "x_y_z":
        ...
        builder.add(
          item.text,
          item.position.params.x,
          item.position.params.y,
          item.position.params.z,
          scale,
          scale,
          0,
        );
        break;
      case "selection":
        const substructure = substructureFromSelector(
          structure,
          item.position.params.selector,
        );
...
function serialIndexOfSubstructure(
  structure: Structure,
  ...
  for (const unit of structure.units) {
    if (
      unit.model.id === theUnit.model.id &&
      SortedArray.has(unit.elements, theElement)
    ) {
