...
export namespace DnatcoUtil {
    ...
    export function getAtomIndex(
        ...
        for (let eI = residue.start; eI < residue.end; eI++) {
            ...
            if (
                names.includes(elName) && (elAltId === altId || elAltId.length === 0) &&
                (elInsCode === insCode)
            ) {
                return loc.element;
            }
    ...
    export function getStepIndices(
        data: DnatcoTypes.Steps,
        loc: StructureElement.Location,
        r: DnatcoUtil.Residue,
    ) {
        ...
        return insCode !== ''
            ? indices.filter((idx) => data.steps[idx].PDB_ins_code_1 === insCode)
            : indices;
    }
