...
export type ConfalPyramidsColorThemeParams =
  typeof ConfalPyramidsColorThemeParams;
...
export function ConfalPyramidsColorTheme(
  ...
  const colorMap =
    props.colors.name === "default" ? PyramidsColors : props.colors.params;

  function color(location: Location, isSecondary: boolean): Color {
    if (CPT.isLocation(location)) {
      const { step, isLower } = location.data;
      const key = (step.NtC +
        `_${isLower ? "Lwr" : "Upr"}`) as keyof PyramidsColors;
...
> = {
  ...
  ensureCustomProperties: {
    ...
    detach: (data) =>
      data.structure &&
      ConfalPyramidsProvider.ref(data.structure.models[0], false),
  },
};
