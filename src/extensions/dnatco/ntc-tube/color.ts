  ...
  const colorMap =
    props.colors.name === "default"
      ...
      : props.colors.name === "custom"
        ...
        : (ColorMap({
            ...Object.fromEntries(
              ObjectKeys(NtCTubeColors).map((item) => [
                item,
                props.colors.params,
              ]),
            ),
  ...
  function color(location: Location, isSecondary: boolean): Color {
    if (NTT.isLocation(location)) {
      ...
      if (kind === "upper") {
        ...
        key = (
          !props.markResidueBoundaries ? step.NtC + "_Lwr" : "residueMarker"
        ) as keyof NtCTubeColors;
      } /* segment-boundary */ else {
        key = (
          !props.markSegmentBoundaries
            ? step.NtC + "_Lwr"
            : "stepBoundaryMarker"
        ) as keyof NtCTubeColors;
      }
      ...
      ObjectKeys(colorMap)
        .map(
          (k) =>
            [
              String(k).replace("_", " "),
              colorMap[k as keyof typeof colorMap],
            ] as [string, Color],
        )
...
export const NtCTubeColorThemeProvider: ColorTheme.Provider<
  NtCTubeColorThemeParams,
  "ntc-tube"
> = {
  name: "ntc-tube",
  label: "NtC Tube",
  ...
  ensureCustomProperties: {
    attach: (ctx: CustomProperty.Context, data: ThemeDataContext) =>
      data.structure
        ...
        : Promise.resolve(),
    detach: (data) =>
      data.structure && NtCTubeProvider.ref(data.structure.models[0], false),
  },
};
