...
import {
  ...
  type TrajectoryFormatProvider,
} from "../../mol-plugin-state/formats/trajectory.ts";
import {
...
async function defaultStructure(
  plugin: PluginContext,
  data: { trajectory: StateObjectRef<SO.Molecule.Trajectory> },
) {
...
export class G3dHeaderObject extends G3dHeaderObjectBase {}
...
export const G3DHeaderFromFile = PluginStateTransform.BuiltIn({
  ...
  apply({ a }, plugin: PluginContext) {
    return Task.create("Parse G3D", async () => {
      ...
      return new G3dHeaderObject(
        { header, urlOrData: a.data, cache: {} },
        {
          label: header.name,
...
export const G3DHeaderFromUrl = PluginStateTransform.BuiltIn({
  ...
  apply({ params }, plugin: PluginContext) {
    return Task.create("Parse G3D", async () => {
      ...
      return new G3dHeaderObject(
        { header, urlOrData: params.url, cache: {} },
        {
          label: header.name,
          description: header.genome,
        },
...
export const G3DTrajectory = PluginStateTransform.BuiltIn({
  ...
  params: (a) => {
    ...
    return {
      resolution: ParamDefinition.Select(
        rs[rs.length - 1],
        rs.map((r) => [r, "" + r] as const),
      ),
    };
  },
  ...
  apply({ a, params }, plugin: PluginContext) {
    return Task.create("G3D Trajectory", async (ctx) => {
      ...
      const data = await getG3dDataBlock(
        plugin,
        a.data.header,
        a.data.urlOrData,
        params.resolution,
      ...
      return new SO.Molecule.Trajectory(traj, {
        label: a.label,
        description: a.description,
      });
    });
...
export const LoadG3D = StateAction.build({
  display: {
    name: "Load Genome 3D (G3D)",
    description: "Load G3D file from the specified URL.",
  },
  from: SO.Root,
  params: { url: ParamDefinition.Text("") },
