...
const CCDParams = (
    ...
    showOriginalCoordinates: PD.Optional(
        PD.Boolean(true, {
            description:
                `Show original coordinates for 'model' and 'ideal' structure and do not align them.`,
        }),
    ),
    shownCoordinateType: PD.Select(
        'ideal',
        PD.arrayToOptions(['ideal', 'model', 'both'] as const),
...
export const ChemicalCompontentTrajectoryHierarchyPreset = TrajectoryHierarchyPresetProvider({
    ...
    async apply(trajectory, params, plugin) {
        ...
        const idealModelProperties = await builder.insertModelProperties(
            idealModel,
        ...
        if (tr.frameCount !== 2) {
            ...
            await builder.representation.applyPreset(
                idealStructureProperties,
                representationPreset,
                { ...representationPresetParams, coordinateType },
            );
        ...
        const modelModelProperties = await builder.insertModelProperties(
            modelModel,
            params.modelProperties,
            { isCollapsed: true },
        );
...
export const ChemicalComponentPreset = StructureRepresentationPresetProvider({
    ...
    params: () => ({
        ...
        coordinateType: PD.Select<CCDFormat.CoordinateType>(
            'ideal',
            PD.arrayToOptions(['ideal', 'model']),
        ),
