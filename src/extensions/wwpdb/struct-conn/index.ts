...
export const wwPDBStructConnExtensionFunctions = {
    ...
    async inspectStructConn(
        ...
        const conns: { [id: string]: StructConnRecord } = structure.model
            ._staticPropertyData['wwpdb-struct-conn-extension-data'] ??= extractStructConns(
                structure.model,
            );
...
function selectStructureNode(plugin: PluginContext, entry: string | undefined) {
    ...
    if (entry) {
        const result = structNodes.find((node) =>
            node.obj && node.obj.data.model.entry.toLowerCase() === entry.toLowerCase()
        );
        if (!result) {
            console.warn(
                `Structure with entry ID "${entry}" was not found. Available structures: ${
                    structNodes.map((node) => node.obj?.data.model.entry)
                }`,
            );
        }
        return result;
        ...
        if (structNodes.length > 1) {
            console.warn(
                `Structure entry ID was not specified, but there is more than one loaded structure (${
                    structNodes.map((node) => node.obj?.data.model.entry)
                }). Taking the first structure.`,
            );
        }
    ...
    for (let i = 0; i < n; i++) {
        const conn: StructConnRecord = {
            ...
            partner1: {
                ...
                authSeqId: authSeq1.valueKind(i) === Column.ValueKinds.Present
                    ? authSeq1.value(i)
                    : undefined,
            ...
            partner2: {
                ...
                authSeqId: authSeq2.valueKind(i) === Column.ValueKinds.Present
                    ? authSeq2.value(i)
                    : undefined,
    ...
    for (const partner of [conn.partner1, conn.partner2]) {
        const propTests: Parameters<typeof struct.generator.atomGroups>[0] = {
            'chain-test': core.rel.eq([
                struct.atomProperty.macromolecular.label_asym_id(),
                partner.asymId,
            ]),
            'group-by': struct.atomProperty.core.operatorName(),
        };
        if (partner.seqId !== undefined) {
            propTests['residue-test'] = core.rel.eq([
                struct.atomProperty.macromolecular.label_seq_id(),
                partner.seqId,
            ]);
            ...
            propTests['residue-test'] = core.logic.and([
                ...
                core.rel.eq([
                    struct.atomProperty.macromolecular.pdbx_PDB_ins_code(),
                    partner.insCode,
                ]),
            ]);
        }
        if (by === 'residues' && partner.altId !== '') {
            propTests['atom-test'] = core.rel.eq([
                struct.atomProperty.macromolecular.label_alt_id(),
                partner.altId,
            ]);
        }
        if (by === 'atoms') {
 ...
 * Return a promise that resolves to the number of involved atoms which were successfully selected (2, 1, or 0).
 */
async function addStructConnInspection(
    plugin: PluginContext,
    structNode: StructNode,
    conn: StructConnRecord,
): Promise<number> {
    ...
    update.to(structNode).apply(
        StructureComponent,
        {
            label: `${conn.id} (residues)`,
            type: { name: 'expression', params: expressionByResidues },
        },
    ...
    plugin.managers.camera.focusRenderObjects(atomsVisual.selector.data?.repr.renderObjects, {
