...
import { DataLoci, EmptyLoci, Loci } from '../../../mol-model/loci.ts';
...
import { ClashesProvider, InterUnitClashes, IntraUnitClashes, ValidationReport } from './prop.ts';
...
function IntraClashLoci(
    ...
    return DataLoci(
        ...
        elements,
        (boundingSphere: Sphere3D) =>
            getIntraClashBoundingSphere(unit, clashes, elements, boundingSphere),
...
function InterClashLoci(structure: Structure, clashes: InterUnitClashes, elements: number[]) {
    return DataLoci(
        ...
        elements,
        (boundingSphere: Sphere3D) =>
            getInterClashBoundingSphere(structure, clashes, elements, boundingSphere),
...
export const ClashesRepresentationProvider = StructureRepresentationProvider({
    ...
    description:
        'Displays clashes between atoms as disks. Data from wwPDB Validation Report, obtained via RCSB PDB.',
