    ...
    ctor: class extends PluginBehavior.Handler<{ autoAttach: boolean; showTooltip: boolean }> {
        ...
        register(): void {
            ...
            this.ctx.representation.structure.themes.colorThemeRegistry.add(
                DensityFitColorThemeProvider,
            );
            this.ctx.representation.structure.themes.colorThemeRegistry.add(
                GeometryQualityColorThemeProvider,
            );
            this.ctx.representation.structure.themes.colorThemeRegistry.add(
                RandomCoilIndexColorThemeProvider,
            );
...
function geometryQualityLabel(loci: Loci): string | undefined {
    if (loci.kind === 'element-loci') {
        ...
        const residueCount = `<small>(${seen.size} ${
            seen.size > 1 ? 'Residues' : 'Residue'
        })</small>`;
...
function densityFitLabel(loci: Loci): string | undefined {
    if (loci.kind === 'element-loci') {
        ...
        if (rsrzSeen.size) {
            const rsrzCount = `<small>(${rsrzSeen.size} ${
                rsrzSeen.size > 1 ? 'Residues avg.' : 'Residue'
            })</small>`;
            const rsrzAvg = rsrzSum / rsrzSeen.size;
            summary.push(`Real-Space R Z-score ${rsrzCount}: ${rsrzAvg.toFixed(2)}`);
        }
        if (rsccSeen.size) {
            const rsccCount = `<small>(${rsccSeen.size} ${
                rsccSeen.size > 1 ? 'Residues avg.' : 'Residue'
            })</small>`;
...
function randomCoilIndexLabel(loci: Loci): string | undefined {
    if (loci.kind === 'element-loci') {
        ...
        const residueCount = `<small>(${seen.size} ${
            seen.size > 1 ? 'Residues avg.' : 'Residue'
        })</small>`;
...
export const ValidationReportDensityFitPreset = StructureRepresentationPresetProvider({
    ...
    display: {
        ...
        group: 'Annotation',
        description:
            'Color structure based on density fit. Data from wwPDB Validation Report, obtained via RCSB PDB.',
    },
    isApplicable(a) {
        return a.data.models.length === 1 && ValidationReport.isApplicable(a.data.models[0]) &&
