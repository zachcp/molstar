...
`);
    ...
    protected async addMeshWithColors(input: AddMeshInput) {
        ...
        for (let instanceIndex = 0; instanceIndex < instanceCount; ++instanceIndex) {
            ...
            const { vertices, normals, indices, groups, vertexCount, drawCount } = UsdzExporter
            ...
            for (let i = 0; i < drawCount; i += 3) {
                const v = isGeoTexture ? i : indices![i];
                const color = UsdzExporter.getColor(
                    v,
                    geoData,
                    interpolatedColors,
            ...
            for (let i = 0; i < drawCount; i += 3) {
                ...
                const transparency = UsdzExporter.getTransparency(
                    i,
                    geoData,
