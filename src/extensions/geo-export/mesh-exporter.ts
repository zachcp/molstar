...
import type { RenderObjectExportData, RenderObjectExporter } from './render-object-exporter.ts';
...
export abstract class MeshExporter<D extends RenderObjectExportData> implements RenderObjectExporter<D> {
    ...
    private async addMesh(values: MeshValues, webgl: WebGLContext, ctx: RuntimeContext) {
        ...
        await this.addMeshWithColors({
            mesh: { vertices: aPosition, normals: aNormal, indices, groups: aGroup, vertexCount, drawCount },
            meshes: undefined,
            values,
            isGeoTexture: false,
            mode: 'triangles',
    ...
    private async addLines(values: LinesValues, webgl: WebGLContext, ctx: RuntimeContext) {
        ...
        if (this.options.linesAsTriangles) {
            ...
            await this.addMeshWithColors({
                mesh: { vertices, normals: undefined, indices: undefined, groups: aGroup, vertexCount, drawCount },
                meshes: undefined,
                values,
                isGeoTexture: false,
                mode: 'lines',
