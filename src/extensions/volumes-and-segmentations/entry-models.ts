...
export class VolsegModelData {
    ...
    private async loadPdb(pdbId: string, parent: StateObjectSelector | StateObjectRef) {
        ...
        const dataNode = await this.entryData.plugin.build().to(parent).apply(Download, { url: url, isBinary: true }, {
    ...
    async showPdbs(pdbIds: string[]) {
        ...
        if (!group) {
            const newGroupNode = await this.entryData.newUpdate().apply(CreateGroup, { label: 'Fitted Models' }, {
                tags: ['fitted-models-group'],
