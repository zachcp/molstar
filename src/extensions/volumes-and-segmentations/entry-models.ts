...
export class VolsegModelData {
    ...
    private async loadPdb(pdbId: string, parent: StateObjectSelector | StateObjectRef) {
        ...
        const dataNode = await this.entryData.plugin.build().to(parent).apply(Download, {
            url: url,
            isBinary: true,
        }, { tags: ['fitted-model-data', `pdbid-${pdbId}`] }).commit();
        const cifNode = await this.entryData.plugin.build().to(dataNode).apply(ParseCif).commit();
        const trajectoryNode = await this.entryData.plugin.build().to(cifNode).apply(
            TrajectoryFromMmCif,
        ).commit();
    ...
    async showPdbs(pdbIds: string[]) {
        ...
        if (!group) {
            const newGroupNode = await this.entryData.newUpdate().apply(CreateGroup, {
                label: 'Fitted Models',
            }, { tags: ['fitted-models-group'], state: { isCollapsed: true } }).commit();
