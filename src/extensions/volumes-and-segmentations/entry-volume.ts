...
export const SimpleVolumeParams = {
    ...
    opacity: PD.Numeric(0.2, { min: 0, max: 1, step: 0.05 }, {
...
export class VolsegVolumeData {
    ...
    async loadVolume() {
        ...
        if (hasVolumes) {
            ...
            const url = this.entryData.api.volumeUrl(
            ...
            const parsed = await this.entryData.plugin.dataFormats.get('dscif')!.parse(
                this.entryData.plugin,
                data,
            );
            const volumeNode: StateObjectSelector<PluginStateObject.Volume.Data> =
    ...
    private changeIsovalueInVolumeVisualParams(
        ...
        switch (params.type.name) {
            ...
            case 'direct-volume':
                ...
                const peakHalfwidth = DIRECT_VOLUME_RELATIVE_PEAK_HALFWIDTH * stats.sigma /
                    (stats.max - stats.min);
