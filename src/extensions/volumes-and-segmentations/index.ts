...
const DEBUGGING = typeof window !== 'undefined'
...
export const Volseg = PluginBehavior.create<{ autoAttach: boolean; showTooltip: boolean }>({
    ...
    ctor: class extends PluginBehavior.Handler<{ autoAttach: boolean; showTooltip: boolean }> {
        ...
        private async initializeEntryLists() {
            const apiUrl = this.ctx.config.get(VolsegVolumeServerConfig.DefaultServer) ??
...
export const LoadVolseg = StateAction.build({
    ...
    Task.create('Loading Volume & Segmentation', (taskCtx) => {
        return state.transaction(async () => {
            ...
            if (!globalStateNode) {
                await state.build().toRoot().apply(VolsegGlobalStateFromRoot, {}, {
                    state: { isGhost: !DEBUGGING },
                }).commit();
            }

            const entryNode = await state.build().toRoot().apply(VolsegEntryFromRoot, entryParams)
                .commit();
            await state.build().to(entryNode).apply(VolsegStateFromEntry, {}, {
                state: { isGhost: !DEBUGGING },
            }).commit();
            if (entryNode.data) {
