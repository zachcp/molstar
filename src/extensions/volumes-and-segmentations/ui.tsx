...
function VolsegControls(
    { plugin, data, setData }: { plugin: PluginContext; data: VolsegUIData; setData: (d: VolsegUIData) => void },
) {
        ...
        <>
            ...
            <ExpandGroup header='Global options'>
                <WaitingParameterControls
                    ...
                    values={globalState}
                    onChangeValues={async (next) => await data.globalState?.updateState(plugin, next)}
                />
            </ExpandGroup>
            ...
            {allPdbs.length > 0 && (
                <ExpandGroup header='Fitted models in PDB' initiallyExpanded>
                    {allPdbs.map((pdb) => (
                        <WaitingButton
                            key={pdb}
                            onClick={() => entryData.actionShowFittedModel(visibleModels.includes(pdb) ? [] : [pdb])}
                            style={{
                        ...
                        <div style={{ maxHeight: 200, overflow: 'hidden', overflowY: 'auto', marginBlock: 1 }}>
                            {allSegments.map((segment) => (
                                ...
                                >
                                    <Button
                                        onClick={() =>
                                            entryData.actionSelectSegment(
                                                segment !== selectedSegment ? segment.id : undefined,
                                            )}
                                        style={{
                                            fontWeight: segment.id === selectedSegment?.id ? 'bold' : undefined,
                                            marginRight: 1,
                                            flexGrow: 1,
                                            textAlign: 'left',
                                        }}
                                    >
                                        <div
                                            title={segment.biological_annotation.name ?? 'Unnamed segment'}
                                            style={{
                                                maxWidth: 240,
                                                whiteSpace: 'nowrap',
                                                overflow: 'hidden',
...
type ComponentParams<T extends React.Component<any, any, any> | ((props: any) => JSX.Element)> = T extends
    React.Component<infer P, any, any> ? P : T extends (props: infer P) => JSX.Element ? P : never;

function WaitingSlider(
    { value, onChange, ...etc }: { value: number; onChange: (value: number) => any } & ComponentParams<Slider>,
) {
...
function WaitingParameterControls<T extends PD.Params>(
    { values, onChangeValues, ...etc }:
        & { values: PD.ValuesFor<T>; onChangeValues: (values: PD.ValuesFor<T>) => any }
