...
import {
    ...
    ExpandGroup,
    IconButton,
} from '../../mol-plugin-ui/controls/common.tsx';
...
import { VolsegEntry, VolsegEntryData } from './entry-root.ts';
import { SimpleVolumeParams, SimpleVolumeParamValues } from './entry-volume.ts';
import {
    VolsegGlobalState,
...
namespace VolsegUIData {
    export function changeAvailableNodes(
        data: VolsegUIData,
        newNodes: VolsegEntry[],
    ): VolsegUIData {
    ...
    export function changeActiveNode(data: VolsegUIData, newActiveRef: string): VolsegUIData {
        const newActiveNode = data.availableNodes.find((node) => node.data.ref === newActiveRef) ??
            data.availableNodes[0];
    ...
    export function equals(data1: VolsegUIData, data2: VolsegUIData) {
        return shallowEqualArrays(data1.availableNodes, data2.availableNodes) &&
            data1.activeNode === data2.activeNode && data1.globalState === data2.globalState;
    }
}
...
export class VolsegUI extends CollapsableControls<{}, { data: VolsegUIData }> {
    ...
    componentDidMount(): void {
        ...
        this.subscribe(this.plugin.state.data.events.changed, (e) => {
            const nodes = e.state.selectQ((q) => q.ofType(VolsegEntry)).map((cell) => cell?.obj)
                .filter(isDefined);
            ...
            if (!this.state.data.globalState?.isRegistered()) {
                const globalState = e.state.selectQ((q) => q.ofType(VolsegGlobalState))[0]?.obj
                    ?.data;
                if (globalState) newData.globalState = globalState;
            }
            if (
                !VolsegUIData.equals(this.state.data, newData) || this.state.isHidden !== isHidden
            ) {
...
function VolsegControls(
    { plugin, data, setData }: {
        plugin: PluginContext;
        data: VolsegUIData;
        setData: (d: VolsegUIData) => void;
    },
) {
    ...
    const params = {
        ...
        entry: PD.Select(
            data.activeNode!.data.ref,
            data.availableNodes.map((entry) => [entry.data.ref, entry.data.entryId]),
        ),
    };
    ...
    return (
        <>
            <ParameterControls
                params={params}
                values={values}
                onChangeValues={(next) => setData(VolsegUIData.changeActiveNode(data, next.entry))}
            />

            <ExpandGroup header='Global options'>
                <WaitingParameterControls
                    params={VolsegGlobalStateParams}
                    values={globalState}
                    onChangeValues={async (next) =>
                        await data.globalState?.updateState(plugin, next)}
                />
            </ExpandGroup>
...
function VolsegEntryControls({ entryData }: { entryData: VolsegEntryData }) {
    ...
    return (
        <>
            ...
            {/* Fitted models */}
            {allPdbs.length > 0 && (
                <ExpandGroup header='Fitted models in PDB' initiallyExpanded>
                    {allPdbs.map((pdb) => (
                        <WaitingButton
                            key={pdb}
                            onClick={() =>
                                entryData.actionShowFittedModel(
                                    visibleModels.includes(pdb) ? [] : [pdb],
                                )}
                            style={{
                                fontWeight: visibleModels.includes(pdb) ? 'bold' : undefined,
                                textAlign: 'left',
                                marginTop: 1,
                            }}
                        >
                ...
                <ControlRow
                    ...
                    control={
                        <WaitingSlider
                            min={0}
                            max={1}
                            value={state.segmentOpacity}
                            step={0.05}
                            onChange={async (v) => await entryData.actionSetOpacity(v)}
                        />
                    }
                />
                ...
                {allSegments.length > 0 && (
                    <>
                        <WaitingButton
                            onClick={async () => {
                                await sleep(20);
                                await entryData.actionToggleAllSegments();
                            }}
                            style={{ marginTop: 1 }}
                        >
                            Toggle All segments
                        ...
                        >
                            {allSegments.map((segment) => (
                                ...
                                >
                                    <Button
                                        onClick={() =>
                                            entryData.actionSelectSegment(
                                                segment !== selectedSegment
                                                    ? segment.id
                                                    : undefined,
                                            )}
                                        style={{
                                            fontWeight: segment.id === selectedSegment?.id
                                                ? 'bold'
                                                : undefined,
                                            marginRight: 1,
                                            flexGrow: 1,
                                    ...
                                    >
                                        <div
                                            title={segment.biological_annotation.name ??
                                                'Unnamed segment'}
                                            style={{
                                                maxWidth: 240,
                                                whiteSpace: 'nowrap',
                                        ...
                                        >
                                            {segment.biological_annotation.name ??
                                                'Unnamed segment'} ({segment.id})
                                        </div>
                                    </Button>
                                    <IconButton
                                        svg={visibleSegments.includes(segment.id)
                                            ? Icons.VisibilityOutlinedSvg
                                            : Icons.VisibilityOffOutlinedSvg}
            ...
            <ExpandGroup header='Selected segment annotation' initiallyExpanded>
                ...
                >
                    ...
                    {selectedSegment && (
                        <b>
                            Segment {selectedSegment.id}:<br />
                            {selectedSegment.biological_annotation.name ?? 'Unnamed segment'}
                        </b>
                    )}
...
type ComponentParams<T extends React.Component<any, any, any> | ((props: any) => JSX.Element)> =
    T extends React.Component<infer P, any, any> ? P
        : T extends (props: infer P) => JSX.Element ? P
        : never;

function WaitingSlider(
    { value, onChange, ...etc }:
        & { value: number; onChange: (value: number) => any }
        & ComponentParams<Slider>,
) {
    ...
    return (
        <Slider
            value={sliderValue}
            disabled={changing}
            onChange={(newValue) => execute(onChange, newValue)}
            {...etc}
        />
    );
}
...
function WaitingButton(
    { onClick, ...etc }: { onClick: () => any } & ComponentParams<typeof Button>,
) {
    const [changing, _, execute] = useAsyncChange(undefined);

    return (
        <Button disabled={changing} onClick={() => execute(onClick, undefined)} {...etc}>
            {etc.children}
        </Button>
    );
}

function WaitingParameterControls<T extends PD.Params>(
    { values, onChangeValues, ...etc }: {
        values: PD.ValuesFor<T>;
        onChangeValues: (values: PD.ValuesFor<T>) => any;
    } & ComponentParams<ParameterControls<T>>,
) {
    ...
    return (
        <ParameterControls
            ...
            onChangeValues={(newValue) => execute(onChangeValues, newValue)}
            {...etc}
        />
    );
}
