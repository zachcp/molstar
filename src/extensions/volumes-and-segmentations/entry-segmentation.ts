...
import { BOX, MAX_VOXELS, VolsegEntryData } from './entry-root.ts';
...
export class VolsegLatticeSegmentationData {
    ...
    async loadSegmentation() {
        const hasLattices =
            this.entryData.metadata.raw.grid.segmentation_lattices.segmentation_lattice_ids.length >
                0;
        if (hasLattices) {
            const url = this.entryData.api.latticeUrl(
            ...
            if (!group) {
                const newGroupNode = await this.entryData.newUpdate().apply(CreateGroup, {
                    label: 'Segmentation',
                    description: 'Lattice',
                }, { tags: [GROUP_TAG], state: { isCollapsed: true } }).commit();
                group = newGroupNode.ref;
            }
            const segmentLabels = this.entryData.metadata.allSegments.map((seg) => ({
                id: seg.id,
                label: seg.biological_annotation.name
                    ? `<b>${seg.biological_annotation.name}</b>`
                    : '',
            }));
            const volumeNode = await this.entryData.newUpdate().to(group)
                .apply(Download, { url, isBinary: true, label: `Segmentation Data: ${url}` })
                .apply(ParseCif)
                .apply(VolumeFromSegmentationCif, {
                    blockHeader: 'SEGMENTATION_DATA',
                    segmentLabels: segmentLabels,
                    ownerId: this.entryData.ref,
                })
                .commit();
    ...
    async showSegments(segments: number[]) {
        ...
        const mustReselect = segments.includes(selectedSegment) &&
            !repr.params?.values.type.params.segments.includes(selectedSegment);
        const update = this.entryData.newUpdate();
    ...
    async setTryUseGpu(tryUseGpu: boolean) {
        ...
        for (const visual of visuals) {
            ...
            if (oldParams.type.params.tryUseGpu === !tryUseGpu) {
                ...
                const update = this.entryData.newUpdate().to(visual.transform.ref).update(
                    newParams,
                );
