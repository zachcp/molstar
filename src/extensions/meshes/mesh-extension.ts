...
import { BaseGeometry, VisualQuality, VisualQualityOptions } from '../../mol-geo/geometry/base.ts';
...
import { StateObjectRef, StateObjectSelector, StateTransformer } from '../../mol-state/index.ts';
...
export const VolsegTransform: StateTransformer.Builder.Root = StateTransformer.builderFactory(
    'volseg',
);
    ...
    apply({ a, params }, globalCtx) { // `a` is the parent node, params are 2nd argument to To.apply(), `globalCtx` is the plugin
        return Task.create('Create Parsed Meshlist', async (ctx) => {
            ...
            return new MeshlistStateObject(meshlistData, {
                label: params.label,
                description:
                    `${meshlistData.segmentName} (${meshlistData.meshIds.length} mesh${es})`,
            });
        });
    },
});
...
export async function createMeshFromUrl(
    ...
    const rawDataNodeRef = update.apply(Download, {
        url: meshDataUrl,
        isBinary: true,
        label: `Downloaded Data ${segmentId}`,
    }, { state: { isCollapsed: collapseTree } }).ref;
