...
export const MeshServerTransformer = VolsegTransform({
    ...
    apply({ a, params }, plugin: PluginContext) { // `a` is the parent node, `params` are 2nd argument to To.apply()
        ...
        return new MeshServerInfo({ ...params }, {
            label: 'Mesh Server',
...
export const MeshStreamingTransformer = VolsegTransform({
    ...
    apply({ a, params }, plugin: PluginContext) {
        return Task.create('Mesh Streaming', async (ctx) => {
            ...
            return new MeshStreaming(behavior, {
    ...
    update({ a, b, oldParams, newParams }) {
        return Task.create('Update Mesh Streaming', async (ctx) => {
            if (
                a.data.source !== b.data.parentData.source ||
                a.data.entryId !== b.data.parentData.entryId
            ) {
...
function trySetAutoOpacity(
    params: StateTransformer.Params<typeof MeshVisualGroupTransformer>,
    parent: MeshStreaming,
) {
    if (params.opacity === -1) {
...
export const MeshVisualTransformer = VolsegTransform({
    ...
    apply({ a, params, spine }, plugin: PluginContext) {
        return Task.create('Mesh Visual', async (ctx) => {
            ...
            const groupData = spine.getAncestorOfType(PluginStateObject.Group)?.data as
                | MeshVisualGroupData
                | undefined;
            ...
            await repr.createOrUpdate(props, visualInfo.data ?? MeshlistData.empty()).runInContext(
                ctx,
            );
            return new PluginStateObject.Shape.Representation3D({
                repr,
                sourceData: visualInfo.data,
            }, { label: 'Mesh Visual', description: params.tag });
        });
    },
...
export const InitMeshStreaming = StateAction.build({
    ...
    return Task.create('Mesh Streaming', async (ctx) => {
        const { params } = p;
        // p.ref
        const serverNode = await plugin.build().to(p.ref).apply(MeshServerTransformer, params)
            .commit();
        // const serverNode = await plugin.build().toRoot().apply(MeshServerTransformer, params).commit();
        const streamingNode = await plugin.build().to(serverNode).apply(
            MeshStreamingTransformer,
            {},
        ).commit();
        ...
        for (const tag in visuals) {
            ...
            if (!segmentGroups[segid]) {
                ...
                const group = await plugin.build().to(streamingNode).apply(
                    MeshVisualGroupTransformer,
                    { label: `Segment ${segid}`, description: description, segmentId: segid },
        ...
        for (const tag in visuals) {
            ...
            const segid = visuals[tag].segmentId;
            visualsUpdate.to(segmentGroups[segid]).apply(MeshVisualTransformer, {
                ref: ref,
                tag: tag,
            }, { ref: ref }); // ref - hack to allow the node make itself invisible
        }
