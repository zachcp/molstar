...
import { PluginCommand } from '../../../mol-plugin/command.ts';
...
export namespace MeshStreaming {
    export namespace Params {
        export const ViewTypeChoice = new Choice(
            { off: 'Off', select: 'Select', all: 'All' },
            'select',
        ); // TODO add camera target?
        ...
        export function create(options: MeshServerInfo.Data) {
            return {
                view: PD.MappedStatic('select', {
                    'off': PD.Group({}),
                    'select': PD.Group({
                        baseDetail: PD.Numeric(
                            DEFAULT_DETAIL,
                            { min: 1, max: MAX_DETAIL, step: 1 },
                            {
                                description:
                                    'Detail level for the non-selected segments (lower number = better)',
                            },
                        ),
                        focusDetail: PD.Numeric(1, { min: 1, max: MAX_DETAIL, step: 1 }, {
                            description:
                                'Detail level for the selected segment (lower number = better)',
                        }),
        ...
        export function detailsEqual(p: Values, q: Values): boolean {
            switch (p.view.name) {
                ...
                case 'select':
                    return q.view.name === 'select' &&
                        p.view.params.baseDetail === q.view.params.baseDetail &&
    ...
    export class Behavior extends PluginBehavior.WithSubscribers<Params.Values> {
        ...
        private focusObservable = this.plugin.behaviors.interaction.click.pipe( // QUESTION is this OK way to get focused segment?
            ...
            map((loci) =>
                (loci.kind === 'group-loci') ? loci.shape.sourceData as MeshlistData : null
            ),
        ...
        private initVisualInfos() {
            ...
            for (const segid of this.metadata!.meshSegmentIds) {
                const name = this.metadata?.getSegment(segid)?.biological_annotation.name ??
        ...
        private updateVisualInfoDetails() {
            ...
            for (const tag in this.visuals) {
                ...
                if (preferredDetail !== undefined) {
                    visual.detail = this.metadata!.getSufficientMeshDetail(
                        visual.segmentId,
                        preferredDetail,
                    );
                }
