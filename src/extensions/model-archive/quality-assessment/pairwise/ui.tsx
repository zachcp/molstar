...
import { StateBuilder, StateTransform } from '../../../../mol-state/index.ts';
...
import { maDrawPairwiseMetricPNG, MAPairwiseMetricDrawing } from './plot.ts';
    ...
    toggleCollapsed() {
        if (!this.state.isCollapsed) {
            ...
            this.setState({
                ...state,
                isCollapsed: false,
                isHidden: state.params.data.options.length === 0 ||
                    state.params.model.options.length === 0,
            });
        }
    }
...
export function MAPairwiseScorePlot(
    { plugin, model, pairwiseMetric }: {
    ...
    useEffect(() => {
        ...
        const highlight = filterHighlightState(interactivity).subscribe((state) =>
            highlightState(plugin, state)
        );
        const paint = filterOverpaintState(interactivity).subscribe((state) =>
    ...
    return (
        <MAPairwiseScorePlotBase
            model={model}
            pairwiseMetric={pairwiseMetric}
...
function filterOverpaintState(state: BehaviorSubject<PlotInteractivityState>) {
    return state.pipe(
        ...
        distinctUntilChanged((a, b) =>
            ...
            (a.mouseDown ? a.crosshairOffset : a.boxEnd) ===
                (b.mouseDown ? b.crosshairOffset : b.boxEnd)
        ),
    );
}

const PlotWrapper = memo(
    (
        { plugin, values, dataSources, interactivity }: {
            plugin: PluginContext;
            values: State['values'];
        ...
        const model: Model | undefined = plugin.managers.structure.hierarchy.current.models.find(
            (m) => m.cell.transform.ref === values.model,
        )?.cell.obj?.data;
        const src = dataSources.find((src) => src.id === values.data);
        const cif: PluginStateObject.Format.Cif | undefined = plugin.state.data.cells.get(
            src?.dataRef!,
        )?.obj;
        const block = cif?.data.blocks[src?.blockIndex!];

        if (!model || !block || !src) {
            return <div className='msp-description'>Data not available</div>;
        }

        const metric = QualityAssessment.pairwiseMetricFromModelArchiveCIF(
            model,
            block,
            src.metridId,
        );
    ...
    (prev, next) =>
        prev.values.data === next.values.data && prev.values.model === next.values.model,
);
...
function getPropsAndValues(plugin: PluginContext, current?: { model?: string; data?: string }) {
    ...
    const cifs = plugin.state.data.selectQ((q) =>
        q.root.subtree().ofType(PluginStateObject.Format.Cif)
    );
    ...
    const params = {
        model: PD.Select(
            models[0]?.cell.transform.ref,
            models.map((m) => [m.cell.transform.ref, m.cell.obj?.data.label!]),
            { isHidden: models.length <= 1 },
        ),
        data: PD.Select(dataSources[0]?.id, dataSources.map((o) => [o.id, o.label]), {
            isHidden: dataSources.length <= 1,
        }),
    };

    const values = {
        model: params.model.options.find((o) => o[0] === current?.model)?.[0] ??
            params.model.options[0]?.[0],
        data: params.data.options.find((o) => o[0] === current?.data)?.[0] ??
            params.data.options[0]?.[0],
    };
...
export const MAPairwiseScorePlotBase = memo(
    (
        { model, pairwiseMetric, interactivity }: {
            model: Model;
            pairwiseMetric: QualityAssessment.Pairwise;
            interactivity: BehaviorSubject<PlotInteractivityState>;
        },
        ...
        useEffect(() => {
            ...
            const moveEvent = (ev: MouseEvent) => {
                ...
                const offset = getPlotMouseOffsetBase(
                    interactivityRect.current!,
                    ev.clientX,
                    ev.clientY,
                );
        ...
        return (
            <div style={{ margin: '8px 8px 0 8px', position: 'relative' }}>
                <svg viewBox={viewBox} width='100%'>
                    ...
                    <line
                        x1={PlotOffset}
                        x2={PlotOffset + PlotSize}
                        y1={PlotOffset}
                        y2={PlotOffset + PlotSize}
                        style={{ stroke: line, strokeDasharray: '15,15' }}
                    />
                    ...
                    <rect
                        ...
                        width={PlotSize}
                        height={legendHeight}
                        style={{ fill: 'url(#legend-gradient)', strokeWidth: 1, stroke: border }}
                    />
                    <text
                        x={PlotOffset + 20}
                        y={legendOffsetY + legendHeight - 22}
                        style={{ fontSize: '45px', fill: 'white', fontWeight: 'bold' }}
                    >
                        {round(metric.valueRange[0], 2)} Å
                    </text>
                    <text
                        x={PlotOffset + PlotSize - 20}
                        y={legendOffsetY + legendHeight - 22}
                        style={{ fontSize: '45px', fill: 'black', fontWeight: 'bold' }}
                        textAnchor='end'
                    >
                        {round(metric.valueRange[1], 2)} Å
                    </text>
                    <text
                        x={PlotOffset + PlotSize / 2}
                        y={legendOffsetY + legendHeight - 22}
                    ...
                    <text
                        ...
                        style={{
                            fontSize: '45px',
                            fontWeight: 'bold',
                            fill: Color.toStyle(PlotColors.ScoredLabel),
                        }}
                        textAnchor='middle'
                    >
                        Scored Residue
                    ...
                    <text
                        className='msp-svg-text'
                        style={{
                            fontSize: '50px',
                            fontWeight: 'bold',
                            fill: Color.toStyle(PlotColors.AlignedLabel),
                        }}
                        transform={`translate(50, ${PlotOffset + PlotSize / 2}) rotate(270)`}
                        textAnchor='middle'
                    >
                        Aligned Residue
                    </text>
                    ...
                    {chains.map(({ startOffset, endOffset, label }) => {
                        const textOffset = PlotOffset +
                            PlotSize * (startOffset + (endOffset - startOffset) / 2) / nResidues;
                    ...
                    <rect
                        ...
                        onMouseDown={(ev) => {
                            interactivity.next({
                                ...interactivity.value,
                                mouseDown: true,
                                boxStart: getPlotMouseOffset(ev),
                            });
                        }}
                        onMouseLeave={(ev) => {
                            interactivity.next({
...
function PlotInteractivity(
    { drawing, interactity }: {
        drawing: MAPairwiseMetricDrawing;
        interactity: BehaviorSubject<PlotInteractivityState>;
    },
) {
    ...
    if (label) {
        const labelStyle: CSSProperties | undefined = label
            ? {
                fontSize: '45px',
                fill: 'black',
                fontWeight: 'bold',
        ...
        labelNode = (
            <text y={y} style={labelStyle} textAnchor={anchor}>
    ...
    return (
        <>
            {inside && crosshairOffset && (
                <line
                    x1={crosshairOffset[0] + PlotOffset}
                    x2={crosshairOffset[0] + PlotOffset}
                    y1={PlotOffset}
                    y2={PlotOffset + PlotSize}
                    style={{ pointerEvents: 'none', stroke: 'black', strokeDasharray: '5,5' }}
                />
            )}
            {inside && crosshairOffset && (
                <line
                    x1={PlotOffset}
                    x2={PlotOffset + PlotSize}
                    y1={crosshairOffset[1] + PlotOffset}
                    y2={crosshairOffset[1] + PlotOffset}
                    style={{ pointerEvents: 'none', stroke: 'black', strokeDasharray: '5,5' }}
                />
            )}
            {box && (
...
function getResidueIndex(drawing: MAPairwiseMetricDrawing, offset: number) {
    const rI = drawing.metric.residueRange[0] +
        Math.round(
            offset / PlotSize *
                (drawing.metric.residueRange[1] - drawing.metric.residueRange[0] + 1),
        ) as ResidueIndex;
    return clamp(
        rI,
        drawing.metric.residueRange[0],
        drawing.metric.residueRange[1],
    ) as ResidueIndex;
}
...
function getResidueLabel(drawing: MAPairwiseMetricDrawing, rI: ResidueIndex) {
    ...
    return `${asym_id.value(AtomicHierarchy.residueChainIndex(hierarchy, rI))} ${
        seq_id.value(rI)
    } ${comp_id.value(AtomicHierarchy.residueFirstAtomIndex(hierarchy, rI))}`;
}
...
function getPlotMouseOffsetBase(
    target: HTMLElement | SVGRectElement,
    clientX: number,
    clientY: number,
) {
...
async function overpaintState(plugin: PluginContext, state: PlotInteractivityState) {
    ...
    if (
        !state.drawing || !state.boxStart || !(state.boxEnd || state.crosshairOffset) || !structure
    ) {
    ...
    const lociScored = StructureQuery.loci(
        atoms({
            residueTest: (ctx) => {
                const rI = resIdx(ctx.element);
    ...
    const lociAligned = StructureQuery.loci(
        atoms({
            residueTest: (ctx) => {
                const rI = resIdx(ctx.element);
    ...
    for (const repr of representations) {
        update.to(repr).apply(OverpaintStructureRepresentation3DFromBundle, { layers }, {
            tags: [tag],
            state: { isGhost: true },
        });
    }
...
async function reApplyRepresentationStates(plugin: PluginContext, update: StateBuilder.Root) {
    await update.commit();
    const states = plugin.state.data.selectQ((q) =>
        q.root.subtree().ofType(PluginStateObject.Molecule.Structure.Representation3DState)
    );
