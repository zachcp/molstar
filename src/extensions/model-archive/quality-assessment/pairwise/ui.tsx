...
function filterOverpaintState(state: BehaviorSubject<PlotInteractivityState>) {
    return state.pipe(
        ...
        distinctUntilChanged((a, b) =>
            ...
            (a.mouseDown ? a.crosshairOffset : a.boxEnd) === (b.mouseDown ? b.crosshairOffset : b.boxEnd)
        ),
    );
}
...
const PlotWrapper = memo(
    (
        ...
        const model: Model | undefined = plugin.managers.structure.hierarchy.current.models.find((m) =>
            m.cell.transform.ref === values.model
        )?.cell.obj?.data;
            ...
            <div style={{ margin: '8px 8px 0 8px', position: 'relative' }}>
                <svg viewBox={viewBox} width='100%'>
                    ...
                    <text
                        x={PlotOffset + PlotSize / 2}
                        y={50}
                        style={{ fontSize: '45px', fontWeight: 'bold', fill: Color.toStyle(PlotColors.ScoredLabel) }}
                        textAnchor='middle'
                    >
                        Scored Residue
                    </text>
                    <text
                        ...
                        style={{ fontSize: '50px', fontWeight: 'bold', fill: Color.toStyle(PlotColors.AlignedLabel) }}
                        ...
                        return (
                            <Fragment key={startOffset}>
                                ...
                                <text
                                    className='msp-svg-text'
                                    style={{ fontSize: '40px' }}
                                    transform={`translate(${PlotOffset - 15}, ${textOffset}) rotate(270)`}
                                    textAnchor='middle'
                                >
                                    {label} {startIndex}-{endIndex}
                                </text>
...
function PlotInteractivity(
    ...
    if (label) {
        const labelStyle: CSSProperties | undefined = label
            ? { fontSize: '45px', fill: 'black', fontWeight: 'bold', pointerEvents: 'none', userSelect: 'none' }
            : undefined;
        ...
        <>
            ...
            {box && (
                <rect
                    ...
                    width={box[2]}
                    height={box[3]}
                    style={{ stroke: '#eee', strokeWidth: 4, fill: 'rgba(0, 0, 0, 0.15)', pointerEvents: 'none' }}
                />
            )}
            {labelNode}
        </>
...
function getResidueIndex(drawing: MAPairwiseMetricDrawing, offset: number) {
    const rI = drawing.metric.residueRange[0] +
        Math.round(
            offset / PlotSize * (drawing.metric.residueRange[1] - drawing.metric.residueRange[0] + 1),
        ) as ResidueIndex;
...
function getResidueLabel(drawing: MAPairwiseMetricDrawing, rI: ResidueIndex) {
    ...
    return `${asym_id.value(AtomicHierarchy.residueChainIndex(hierarchy, rI))} ${seq_id.value(rI)} ${
