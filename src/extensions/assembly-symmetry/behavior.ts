...
export const AssemblySymmetry: StateTransformer<PluginBehavior.Category, PluginBehavior.Behavior, { autoAttach: boolean }> = PluginBehavior.create<{ autoAttach: boolean }>({
  ...
  display: {
    ...
    description:
      "Assembly Symmetry data provided by RCSB PDB (calculated with BioJava) or by PDBe.",
  },
  ctor: class extends PluginBehavior.Handler<{ autoAttach: boolean }> {
    ...
    register(): void {
      ...
      this.ctx.genericRepresentationControls.set(
        Tag.Representation,
        (selection) => {
          const refs: GenericRepresentationRef[] = [];
          selection.structures.forEach((structure) => {
            const symmRepr = structure.genericRepresentations?.filter(
              (r) => r.cell.transform.transformer.id === AssemblySymmetry3D.id,
            )[0];
      ...
      this.ctx.customStructureControls.set(
        Tag.Representation,
        AssemblySymmetryControls as any,
      );
...
export const InitAssemblySymmetry3D = StateAction.build({
  ...
  Task.create("Init Assembly Symmetry", async (ctx) => {
    try {
      ...
      const assemblySymmetryData = AssemblySymmetryDataProvider.get(
        a.data,
      ).value;
      const symmetryIndex = assemblySymmetryData
        ? AssemblySymmetryData.firstNonC1(assemblySymmetryData)
        : -1;
      await AssemblySymmetryProvider.attach(propCtx, a.data, {
        ...params,
        symmetryIndex,
      });
    } catch (e) {
...
const AssemblySymmetry3D = PluginStateTransform.BuiltIn({
  ...
  display: {
    name: "Assembly Symmetry",
    description:
      "Assembly Symmetry axes and cage. Data provided by RCSB PDB (calculated with BioJava) or by PDBe.",
  },
  from: PluginStateObject.Molecule.Structure,
  ...
  async apply(ref, params, plugin) {
    ...
    if (!AssemblySymmetryDataProvider.get(structure).value) {
      await plugin.runTask(
        Task.create("Assembly Symmetry", async (runtime) => {
          const propCtx = {
            runtime,
            assetManager: plugin.managers.asset,
            errorContext: plugin.errorContext,
          ...
          await AssemblySymmetryDataProvider.attach(
            propCtx,
            structure,
            propProps,
          );
          const assemblySymmetryData =
            AssemblySymmetryDataProvider.get(structure).value;
          const symmetryIndex = assemblySymmetryData
            ? AssemblySymmetryData.firstNonC1(assemblySymmetryData)
            : -1;
    ...
    const assemblySymmetry = await tryCreateAssemblySymmetry(
      plugin,
      structureCell,
    );
    const colorTheme =
      getAssemblySymmetryConfig(plugin).ApplyColors && assemblySymmetry.isOk
        ? (Tag.Cluster as any)
        : undefined;
    ...
    return {
      ...
      representations: { ...preset.representations, assemblySymmetry },
    };
  },
});
...
export function getAssemblySymmetryConfig(plugin: PluginContext): {
  [key in keyof typeof AssemblySymmetryConfig]: NonNullable<
    (typeof AssemblySymmetryConfig)[key]["defaultValue"]
  >;
} {
  return {
    ApplyColors:
      plugin.config.get(AssemblySymmetryConfig.ApplyColors) ??
      AssemblySymmetryConfig.ApplyColors.defaultValue ??
      true,
    DefaultServerType:
      plugin.config.get(AssemblySymmetryConfig.DefaultServerType) ??
      AssemblySymmetryConfig.DefaultServerType.defaultValue ??
      AssemblySymmetryDataParams.serverType.defaultValue,
    DefaultServerUrl:
      plugin.config.get(AssemblySymmetryConfig.DefaultServerUrl) ??
      AssemblySymmetryConfig.DefaultServerUrl.defaultValue ??
      AssemblySymmetryDataParams.serverUrl.defaultValue,
  };
}
