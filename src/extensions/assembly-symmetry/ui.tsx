...
> {
    ...
    this.subscribe(
      ...
      () => {
        this.setState({
          ...
          description:
            StructureHierarchyManager.getSelectedStructuresDescription(
              this.plugin,
            ),
        });
      },
  ...
  async updateAssemblySymmetry(values: AssemblySymmetryProps) {
    ...
    for (const components of this.plugin.managers.structure.hierarchy
      .currentComponentGroups) {
      if (values.symmetryIndex === -1) {
        const name =
          components[0]?.representations[0]?.cell.transform.params?.colorTheme
            .name;
...
const EnableAssemblySymmetry3D = StateAction.build({
  ...
  Task.create("Enable Assembly Symmetry", async (ctx) => {
    const presetParams = AssemblySymmetryPreset.params?.(a, plugin) as
      | PD.Params
      | undefined;
    const presetProps = presetParams
      ? PD.getDefaultValues(presetParams)
      : Object.create(null);
