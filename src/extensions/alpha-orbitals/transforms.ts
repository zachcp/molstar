...
export class BasisAndOrbitals extends PluginStateObject.Create<
    { basis: Basis; order: SphericalBasisOrder; orbitals: AlphaOrbital[] }
>({ name: 'Basis', typeClass: 'Object' }) {}
...
export const CreateOrbitalVolume = PluginStateTransform.BuiltIn({
    ...
    params: (a) => {
        ...
        return {
            index: PD.Select(
                0,
                a.data.orbitals.map((o, i) => [i, `[${i + 1}] ${o.energy.toFixed(4)}`]),
            ),
            ...
            const data = await createSphericalCollocationGrid(
                {
                    ...
                    gridSpacing: params.gridSpacing.map((e) =>
                        [e.atomCount, e.spacing] as [number, number]
                    ),
                },
...
export const CreateOrbitalRepresentation3D = PluginStateTransform.BuiltIn({
    ...
    params: {
        ...
        kind: PD.Select<'positive' | 'negative'>('positive', [['positive', 'Positive'], [
            'negative',
            'Negative',
        ]]),
    ...
    apply({ a, params: srcParams }, plugin: PluginContext) {
        return Task.create('Orbitals Representation 3D', async (ctx) => {
            ...
            repr.setTheme(
                Theme.create(plugin.representation.volume.themes, { volume: a.data }, params),
            );
    ...
    update({ a, b, newParams: srcParams }, plugin: PluginContext) {
        return Task.create('Orbitals Representation 3D', async (ctx) => {
            ...
            b.data.repr.setTheme(
                Theme.create(plugin.representation.volume.themes, { volume: a.data }, newParams),
            );
...
function volumeParams(
    ...
    return createVolumeRepresentationParams(plugin, volume.data, {
        ...
        typeParams: {
            isoValue: {
                kind: 'absolute',
                absoluteValue: (value ?? 1000) * params.relativeIsovalue,
            },
            alpha: params.alpha,
