      ...
      this.ctx.genericRepresentationControls.set(
        ...
        (selection) => {
          ...
          selection.structures.forEach((structure) => {
            const memRepr = structure.genericRepresentations?.filter(
              (r) =>
...
export const isTransmembrane: StructureSelectionQueryType = StructureSelectionQuery(
  ...
  {
    description:
      "Select residues that are embedded between the membrane layers.",
...
const _MembraneOrientation3D = PluginStateTransform.BuiltIn({
  ...
  display: {
    name: "Membrane Orientation",
    description:
      "Membrane Orientation planes and rims. Data calculated with ANVIL algorithm.",
  },
...
const _MembraneOrientationPreset = StructureRepresentationPresetProvider({
  ...
  display: {
    ...
    group: "Annotation",
    description:
      "Shows orientation of membrane layers. Data calculated with ANVIL algorithm.", // TODO add ' or obtained via RCSB PDB'
  },
  isApplicable(a): boolean {
    return MembraneOrientationProvider.isApplicable(a.data);
  },
  ...
  async apply(ref, params, plugin): Promise<StructureRepresentationPresetProvider.Result> {
    ...
    if (!MembraneOrientationProvider.get(structure).value) {
      await plugin.runTask(
        Task.create("Membrane Orientation", async (runtime) => {
          await MembraneOrientationProvider.attach(
            {
              runtime,
              assetManager: plugin.managers.asset,
    ...
    const membraneOrientation = await tryCreateMembraneOrientation(
      plugin,
      structureCell,
    );
    ...
    return {
      ...
      representations: { ...preset.representations, membraneOrientation },
    };
  },
});
