...
async function initialize(
    ...
    function isPartOfEntity(l: StructureElement.Location): boolean {
        return !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.residues.label_seq_id.valueKind(l.unit.residueIndex[l.element]) === 0;
    }
...
function membraneSegments(ctx: ANVILContext, membrane: MembraneCandidate): ArrayLike<{ start: number; end: number }> {
    ...
    let authSeqId;
    let lastAuthSeqId =
        units[0].model.atomicHierarchy.residues.auth_seq_id.value((units[0] as Unit.Atomic).chainIndex[0]) - 1;
    let startOffset = 0;
    let endOffset = 0;
...
function qValue(currentStats: HphobHphil, initialStats: HphobHphil): number {
    ...
    return (currentStats.hphob * (initialStats.hphil - currentStats.hphil) -
        ...
        Math.sqrt(
            part_tot * initialStats.hphob * initialStats.hphil * (initialStats.hphob + initialStats.hphil - part_tot),
        );
}
