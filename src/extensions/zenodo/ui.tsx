...
function createImportParams(files: ZenodoFile[], plugin: PluginContext) {
    ...
    for (
        const { provider: { category, binaryExtensions, stringExtensions }, name } of plugin
            .dataFormats.list
    ) {
    ...
    return {
        type: PD.MappedStatic(
            defaultType,
            Object.keys(params).length ? params : { '': PD.EmptyGroup() },
        ),
    };
}
    ...
    private loadRecord = async () => {
        try {
            ...
            this.plugin.log.error(
                `Failed to load Zenodo record '${this.state.recordValues.record}'`,
            );
