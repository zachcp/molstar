...
namespace StringBuilder {
  ...
  export function getString(builder: StringBuilder): string {
    ...
    if (builder.offset > 0) {
      builder.chunks[builder.chunks.length] =
        builder.current.length === builder.offset
  ...
  export function getChunks(builder: StringBuilder): string[] {
    if (builder.offset > 0) {
      if (builder.current.length === builder.offset)
        ...
        builder.chunks[builder.chunks.length] = builder.current
          .slice(0, builder.offset)
          .join("");
      builder.offset = 0;
    }
  ...
  export function writeIntegerPadLeft(
    builder: StringBuilder,
    val: number,
    totalWidth: number,
  ) {
  ...
  export function writeIntegerPadRight(
    builder: StringBuilder,
    val: number,
    totalWidth: number,
  ) {
