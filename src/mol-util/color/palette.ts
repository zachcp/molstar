...
import { type ScaleLegend, TableLegend } from "../legend.ts";
...
export function getPalette(
  ...
  if (
    props.palette.name === "colors" &&
    props.palette.params.list.kind === "interpolate"
  ) {
    ...
    if (props.palette.name === "colors") {
      colors = props.palette.params.list.colors.map((c) =>
    ...
    const valueLabel =
      labelOptions.valueLabel ?? DefaultLabelOptions.valueLabel;
