...
import { ajaxGet, type DataResponse, type DataType, readFromFile } from './data-source.ts';
...
class AssetManager {
    ...
    set(
        asset: Asset,
        file: File,
        options?: { isStatic?: boolean; tag?: string },
    ) {
    ...
    resolve<T extends DataType>(
        asset: Asset,
        type: T,
        store = true,
    ): Task<Asset.Wrapper<T>> {
        if (Asset.isUrl(asset)) {
            return Task.create(
                `Download ${asset.title || asset.url}`,
                async (ctx) => {
                    ...
                    const data = await ajaxGet({ ...asset, type: 'binary' }).runInContext(
                        ctx,
                    );
                    const file = new File([data], 'raw-data');
                    this._assets.set(asset.id, { asset, file, refCount: 1 });
                    return Asset.Wrapper(
                        await readFromFile(file, type).runInContext(ctx),
                        asset,
                        this,
                    );
                },
            ...
            return Task.create(`Read ${asset.name}`, async (ctx) => {
                ...
                if (!(asset.file instanceof File)) {
                    throw new Error(
                        `Cannot resolve file asset '${asset.name}' (${asset.id})`,
                    );
                }
                if (store) {
