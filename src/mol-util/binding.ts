...
namespace Binding {
  ...
  export function isEmpty(binding: Binding): boolean {
    return (
      binding.triggers.length === 0 ||
      binding.triggers.every(
        (t) => t.buttons === undefined && t.modifiers === undefined && !t.code,
      )
    );
  }
  ...
  export function matchKey(
    ...
    return binding.triggers.some((t) =>
      Trigger.matchKey(t, code, modifiers, key),
    );
  }
