...
namespace ValueRef {
    ...
    export function set<T>(ref: ValueRef<T>, value: T): ValueRef<T> {
        ref.ref = value;
        return ref;
    }
}
...
namespace ValueCell {
    export function create<T, D = never>(
        value: T,
        metadata?: D,
    ): ValueCell<T, D> {
    ...
    export function updateIfChanged<T, D>(
        cell: ValueCell<T, D>,
        value: T,
    ): ValueCell<T, D> {
