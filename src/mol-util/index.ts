...
const hasOwnProperty: (v: PropertyKey) => boolean =
...
export function shallowEqualObjects(a: {}, b: {}): boolean {
  if (a === b) return true;
  if (!a || !b) return false;
  const keys: string[] = Object.keys(a);
...
export const assign: <T>(o: T, ...from: any[]) => T =
  (Object as any).assign || _assign;
...
function _shallowMerge<T>(source: T) {
  ...
  for (let s: number = 1; s < arguments.length; s++) {
    ...
    if (ret !== source) {
      for (let i: number = s + 1; i < arguments.length; i++) {
...
export function formatProgress(p: Progress): string {
  const tp: Task.Progress = p.root.progress;
  if (tp.isIndeterminate) return tp.message;
  const x: string = ((100 * tp.current) / tp.max).toFixed(2);
  return `${tp.message} ${x}%`;
}

export function formatBytes(count: number): string {
  const units: string[] = ["B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB"];
  const i: number = Math.floor(Math.log(count) / Math.log(1024));
