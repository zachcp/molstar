...
export class MonadicParser<A> {
  ...
  wrap<L, R>(
    ...
    return seqPick(
      1,
      typeof leftParser === "string"
        ? MonadicParser.string(leftParser)
        : leftParser,
      this,
      typeof rightParser === "string"
        ? MonadicParser.string(rightParser)
        : rightParser,
    );
  }
  ...
  atLeast(n: number) {
    return MonadicParser.seq(this.times(n), this.many()).map((r) => [
      ...r[0],
      ...r[1],
    ]);
  }
  ...
  mark(): MonadicParser<MonadicParser.Mark<A>> {
    return MonadicParser.seq(
      MonadicParser.index,
      this,
      MonadicParser.index,
    ).map((r) => ({ start: r[0], value: r[1], end: r[2] }));
  }
  ...
  node(name: string): MonadicParser<MonadicParser.Node<A>> {
    return MonadicParser.seq(
      ...
      this,
      MonadicParser.index,
    ).map((r) => ({ name, start: r[0], value: r[1], end: r[2] }));
  }
...
export namespace MonadicParser {
  ...
  export function string(str: string) {
    ...
    if (str.length === 1) {
      ...
      return new MonadicParser((input, i) =>
        input.charCodeAt(i) === code
          ? makeSuccess(i + 1, str)
          : makeFailure(i, expected),
      );
    }
  ...
  export function lookahead<A>(
    x: MonadicParser<A> | string | RegExp,
  ): MonadicParser<null> {
    if (isParser(x)) {
      return new MonadicParser((input, i) => {
  ...
  export function notFollowedBy<A>(
    parser: MonadicParser<A>,
  ): MonadicParser<null> {
    return new MonadicParser((input, i) => {
