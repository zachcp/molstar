            ...
            source: PD.MappedStatic('pdb-xray', {
                ...
                'pdb-emd-ds': PD.Group({
                    provider: PD.Group({
                        ...
                        server: PD.Select<EmdbDownloadProvider>('pdbe', [['pdbe', 'PDBe'], [
                            'rcsb',
                            'RCSB PDB',
                        ]]),
                    }, { pivot: 'id' }),
        ...
        switch (src.name) {
            case 'url':
                ...
                provider = src.params.format === 'auto'
                    ? plugin.dataFormats.auto(
                        getFileNameInfo(Asset.getUrl(downloadParams.url)),
                        data.cell?.obj!,
                    )
...
export const AssignColorVolume = StateAction.build({
    ...
    params(a, plugin: PluginContext) {
        const cells = plugin.state.data.select(
            StateSelection.Generators.root.subtree().ofType(PluginStateObject.Volume.Data).filter(
                (cell) => !!cell.obj && !cell.obj?.data.colorVolume && cell.obj !== a,
            ),
        );
