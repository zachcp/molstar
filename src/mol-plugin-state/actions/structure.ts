...
const DownloadModelRepresentationOptions = (plugin: PluginContext) => {
    ...
    return PD.Group({
        ...
        representation: PD.Select(
            ...
            plugin.builders.structure.representation.getPresets().map((p) =>
                [p.id, p.display.name, p.display.group] as any
            ),
        ...
        asTrajectory: PD.Optional(
            PD.Boolean(false, { description: 'Load all entries into a single trajectory.' }),
        ),
    }, { isExpanded: false });
};
...
export const PdbDownloadProvider = {
    ...
    'pdbe': PD.Group({
        variant: PD.Select(
            'updated-bcif',
            [['updated-bcif', 'Updated (bcif)'], ['updated', 'Updated'], [
                'archival',
                'Archival',
            ]] as ['updated' | 'updated-bcif' | 'archival', string][],
        ),
...
const DownloadStructure = StateAction.build({
    ...
    params: (_, plugin: PluginContext) => {
        ...
        const defaultPdbProvider = plugin.config.get(PluginConfig.Download.DefaultPdbProvider) ||
            'pdbe';
        return {
            source: PD.MappedStatic('pdb', {
                ...
                'alphafolddb': PD.Group({
                    ...
                    isFlat: true,
                    label: 'AlphaFold DB',
                    description: 'Loads the predicted model if available',
                }),
                ...
                'url': PD.Group({
                    ...
                    format: PD.Select<BuiltInTrajectoryFormat>(
                        'mmcif',
                        PD.arrayToOptions(BuiltInTrajectoryFormats.map((f) => f[0]), (f) => f),
                    ),
    ...
    Task.create('Download Structure', async (ctx) => {
        ...
        switch (src.name) {
            case 'url':
                downloadParams = [{
                    ...
                    label: src.params.label || undefined,
                }];
            ...
            case 'pdb-ihm':
                const map = (id: string) =>
                    id.startsWith('PDBDEV_') ? id : `PDBDEV_${id.padStart(8, '0')}`;
                downloadParams = await getDownloadParams(src.params.provider.id, (id) => {
                    ...
                    return nId.match(/^[1-9][A-Z0-9]{3}$/) ? `PDB-IHM: ${nId}` : map(nId);
            ...
            case 'swissmodel':
                downloadParams = await getDownloadParams(
                    src.params.id,
                    (id) =>
                        `https://swissmodel.expasy.org/repository/uniprot/${id.toUpperCase()}.pdb`,
                    (id) => `SWISS-MODEL: ${id}`,
                    false,
                );
                asTrajectory = !!src.params.options.asTrajectory;
            ...
            case 'alphafolddb':
                downloadParams = await getDownloadParams(src.params.provider.id, async (id) => {
                    const url =
                        `https://www.alphafold.ebi.ac.uk/api/prediction/${id.toUpperCase()}`;
                    ...
                    `AlphaFold DB: ${id}`, src.params.provider.encoding === 'bcif');
                asTrajectory = !!src.params.options.asTrajectory;
                format = 'mmcif';
                break;
            case 'modelarchive':
                downloadParams = await getDownloadParams(src.params.id, (id) =>
                    `https://www.modelarchive.org/doi/10.5452/${id.toLowerCase()}.cif`, (id) =>
                    `Model Archive: ${id}`, false);
                asTrajectory = !!src.params.options.asTrajectory;
                format = 'mmcif';
                break;
            case 'pubchem':
                downloadParams = await getDownloadParams(
                    src.params.id,
                    (id) =>
                        `https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/CID/${id.trim()}/record/SDF/?record_type=3d`,
                    (id) =>
                        `PubChem: ${id}`,
                    false,
                );
        ...
        const representationPreset: any = params.source.params.options.representation ||
            plugin.config.get(PluginConfig.Structure.DefaultRepresentationPreset) ||
            PresetStructureRepresentations.auto.id;
        const showUnitcell = representationPreset !== PresetStructureRepresentations.empty.id;

        const structure = src.params.options.type.name === 'auto'
            ? void 0
            : src.params.options.type;
        await state.transaction(async () => {
            if (downloadParams.length > 0 && asTrajectory) {
                const blob = await plugin.builders.data.downloadBlob({
                    sources: downloadParams.map((src, i) => ({
                        id: '' + i,
                        url: src.url,
                ...
                for (const download of downloadParams) {
                    const data = await plugin.builders.data.download(download, {
                        state: { isGhost: true },
                    });
                    ...
                    await plugin.builders.structure.hierarchy.applyPreset(trajectory, 'default', {
                        ...
                        representationPresetParams:
                            params.source.params.options.representationParams,
                    });
                }
            }
        }).runInContext(ctx);
...
async function getDownloadParams(
    ...
    const ids = src.split(/[,\s]/).map((id) => id.trim()).filter((id) =>
        !!id && (id.length >= 4 || /^[1-9][0-9]*$/.test(id))
    );
...
async function getPdbeDownloadParams(
    src: ReturnType<DownloadStructure['createDefaultParams']>['source'],
) {
...
async function getPdbjDownloadParams(
    src: ReturnType<DownloadStructure['createDefaultParams']>['source'],
) {
    ...
    return getDownloadParams(
        src.params.provider.id,
        (id) =>
            `https://data.pdbjlc1.pdbj.org/pub/pdb/data/structures/divided/mmCIF/${
                id.toLowerCase().substring(1, 3)
            }/${id.toLowerCase()}.cif`,
        (id) => `PDBj: ${id} (cif)`,
        false,
    );
...
async function getRcsbDownloadParams(
    src: ReturnType<DownloadStructure['createDefaultParams']>['source'],
) {
...
export const UpdateTrajectory = StateAction.build({
    ...
    if (params.action === 'reset') {
        ...
        for (const m of models) {
            const parent = StateSelection.findAncestorOfType(
                state.tree,
                state.cells,
                m.transform.ref,
...
})(({ ref, params }, ctx: PluginContext) =>
    ctx.builders.structure.insertModelProperties(ref, params)
);
...
export const EnableStructureCustomProps = StateAction.build({
    ...
    ctx.builders.structure.insertStructureProperties(ref, params)
);
...
export const AddTrajectory = StateAction.build({
    ...
    params(a, ctx: PluginContext) {
        ...
        const modelOptions = models.map((t) => [t.transform.ref, t.obj!.label]) as [
            string,
            string,
        ][];
        const coords = state.selectQ((q) => q.rootsOfType(PluginStateObject.Molecule.Coordinates));
        const coordOptions = coords.map((c) => [c.transform.ref, c.obj!.label]) as [
            string,
            string,
        ][];
        return {
    ...
    Task.create('Add Trajectory', (taskCtx) => {
...
export const LoadTrajectory = StateAction.build({
    ...
    params(a, ctx: PluginContext) {
        ...
        const modelOptions = options.filter((o) =>
            o[2] === TrajectoryFormatCategory || o[2] === TopologyFormatCategory
        );
        ...
        for (const { provider } of ctx.dataFormats.list) {
            if (
                provider.category === TrajectoryFormatCategory ||
                provider.category === TopologyFormatCategory
            ) {
        ...
        return {
            source: PD.MappedStatic('file', {
                ...
                file: PD.Group({
                    model: PD.File({
                        accept: modelExts.map((e) => `.${e}`).join(','),
                        label: 'Model',
                    }),
                    coordinates: PD.File({
                        accept: coordinatesExts.map((e) => `.${e}`).join(','),
                        label: 'Coordinates',
                    }),
                }, { isFlat: true }),
    ...
    Task.create('Load Trajectory', (taskCtx) => {
        return state.transaction(async () => {
            ...
            try {
                const modelParsed = s.name === 'url'
                    ? await processUrl(
                        s.params.model.url,
                        s.params.model.format,
                        s.params.model.isBinary,
                    )
