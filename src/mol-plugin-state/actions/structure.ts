...
import type { Download } from '../transforms/data.ts';
import {
...
const _PdbDownloadProvider = {
    ...
    'pdbe': PD.Group({
        variant: PD.Select(
            'updated-bcif',
            [['updated-bcif', 'Updated (bcif)'], ['updated', 'Updated'], ['archival', 'Archival']] as [
                'updated' | 'updated-bcif' | 'archival',
                string,
            ][],
        ),
        ...
        switch (src.name) {
            ...
            case 'swissmodel':
                downloadParams = await getDownloadParams(src.params.id, (id) =>
                    `https://swissmodel.expasy.org/repository/uniprot/${id.toUpperCase()}.pdb`, (id) =>
                    `SWISS-MODEL: ${id}`, false);
                asTrajectory = !!src.params.options.asTrajectory;
                format = 'pdb';
                break;
            case 'alphafolddb':
