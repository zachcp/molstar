...
import {
  ...
  type StructureHierarchyRef,
  type ModelRef,
  type StructureComponentRef,
  StructureHierarchy,
  type StructureRef,
  type TrajectoryRef,
...
export class StructureHierarchyManager extends PluginComponent {
  ...
  get currentComponentGroups(): ReturnType<
    (typeof StructureHierarchyManager)["getComponentGroups"]
  > {
  ...
  findStructure(structure: Structure | undefined): StructureRef | undefined {
    ...
    const root = this.plugin.state.data.selectQ((q) =>
      q.byValue(parent).rootOfType(PluginStateObject.Molecule.Structure),
    )[0];
    if (!root) return undefined;

    return this.behaviors.selection.value.structures.find(
      (s) => s.cell === root,
    );
  }
  ...
  private sync(notify: boolean) {
    ...
    const update = buildStructureHierarchy(
  ...
  updateCurrent(refs: StructureHierarchyRef[], action: "add" | "remove") {
    ...
    const set =
      action === "add"
        ? SetUtils.union(
            this.seletionSet,
            new Set(refs.map((r) => r.cell.transform.ref)),
          )
        : SetUtils.difference(
            this.seletionSet,
            new Set(refs.map((r) => r.cell.transform.ref)),
  ...
  toggleVisibility(
    refs: ReadonlyArray<StructureHierarchyRef>,
    action?: "show" | "hide",
  ) {
    if (refs.length === 0) return;
  ...
  async updateStructure(s: StructureRef, params: any) {
    await this.plugin.dataTransaction(
      async () => {
        const root = StateTree.getDecoratorRoot(
          this.dataState.tree,
...
export namespace StructureHierarchyManager {
  ...
  export function getSelectedStructuresDescription(plugin: PluginContext) {
    ...
    if (structures.length === 1) {
      ...
      const model =
        data.models[0] || data.representativeModel || data.masterModel;
