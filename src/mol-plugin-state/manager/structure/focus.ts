...
export class StructureFocusManager extends StatefulPluginComponent<StructureFocusManagerState> {
  readonly events: {
    historyUpdated: Subject<undefined>;
  } = {
    historyUpdated: this.ev<undefined>(),
  };

  readonly behaviors: {
    current: BehaviorSubject<FocusEntry | undefined>;
  } = {
  ...
  private tryAddHistory(entry: FocusEntry) {
    ...
    let idx = 0,
      existingEntry: FocusEntry | undefined = void 0;
  ...
  addFromLoci(anyLoci: Loci) {
    const union =
      this.state.current &&
      StructureElement.Loci.is(anyLoci) &&
    ...
    this.referenceLoci = StructureElement.Loci.is(refLoci)
      ? refLoci
      : undefined;
  }
  ...
  toggleFromLoci(anyLoci: Loci) {
    ...
    const refLoci = Loci.normalize(anyLoci);
    this.referenceLoci =
      StructureElement.Loci.is(refLoci) && kind !== "subtract"
        ? refLoci
        : undefined;
  }
  ...
  getSnapshot(): StructureFocusSnapshot {
    ...
    const node = this.plugin.helpers.substructureParent.get(
      this.current.loci.structure,
    );
  ...
  setSnapshot(snapshot: StructureFocusSnapshot) {
    ...
    const structure = this.plugin.state.data.select(
      StateSelection.Generators.byRef(ref),
    )[0]?.obj?.data as Structure;
    if (!structure) return;
  ...
  constructor(private plugin: PluginContext) {
    ...
    plugin.state.data.events.object.updated.subscribe(
      ({ oldData, obj, action }) => {
        if (!PluginStateObject.Molecule.Structure.is(obj)) return;
...
function toggleLoci(
  currentLoci: StructureElement.Loci | undefined,
  newLoci: Loci,
) {
  if (
    currentLoci &&
    StructureElement.Loci.is(newLoci) &&
    newLoci.structure === currentLoci.structure
  ) {
    if (StructureElement.Loci.isSubset(currentLoci, newLoci)) {
      return {
        kind: "subtract",
        loci: StructureElement.Loci.subtract(currentLoci, newLoci),
      };
    } else {
      return {
        kind: "add",
        loci: StructureElement.Loci.union(newLoci, currentLoci),
      };
    }
  } else {
