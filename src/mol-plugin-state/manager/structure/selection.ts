...
export class StructureSelectionManager extends StatefulPluginComponent<StructureSelectionManagerState> {
    ...
    private tryAddHistory(loci: StructureElement.Loci) {
        ...
        let idx = 0,
            entry: StructureSelectionHistoryEntry | undefined = void 0;
    ...
    private onUpdate(
        ...
        const structure: Structure | undefined = this.plugin.helpers.substructureParent.get(obj.data)?.obj?.data;
        if (!structure) return;

        // oldObj is not defined for inserts (e.g. TransformStructureConformation)
        if (
            !oldObj?.data ||
            Structure.areUnitIdsAndIndicesEqual(oldObj.data, obj.data)
        ) {
            this.entries.set(
                ref,
                remapSelectionEntry(this.entries.get(ref)!, structure),
            );

            // remap referenceLoci & prevHighlight if needed and possible
            if (this.referenceLoci?.structure.root === structure.root) {
                this.referenceLoci = StructureElement.Loci.remap(
                    this.referenceLoci,
                    structure,
                );
            }
            ...
            this.entries.set(
                ref,
                new SelectionEntry(StructureElement.Loci(structure, [])),
            );
