...
class PluginAnimationManager extends StatefulPluginComponent<PluginAnimationManager.State> {
  ...
  get animations(): PluginStateAnimation[] {
    return this._animations;
  }

  get isAnimatingStateTransition(): boolean {
    return (
      this._current.anim.name === "built-in.animate-state-snapshot-transition"
    );
  }
      ...
      this._params = {
        current: PD.Select(
          ...
          this._animations.map(
            (a) => [a.name, a.display.name] as [string, string],
          ),
          { label: "Animation" },
        ),
      };
  ...
  register(animation: PluginStateAnimation) {
    if (this.map.has(animation.name)) {
      this.context.log.error(
        `Animation '${animation.name}' is already registered.`,
      );
      return;
    }
    this._params = void 0;
  ...
  async start() {
    ...
    let initialState = this._current.anim.initialState(
      this._current.paramValues,
      this.context,
    );
    if (anim.setup) {
      const state = await anim.setup(
        this._current.paramValues,
        initialState,
        this.context,
      );
