...
import type { StructureComponentRef } from '../structure/hierarchy-state.ts';
import { Camera } from '../../../mol-canvas3d/camera.ts';

function getPolymerResiduePositions(
    structure: Structure,
    ...
    const size = stride === 1 ? polymerResidueCount : Math.ceil(polymerResidueCount / stride) + structure.units.length;
...
function getFirstResidueOrAveragePosition(
    ...
    if (structure.units.length === 1) {
        ...
        const firstPolymerUnit = structure.units.find(
            (u) => u.props.polymerElements,
        );
        if (firstPolymerUnit) {
            const pos = Vec3();
            const center = Vec3();
...
export function pcaFocus(
    ...
    if (options.positionToFlip) {
        const { aroundX, aroundY } = getAxesToFlip(
            options.positionToFlip,
            origin,
            up,
            dirB,
        );
    ...
    return plugin.canvas3d.camera.getFocus(
        origin,
        radius,
        up,
        dir,
...
export function getPcaTransform(
    group: StructureComponentRef[],
): PcaTransformData | undefined {
