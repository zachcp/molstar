...
import { StateObject, StateTransform } from '../../../mol-state/index.ts';
...
export function getFocusSnapshot(
    ...
    const targetSpheres = options.targets?.map((target) => {
        ...
        const radius = target.radius ??
            bounding.radius * (target.radiusFactor ?? 1) + (target.extraRadius ?? 0);
...
export function getCellBoundingSphere(
    plugin: PluginContext,
    cellRef: StateTransform.Ref,
): Sphere3D | undefined {
...
function collectCellBoundingSpheres(
    out: Sphere3D[],
    plugin: PluginContext,
    cellRef: StateTransform.Ref,
): Sphere3D[] {
...
export function getPluginBoundingSphere(plugin: PluginContext) {
    ...
    const spheres = renderObjects.map((r) => r.values.boundingSphere.ref.value).filter((sphere) =>
        sphere.radius > 0
    );
    return boundingSphereOfSpheres(spheres);
}
