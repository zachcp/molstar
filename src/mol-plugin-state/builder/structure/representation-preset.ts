...
export interface StructureRepresentationPresetProvider<P = any, S extends _Result = _Result>
...
export namespace StructureRepresentationPresetProvider {
    ...
    export type State<P extends StructureRepresentationPresetProvider> = P extends
        StructureRepresentationPresetProvider<infer _, infer S> ? S : never;
    export type Result = {
    ...
    export const CommonParams = {
        ...
        theme: PD.Optional(PD.Group({
            ...
            carbonColor: PD.Optional(
                PD.Select(
                    'chain-id',
                    PD.arrayToOptions(['chain-id', 'operator-name', 'element-symbol'] as const),
                ),
            ),
    ...
    function isSymmetry(structure: Structure) {
        return structure.units.some((u) =>
            !u.conformation.operator.assembly && u.conformation.operator.spgrOp >= 0
        );
    }
...
const polymerAndLigand = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const waterType = (components.water?.obj?.data?.elementCount || 0) > 50_000
            ? 'line'
            : 'ball-and-stick';
        const lipidType = (components.lipid?.obj?.data?.elementCount || 0) > 20_000
            ? 'line'
            : 'ball-and-stick';
...
const atomicDetail = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const { update, builder, typeParams, color, ballAndStickColor, globalColorParams } =
            reprBuilder(plugin, params, structure);
...
const autoLod = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const representations = {
            gaussianSurface: builder.buildRepresentation(update, components.all, {
                type: 'gaussian-surface',
                typeParams: { ...typeParams, lod: Vec3.create(30, 10000000, 100) },
                color: symmetryColor,
                colorParams: symmetryColorParams,
            }, { tag: 'gaussian-surface' }),
            cartoon: builder.buildRepresentation(update, components.all, {
                type: 'cartoon',
                typeParams: { ...typeParams, lod: Vec3.create(-20, 300, 100) },
                color: symmetryColor,
                colorParams: symmetryColorParams,
            }, { tag: 'cartoon' }),
            ballAndStick: builder.buildRepresentation(update, components.all, {
                type: 'ball-and-stick',
