...
import { type VisualQuality, VisualQualityOptions } from '../../../mol-geo/geometry/base.ts';
import type { ColorTheme } from '../../../mol-theme/color.ts';
import { Structure } from '../../../mol-model/structure.ts';
import type { PluginContext } from '../../../mol-plugin/context.ts';
import { type StateBuilder, StateObjectRef, type StateObjectSelector } from '../../../mol-state/index.ts';
...
export namespace StructureRepresentationPresetProvider {
    export type Params<P extends StructureRepresentationPresetProvider> = P extends
        StructureRepresentationPresetProvider<infer T> ? T : never;
    export type State<P extends StructureRepresentationPresetProvider> = P extends
        StructureRepresentationPresetProvider<infer _, infer S> ? S
        : never;
    export type Result = {
...
const auto = StructureRepresentationPresetProvider({
    ...
    apply(ref, params, plugin) {
        ...
        const thresholds = plugin.config.get(PluginConfig.Structure.SizeThresholds) ||
            Structure.DefaultSizeThresholds;
        const size = Structure.getSize(structure, thresholds);

        const gapFraction: number = structure.polymerResidueCount / structure.polymerGapCount;
...
const polymerAndLigand = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const waterType: 'ball-and-stick' | 'line' = (components.water?.obj?.data?.elementCount || 0) > 50_000
            ? 'line'
            : 'ball-and-stick';
        const lipidType: 'ball-and-stick' | 'line' = (components.lipid?.obj?.data?.elementCount || 0) > 20_000
            ? 'line'
            : 'ball-and-stick';
...
const proteinAndNucleic = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const { update, builder, typeParams, symmetryColor, symmetryColorParams } = reprBuilder(
            plugin,
            params,
            structure,
        );
...
const atomicDetail = StructureRepresentationPresetProvider({
    ...
    async apply(ref, params, plugin) {
        ...
        const lowResidueElementRatio: boolean | 0 = structure.atomicResidueCount &&
        ...
        const showCarbohydrateSymbol: boolean = params.showCarbohydrateSymbol &&
            !highElementCount &&
