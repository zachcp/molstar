...
> extends PresetProvider<PluginStateObject.Molecule.Structure, P, S> {}
export function StructureRepresentationPresetProvider<P, S extends _Result>(
  repr: StructureRepresentationPresetProvider<P, S>,
): StructureRepresentationPresetProvider<P, S> {
  return repr;
}
export namespace StructureRepresentationPresetProvider {
  ...
  export type State<P extends StructureRepresentationPresetProvider> =
    P extends StructureRepresentationPresetProvider<infer _, infer S>
      ? S
      : never;
  export type Result = {
  ...
  export const CommonParams = {
    ...
    ignoreHydrogensVariant: PD.Optional(
      PD.Select("all", PD.arrayToOptions(["all", "non-polar"] as const)),
    ),
    ignoreLight: PD.Optional(PD.Boolean(false)),
    quality: PD.Optional(
      PD.Select<VisualQuality>("auto", VisualQualityOptions),
    ),
    theme: PD.Optional(
      PD.Group({
        ...
        focus: PD.Optional(
          PD.Group({
            ...
            params: PD.Optional(
              PD.Value<ColorTheme.BuiltInParams<ColorTheme.BuiltIn>>({} as any),
            ),
          }),
        ),
      }),
  ...
  function getCarbonColorParams(
    name: "chain-id" | "operator-name" | "element-symbol",
  ) {
  ...
  function isSymmetry(structure: Structure): boolean {
    return structure.units.some(
      (u) =>
        !u.conformation.operator.assembly &&
        u.conformation.operator.spgrOp >= 0,
    );
  }
  ...
  export function reprBuilder(
    ...
    const h: "all" | "hide-all" | "only-polar" =
      plugin.managers.structure.component.state.options.hydrogens;
    const typeParams = {
      quality: plugin.managers.structure.component.state.options.visualQuality,
      ignoreHydrogens: h !== "all",
      ignoreHydrogensVariant: (h === "only-polar" ? "non-polar" : "all") as
        | "all"
        | "non-polar",
      ignoreLight:
        plugin.managers.structure.component.state.options.ignoreLight,
    };
    ...
    const color: ColorTheme.BuiltIn | undefined = params.theme?.globalName
      ? params.theme?.globalName
      : void 0;
    ...
    const symmetryColor: ColorTheme.BuiltIn | undefined =
      structure && params.theme?.symmetryColor
        ? isSymmetry(structure)
          ? params.theme?.symmetryColor
          : color
        : color;
    const symmetryColorParams = params.theme?.symmetryColorParams
      ? {
          ...params.theme?.globalColorParams,
          ...params.theme?.symmetryColorParams,
        }
      : { ...params.theme?.globalColorParams };
    const globalColorParams = params.theme?.globalColorParams
      ? { ...params.theme?.globalColorParams }
      : undefined;
...
const auto = StructureRepresentationPresetProvider({
  ...
  apply(ref, params, plugin) {
    const structure: Structure | undefined = StateObjectRef.resolveAndCheck(
      plugin.state.data,
      ref,
    )?.obj?.data;
    if (!structure) return {};
    ...
    const thresholds =
      plugin.config.get(PluginConfig.Structure.SizeThresholds) ||
      Structure.DefaultSizeThresholds;
    const size = Structure.getSize(structure, thresholds);

    const gapFraction: number =
      structure.polymerResidueCount / structure.polymerGapCount;
    ...
    switch (size) {
      ...
      case Structure.Size.Small:
        ...
        return atomicDetail.apply(
          ref,
          { ...params, showCarbohydrateSymbol: true },
          plugin,
        );
      default:
...
const polymerAndLigand = StructureRepresentationPresetProvider({
  ...
  async apply(ref, params, plugin) {
    ...
    const components = {
      ...
      nonStandard: await presetStaticComponent(
        plugin,
        structureCell,
        "non-standard",
      ),
    ...
    const waterType: "ball-and-stick" | "line" =
      (components.water?.obj?.data?.elementCount || 0) > 50_000
        ? "line"
        : "ball-and-stick";
    const lipidType: "ball-and-stick" | "line" =
      ...
      lipid: builder.buildRepresentation(
        ...
        components.lipid,
        {
          ...
          typeParams: {
            ...typeParams,
            alpha: 0.6,
            visuals: lipidType === "line" ? ["intra-bond"] : undefined,
          },
          color,
          colorParams: {
            carbonColor: { name: "element-symbol", params: {} },
...
const proteinAndNucleic = StructureRepresentationPresetProvider({
  ...
  async apply(ref, params, plugin) {
    ...
    const { update, builder, typeParams, symmetryColor, symmetryColorParams } =
      reprBuilder(plugin, params, structure);
...
const atomicDetail = StructureRepresentationPresetProvider({
  ...
  async apply(ref, params, plugin) {
    ...
    const structure = structureCell.obj!.data;
    const highElementCount: boolean = structure.elementCount > 100_000; // TODO make configurable
    const veryHighElementCount: boolean = structure.elementCount > 1_000_000; // TODO make configurable
    const highUnitCount: boolean = structure.units.length > 5_000; // TODO make configurable
    const lowResidueElementRatio: boolean | 0 =
    ...
    const bondsGiven =
      !!IndexPairBonds.Provider.get(m) || StructConn.isExhaustive(m);
    ...
    const showCarbohydrateSymbol: boolean =
      params.showCarbohydrateSymbol &&
      !highElementCount &&
      !lowResidueElementRatio;

    if (showCarbohydrateSymbol) {
      Object.assign(components, {
        branched: await presetStaticComponent(
          plugin,
          structureCell,
          "branched",
          { label: "Carbohydrate" },
...
const illustrative = StructureRepresentationPresetProvider({
  ...
  display: {
    ...
    group: "Miscellaneous",
    description:
      "Show everything in spacefill representation with illustrative colors and ignore light.",
  },
  params: () => CommonParams,
  async apply(ref, params, plugin) {
    const structureCell = StateObjectRef.resolveAndCheck(
    ...
    const { update, builder, typeParams, color } = reprBuilder(
      plugin,
      params,
      structure,
    );
    ...
    const representations = {
      all: builder.buildRepresentation(
        ...
        components.all,
        {
          ...
          color: "illustrative",
          colorParams: {
            style: { name: "entity-id", params: { overrideWater: true } },
          },
        },
        { tag: "all" },
      ),
...
const molecularSurface = StructureRepresentationPresetProvider({
  ...
  display: {
    ...
    group: "Miscellaneous",
    description:
      "Show everything in molecular surface representation with illustrative colors.",
  },
  params: () => CommonParams,
  async apply(ref, params, plugin) {
    const structureCell = StateObjectRef.resolveAndCheck(
...
const autoLod = StructureRepresentationPresetProvider({
  ...
  display: {
    ...
    group: "Miscellaneous",
    description:
      "Shows more (or less) detailed representations automatically based on camera distance.",
  },
  params: () => CommonParams,
  async apply(ref, params, plugin) {
    const structureCell = StateObjectRef.resolveAndCheck(
    ...
    const representations = {
      gaussianSurface: builder.buildRepresentation(
        ...
        {
          type: "gaussian-surface",
          typeParams: { ...typeParams, lod: Vec3.create(30, 10000000, 100) },
          color: symmetryColor,
          colorParams: symmetryColorParams,
        },
...
export function presetStaticComponent(
  ...
  return plugin.builders.structure.tryCreateComponentStatic(
    structure,
    type,
    params,
  );
...
export type PresetStructureRepresentations =
  typeof PresetStructureRepresentations;
