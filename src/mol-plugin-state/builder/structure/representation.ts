...
import {
  ...
  type StructureRepresentationPresetProvider,
} from "./representation-preset.ts";
...
export class StructureRepresentationBuilder {
  ...
  resolveProvider(
    ref: StructureRepresentationPresetProviderRef,
  ): StructureRepresentationPresetProvider | undefined {
    return typeof ref === "string"
      ? (PresetStructureRepresentations[
          ref as keyof PresetStructureRepresentations
  ...
  getPresets(
    s?: PluginStateObject.Molecule.Structure,
  ): ReadonlyArray<StructureRepresentationPresetProvider> {
  ...
  getPresetsWithOptions(
    s: PluginStateObject.Molecule.Structure,
  ): PD.Mapped<PD.NamedParams<PD.Normalize<unknown>, string>> {
    ...
    for (const p of this._providers) {
      ...
      map[p.id] = p.params
        ? PD.Group(p.params(s, this.plugin))
        : PD.EmptyGroup();
    }
    if (options.length === 0)
  ...
  registerPreset(provider: StructureRepresentationPresetProvider) {
    if (this.providerMap.has(provider.id)) {
      throw new Error(
        `Representation provider with id '${provider.id}' already registered.`,
      );
    }
  ...
  applyPreset<K extends keyof PresetStructureRepresentations>(
    parent: StateObjectRef<PluginStateObject.Molecule.Structure>,
    preset: K,
    params?: StructureRepresentationPresetProvider.Params<
      PresetStructureRepresentations[K]
    >,
  ):
    | Promise<
        StructureRepresentationPresetProvider.State<
          PresetStructureRepresentations[K]
        >
      >
    | undefined;
  ...
  applyPreset(
    ...
    const defaults = this.plugin.config.get(
  ...
  buildRepresentation(
    ...
    if (!structure) return;
    const data = StateObjectRef.resolveAndCheck(this.dataState, structure)?.obj
      ?.data;
    if (!data) return;
    ...
    return options?.tag
      ? builder
          .to(structure)
          .applyOrUpdateTagged(options.tag, StructureRepresentation3D, params, {
            state: options?.initialState,
          }).selector
      : builder
          .to(structure)
          .apply(StructureRepresentation3D, params, {
            state: options?.initialState,
          }).selector;
  }

  constructor(public plugin: PluginContext) {
    objectForEach(PresetStructureRepresentations, (r) =>
      this.registerPreset(r),
    );
  }
}
