...
import type { StructureBuilder } from '../structure.ts';
...
export function TrajectoryHierarchyPresetProvider<P, S>(
    preset: TrajectoryHierarchyPresetProvider<P, S>,
...
const defaultPreset = TrajectoryHierarchyPresetProvider({
    ...
    async apply(trajectory, params, plugin) {
        ...
        const unitcell = params.showUnitcell === void 0 || !!params.showUnitcell
            ? await builder.tryCreateUnitcell(modelProperties, undefined, {
                isHidden: true,
            })
            : void 0;
        const representationPreset: string = params.representationPreset ||
...
const allModels = TrajectoryHierarchyPresetProvider({
    ...
    async apply(trajectory, params, plugin) {
        const tr: PluginStateObject.Molecule.Trajectory['data'] | undefined = StateObjectRef.resolveAndCheck(
            plugin.state.data,
            trajectory,
        )?.obj?.data;
        if (!tr) return {};
        ...
        for (let i = 0; i < tr.frameCount; i++) {
            ...
            const quality:
                ...
                | 'lowest' = structure.obj
                    ...
                    : 'medium';
            const representationPreset = params.representationPreset ||
                plugin.config.get(PluginConfig.Structure.DefaultRepresentationPreset) ||
                PresetStructureRepresentations.auto.id;
            await builder.representation.applyPreset(
                structureProperties,
                representationPreset,
                { theme: { globalName: 'trajectory-index' }, quality },
            );
        }
...
const crystalContacts = TrajectoryHierarchyPresetProvider({
    ...
    async apply(trajectory, params, plugin) {
        ...
        const unitcell = await builder.tryCreateUnitcell(
            modelProperties,
            undefined,
            { isHidden: true },
        );
