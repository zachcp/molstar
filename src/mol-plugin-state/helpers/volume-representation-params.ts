...
import { VolumeRepresentation3D } from '../transforms/representation.ts';
...
export function createVolumeColorThemeParams(
    ...
    const colorDefaultParams = PD.getDefaultValues(
        color.getParams({ volume: volume || Volume.One }),
    );
...
function createParamsByName(
    ...
    const colorProvider =
        ...
        ctx.representation.volume.themes.colorThemeRegistry.get(
