...
export namespace RootStructureDefinition {
    export function getParams(
        model?: Model,
        defaultValue?: 'auto' | 'model' | 'assembly' | 'symmetry' | 'symmetry-mates' | 'symmetry-assembly',
    ) {
        const symmetry = model && ModelSymmetry.Provider.get(model);

        const assemblyIds = symmetry
            ? symmetry.assemblies.map((a) => [a.id, `${a.id}: ${stringToWords(a.details)}`] as [string, string])
            : [];
    ...
    export function canAutoUpdate(oldParams: Params, newParams: Params) {
        if (
            newParams.name === 'symmetry-assembly' || (newParams.name === 'symmetry' && oldParams.name === 'symmetry')
        ) return false;
        return true;
    }
