...
import { Model, Structure, StructureSymmetry } from '../../mol-model/structure.ts';
...
const CommonStructureParams = {
    dynamicBonds: PD.Optional(
        PD.Boolean(false, {
            description:
                'Ensure bonds are recalculated upon model changes. Also enables calculation of inter-unit bonds in water molecules and ions.',
        }),
    ),
};
...
export namespace RootStructureDefinition {
    export function getParams(
        ...
        const assemblyIds = symmetry
            ? symmetry.assemblies.map((a) =>
                [a.id, `${a.id}: ${stringToWords(a.details)}`] as [string, string]
            )
            : [];
        ...
        const modes = {
            ...
            assembly: PD.Group({
                id: PD.Optional(
                    model
                        ? PD.Select(assemblyIds.length ? assemblyIds[0][0] : '', assemblyIds, {
                            label: 'Asm Id',
            ...
            'symmetry': PD.Group({
                ijkMin: PD.Vec3(Vec3.create(-1, -1, -1), { step: 1 }, {
                    label: 'Min IJK',
                    fieldLabels: { x: 'I', y: 'J', z: 'K' },
                }),
                ijkMax: PD.Vec3(Vec3.create(1, 1, 1), { step: 1 }, {
                    label: 'Max IJK',
                    fieldLabels: { x: 'I', y: 'J', z: 'K' },
                }),
                ...CommonStructureParams,
            ...
            'symmetry-assembly': PD.Group({
                generators: PD.ObjectList(
                    ...
                    {
                        defaultValue: [] as {
                            operators: { index: number; shift: Vec3 }[];
                            asymIds: string[];
                        }[],
                    },
                ),
    ...
    export function canAutoUpdate(oldParams: Params, newParams: Params) {
        if (
            newParams.name === 'symmetry-assembly' ||
            (newParams.name === 'symmetry' && oldParams.name === 'symmetry')
        ) return false;
        return true;
    }
    ...
    async function buildSymmetry(
        ...
        const base = Structure.ofModel(model, props);
        const s = await StructureSymmetry.buildSymmetryRange(base, ijkMin, ijkMax).runInContext(
            ctx,
        );
        const objProps = {
    ...
    async function buildSymmetryMates(
        ctx: RuntimeContext,
        model: Model,
        radius: number,
        props?: CommonStructureProps,
    ...
    async function buildSymmetryAssembly(
        ctx: RuntimeContext,
        model: Model,
        generators: StructureSymmetry.Generators,
        symmetry: Symmetry,
        props?: CommonStructureProps,
    ) {
        const base = Structure.ofModel(model, props);
        const s = await StructureSymmetry.buildSymmetryAssembly(base, generators, symmetry)
            .runInContext(ctx);
        const objProps = {
