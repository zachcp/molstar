...
import { StateBuilder } from '../../mol-state/index.ts';
...
const current = StructureSelectionQuery('Current Selection', MS.internal.generator.current(), {
    category: '',
    ...
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
            'entity-test': MS.core.logic.and([
                ...
                MS.core.str.match([
                    MS.re(
                        '(polypeptide|cyclic-pseudo-peptide|peptide-like|nucleotide|peptide nucleic acid)',
                        'i',
                    ),
...
const trace = StructureSelectionQuery(
    'Trace',
    MS.struct.modifier.union([
...
const backbone = StructureSelectionQuery(
    'Backbone',
    MS.struct.modifier.union([
        MS.struct.combinator.merge([
            MS.struct.modifier.union([
                MS.struct.generator.atomGroups({
                    ...
                    'atom-test': MS.core.set.has([
                        MS.set(...SetUtils.toArray(ProteinBackboneAtoms)),
            ...
            MS.struct.modifier.union([
                MS.struct.generator.atomGroups({
                    ...
                    'residue-test': MS.core.logic.not([_nonPolymerResidueTest]),
                    'atom-test': MS.core.set.has([
                        MS.set(...SetUtils.toArray(NucleicBackboneAtoms)),
                        MS.ammp('label_atom_id'),
                    ]),
                }),
            ]),
...
const sidechain = StructureSelectionQuery(
    'Sidechain',
    MS.struct.modifier.union([
        MS.struct.combinator.merge([
            MS.struct.modifier.union([
...
// TODO maybe pre-calculate sidechain atom property
const sidechainWithTrace = StructureSelectionQuery(
...
const protein = StructureSelectionQuery(
    'Protein',
    MS.struct.modifier.union([
...
const nucleic = StructureSelectionQuery(
    'Nucleic',
    MS.struct.modifier.union([
...
const helix = StructureSelectionQuery(
    'Helix',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
            'entity-test': _proteinEntityTest,
...
const beta = StructureSelectionQuery(
    'Beta Strand/Sheet',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
            'entity-test': _proteinEntityTest,
...
const water = StructureSelectionQuery(
    'Water',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
...
const ion = StructureSelectionQuery(
    'Ion',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
...
const lipid = StructureSelectionQuery(
    'Lipid',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
...
const branched = StructureSelectionQuery(
    'Carbohydrate',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
...
const branchedPlusConnected = StructureSelectionQuery(
...
const branchedConnectedOnly = StructureSelectionQuery(
...
const ligand = StructureSelectionQuery(
    'Ligand',
    MS.struct.modifier.union([
        MS.struct.modifier.exceptBy({
                ...
                MS.struct.modifier.union([
                    MS.struct.generator.atomGroups({
                        ...
                        'residue-test': MS.core.set.has([
                            MS.set(...SetUtils.toArray(PolymerNames)),
...
const ligandPlusConnected = StructureSelectionQuery(
...
const ligandConnectedOnly = StructureSelectionQuery(
    'Connected to Ligand',
    MS.struct.modifier.union([
        MS.struct.modifier.exceptBy({
...
const connectedOnly = StructureSelectionQuery(
...
const disulfideBridges = StructureSelectionQuery(
    'Disulfide Bridges',
    MS.struct.modifier.union([
        MS.struct.combinator.merge([
            MS.struct.modifier.union([
...
const nosBridges = StructureSelectionQuery(
    'NOS Bridges',
    MS.struct.modifier.union([
        MS.struct.modifier.wholeResidues([
            MS.struct.filter.isConnectedTo({
...
const nonStandardPolymer = StructureSelectionQuery(
...
const coarse = StructureSelectionQuery(
    'Coarse Elements',
    MS.struct.modifier.union([
        MS.struct.generator.atomGroups({
            'chain-test': MS.core.set.has([
...
const ring = StructureSelectionQuery(
    'Rings in Residues',
    MS.struct.modifier.union([
...
const aromaticRing = StructureSelectionQuery(
...
const surroundings = StructureSelectionQuery(
...
const surroundingLigands = StructureSelectionQuery(
...
const surroundingAtoms = StructureSelectionQuery(
...
const complement = StructureSelectionQuery(
...
const covalentlyBonded = StructureSelectionQuery(
...
const covalentlyBondedComponent = StructureSelectionQuery(
...
const covalentlyOrMetallicBonded = StructureSelectionQuery(
...
const wholeResidues = StructureSelectionQuery(
...
export function ElementSymbolQuery(
    [names, label]: [string[], string],
    category: string,
    priority: number,
) {
...
export function EntityDescriptionQuery(
    [names, label]: [string[], string],
    category: string,
    priority: number,
) {
    ...
    return StructureSelectionQuery(
        ...
        MS.struct.modifier.union([
            MS.struct.generator.atomGroups({
                'entity-test': MS.core.list.equal([
                    MS.list(...names),
                    MS.ammp('entityDescription'),
                ]),
            }),
        ]),
        { category, priority, description },
...
export function applyBuiltInSelection(
    to: StateBuilder.To<PluginStateObject.Molecule.Structure>,
    query: keyof typeof StructureSelectionQueries,
    customTag?: string,
) {
...
export class StructureSelectionQueryRegistry {
    ...
    constructor() {
        ...
        this.list.push(
            ...
            ...StandardNucleicBases.map((v) =>
                ResidueQuery(v, StructureSelectionCategory.NucleicBase)
            ),
        );
        this.options.push(
            ...this.list.map((q) =>
                [q, q.label, q.category] as [StructureSelectionQuery, string, string]
            ),
        );
    }
}
