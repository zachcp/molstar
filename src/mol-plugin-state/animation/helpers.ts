...
import { Sphere3D, SymmetryOperator } from '../../mol-math/geometry.ts';
...
export function unwindStructureAssembly(
    structure: Structure,
...
export function explodeStructure(
    ...
    for (let i = 0, _i = structure.units.length; i < _i; i++) {
        ...
        Vec3.transformMat4(
            _centerVec,
            u.lookup3d.boundary.sphere.center,
            u.conformation.operator.matrix,
        );
...
export const SpinStructureParams = {
    axis: PD.MappedStatic('custom', {
        structure: PD.Group({
            principalAxis: PD.Select(
                'dirA',
                [['dirA', 'A'], ['dirB', 'B'], ['dirC', 'C']] as const,
            ),
        }),
        custom: PD.Group({
