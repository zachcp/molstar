...
namespace PluginStateAnimation {
    ...
    export type Duration = { kind: 'unknown' } | { kind: 'infinite' } | {
        kind: 'fixed';
        durationMs: number;
    };
    ...
    export type ApplyResult<S> = { kind: 'finished' } | { kind: 'skip' } | {
        kind: 'next';
        state: S;
    };
    ...
    export type Params<A extends PluginStateAnimation> = A extends PluginStateAnimation<infer P> ? P
        : never;
