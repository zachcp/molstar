...
interface PluginStateAnimation<P = any, S = any> {
  ...
  apply(
    state: S,
    t: PluginStateAnimation.Time,
    ctx: PluginStateAnimation.Context<P>,
  ): Promise<PluginStateAnimation.ApplyResult<S>>;
...
namespace PluginStateAnimation {
  export type CanApply =
    | { canApply: true }
    | { canApply: false; reason?: string };
  export type Duration =
    | { kind: "unknown" }
    | { kind: "infinite" }
    | { kind: "fixed"; durationMs: number };
  ...
  export type Params<A extends PluginStateAnimation> =
    A extends PluginStateAnimation<infer P> ? P : never;

  export function create<P, S>(
    params: PluginStateAnimation<P, S>,
  ): PluginStateAnimation<P, S> {
    return params;
  }

  export function getDuration<A extends PluginStateAnimation>(
    ctx: PluginContext,
    instance: Instance<A>,
  ): number | undefined {
