...
export const AnimateAssemblyUnwind = PluginStateAnimation.create({
    ...
    setup(params, _, plugin) {
        const state = plugin.state.data;
        const root = !params.target || params.target === 'all'
        ...
        for (const r of reprs) {
            ...
            changed = true;
            update.to(r)
                .apply(StateTransforms.Representation.UnwindStructureAssemblyRepresentation3D, {
                    t: 0,
    ...
    teardown(_, __, plugin) {
        ...
        const reprs = state.select(
            StateSelection.Generators.ofType(
                PluginStateObject.Molecule.Structure.Representation3DState,
            )
    ...
    async apply(animState, t, ctx) {
        ...
        const root = !ctx.params.target || ctx.params.target === 'all'
            ? StateTransform.RootRef
            : ctx.params.target;
