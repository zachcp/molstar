...
export const AnimateAssemblyUnwind = PluginStateAnimation.create({
    ...
    canApply(plugin) {
        ...
        const reprs = state.select(
            StateSelection.Generators.ofType(PluginStateObject.Molecule.Structure.Representation3D, root),
        );
    ...
    setup(params, _, plugin) {
        ...
        for (const r of reprs) {
            ...
            changed = true;
            update.to(r)
                .apply(StateTransforms.Representation.UnwindStructureAssemblyRepresentation3D, { t: 0 }, {
