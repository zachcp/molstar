...
export const AnimateModelIndex = PluginStateAnimation.create({
    ...
    params: () => ({
        ...
        duration: PD.MappedStatic('fixed', {
            fixed: PD.Group({
                durationInS: PD.Numeric(5, { min: 1, max: 120, step: 0.1 }, {
        ...
        if (p.duration?.name === 'fixed') {
            ...
            for (const m of models) {
                ...
                maxDuration = Math.max(
                    Math.ceil(1000 * traj.data.frameCount / p.duration.params.targetFps),
                    maxDuration,
                );
            }
    ...
    async apply(animState, t, ctx) {
        ...
        if (
            ctx.params.duration.name === 'sequential' && t.current > 0 &&
        ...
        for (const m of models) {
            ...
            update.to(m).update((old) => {
                ...
                if (params.duration.name === 'sequential') {
                    ...
                    isEnd = isEnd || (dir === -1 && modelIndex === 0) ||
                    ...
                    const modelIndex = Math.min(
                        Math.floor(traj.data.frameCount * phase),
                        traj.data.frameCount - 1,
                    );
                    return { modelIndex };
                }
            });
