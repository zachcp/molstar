...
import { parseDcd } from '../../mol-io/reader/dcd/parser.ts';
import { parseGRO } from '../../mol-io/reader/gro/parser.ts';
import { parsePDB } from '../../mol-io/reader/pdb/parser.ts';
import { Mat4, Vec3 } from '../../mol-math/linear-algebra.ts';
import { shapeFromPly } from '../../mol-model-formats/shape/ply.ts';
import { coordinatesFromDcd } from '../../mol-model-formats/structure/dcd.ts';
import { trajectoryFromGRO } from '../../mol-model-formats/structure/gro.ts';
import { trajectoryFromCCD, trajectoryFromMmCIF } from '../../mol-model-formats/structure/mmcif.ts';
import { trajectoryFromPDB } from '../../mol-model-formats/structure/pdb.ts';
import { topologyFromPsf } from '../../mol-model-formats/structure/psf.ts';
import {
    ArrayTrajectory,
    Coordinates,
    Frame,
...
import { StructureQueryHelper } from '../helpers/structure-query.ts';
import { StructureSelectionQueries } from '../helpers/structure-selection-query.ts';
...
import { trajectoryFromCifCore } from '../../mol-model-formats/structure/cif-core.ts';
import { trajectoryFromCube } from '../../mol-model-formats/structure/cube.ts';
import { parseMol2 } from '../../mol-io/reader/mol2/parser.ts';
import { trajectoryFromMol2 } from '../../mol-model-formats/structure/mol2.ts';
import { parseXtc } from '../../mol-io/reader/xtc/parser.ts';
import { coordinatesFromXtc } from '../../mol-model-formats/structure/xtc.ts';
import { parseXyz } from '../../mol-io/reader/xyz/parser.ts';
import { trajectoryFromXyz } from '../../mol-model-formats/structure/xyz.ts';
import { UnitStyles } from '../../mol-io/reader/lammps/schema.ts';
import { parseLammpsData } from '../../mol-io/reader/lammps/data/parser.ts';
import { trajectoryFromLammpsData } from '../../mol-model-formats/structure/lammps-data.ts';
import { parseLammpsTrajectory } from '../../mol-io/reader/lammps/traj/parser.ts';
import {
    coordinatesFromLammpsTrajectory,
    trajectoryFromLammpsTrajectory,
} from '../../mol-model-formats/structure/lammps-trajectory.ts';
import { parseSdf } from '../../mol-io/reader/sdf/parser.ts';
import { trajectoryFromSdf } from '../../mol-model-formats/structure/sdf.ts';
import { assertUnreachable } from '../../mol-util/type-helpers.ts';
import { parseTrr } from '../../mol-io/reader/trr/parser.ts';
import { coordinatesFromTrr } from '../../mol-model-formats/structure/trr.ts';
import { parseNctraj } from '../../mol-io/reader/nctraj/parser.ts';
import { coordinatesFromNctraj } from '../../mol-model-formats/structure/nctraj.ts';
import { topologyFromPrmtop } from '../../mol-model-formats/structure/prmtop.ts';
import { topologyFromTop } from '../../mol-model-formats/structure/top.ts';
...
const CoordinatesFromDcd = PluginStateTransform.BuiltIn({
    ...
    apply({ a }) {
        return Task.create('Parse DCD', async (ctx) => {
            ...
            return new SO.Molecule.Coordinates(coordinates, {
                label: a.label,
...
const CoordinatesFromLammpstraj = PluginStateTransform.BuiltIn({
    ...
    apply({ a }) {
        return Task.create('Parse LAMMPSTRAJ', async (ctx) => {
            ...
            if (parsed.isError) throw new Error(parsed.message);
            const coordinates = await coordinatesFromLammpsTrajectory(parsed.result).runInContext(
                ctx,
            );
            return new SO.Molecule.Coordinates(coordinates, {
                label: a.label,
...
const TopologyFromPsf = PluginStateTransform.BuiltIn({
    ...
    apply({ a }) {
        return Task.create('Create Topology', async (ctx) => {
            ...
            return new SO.Molecule.Topology(topology, {
...
async function getTrajectory(ctx: RuntimeContext, obj: StateObject, coordinates: Coordinates) {
    if (obj.type === SO.Molecule.Topology.type) {
        ...
        return await Model.trajectoryFromTopologyAndCoordinates(topology, coordinates).runInContext(
            ctx,
        );
...
const TrajectoryFromMmCif = PluginStateTransform.BuiltIn({
    ...
    params(a) {
        if (!a) {
            return {
                ...
                blockHeader: PD.Optional(
                    PD.Text(void 0, {
                        description:
                            'Header of the block to parse. If not specifed, Block Index parameter applies.',
                        hideIf: (p) => p.loadAllBlocks === true,
                    }),
                ),
                blockIndex: PD.Optional(
                    PD.Numeric(0, { min: 0, step: 1 }, {
                        description:
        ...
        return {
            ...
            blockIndex: PD.Optional(
                PD.Numeric(0, { min: 0, step: 1, max: blocks.length - 1 }, {
                    description:
                        'Zero-based index of the block to parse. Only applies when Block Header parameter is not specified.',
                    hideIf: (p) => p.loadAllBlocks === true || p.blockHeader,
                }),
    ...
    apply({ a, params }) {
        return Task.create('Parse mmCIF', async (ctx) => {
            ...
            if (params.loadAllBlocks) {
                ...
                const isCcd = block.categoryNames.includes('chem_comp_atom') &&
...
const TrajectoryFromLammpsData = PluginStateTransform.BuiltIn({
    ...
    display: {
        name: 'Parse Lammps Data',
        description: 'Parse Lammps Data from string and create trajectory.',
    },
    from: [SO.Data.String],
    to: SO.Molecule.Trajectory,
    params: {
    ...
    apply({ a, params }) {
        return Task.create('Parse Lammps Data', async (ctx) => {
            ...
            if (parsed.isError) throw new Error(parsed.message);
            const models = await trajectoryFromLammpsData(parsed.result, params.unitsStyle)
                .runInContext(ctx);
            const props = trajectoryProps(models);
            return new SO.Molecule.Trajectory(models, props);
        });
...
const TrajectoryFromLammpsTrajData = PluginStateTransform.BuiltIn({
    ...
    display: {
        name: 'Parse Lammps traj Data',
        description: 'Parse Lammps Traj Data string and create trajectory.',
    },
    from: [SO.Data.String],
    to: SO.Molecule.Trajectory,
    params: {
    ...
    apply({ a, params }) {
        return Task.create('Parse Lammps Data', async (ctx) => {
            const parsed = await parseLammpsTrajectory(a.data).runInContext(ctx);
            if (parsed.isError) throw new Error(parsed.message);
            const models = await trajectoryFromLammpsTrajectory(parsed.result, params.unitsStyle)
                .runInContext(ctx);
            const props = trajectoryProps(models);
            return new SO.Molecule.Trajectory(models, props);
        });
    ...
    params: (a) => {
        ...
        return {
            modelIndex: PD.Converted(
                plus1,
                minus1,
                PD.Numeric(1, { min: 1, max: a.data.frameCount, step: 1 }, {
...
const StructureFromModel = PluginStateTransform.BuiltIn({
    ...
    display: {
        name: 'Structure',
        description:
            'Create a molecular structure (model, assembly, or symmetry) from the specified model.',
    },
...
const TransformStructureConformation = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        ...
        if (params.transform.name === 'components') {
            ...
            const rot = Mat4.fromRotation(
                Mat4(),
                Math.PI / 180 * angle,
                Vec3.normalize(Vec3(), axis),
            );
...
const ModelWithCoordinates = PluginStateTransform.BuiltIn({
    ...
    display: {
        name: 'Model With Coordinates',
        description: 'Updates the current model with provided coordinate frame',
    },
    from: SO.Molecule.Model,
    to: SO.Molecule.Model,
    params: {
        ...
        atomicCoordinateFrame: PD.Optional(
            PD.Value<Frame | undefined>(undefined, { isHidden: true }),
        ),
    },
    isDecorator: true,
})({
    apply({ a, params }) {
        ...
        const model: Model = {
            ...a.data,
            atomicConformation: Model.getAtomicConformationFromFrame(
                a.data,
                params.atomicCoordinateFrame,
            ),
        };
        Model.TrajectoryInfo.set(model, {
            index: params.frameIndex ?? 0,
            size: params.frameCount ?? 1,
        });
    ...
    update: ({ a, b, oldParams, newParams }) => {
        ...
        if (!newParams.atomicCoordinateFrame) {
            ...
            b.data = {
                ...
                atomicConformation: Model.getAtomicConformationFromFrame(
                    b.data,
                    newParams.atomicCoordinateFrame,
                ),
            };
        }
        Model.TrajectoryInfo.set(b.data, {
            index: newParams.frameIndex ?? 0,
            size: newParams.frameCount ?? 1,
        });
        return StateTransformer.UpdateResult.Updated;
    },
});
    ...
    params: () => ({
        ...
        isTransitive: PD.Optional(
            PD.Boolean(false, {
                isHidden: true,
                description:
                    'Remap the selections from the original structure if structurally equivalent.',
            }),
        ),
        label: PD.Optional(PD.Text('', { isHidden: true })),
    }),
    ...
    update: ({ b, oldParams, newParams, cache, dependencies }) => {
        ...
        for (const sel of newParams.selections) {
            ...
            if (cacheEntries.has(sel.key)) {
                ...
                if (
                    StructureQueryHelper.isUnchanged(entry, sel.expression, structure) &&
                    current.has(sel.key)
                ) {
                ...
                if (entry.expression !== sel.expression) {
                    ...
                    if (!!newParams.isTransitive) {
                        if (
                            Structure.areUnitIdsAndIndicesEqual(entry.originalStructure, structure)
                        ) {
                            const selection = StructureQueryHelper.run(
                                entry,
                            ...
                            const loci = StructureElement.Loci.remap(
                                Sel.toLociWithSourceUnits(selection),
                                structure,
                            );
                            selections.push({
                                key: sel.key,
                                structureRef: sel.ref,
                                loci,
                                groupId: sel.groupId,
                            });
                            totalSize += StructureElement.Loci.size(loci);
    ...
    update: ({ b, oldParams, newParams, cache, dependencies }) => {
        ...
        for (const sel of newParams.selections) {
            ...
            if (cacheEntries.has(sel.key)) {
                ...
                if (
                    prev && source === entry.source && sel.bundle.hash === entry.source.hashCode &&
...
const StructureComplexElement = PluginStateTransform.BuiltIn({
    ...
    to: SO.Molecule.Structure,
    params: {
        type: PD.Select<StructureComplexElementTypes>(
            'atomic-sequence',
            StructureComplexElementTypeTuples,
            { isHidden: true },
        ),
    },
})({
...
const CustomStructureProperties = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }, ctx: PluginContext) {
        return Task.create('Custom Props', async (taskCtx) => {
            ...
            return new SO.Molecule.Structure(a.data, {
                label: a.label,
