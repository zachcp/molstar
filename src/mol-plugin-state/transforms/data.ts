...
const Download = PluginStateTransform.BuiltIn({
    ...
    apply({ params: p, cache }, plugin: PluginContext) {
        return Task.create('Download', async (ctx) => {
            ...
            const asset = await plugin.managers.asset.resolve(url, p.isBinary ? 'binary' : 'string')
                .runInContext(ctx);
            (cache as any).asset = asset;
            return p.isBinary
                ? new SO.Data.Binary(asset.data as Uint8Array, {
    ...
    update({ oldParams, newParams, b }) {
        ...
        if (oldParams.label !== newParams.label) {
            b.label = newParams.label ||
                ((typeof newParams.url === 'string') ? newParams.url : newParams.url.url);
...
const DownloadBlob = PluginStateTransform.BuiltIn({
    ...
    params: {
        sources: PD.ObjectList({
            ...
            canFail: PD.Optional(
                PD.Boolean(false, {
                    description:
                        'Indicate whether the download can fail and not be included in the blob as a result.',
                }),
            ),
        }, (e) => `${e.id}: ${e.url}`),
        maxConcurrency: PD.Optional(
            PD.Numeric(4, { min: 1, max: 12, step: 1 }, {
    ...
    apply({ params, cache }, plugin: PluginContext) {
        return Task.create('Download Blob', async (ctx) => {
            ...
            const data = await ajaxGetMany(
                ctx,
                plugin.managers.asset,
                params.sources,
    ...
    apply({ params: p }) {
        return Task.create('Raw Data', async () => {
            if (typeof p.data === 'string') {
                return new SO.Data.String(p.data as string, {
                    label: p.label ? p.label : 'String',
                });
            } else if (Array.isArray(p.data)) {
                return new SO.Data.Binary(new Uint8Array(p.data), {
...
const ReadFile = PluginStateTransform.BuiltIn({
    ...
    apply({ params: p, cache }, plugin: PluginContext) {
        return Task.create('Open File', async (ctx) => {
            ...
            const asset = await plugin.managers.asset.resolve(
                p.file,
                p.isBinary ? 'binary' : 'string',
            ).runInContext(ctx);
            (cache as any).asset = asset;
            const o = p.isBinary
                ? new SO.Data.Binary(asset.data as Uint8Array, {
                ...
                : new SO.Data.String(asset.data as string, {
                    label: p.label ? p.label : p.file.name,
                });
    ...
    apply({ a, params }, plugin: PluginContext) {
        return Task.create('Parse Blob', async (ctx) => {
            ...
            for (const e of a.data) {
                ...
                const parsed =
                    await (e.kind === 'string' ? CIF.parse(e.data) : CIF.parseBinary(e.data))
                        .runInContext(ctx);
    ...
    apply({ a, params }) {
        return Task.create('Lazy Volume', async (ctx) => {
            const entryId = Array.isArray(params.entryId)
                ? params.entryId.join(', ')
                : params.entryId;
            return new SO.Volume.Lazy(params, {
