...
const VolumeFromCcp4 = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        return Task.create('Create volume from CCP4/MRC/MAP', async (ctx) => {
            const volume = await volumeFromCcp4(a.data, {
                ...params,
            ...
            const props = {
                ...
                description:
                    `Volume ${a.data.header.NX}\u00D7${a.data.header.NX}\u00D7${a.data.header.NX}`,
            };
...
const VolumeFromDsn6 = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        return Task.create('Create volume from DSN6/BRIX', async (ctx) => {
            const volume = await volumeFromDsn6(a.data, {
                ...params,
                label: a.data.name || a.label,
            }).runInContext(ctx);
            const props = {
...
const VolumeFromCube = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        return Task.create('Create volume from Cube', async (ctx) => {
            ...
            const props = {
                label: volume.label || 'Volume',
                description: `Volume ${a.data.header.dim[0]}\u00D7${a.data.header.dim[1]}\u00D7${
                    a.data.header.dim[2]
                }`,
            };
            return new SO.Volume.Data(volume, props);
...
const VolumeFromDx = PluginStateTransform.BuiltIn({
    ...
    apply({ a }) {
        return Task.create('Parse DX', async (ctx) => {
            const volume = await volumeFromDx(a.data, { label: a.data.name || a.label })
                .runInContext(ctx);
            const props = {
                label: volume.label || 'Volume',
                description: `Volume ${a.data.header.dim[0]}\u00D7${a.data.header.dim[1]}\u00D7${
...
const VolumeFromDensityServerCif = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        return Task.create('Parse density-server CIF', async (ctx) => {
            ...
            const volume = await volumeFromDensityServerData(densityServerCif, {
...
const VolumeFromSegmentationCif = PluginStateTransform.BuiltIn({
    ...
    params(a) {
        ...
        return {
            ...
            segmentLabels: PD.ObjectList(
                { id: PD.Numeric(-1), label: PD.Text('') },
                (s) => `${s.id} = ${s.label}`,
...
export const VolumeInstances = PluginStateTransform.BuiltIn({
    ...
    apply({ a, params }) {
        const instances = params.transforms.map((t) => ({
            transform: getTransformFromParams(t.transform),
        }));
        if (!instances.length) {
            return a;
        }
        return new SO.Volume.Data({
            ...a.data,
            instances: params.transforms.map((t) => ({
                transform: getTransformFromParams(t.transform),
            })),
        }, {
            label: a.label,
