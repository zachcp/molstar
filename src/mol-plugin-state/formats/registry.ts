...
import type { DataFormatProvider } from "./provider.ts";
...
export class DataFormatRegistry {
  ...
  get types(): [name: string, label: string][] {
    return this._list.map(
      (e) => [e.name, e.provider.label] as [name: string, label: string],
    );
  }
  ...
  auto(
    ...
    for (let i = 0, il = this.list.length; i < il; ++i) {
      ...
      if (
        hasExt &&
        (!p.isApplicable || p.isApplicable(info, dataStateObject.data))
      )
        return p;
    }
    return;
