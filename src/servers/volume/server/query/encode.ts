...
import { ArrayEncoder, ArrayEncoding } from '../../../../mol-io/common/binary-cif.ts';
import {
...
export function encode(query: Data.QueryContext, output: Data.QueryOutputStream) {
    const w = CifWriter.createEncoder({
        binary: query.params.asBinary,
        encoderName: `VolumeServer ${VERSION}`,
    });
    write(w, query);
    w.encode();
    w.writeTo(output);
...
function float64<T>(
    ...
    return CifWriter.Field.float(name, (i, d) => value(d), {
...
const _volume_data_3d_info: CifWriter.Category<ResultContext> = {
    ...
    instance(result) {
        const ctx: _vd3d_Ctx = {
            ...
            sampledValuesInfo:
                result.query.data.header.sampling[result.query.samplingInfo.sampling.index]
                    .valuesInfo[result.channelIndex],
        };
...
const _volume_data_3d: CifWriter.Category<ResultContext> = {
    ...
    instance(ctx) {
        ...
        let typedArray: any;
        if (
            ctx.query.data.header.valueType === TypedArrayValueType.Float32 ||
            ctx.query.data.header.valueType === TypedArrayValueType.Int16
        ) {
        ...
        const fields = [
            CifWriter.Field.float('values', _volume_data_3d_number, {
                encoder,
                typedArray,
                digitCount: 6,
            }),
...
function queryBoxDimension(e: 'a' | 'b', d: number) {
    return string<Data.QueryContext>(
        `query_box_${e}[${d}]`,
        (ctx) => pickQueryBoxDimension(ctx, e, d),
        (ctx) => ctx.params.box.kind !== 'Cell',
    );
}

const _density_server_result_fields = [
    string<Data.QueryContext>('server_version', (ctx) => VERSION),
    string<Data.QueryContext>(
        'datetime_utc',
        (ctx) => new Date().toISOString().replace(/T/, ' ').replace(/\..+/, ''),
    ),
    string<Data.QueryContext>('guid', (ctx) => ctx.guid),
    string<Data.QueryContext>(
        'is_empty',
        (ctx) => ctx.kind === 'Empty' || ctx.kind === 'Error' ? 'yes' : 'no',
    ),
    string<Data.QueryContext>('has_error', (ctx) => ctx.kind === 'Error' ? 'yes' : 'no'),
    string<Data.QueryContext>(
        'error',
        (ctx) => ctx.kind === 'Error' ? ctx.message : '',
        (ctx) => ctx.kind === 'Error',
    ),
...
const _density_server_result: CifWriter.Category<Data.QueryContext> = {
    ...
    instance: (ctx) =>
        CifWriter.categoryInstance(_density_server_result_fields, { data: ctx, rowCount: 1 }),
};
