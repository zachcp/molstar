...
import { type ArrayEncoder, ArrayEncoding } from '../../../../mol-io/common/binary-cif.ts';
import { type TypedArrayValueArray, TypedArrayValueType } from '../../../../mol-io/common/typed-array.ts';
...
function string<T>(
    ...
    if (isSpecified) {
        return CifWriter.Field.str(name, (i, d) => str(d), {
            valueKind: (i, d) => isSpecified(d) ? Column.ValueKinds.Present : Column.ValueKinds.NotPresent,
        });
    }
    return CifWriter.Field.str(name, (i, d) => str(d));
}
...
const _volume_data_3d_info: CifWriter.Category<ResultContext> = {
    ...
    instance(result) {
        const ctx: _vd3d_Ctx = {
            ...
            sampledValuesInfo: result.query.data.header.sampling[result.query.samplingInfo.sampling.index]
...
const _volume_data_3d: CifWriter.Category<ResultContext> = {
    ...
    instance(ctx) {
        ...
        const fields = [
            CifWriter.Field.float('values', _volume_data_3d_number, { encoder, typedArray, digitCount: 6 }),
        ];
