...
async function readBlock(
    ...
    blockBox: Box.Fractional,
): Promise<Data.BlockData> {
    ...
    const blockSampleCount = Box.dimensions(
        Box.fractionalToGrid(blockBox, query.samplingInfo.sampling.dataDomain),
    );
    ...
    const dataSampleCount =
        query.data.header.sampling[query.samplingInfo.sampling.index].sampleCount;
...
function fillData(
    ...
    blockGridBox: Box.Grid<'BlockGrid'>,
    queryGridBox: Box.Grid<'Query'>,
) {
    const source = blockData.values;

    const { sizeX: tSizeH, sizeXY: tSizeHK } = Coords.gridMetrics(
        query.samplingInfo.gridDomain.sampleCount,
    );
    const { sizeX: sSizeH, sizeXY: sSizeHK } = Coords.gridMetrics(blockData.sampleCount);

    const offsetTarget = queryGridBox.a[0] + queryGridBox.a[1] * tSizeH +
        queryGridBox.a[2] * tSizeHK;

    const [maxH, maxK, maxL] = Box.dimensions(blockGridBox);

    for (
        let channelIndex = 0, _ii = query.data.header.channels.length;
        channelIndex < _ii;
        channelIndex++
    ) {
...
function createBlockGridDomain(
    block: Coords.Grid<'Block'>,
    grid: Coords.GridDomain<'Data'>,
): Coords.GridDomain<'BlockGrid'> {
    ...
    return Coords.domain<'BlockGrid'>('BlockGrid', {
        origin,
        dimensions,
        delta: grid.delta,
        sampleCount,
...
async function fillBlock(query: Data.QueryContext.Data, block: Data.QueryBlock) {
    ...
    const blockGridDomain = createBlockGridDomain(
        block.coord,
        query.samplingInfo.sampling.dataDomain,
    );
    ...
    for (const offset of block.offsets) {
        ...
        const queryGridBox = Box.clampGridToSamples(
            Box.fractionalToGrid(offsetDataBox, query.samplingInfo.gridDomain),
        );
