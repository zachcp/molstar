...
export function getSchema() {
    function detail(i: number) {
        return `${i} (${
            Math.round(100 * LimitsConfig.maxOutputSizeInVoxelCountByPrecisionLevel[i] / 1000 / 1000) / 100
        }M voxels)`;
    }
                ...
                get: {
                    ...
                    parameters: [
                        ...
                        { $ref: '#/components/parameters/id' },
                        {
                            ...
                            in: 'path',
                            description:
                                'Bottom left corner of the query region in Cartesian or fractional coordinates (determined by the `space` query parameter).',
                            required: true,
                            schema: {
                                type: 'list',
                                items: {
                        ...
                        },
                        {
                            ...
                            in: 'path',
                            description:
                                'Top right corner of the query region in Cartesian or fractional coordinates (determined by the `space` query parameter).',
                            required: true,
                            schema: {
                                type: 'list',
                                items: {
                        ...
                        { $ref: '#/components/parameters/detail' },
                        {
                            ...
                            in: 'query',
                            description:
                                'Determines the coordinate space the query is in. Can be cartesian or fractional. An optional argument, default values is cartesian.',
                            schema: {
                                type: 'string',
        ...
        components: {
            ...
            parameters: {
                source: {
                    ...
                    in: 'path',
                    description:
                        `Specifies the data source (determined by the experiment method). Currently supported sources are: ${
                            sources.join(', ')
                        }.`,
                    required: true,
                    schema: {
                ...
                id: {
                    ...
                    in: 'path',
                    description:
                        'Id of the entry. For x-ray, use PDB ID (i.e. 1cbs) and for em use EMDB id (i.e. emd-8116).',
                    required: true,
                    schema: {
                        type: 'string',
                    },
                ...
                encoding: {
                    ...
                    in: 'query',
                    description:
                        'Determines if text based CIF or binary BinaryCIF encoding is used. An optional argument, default is BinaryCIF encoding.',
                    schema: {
                        type: 'string',
                        enum: ['cif', 'bcif'],
                    },
                ...
                detail: {
                    ...
                    in: 'query',
                    description:
                        `Determines the maximum number of voxels the query can return. Possible values are in the range from ${
                            detail(0)
                        } to ${
                            detail(detailMax)
                        }. Default value is 0. Note: different detail levels might lead to the same result.`,
                    schema: {
                        type: 'integer',
                    },
                    style: 'form',
