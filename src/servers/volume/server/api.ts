...
export function getOutputFilename(
    source: string,
    id: string,
    { asBinary, box, detail, forcedSamplingLevel }: Data.QueryParams,
) {
    function n(s: string) {
        return (s || '').replace(/[ \n\t]/g, '').toLowerCase();
    }
    ...
    const det = forcedSamplingLevel !== void 0 ? `l${forcedSamplingLevel}` : `d${
        Math.min(
            Math.max(0, detail | 0),
    ...
    const boxInfo = box.kind === 'Cell'
        ? 'cell'
        : `${box.kind === 'Cartesian' ? 'cartn' : 'frac'}_${r(box.a[0])}_${r(box.a[1])}_${
            r(box.a[2])
        }_${r(box.b[0])}_${r(box.b[1])}_${r(box.b[2])}`;
