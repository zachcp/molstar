...
export function init(app: express.Express) {
    ...
    app.get(
        makePath(':source/:id/cell/'),
        (req, res) => queryBox(req, res, getQueryParams(req, true)),
    );
...
function validateSourceAndId(req: express.Request, res: express.Response) {
    if (
        !req.params.source || req.params.source.length > 32 || !req.params.id ||
...
function getQueryParams(req: express.Request, isCell: boolean): Data.QueryParams {
    ...
    const box: Data.QueryParamsBox = isCell ? { kind: 'Cell' } : (isCartesian
        ? {
            kind: 'Cartesian',
            a: Coords.cartesian(a[0], a[1], a[2]),
            b: Coords.cartesian(b[0], b[1], b[2]),
        }
        : {
            kind: 'Fractional',
            a: Coords.fractional(a[0], a[1], a[2]),
            b: Coords.fractional(b[0], b[1], b[2]),
        });
...
async function queryBox(req: express.Request, res: express.Response, params: Data.QueryParams) {
    ...
    try {
        ...
        ConsoleLogger.error(
            `Query Box ${JSON.stringify(req.params || {})} | ${JSON.stringify(req.query || {})}`,
            e,
        );
        response.do404();
    } finally {
