...
export const CommonQueryParamsInfo: QueryParamInfo[] = [
    {
        ...
        description:
            `A comma-separated list of model ids (i.e. 1,2). If set, only include atoms with the corresponding '_atom_site.pdbx_PDB_model_num' field.`,
    },
    {
        name: 'encoding',
        type: QueryParamType.String,
        defaultValue: 'cif',
        description:
            `Determines the output encoding (text based 'CIF' or binary 'BCIF'). Ligands can also be exported as 'SDF', 'MOL', or 'MOL2'.`,
        supportedValues: ['cif', 'bcif', 'sdf', 'mol', 'mol2'],
    },
    {
    ...
    },
    {
        ...
        defaultValue: '',
        description:
            'Allows to control how the provided data source ID maps to input file (as specified by the server instance config).',
    },
    {
        name: 'transform',
        type: QueryParamType.String,
        description:
            `Transformation to apply to coordinates in '_atom_site'. Accepts a 4x4 transformation matrix, provided as array of 16 float values.`,
    },
    {
        name: 'download',
        type: QueryParamType.Boolean,
    ...
    },
    {
        ...
        defaultValue: '',
        description:
            `Controls the filename for downloaded files. Will force download if specified.`,
    },
];
...
const AtomSiteTestJsonParam: QueryParamInfo = {
    ...
    description:
        'Object or array of objects describing atom properties. Names are same as in wwPDB mmCIF dictionary of the atom_site category.',
    exampleValues: [[{ label_seq_id: 30, label_asym_id: 'A' }, {
        label_seq_id: 31,
        label_asym_id: 'A',
    }], { label_comp_id: 'ALA' }],
};
...
const RadiusParam: QueryParamInfo = {
    ...
    validation(v: any) {
        if (v < 1 || v > 10) {
            throw new Error(
                'Invalid radius for residue interaction query (must be a value between 1 and 10).',
            );
        }
    },
};
...
const AssemblyNameParam: QueryParamInfo = {
    ...
    type: QueryParamType.String,
    description:
        'Assembly name. If none is provided, crystal symmetry (where available) or deposited model is used.',
};
...
const QueryMap = {
    ...
    'ligand': Q<{ atom_site: AtomSiteSchema }>({
        ...
        query: (p, _s, numModels) => {
            ...
            const ligands = Queries.combinators.merge(tests.map((test) =>
                Queries.generators.atoms({
                    ...test,
                    unitTest: (ctx) =>
                        StructureProperties.unit.model_num(ctx.element) === numModels[0],
    ...
    'atoms': Q<{ atom_site: AtomSiteSchema }>({
        ...
        query: (p) => {
            return Queries.combinators.merge(
                getAtomsTests(p.atom_site).map((test) => Queries.generators.atoms(test)),
            );
        },
        jsonParams: [AtomSiteTestJsonParam],
        restParams: AtomSiteTestRestParams,
    ...
    'residueInteraction': Q<{ atom_site: AtomSiteSchema; radius: number; assembly_name: string }>({
        ...
        query(p) {
            const tests = getAtomsTests(p.atom_site);
            const center = Queries.combinators.merge(tests.map((test) =>
                Queries.generators.atoms({
                    ...test,
                    entityTest: test.entityTest
                        ? (ctx) =>
                            test.entityTest!(ctx) &&
                            ctx.element.unit.conformation.operator.isIdentity
                        : (ctx) => ctx.element.unit.conformation.operator.isIdentity,
                })
            ));
            return Queries.modifiers.includeSurroundings(center, {
                radius: p.radius !== void 0 ? p.radius : 5,
                wholeResidues: true,
            });
        },
        structureTransform(p, s) {
            ...
            return StructureSymmetry.builderSymmetryMates(s, p.radius !== void 0 ? p.radius : 5)
                .run();
        },
    ...
    'residueSurroundings': Q<{ atom_site: AtomSiteSchema; radius: number }>({
        ...
        query(p) {
            const center = Queries.combinators.merge(
                getAtomsTests(p.atom_site).map((test) => Queries.generators.atoms(test)),
            );
            return Queries.modifiers.includeSurroundings(center, {
                radius: p.radius,
    ...
    'surroundingLigands': Q<
        { atom_site: AtomSiteSchema; radius: number; assembly_name: string; omit_water: boolean }
    >({
...
export const QueryList = (function () {
    ...
    for (const k of Object.keys(QueryMap)) {
        list.push({ name: k, definition: <QueryDefinition> QueryMap[k as QueryName] });
    }
...
function _normalizeQueryParams(
    params: { [p: string]: string },
    paramList: QueryParamInfo[],
...
export function normalizeRestCommonParams(params: any): CommonQueryParamsInfo {
    return {
        model_nums: params.model_nums
            ? ('' + params.model_nums).split(',').map((n) => n.trim()).filter((n) => !!n).map((n) =>
                +n
            )
            : void 0,
        ...
        transform: params.transform
            ? ('' + params.transform).split(',').map((n) => n.trim()).map((n) => +n) as Mat4
            : Mat4.identity(),
