...
export let mapSourceAndIdToFilename: (
    source: string,
    id: string,
) => [string, ModelServerFetchFormats] = () => {
    throw new Error(
        'call setupConfig & validateConfigAndSetupSourceMap to initialize this function',
    );
};
...
function addServerArgs(parser: argparse.ArgumentParser) {
    ...
    parser.add_argument('--sourceMapUrl', {
        ...
        help: [
            ...
            `Supported formats: ${
                ModelServerFetchFormats.join(', ')
            }. Supported protocols: http://, https://, gs://`,
        ].join('\n'),
    });
...
function validateConfigAndSetupSourceMap() {
    if (!ModelServerConfig.sourceMap || ModelServerConfig.sourceMap.length === 0) {
        throw new Error(
            `Please provide 'sourceMap' configuration. See [-h] for available options.`,
        );
    }
