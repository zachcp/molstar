...
import { PreprocessEntry, runMaster } from './parallel.ts';
...
cmdParser.add_argument('--folderOutCIF', '-foc', {
    help: 'Convert folder text output',
    required: false,
});
cmdParser.add_argument('--folderOutBCIF', '-fob', {
    help: 'Convert folder binary output',
    required: false,
});
cmdParser.add_argument('--folderNumProcesses', '-fp', {
    help: 'Convert folder num processes',
    required: false,
});
...
if (
    Object.keys(cmdArgs).filter((k) => (cmdArgs as any)[k] !== null).length === 0 ||
    typeof cmdArgs.help !== 'undefined'
) {
...
function findEntries() {
    ...
    for (const f of files) {
        ...
        entries.push({
            ...
            bcif: cmdArgs.folderOutBCIF
                ? path.join(cmdArgs.folderOutBCIF!, path.parse(f).name + '.bcif')
                : void 0,
        });
    }
}
