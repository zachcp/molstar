...
import { EncodedData, Encoding } from './encoding.ts';
import { flipByteOrder, IsNativeEndianLittle } from '../binary.ts';
...
function integerPacking(
    data: Int8Array | Int16Array | Uint8Array | Uint16Array,
    encoding: Encoding.IntegerPacking,
) {
    if (data.length === encoding.srcSize) return data;
    return encoding.isUnsigned
        ? integerPackingUnsigned(data as any, encoding)
        : integerPackingSigned(data as any, encoding);
}
