...
function record(
    buffer: IOBuffer,
    variable: { type: string; size: number },
    recordDimension: NetCDFRecordDimension,
) {
    // variable type
    const type = str2num(variable.type);
...
function header(buffer: IOBuffer, version: number) {
    ...
    // List of dimensions
    const dimList = dimensionsList(buffer) as {
    ...
    // List of variables
    const variables = variablesList(buffer, dimList.recordId, version) as {
        variables: any[];
        recordStep: number;
    };
...
export class NetcdfReader {
    ...
    hasDataVariable(variableName: string) {
        return this.header.variables &&
            this.header.variables.findIndex((val) => val.name === variableName) !== -1;
    }
