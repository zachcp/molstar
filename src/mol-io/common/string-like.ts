...
export class ChunkedBigString implements StringLikeInterface {
  ...
  static fromUtf8Data(
    ...
    while (readStart < end) {
      ...
      if (readEnd < end) {
        ...
        while ((data[readEnd] & 0xc0) === 0x80) {
          // Byte after the cut is a continuation byte (10xxxxxx)
          readEnd--;
  ...
  substring(start?: number, end?: number): string {
    // optional `start` not part of contract but works in Chrome
    ...
    if (end_ - start_ > MAX_STRING_LENGTH) {
      throw new Error(
        `Trying to create get a substring longer (${end_ - start_}) than maximum allowed string length (${MAX_STRING_LENGTH}).`,
      );
    }
    ...
    if (iFirstChunk === iLastChunk) {
      return this._chunks[iFirstChunk].substring(
        indexInChunkFrom,
        indexInChunkTo,
      );
    } else {
      ...
      out.push(
        this._chunks[iFirstChunk].substring(
          indexInChunkFrom,
          this.STRING_CHUNK_SIZE,
        ),
      );
  ...
  indexOf(searchString: string, position: number = 0): number {
    ...
    for (let iChunk = iFirstChunk; iChunk < this._chunks.length; iChunk++) {
      ...
      const positionInChunk =
        iChunk === iFirstChunk ? this._getIndexInChunk(position) : 0;
      ...
      if (iChunk !== this._chunks.length - 1) {
        ...
        const aroundBoundary =
          chunk.substring(start, undefined) +
  ...
  startsWith(searchString: string, position: number = 0): boolean {
    ...
    return (
      this.substring(position, position + searchString.length) === searchString
    );
  }
