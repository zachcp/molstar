...
import {
    ...
    type EncodedColumn,
    type EncodedData,
    type EncodedDataBlock,
    type EncodedFile,
    VERSION,
} from '../../../common/binary-cif.ts';
import { Category, type Encoder, Field } from '../encoder.ts';
import type { Writer } from '../../writer.ts';
import { type CategoryInstanceData, getCategoryInstanceData, getIncludedFields } from './util.ts';
...
export class BinaryEncoder implements Encoder<Uint8Array<ArrayBuffer>> {
    ...
    writeCategory<Ctx>(category: Category<Ctx>, context?: Ctx, options?: Encoder.WriteCategoryOptions) {
        ...
        for (const f of fields) {
            ...
            cat.columns.push(
                encodeField(category.name, f, source, rowCount, format, this.binaryEncodingProvider, this.autoClassify),
            );
        }
        // no columns included.
