...
import { Category, Field } from '../encoder.ts';
...
export function getIncludedFields(category: Category.Instance) {
    return category.fields.some((f) => !!f.shouldInclude)
        ? category.fields.filter((f) =>
            !f.shouldInclude || category.source.some((src) => f.shouldInclude!(src.data))
        )
        : category.fields;
}
...
export function getCategoryInstanceData<Ctx>(
    category: Category<Ctx>,
    ctx?: Ctx,
): CategoryInstanceData<Ctx> {
