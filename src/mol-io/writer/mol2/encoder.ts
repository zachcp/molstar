...
export class Mol2Encoder extends LigandEncoder {
    ...
    _writeCategory<Ctx>(category: Category<Ctx>, context?: Ctx): void {
        ...
        atoms.forEach((atom1, label_atom_id1) => {
                ...
                bondMap.map.get(label_atom_id1)!.forEach((bond, label_atom_id2) => {
                    ...
                    if (i1 < i2) {
                        ...
                        StringBuilder.writeSafe(
                            b,
                            `${++bondCount} ${i1 + 1} ${i2 + 1} ${ar ? 'ar' : order}`,
                        );
            ...
            const sybyl = bondMap?.map
                ? this.mapToSybyl(label_atom_id1, type_symbol1, bondMap)
                : type_symbol1;
            StringBuilder.writeSafe(
                a,
                `${i1 + 1} ${label_atom_id1} ${atom1.Cartn_x.toFixed(3)} ${
                    atom1.Cartn_y.toFixed(3)
                } ${atom1.Cartn_z.toFixed(3)} ${sybyl} 1 ${name} 0.000\n`,
            );
            atomCount++;
        });
    ...
    private count<K, V, C>(
        map: Map<K, V>,
        ctx: C,
        predicate: (k: K, v: V, ctx: C) => boolean,
    ): number {
        let count = 0;
    ...
    private mapToSybyl(label_atom_id1: string, type_symbol1: string, bondMap: ComponentBond.Entry) {
        ...
        if (type_symbol1 === 'C') { // 1.6
            ...
            if (
                numBonds >= 2 &&
                this.count(bonds, this, (_k, v) => BondType.is(BondType.Flag.Aromatic, v.flags)) >=
                    2
            ...
            if (
                (numBonds === 1 || numBonds === 2) &&
                this.count(bonds, this, (_k, v) => v.order === 3)
        ...
        const nonmets =
            this.count(bonds, this, (k, _v, ctx) => ctx.isNonMetalBond(k)) === bonds.size
                ? bonds
        ...
        if (type_symbol1 === 'O') { // 1.7
            ...
            if (
                numNonmets >= 2 && this.count(bonds, this, (_k, v) => v.order === 1) === bonds.size
        ...
        while (!result.done) {
            ...
            if (
                this.count(
                    adjacentBonds,
                    this,
                    (_k, v) => v.order > 1 || BondType.is(BondType.Flag.Aromatic, v.flags),
                )
            ) {
    ...
    private isOC(nonmets: BondMap, bondMap: ComponentBond.Entry): boolean {
        const nonmet = nonmets.entries().next()!.value as [
            string,
            { order: number; flags: number },
        ];
            ...
            if (label_atom_id.startsWith('C')) {
                ...
                if (
                    this.count(
                        adjacentBonds,
                        this,
                        (k, v) => k.startsWith('O') || k.startsWith('S') && v.order === 2,
                    )
                ) return true;
            }
            result = iter.next();
