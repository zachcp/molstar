...
function eatTripleQuote(state: TokenizerState) {
    ...
    while (state.position < state.length) {
        if (
            state.data.charCodeAt(state.position) === 39 /* ' */ && isTripleQuoteAtPosition(state)
        ) {
            // get rid of the quotes.
            state.tokenStart += 3;
            state.tokenEnd = state.position;
    ...
    switch (c) {
        ...
        default:
            ...
            if (state.isEscaped) {
                ...
                state.tokenEnd - state.tokenStart >= 5 &&
                state.data.charCodeAt(state.tokenStart + 4) === 95
            ) {
...
function CifCategories(
    categoryNames: string[],
    categoryData: { [name: string]: CifCategoryData },
): { [name: string]: Data.CifCategory } {
...
function CifBlock(ctx: FrameContext, header: string, saveFrames?: Data.CifFrame[]): Data.CifBlock {
    return Data.CifBlock(
        ctx.categoryNames,
        CifCategories(ctx.categoryNames, ctx.categoryData),
        header,
        saveFrames,
    );
}
...
function handleSingle(tokenizer: TokenizerState, ctx: FrameContext): CifCategoryResult {
    ...
    while (readingNames) {
        if (
            tokenizer.tokenType !== CifTokenType.ColumnName ||
...
async function handleLoop(
    ...
    if (state.tokenCount % fieldCount !== 0) {
        return {
            ...
            errorLine: tokenizer.lineNumber,
            errorMessage:
                `The number of values for loop starting at line ${loopLine} is not a multiple of the number of columns.`,
        };
    }
...
async function parseInternal(data: StringLike, runtimeCtx: RuntimeContext) {
    ...
    const saveFrame: Data.CifFrame = Data.CifSaveFrame(
        saveCtx.categoryNames,
        CifCategories(saveCtx.categoryNames, saveCtx.categoryData),
        '',
    );
