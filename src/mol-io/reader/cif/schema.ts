...
import { Column, ColumnHelpers, Database, type DatabaseCollection, type Table } from '../../../mol-data/db.ts';
...
export function toDatabase<Schema extends Database.Schema, Frame extends Database<Schema> = Database<Schema>>(
    schema: Schema,
    frame: Data.CifFrame,
    aliases?: Data.CifAliases,
): Frame {
...
function createListColumn<T extends number | string>(
    ...
    const value = f
        ? (row: number) => f.str(row).split(separator).map((x) => itemParse(x.trim())).filter((x) => !!x)
        : (row: number) => [];
