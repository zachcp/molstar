...
export function toDatabaseCollection<Schema extends Database.Schema>(
    schema: Schema,
    file: Data.CifFile,
    aliases?: Data.CifAliases,
...
export function toDatabase<
    Schema extends Database.Schema,
    Frame extends Database<Schema> = Database<Schema>,
>(schema: Schema, frame: Data.CifFrame, aliases?: Data.CifAliases): Frame {
    return createDatabase(schema, frame, aliases) as Frame;
}

export function toTable<Schema extends Table.Schema, R extends Table<Schema> = Table<Schema>>(
    schema: Schema,
    category: Data.CifCategory,
): R {
...
function createStringColumn<T extends string>(
    schema: Column.Schema.Str | Column.Schema.Aliased<T>,
    field: Data.CifField,
    value: (row: number) => T,
    toArray: Column<T>['toArray'],
): Column<T> {
    return {
        schema,
...
function createListColumn<T extends number | string>(
    schema: Column.Schema.List<T>,
    category: Data.CifCategory,
    key: string,
    ...
    const value = f
        ? (row: number) =>
            f.str(row).split(separator).map((x) => itemParse(x.trim())).filter((x) => !!x)
        : (row: number) => [];
    const toArray: Column<T[]>['toArray'] = (params) =>
        ColumnHelpers.createAndFillArray(category.rowCount, value, params);
...
function createTensorColumn(
    schema: Column.Schema.Tensor,
    category: Data.CifCategory,
    key: string,
): Column<Tensor.Data> {
    ...
    const first = category.getField(getName(fst, fst, fst)) ||
        Column.Undefined(category.rowCount, schema);
    const value = (row: number) => Data.getTensor(category, space, row, getName);
    const toArray: Column<Tensor.Data>['toArray'] = (params) =>
        ColumnHelpers.createAndFillArray(category.rowCount, value, params);
            ...
            Object.defineProperty(this, k, {
                get: function () {
                    ...
                    if (fType.valueType === 'list') {
                        ...
                        cache[k] = !!field
                            ? ctor(field, category, k)
                            : Column.Undefined(category.rowCount, fType);
                    }
                    return cache[k];
                },
                enumerable: true,
