...
import type { Ccp4File, Ccp4Header } from "./schema.ts";
...
function getCcp4DataType(mode: number) {
  switch (mode) {
    ...
    case 16:
      throw new Error(
        "mode 16 unsupported, unsigned char * 3 (for rgb data, non-standard)",
      );
  }
...
export function getCcp4ValueType(header: Ccp4Header) {
  return isMapmode2to0(header)
    ? TypedArrayValueType.Float32
    : getCcp4DataType(header.MODE);
}
