...
import '../../apps/mvs-stories/elements/index.ts';
...
type Story = { kind: 'built-in'; id: string } | {
    kind: 'url';
    url: string;
    format: 'mvsx' | 'mvsj';
} | undefined;
...
function SelectStoryUI({ subject }: { subject: BehaviorSubject<Story> }) {
    ...
    const selectedId = current?.kind === 'built-in'
        ? current.id
        : current?.kind === 'url'
        ? 'url'
        : '';
...
function init() {
    CurrentStory.subscribe((story) => {
        if (!story) {
            ...
            history.replaceState(
                ...
                '',
                story
                    ? `?story-url=${encodeURIComponent(story.url)}&data-format=${story.format}`
                    : '',
            );
            getContext().dispatch({
                kind: 'load-mvs',
    ...
    createRoot(document.getElementById('select-story')!).render(
        <SelectStoryUI subject={CurrentStory} />,
    );
}
