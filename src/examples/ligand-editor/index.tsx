...
import { JSONCifDataBlock, JSONCifFile } from '../../extensions/json-cif/model.ts';
...
import { GeometryEditFn, GeometryEdits, TopologyEdits } from './edits.ts';
import { ExampleMol } from './example-data.ts';
import './index.html';
import { jsonCifToMolfile } from './molfile.ts';
import { RGroupName } from './r-groups.ts';
import { SingleTaskQueue } from './utils.ts';
        ...
        >
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
                ...
                <div>
                    ...
                    <button onClick={model.setElement} style={{ borderLeft: 'none' }}>
                        Set Element
                    </button>
                    <button onClick={model.addElement} style={{ borderLeft: 'none' }}>
                        Add Element
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
                ...
                <button
                    onClick={() => model.updateBonds({ value_order: 'sing', type_id: 'covale' })}
                >
                    -
                </button>
                <button
                    onClick={() => model.updateBonds({ value_order: 'doub', type_id: 'covale' })}
                >
                    =
                </button>
                <button
                    onClick={() => model.updateBonds({ value_order: 'trip', type_id: 'covale' })}
                >
                    ≡
                </button>
            </div>
        ...
        <>
            ...
            <div style={{ display: 'flex', gap: '5px' }}>
                ...
                <button
                    onClick={() =>
                        download(
                            new Blob([molfile], { type: 'text/plain' }),
                            `edited-molecule-${Date.now()}.mol`,
                        )}
                >
                    Save
                </button>
...
function UndoButton({ model }: { model: EditorModel }) {
    ...
    return (
        <button onClick={model.undo} disabled={history.length === 0}>
            Undo [{history.length}]
        </button>
    );
}
