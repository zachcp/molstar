...
import { JSONCifLigandGraph, type JSONCifLigandGraphBondProps } from '../../extensions/json-cif/ligand-graph.ts';
import type { JSONCifDataBlock, JSONCifFile } from '../../extensions/json-cif/model.ts';
import { ParseJSONCifFileData } from '../../extensions/json-cif/transformers.ts';
...
import './index.html';
import { jsonCifToMolfile } from './molfile.ts';
import type { RGroupName } from './r-groups.ts';
...
class EditorModel {
    ...
    async editGraphTopology<Args extends any[], T>(
        fn: (graph: JSONCifLigandGraph, ...args: Args) => Promise<T>,
        ...args: Args
    ) {
        try {
    ...
    beginGeometryEdit<Args extends any[], T>(
        fn: (graph: JSONCifLigandGraph, ...args: Args) => GeometryEditFn,
        initial: number,
        ...args: Args
    ) {
        try {
...
function ControlsUI({ model }: { model: EditorModel }) {
    return (
        ...
        >
            ...
            <b>R-groups</b>
            <div style={{ display: 'flex', gap: '5px' }}>
                <button onClick={() => model.attachRgroup('CH3')}>
                    -CH<sub>3</sub>
                </button>
            </div>
            <b>Geometry</b>
    ...
    return (
        <>
            <textarea
                value={molfile}
                readOnly
                style={{ width: '100%', height: 200, fontFamily: 'monospace', fontSize: '10px' }}
            />
            <div style={{ display: 'flex', gap: '5px' }}>
                <button onClick={() => navigator.clipboard.writeText(molfile)}>Copy</button>
                <button
                    onClick={() =>
                        download(new Blob([molfile], { type: 'text/plain' }), `edited-molecule-${Date.now()}.mol`)}
                >
                    Save
                </button>
            </div>
...
function ElementEditUI({ model }: { model: EditorModel }) {
    ...
    return (
        <input
            ...
            value={element}
            style={{ width: 50 }}
            onChange={(e) => model.state.element.next(e.target.value)}
        />
    );
}
...
function TwistUI({ model }: { model: EditorModel }) {
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <i style={{ width: GeometryLabelWidth }}>Twist</i>{' '}
            <input
                type='range'
                min={-60}
                max={60}
...
function StretchUI({ model }: { model: EditorModel }) {
    ...
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <i style={{ width: GeometryLabelWidth }}>Stretch</i>{' '}
            <input
                type='range'
                min={-60}
                max={60}
