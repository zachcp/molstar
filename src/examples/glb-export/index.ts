...
import { ExternalModules } from '../../mol-plugin/util/headless-screenshot.ts';
...
function parseArguments(): Args {
    const parser = new ArgumentParser({
        description:
            'Example command-line application exporting .glb file of SDF structures from PubChem',
    });
...
async function main() {
    ...
    const meshes = structure.data!.repr.renderObjects.filter((obj) =>
        obj.type === 'mesh'
    ...
    await plugin.runTask(Task.create('Export Geometry', async (ctx) => {
        ...
        await fs.promises.writeFile(
            path.join(args.outDirectory, `${args.cid}.glb`),
            Buffer.from(buffer),
        );
    }));
