...
export class AlphaOrbitalsExample {
    ...
    private async syncVolume() {
        ...
        if (state.show.name === 'orbital') {
            if (!this.selectors) {
                ...
                const positive =
                    volume.apply(CreateOrbitalRepresentation3D, this.volumeParams('positive', ColorNames.blue))
                        .selector;
                const negative =
                    volume.apply(CreateOrbitalRepresentation3D, this.volumeParams('negative', ColorNames.red)).selector;
    ...
    async load(input: DemoInput) {
        ...
        this.params.next({
            ...
            isoValue: ParamDefinition.Numeric(this.currentParams.isoValue, { min: 0.5, max: 3, step: 0.1 }, {
                immediateUpdate: true,
                isEssential: false,
            }),
        ...
        this.state.pipe(skip(1), debounceTime(1000 / 24)).subscribe(async (params) => {
            if (
                ...
                params.isoValue !== this.currentParams.isoValue || params.gpuSurface !== this.currentParams.gpuSurface
            ) {
                this.setIsovalue();
            }
        });
