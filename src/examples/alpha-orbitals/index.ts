...
} from '../../extensions/alpha-orbitals/transforms.ts';
...
import { consoleStats, setDebugMode, setTimingMode } from '../../mol-util/debug.ts';
...
type Selectors = {
    ...
    positive: StateObjectSelector<
        PluginStateObject.Volume.Representation3D,
        typeof CreateOrbitalRepresentation3D
    >;
    negative: StateObjectSelector<
        PluginStateObject.Volume.Representation3D,
        typeof CreateOrbitalRepresentation3D
    >;
} | {
...
export class AlphaOrbitalsExample {
    ...
    async init(target: string | HTMLElement) {
        ...
        if (!canComputeGrid3dOnGPU(this.plugin.canvas3d?.webgl)) {
            PluginCommands.Toast.Show(this.plugin, {
                title: 'Error',
                message:
                    `Browser/device does not support required WebGL extension (OES_texture_float).`,
            });
            return;
        }
    ...
    readonly state = new BehaviorSubject<Params>({
        show: { name: 'orbital', params: { index: 32 } },
        isoValue: 1,
        gpuSurface: true,
    });
    ...
    private async syncVolume() {
        ...
        if (state.show.name === 'orbital') {
            if (!this.selectors) {
                ...
                const positive = volume.apply(
                    CreateOrbitalRepresentation3D,
                    this.volumeParams('positive', ColorNames.blue),
                ).selector;
                const negative = volume.apply(
                    CreateOrbitalRepresentation3D,
                    this.volumeParams('negative', ColorNames.red),
                ).selector;
    ...
    private setIsovalue() {
        ...
        if (this.selectors?.type === 'orbital') {
            update.to(this.selectors.negative).update(
                this.volumeParams('negative', ColorNames.red),
            );
        }
    ...
    async load(input: DemoInput) {
        ...
        const data = await this.plugin.builders.data.rawData({ data: input.moleculeSdf }, {
            state: { isGhost: true },
        });
        ...
        if (all) {
            await this.plugin.builders.structure.representation.addRepresentation(all, {
                type: 'ball-and-stick',
        ...
        this.params.next({
            show: ParamDefinition.MappedStatic('orbital', {
                'orbital': ParamDefinition.Group({
                    index: ParamDefinition.Numeric(32, { min: 0, max: input.orbitals.length - 1 }, {
            ...
            isoValue: ParamDefinition.Numeric(this.currentParams.isoValue, {
                min: 0.5,
                max: 3,
                step: 0.1,
            }, { immediateUpdate: true, isEssential: false }),
        ...
        this.state.pipe(skip(1), debounceTime(1000 / 24)).subscribe(async (params) => {
            if (
                params.show.name !== this.currentParams.show.name ||
                (params.show.name === 'orbital' && this.currentParams.show.name === 'orbital' &&
                    params.show.params.index !== this.currentParams.show.params.index)
            ) {
                this.syncVolume();
            } else if (
                params.isoValue !== this.currentParams.isoValue ||
                params.gpuSurface !== this.currentParams.gpuSurface
            ) {
                this.setIsovalue();
            }
        });
