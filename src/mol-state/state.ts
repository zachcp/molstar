...
class State {
  ...
  readonly behaviors = {
    currentObject: this.ev.behavior<State.ObjectEvent>({
      state: this,
  ...
  get transforms(): StateTree.Transforms {
    return (this._tree as StateTree).transforms;
  }
  get current(): StateTransform.Ref {
    return this.behaviors.currentObject.value.ref;
  }
  get root(): StateObjectCell {
    return this.cells.get((this._tree as StateTree).root.ref)!;
  }
  ...
  updateCellState(
    ...
    stateOrProvider:
      | ((old: StateTransform.State) => Partial<StateTransform.State>)
      | Partial<StateTransform.State>,
  ) {
   ...
   */
  selectQ<C extends StateObjectCell>(
    selector: (
      q: typeof StateSelection.Generators,
    ) => StateSelection.Selector<C>,
  ) {
   ...
   */
  applyAction<A extends StateAction>(
    action: A,
    params: StateAction.Params<A>,
    ref: StateTransform.Ref = StateTransform.RootRef,
  ): Task<void> {
    return Task.create("Apply Action", (ctx) => {
      ...
      if (cell.status !== "ok")
        throw new Error(
          `Action cannot be applied to a cell with status '${cell.status}'`,
        );

      return runTask(
        action.definition.run(
          { ref, cell, a: cell.obj!, params, state: this },
          this.globalContext,
        ),
        ctx,
      );
  ...
  transaction(
    edits: (ctx: RuntimeContext) => Promise<void> | void,
    ...
    return Task.create("State Transaction", async (ctx) => {
      ...
      try {
        ...
        if (!restored) {
          restored = true;
          await this.updateTree(snapshot).runInContext(ctx);
          this.events.log.next(
            LogEntry.error("Error during state transaction, reverting"),
          );
        }
        if (isNested) {
        ...
        if (!isNested) {
          ...
          if (!restored) {
            if (options?.canUndo)
              this.addHistory(
                snapshot,
                typeof options.canUndo === "string" ? options.canUndo : void 0,
              );
   ...
   */
  updateTree<T extends StateObject>(
    tree: StateBuilder.To<T, any>,
    options?: Partial<State.UpdateOptions>,
  ): Task<StateObjectSelector<T>>;
  updateTree(
    tree: StateTree | StateBuilder,
    options?: Partial<State.UpdateOptions>,
  ): Task<void>;
  updateTree(
    tree: StateTree | StateBuilder,
        ...
        try {
          if (StateBuilder.is(tree)) {
            if (tree.editInfo.applied)
              throw new Error(
                "This builder has already been applied. Create a new builder for further state updates",
              );
          ...
          this.reverted = false;
          const ret =
            options && (options.revertIfAborted || options.revertOnError)
  ...
  private async _revertibleTreeUpdate(
    ...
    const revert =
      ((ret.ctx.hadError || ret.ctx.wasAborted) && options.revertOnError) ||
      (ret.ctx.wasAborted && options.revertIfAborted);
    if (revert) {
      this.reverted = true;
      return await this._updateTree(taskCtx, {
        tree: old,
  ...
  private async _updateTree(taskCtx: RuntimeContext, params: UpdateParams) {
    let updated = false;
    const ctx = this.updateTreeAndCreateCtx(
      params.tree,
      taskCtx,
      params.options,
    );
  ...
  private updateTreeAndCreateCtx(
    ...
    const _tree: TransientTree = (
      StateBuilder.is(tree) ? tree.getTree() : tree
    ).asTransient();
  ...
  constructor(rootObject: StateObject, params: State.Params) {
    this._tree = StateTree.createEmpty(
      StateTransform.createRoot(params && params.rootState),
    ).asTransient();
    const tree: TransientTree = this._tree;
    const root = tree.root;
    this.runTask = params.runTask;

    if (params?.historyCapacity !== void 0)
...
async function update(ctx: UpdateContext) {
  ...
  const fastTrack = !!(
    ctx.editInfo &&
    ctx.editInfo.count === 1 &&
    ctx.editInfo.lastUpdate &&
    ctx.editInfo.sourceTree === ctx.oldTree
  );
  let deletes: StateTransform.Ref[],
    deletedObjects: (StateObject | undefined)[] = [],
    roots: StateTransform.Ref[];
  ...
  if (fastTrack) {
    ...
    if (hasCurrent) {
      const newCurrent: string = findNewCurrent(
        ctx.oldTree,
        current,
        deletes,
        ctx.cells,
    ...
    for (let i = deletes.length - 1; i >= 0; i--) {
      ...
      if (cell) {
        dispose(
          cell.transform,
          cell.obj,
          cell?.transform.params,
          cell.cache,
          ctx.parent.globalContext,
        );
      }
    }
  ...
  for (const update of ctx.results) {
    if (update.action === "created") {
      ...
      if (!ctx.newCurrent) {
        ...
        if (!transform.state.isGhost && update.obj !== StateObject.Null)
    ...
    if (
      ...
      (currentCell.obj === StateObject.Null ||
        (currentCell.status === "error" &&
          currentCell.errorText === ParentNullErrorText))
    ) {
...
function findUpdateRootsVisitor(
  n: StateTransform,
  _: any,
  s: { roots: Ref[]; cells: Map<Ref, StateObjectCell> },
) {
...
function findDeletes(ctx: UpdateContext): Ref[] {
  const deleteCtx: FindDeletesCtx = {
    newTree: ctx.tree,
    cells: ctx.cells,
    deletes: [],
  };
  StateTree.doPostOrder(
    ctx.oldTree,
    ctx.oldTree.root,
    deleteCtx,
    checkDeleteVisitor,
...
type InitCellsCtx = {
  ctx: UpdateContext;
  visited: Set<Ref>;
  added: StateObjectCell[];
};

function addCellsVisitor(
  transform: StateTransform,
  _: any,
  { ctx, added, visited }: InitCellsCtx,
) {
...
function findNewCurrent(
  tree: StateTree,
  start: Ref,
  deletes: Ref[],
  cells: Map<Ref, StateObjectCell>,
) {
  const deleteSet = new Set(deletes);
  return _findNewCurrent(tree, start, deleteSet, cells);
}

function _findNewCurrent(
  tree: StateTree,
  ref: Ref,
  deletes: Set<Ref>,
  cells: Map<Ref, StateObjectCell>,
  ...
  let prevCandidate: Ref | undefined = void 0,
    seenRef = false;
...
function doError(
  ...
  const children: IterableIterator<string> = ctx.tree.children
    .get(ref)
    .values();
  while (true) {
  ...
  try {
    ...
    if (update.action === "created") {
      ...
      if (!isNull && !ctx.options.doNotLogTiming)
        ctx.parent.events.log.next(
          LogEntry.info(
            `Created ${update.obj.label} in ${formatTimespan(time)}.`,
          ),
        );
    } else if (update.action === "updated") {
      isNull = update.obj === StateObject.Null;
      if (!isNull && !ctx.options.doNotLogTiming)
        ctx.parent.events.log.next(
          LogEntry.info(
            `Updated ${update.obj.label} in ${formatTimespan(time)}.`,
          ),
        );
    } else if (update.action === "replaced") {
      isNull = update.obj === StateObject.Null;
...
function resolveParams(
  ...
  if (transform.version !== (transform as any)._normalized_param_version) {
    (transform.params as any) = ParamDefinition.normalizeParams(
      definition,
      transform.params,
      "all",
    );
...
async function updateNode(
  ...
  const parentCell =
    transform.transformer.definition.from.length === 0
      ? treeParent
      : StateSelection.findAncestorOfType(
          tree,
          ctx.cells,
          currentRef,
          transform.transformer.definition.from,
        );
  if (!parentCell) {
  ...
  if (!oldTree.transforms.has(currentRef) || !current.params) {
    ...
    const updateKind: StateTransformer.UpdateResult =
      !!current.obj && current.obj !== StateObject.Null
        ? await updateObject(
            ctx,
            current,
            transform.transformer,
...
function runTask<T>(t: T | Task<T>, ctx: RuntimeContext) {
  if (typeof (t as any).runInContext === "function")
    return (t as Task<T>).runInContext(ctx);
  return t as T;
}
