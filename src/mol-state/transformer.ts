...
interface Transformer<
    A extends StateObject = StateObject,
    B extends StateObject = StateObject,
    P extends {} = any,
> {
    apply(
        parent: StateTransform.Ref,
        params?: P,
        props?: Partial<StateTransform.Options>,
    ): StateTransform<this>;
...
namespace Transformer {
    ...
    export type Params<T extends Transformer<any, any, any>> = T extends
        Transformer<any, any, infer P> ? P : unknown;
    export type From<T extends Transformer<any, any, any>> = T extends
        Transformer<infer A, any, any> ? A : unknown;
    export type To<T extends Transformer<any, any, any>> = T extends Transformer<any, infer B, any>
        ? B
        : unknown;
    export type Cell<T extends Transformer<any, any, any>> = T extends
        Transformer<any, infer B, any> ? StateObjectCell<B> : unknown;

    export function getParamDefinition<T extends Transformer>(
        t: T,
        a: From<T> | undefined,
        globalCtx: unknown,
    ): PD.For<Params<T>> {
    ...
    export function is(obj: any): obj is Transformer {
        return !!obj && typeof (obj as Transformer).toAction === 'function' &&
            typeof (obj as Transformer).apply === 'function';
    }
    ...
    export interface AutoUpdateParams<
        A extends StateObject = StateObject,
        B extends StateObject = StateObject,
        P extends {} = {},
    > {
    ...
    > {
         ...
         */
        update?(
            params: UpdateParams<A, B, P>,
            globalCtx: unknown,
        ): Task<UpdateResult> | UpdateResult;
        ...
        isSerializable?(
            params: P,
        ): { isSerializable: true } | { isSerializable: false; reason: string };
    ...
    export function create<A extends StateObject, B extends StateObject, P extends {} = {}>(
        ...
        if (registry.has(id)) {
            console.warn(
                `A transform with id '${name}' is already registered. Please pick a unique identifier for your transforms and/or register them only once. This is to ensure that transforms can be serialized and replayed.`,
            );
        }

        const t: Transformer<A, B, P> = {
            apply(parent, params, props) {
                return StateTransform.create<Transformer<A, B, P>>(parent, t, params, props);
            },
            toAction() {
            ...
            id,
            definition,
            createDefaultParams(a, globalCtx) {
                return definition.params
                    ? PD.getDefaultValues(definition.params(a, globalCtx))
                    : {} as any;
            },
        };
        registry.set(id, t);
    ...
    export function factory(namespace: string) {
        return <A extends StateObject, B extends StateObject, P extends {} = {}>(
            definition: Definition<A, B, P>,
    ...
    export namespace Builder {
        export interface Type<
            A extends StateObject.Ctor,
            B extends StateObject.Ctor,
            P extends {},
        > {
            name: string;
            from: A | A[];
            ...
            params?:
                | PD.For<P>
                | ((a: StateObject.From<A> | undefined, globalCtx: any) => PD.For<P>);
            display?: string | { name: string; description?: string };
            isDecorator?: boolean;
        }

        export interface Root {
            <A extends StateObject.Ctor, B extends StateObject.Ctor, P extends {}>(
                info: Type<A, B, P>,
            ): Define<StateObject.From<A>, StateObject.From<B>, PD.Normalize<P>>;
        }
