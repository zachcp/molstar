...
interface StateObject<
  D = any,
  T extends StateObject.Type = StateObject.Type<any>,
> {
  readonly id: UUID;
  readonly type: T;
  readonly data: D;
...
namespace StateObject {
  ...
  export type Type<Cls extends string = string> = {
    name: string;
    typeClass: Cls;
  };
  export type Ctor<T extends StateObject = StateObject> = {
    new (...args: any[]): T;
...
interface StateObjectCell<
  T extends StateObject = StateObject,
  F extends StateTransform = StateTransform,
> {
  parent?: State;
...
namespace StateObjectCell {
  ...
  export type Obj<C extends StateObjectCell> =
    C extends StateObjectCell<infer T> ? T : never;
  export type Transform<C extends StateObjectCell> =
    C extends StateObjectCell<any, infer T> ? T : never;
  export type Transformer<C extends StateObjectCell> =
    C extends StateObjectCell<any, StateTransform<infer T>> ? T : never;
...
export class StateObjectSelector<
...
> {
  get cell(): StateObjectCell<S, StateTransform<T>> | undefined {
    return this.state?.cells.get(this.ref) as
      | StateObjectCell<S, StateTransform<T>>
      | undefined;
  }
  ...
  update(
    ...
    builder?: StateBuilder.Root | StateBuilder.To<any>,
  ): StateBuilder;
  update(
    params: (
      old: StateTransformer.Params<T>,
    ) => StateTransformer.Params<T> | void,
    builder?: StateBuilder.Root | StateBuilder.To<any>,
  ): StateBuilder;
  update(
    params:
      | ((old: StateTransformer.Params<T>) => StateTransformer.Params<T> | void)
      | StateTransformer.Params<T>,
    builder?: StateBuilder.Root | StateBuilder.To<any>,
  ): StateBuilder {
    if (!this.state)
      throw new Error(
        `To use update() from StateObjectSelector, 'state' must be defined.`,
      );
  ...
  checkValid() {
    ...
    if (!cell) {
      throw new Error(
        `Not created at all. Did you await/then the corresponding state update?`,
      );
    }
    ...
    throw new Error(
      `Unresolved. Did you await/then the corresponding state update?`,
    );
  }
  ...
  constructor(
    public ref: StateTransform.Ref,
    public state?: State,
  ) {}
}

export namespace StateObjectSelector {
  export type Obj<S extends StateObjectSelector> =
    S extends StateObjectSelector<infer A> ? A : never;
  export type Transformer<S extends StateObjectSelector> =
    S extends StateObjectSelector<any, infer T> ? T : never;
}

export type StateObjectRef<S extends StateObject = StateObject> =
  | StateObjectSelector<S>
  | StateObjectCell<S>
  | StateTransform.Ref;

export namespace StateObjectRef {
  export function resolveRef<S extends StateObject>(
    ref?: StateObjectRef<S>,
  ...
  export function resolve<S extends StateObject>(
    state: State,
    ref?: StateObjectRef<S>,
  ): StateObjectCell<S> | undefined {
    if (!ref) return;
    if (StateObjectCell.is(ref)) return ref;
    if (typeof ref === "string")
      return state.cells.get(ref) as StateObjectCell<S> | undefined;
    return ref.cell;
  }

  export function resolveAndCheck<S extends StateObject>(
    state: State,
    ref?: StateObjectRef<S>,
  ): StateObjectCell<S> | undefined {
