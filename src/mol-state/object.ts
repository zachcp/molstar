...
import { StateSelection, StateTransformer } from './index.ts';
...
namespace StateObjectCell {
    ...
    export type Transform<C extends StateObjectCell> = C extends StateObjectCell<any, infer T> ? T
        : never;
    export type Transformer<C extends StateObjectCell> = C extends
        StateObjectCell<any, StateTransform<infer T>> ? T : never;
    ...
    export function resolve(
        state: State,
        refOrCellOrSelector: StateTransform.Ref | StateObjectCell | StateObjectSelector,
    ) {
...
> {
    ...
    update(
        params: (old: StateTransformer.Params<T>) => StateTransformer.Params<T> | void,
        builder?: StateBuilder.Root | StateBuilder.To<any>,
    ): StateBuilder;
    update(
        params:
...
export namespace StateObjectSelector {
    export type Obj<S extends StateObjectSelector> = S extends StateObjectSelector<infer A> ? A
        : never;
    export type Transformer<S extends StateObjectSelector> = S extends
        StateObjectSelector<any, infer T> ? T : never;
}
