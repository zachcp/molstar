...
namespace StateSelection {
    export type Selector<C extends StateObjectCell = StateObjectCell> =
        | Query<C>
        | Builder<C>
        | string
        | C;
    ...
    function isObj(arg: any): arg is StateObjectCell {
        return (arg as StateObjectCell).transform !== void 0 &&
            (arg as StateObjectCell).status !== void 0;
    }
    ...
    export interface Builder<C extends StateObjectCell = StateObjectCell> {
        ...
        withTransformer<T extends StateTransformer<any, StateObjectCell.Obj<C>, any>>(
            t: T,
        ): Builder<StateObjectCell<StateObjectCell.Obj<C>, StateTransform<T>>>;
        ...
        ancestor<T extends StateObject.Ctor>(
            test: (c: StateObjectCell) => boolean | void | undefined,
        ): Builder<StateObjectCell<StateObject.From<T>>>;
        ancestorOfType<T extends StateObject.Ctor>(
            t: T | T[],
        ): Builder<StateObjectCell<StateObject.From<T>>>;
        ancestorWithTransformer<T extends StateTransformer>(
            transfomers: T | T[],
        ): Builder<StateObjectCell<StateTransformer.To<T>>>;

        root<T extends StateObject.Ctor>(
            test: (c: StateObjectCell) => boolean | void | undefined,
        ): Builder<StateObjectCell<StateObject.From<T>>>;
        rootOfType<T extends StateObject.Ctor>(
            t: T | T[],
        ): Builder<StateObjectCell<StateObject.From<T>>>;
    ...
    function registerModifier(name: string, f: Function) {
        BuilderPrototype[name] = function (this: any, ...args: any[]) {
    ...
    function build<C extends StateObjectCell>(compile: () => Query<C>): Builder<C> {
        return Object.create(BuilderPrototype, {
            compile: { writable: false, configurable: false, value: compile },
        });
    }
    ...
    export namespace Generators {
        ...
        export function rootsOfType<T extends StateObject.Ctor>(
            ...
            return build(() => (state) => {
                ...
                StateTree.doPreOrder(
                    state.tree,
                    state.tree.transforms.get(root),
                    ctx,
                    _findRootsOfType,
                );
                return ctx.roots;
        ...
        >(t: T, root: StateTransform.Ref = StateTransform.RootRef) {
            return build(() => (state) => {
                ...
                StateTree.doPreOrder(
                    ...
                    state.tree.transforms.get(root),
                    ctx,
                    _findOfTransformerWithError,
                );
                return ctx.ret;
            });
        }

        function _findRootsOfType(
            n: StateTransform,
            _: any,
            s: { type: StateObject.Type; roots: StateObjectCell[]; cells: State.Cells },
        ) {
        ...
        function _findOfType(
            n: StateTransform,
            _: any,
            s: { type: StateObject.Type; ret: StateObjectCell[]; cells: State.Cells },
        ) {
            const cell = s.cells.get(n.ref);
            if (cell && cell.obj && cell.obj.type === s.type) {
                s.ret.push(cell);
        ...
        function _findOfTransformer(
            n: StateTransform,
            _: any,
            s: { t: StateTransformer; ret: StateObjectCell[]; cells: State.Cells },
        ) {
            const cell = s.cells.get(n.ref);
        ...
        function _findOfTransformerWithError(
            n: StateTransform,
            _: any,
            s: { t: StateTransformer; ret: StateObjectCell[]; cells: State.Cells },
        ) {
    ...
    export function filter(b: Selector, p: (n: StateObjectCell) => boolean) {
        return flatMap(b, (n) => p(n) ? [n] : []);
    }

    registerModifier('withStatus', withStatus);
    export function withStatus(b: Selector, s: StateObjectCell.Status) {
        return filter(b, (n) => n.status === s);
    }

    registerModifier('withTag', withTag);
    export function withTag(b: Selector, tag: string) {
        return filter(b, (n) => !!n.transform.tags && n.transform.tags.indexOf(tag) >= 0);
    }
    ...
    export function subtree(b: Selector) {
        return flatMap(b, (n, s) => {
            ...
            StateTree.doPreOrder(
                s.tree,
                s.tree.transforms.get(n.transform.ref),
                nodes,
                (x, _, ctx) => {
                    ctx.push(x.ref);
                },
    ...
    export function ofType(b: Selector, t: StateObject.Ctor) {
        return filter(b, (n) => n.obj ? n.obj.type === t.type : false);
    }
    ...
    export function ancestor(
        ...
        return unique(mapObject(b, (n, s) => findAncestor(s.tree, s.cells, n.transform.ref, test)));
    }

    registerModifier('ancestorOfType', ancestorOfType);
    export function ancestorOfType(b: Selector, types: StateObject.Ctor[]) {
        return unique(
            mapObject(b, (n, s) => findAncestorOfType(s.tree, s.cells, n.transform.ref, types)),
        );
    }

    registerModifier('ancestorWithTransformer', ancestorWithTransformer);
    export function ancestorWithTransformer(b: Selector, transfomers: StateTransformer[]) {
        return unique(
            mapObject(
                b,
                (n, s) =>
                    findAncestorWithTransformer(s.tree, s.cells, n.transform.ref, transfomers),
            ),
        );
    }

    registerModifier('withTransformer', withTransformer);
    export function withTransformer(b: Selector, t: StateTransformer) {
        return filter(b, (o) => o.transform.transformer === t);
    }

    registerModifier('root', root);
    export function root(b: Selector, test: (c: StateObjectCell) => boolean | void | undefined) {
        return unique(mapObject(b, (n, s) => findRoot(s.tree, s.cells, n.transform.ref, test)));
    }

    registerModifier('rootOfType', rootOfType);
    export function rootOfType(b: Selector, types: StateObject.Ctor | StateObject.Ctor[]) {
        return unique(
            mapObject(b, (n, s) => findRootOfType(s.tree, s.cells, n.transform.ref, types)),
        );
    }

    registerModifier('parent', parent);
    export function parent(b: Selector) {
        return unique(
            mapObject(b, (n, s) => s.cells.get(s.tree.transforms.get(n.transform.ref)!.parent)),
        );
    }

    function _findAncestor<T extends StateObject.Ctor>(
        tree: StateTree,
        cells: State.Cells,
    ...
    export function findAncestor<T extends StateObject.Ctor>(
        tree: StateTree,
        cells: State.Cells,
        root: StateTransform.Ref,
        test: (c: StateObjectCell) => boolean | void | undefined,
    ...
    export function findRoot<T extends StateObject.Ctor>(
        tree: StateTree,
        cells: State.Cells,
        root: StateTransform.Ref,
        test: (c: StateObjectCell) => boolean | void | undefined,
    ...
    export function findAncestorWithTransformer<T extends StateTransformer>(
        tree: StateTree,
        cells: State.Cells,
        root: StateTransform.Ref,
        transfomers: T | T[],
    ): StateObjectCell<StateTransformer.To<T>> | undefined {
        return findAncestor<StateObject.Ctor<StateTransformer.To<T>>>(
            tree,
            cells,
            root,
            Array.isArray(transfomers)
                ? (cell) => transfomers.indexOf(cell.transform.transformer as any) >= 0
                : (cell) => cell.transform.transformer === transfomers,
        );
    }

    export function findAncestorOfType<T extends StateObject.Ctor>(
        tree: StateTree,
        cells: State.Cells,
        root: StateTransform.Ref,
        types: T | T[],
    ): StateObjectCell<StateObject.From<T>> | undefined {
        return findAncestor<StateObject.Ctor<StateObject.From<T>>>(
            tree,
            cells,
            root,
            _testTypes(types),
    ...
    export function findRootOfType<T extends StateObject.Ctor>(
        tree: StateTree,
        cells: State.Cells,
        root: StateTransform.Ref,
        types: T | T[],
    ...
    export function findUniqueTagsInSubtree<K extends string = string>(
        tree: StateTree,
        root: StateTransform.Ref,
        tags: Set<K>,
        ...
        return StateTree.doPreOrder(
            tree,
            tree.transforms.get(root),
            { refs: {}, tags },
            _findUniqueTagsInSubtree,
        ).refs;
    }

    function _findUniqueTagsInSubtree(
        n: StateTransform,
        _: any,
        s: { refs: { [name: string]: StateTransform.Ref }; tags: Set<string> },
    ) {
        if (n.tags) {
    ...
    export function findTagInSubtree(
        tree: StateTree,
        root: StateTransform.Ref,
        tag: string,
    ): StateTransform.Ref | undefined {
        return StateTree.doPreOrder(
            tree,
            tree.transforms.get(root),
            { ref: void 0, tag },
    ...
    function _findTagInSubtree(
        n: StateTransform,
        _: any,
        s: { ref: string | undefined; tag: string },
    ) {
    ...
    export function findWithAllTags<K extends string = string>(
        tree: StateTree,
        root: StateTransform.Ref,
        tags: Set<K>,
    ): StateTransform[] {
    ...
    function _findWithAllTags(
        n: StateTransform,
        _: any,
        s: { refs: StateTransform[]; tags: Set<string> },
    ) {
        if (n.tags) {
    ...
    export function tryFindDecorator<T extends StateTransformer>(
        state: State,
        root: StateTransform.Ref,
        transformer: T,
    ): StateObjectCell<StateTransformer.To<T>, StateTransform<T>> | undefined {
