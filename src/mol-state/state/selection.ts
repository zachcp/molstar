...
namespace StateSelection {
    ...
    export type Query<C extends StateObjectCell = StateObjectCell> = (
        state: State,
    ) => CellSeq<C>;
    ...
    export function compile<C extends StateObjectCell>(s: Selector<C>): Query<C> {
        ...
        if (isBuilder(selector)) query = (selector as any).compile();
            ...
            query = (Generators.byValue(selector) as any).compile();
    ...
    function isObj(arg: any): arg is StateObjectCell {
        return (
            (arg as StateObjectCell).transform !== void 0 &&
    ...
    export interface Builder<C extends StateObjectCell = StateObjectCell> {
        ...
        withTransformer<
            T extends StateTransformer<any, StateObjectCell.Obj<C>, any>,
        >(
            t: T,
        ...
        subtree(): Builder;
        children(): Builder;
        ofType<T extends StateObject.Ctor>(
            t: T,
    ...
    const BuilderPrototype: any = {
        select(state?: State): CellSeq<any> {
    ...
    function build<C extends StateObjectCell>(
        compile: () => Query<C>,
    ): Builder<C> {
    ...
    export namespace Generators {
        export const root: Builder<
            StateObjectCell<
                ...
                >
            >
        > = build(() => (state: State) => [state.cells.get(state.tree.root.ref)!]);
        ...
        function _findOfTransformerWithError(
            ...
            if (
                cell &&
                cell.status === 'error' &&
                cell.transform.transformer === s.t
            ) {
                s.ret.push(cell);
            }
            return true;
    ...
    > {
        return filter(
            b,
            (n) => !!n.transform.tags && n.transform.tags.indexOf(tag) >= 0,
        );
    }
    ...
    > {
        return flatMap(b, (n, s) => {
            ...
            s.tree.children
                .get(n.transform.ref)
                .forEach((c) => nodes.push(s.cells.get(c!)!));
            return nodes;
        });
    }
    ...
    > {
        return unique(
            mapObject(b, (n, s) => findAncestor(s.tree, s.cells, n.transform.ref, test)),
        );
    }
    ...
    > {
        return unique(
            mapObject(b, (n, s) => findRoot(s.tree, s.cells, n.transform.ref, test)),
        );
    }

    registerModifier('rootOfType', rootOfType);
    export function rootOfType(
        b: Selector,
        types: StateObject.Ctor | StateObject.Ctor[],
    ): Builder<
        StateObjectCell<
            StateObject<any, StateObject.Type<any>>,
            StateTransform<
    ...
    function _testTypes<T extends StateObject.Ctor>(types: T | T[]) {
        return Array.isArray(types)
            ? (cell: StateObjectCell): true | undefined => {
                for (const t of types) {
                    if (t.type === cell.obj!.type) return true;
                }
            }
            : (cell: StateObjectCell): boolean => {
    ...
    export function tryFindDecorator<T extends StateTransformer>(
        ...
        const children: StateTree.ChildSet = state.tree.children.get(root);
        if (children.size !== 1) return;
        const first: string | undefined = children.first();
        if (first && state.transforms.get(first).transformer.definition.isDecorator) {
            return tryFindDecorator(state, first, transformer);
        }
    }

    export function getDecorated<T extends StateObject>(
        state: State,
        root: StateTransform.Ref,
    ): StateObjectCell<T> {
        const children: StateTree.ChildSet = state.tree.children.get(root);
        ...
        const first: string | undefined = children.first();
        if (first && state.transforms.get(first).transformer.definition.isDecorator) {
            return getDecorated(state, first);
        }
