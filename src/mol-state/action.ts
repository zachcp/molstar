...
namespace StateAction {
    ...
    export type Params<T extends StateAction<any, any, any>> = T extends
        StateAction<any, any, infer P> ? P : unknown;
    export type ReType<T extends StateAction<any, any, any>> = T extends
        StateAction<any, infer T, any> ? T : unknown;
    ...
    export function fromTransformer<T extends StateTransformer>(transformer: T) {
        ...
        return create<StateTransformer.From<T>, void, StateTransformer.Params<T>>({
            from: def.from,
            display: def.display,
            params: def.params as StateTransformer.Definition<
                StateTransformer.From<T>,
                any,
    ...
    export namespace Builder {
        ...
        export interface Root {
            <A extends StateObject.Ctor, P extends {}>(
                info: Type<A, P>,
            ): Define<StateObject.From<A>, PD.Normalize<P>>;
        }

        export interface Define<A extends StateObject, P extends {}> {
            <T>(
                def: DefinitionBase<A, T, P> | DefinitionBase<A, T, P>['run'],
            ): StateAction<A, T, P>;
        }
