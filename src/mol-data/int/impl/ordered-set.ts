...
export function size(set: OrderedSetImpl) {
    return I.is(set) ? I.size(set) : S.size(set);
}
export function has(set: OrderedSetImpl, x: number) {
...
export function isEmpty(set: OrderedSetImpl) {
    return size(set) === 0;
}
...
export function toString(set: OrderedSetImpl) {
...
export function forEach(
    set: OrderedSetImpl,
    f: (value: number, i: number, ctx: any) => void,
    ctx: any,
) {
    if (I.is(set)) {
