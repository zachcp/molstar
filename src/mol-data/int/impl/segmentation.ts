...
export function count({ count }: Segmentation) {
  return count;
}
export function getSegment({ index }: Segmentation, value: number) {
  return index[value];
}
...
export function projectValue(
  ...
  const idx =
    value >= last ? -1 : OrderedSet.findPredecessorIndex(offsets, value - 1);
  return OrderedSet.findRange(
    set,
    OrderedSet.getAt(offsets, idx),
    OrderedSet.getAt(offsets, idx + 1) - 1,
  );
}

export class SegmentIterator<I extends number = number>
  implements Iterator<Segs.Segment<I>>
{
  ...
  move(): Segs.Segment<I> {
    while (this.hasNext) {
      if (this.updateValue()) {
        ...
        this.hasNext =
          this.segmentMax >= this.segmentMin &&
          Interval.size(this.setRange) > 0;
        break;
      } else {
  ...
  private updateValue() {
    ...
    const setEnd = OrderedSet.findPredecessorIndexInInterval(
      this.set,
      segmentEnd,
      this.setRange,
    );
  ...
  private updateSegmentRange() {
    const sMin = Interval.min(this.setRange),
  ...
  constructor(
    private segments: SortedArray,
    private segmentMap: Int32Array,
...
export function segments(
  ...
  const int =
    typeof segment !== "undefined"
      ? Interval.ofBounds(segment.start, segment.end)
      : Interval.ofBounds(0, OrderedSet.size(set));
