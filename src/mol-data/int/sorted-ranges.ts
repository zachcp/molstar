...
import { Interval, OrderedSet, type Segmentation, SortedArray } from '../int.ts';
import type { Iterator as _Iterator } from '../iterator.ts';
...
namespace SortedRanges {
    export function ofSortedRanges<T extends number = number>(
        array: ArrayLike<T>,
    ): SortedRanges<T> {
        return SortedArray.ofSortedArray<T>(array);
    }
    export function start<T extends number = number>(ranges: SortedRanges<T>): T {
        return ranges[0];
    }
    export function end<T extends number = number>(
        ranges: SortedRanges<T>,
    ): number {
        return ranges[ranges.length - 1] + 1;
    }
    export function min<T extends number = number>(ranges: SortedRanges<T>): T {
        return ranges[0];
    }
    ...
    export function firstIntersectionIndexFrom<T extends number = number>(
        ...
        if (
            minAt(ranges, from) > OrderedSet.max(set) ||
            max(ranges) < OrderedSet.min(set)
        ) {
            return -1;
        }
    ...
    export class Iterator<T extends number = number, I extends number = number>
        ...
        private value: Segmentation.Segment<I> = {
            index: 0 as I,
            start: 0 as T,
            end: 0 as T,
        };
