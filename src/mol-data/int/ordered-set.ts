...
namespace OrderedSet {
    ...
    export const ofSingleton: <T extends number = number>(value: T) => OrderedSet<T> = Base
        .ofSingleton as any;
    /** Create interval from range [min, max] */
    export const ofRange: <T extends number = number>(min: T, max: T) => OrderedSet<T> = Base
        .ofRange as any;
    /** Create interval from bounds [start, end), i.e. [start, end - 1] */
    export const ofBounds: <T extends number = number>(start: T, end: T) => OrderedSet<T> = Base
        .ofBounds as any;
    /** It is the responsibility of the caller to ensure the array is sorted and contains unique values. */
    export const ofSortedArray: <T extends number = number>(xs: ArrayLike<T>) => OrderedSet<T> =
        Base.ofSortedArray as any;

    export const has: <T extends number = number>(set: OrderedSet<T>, x: T) => boolean = Base
        .has as any;
    /** Returns the index of `x` in `set` or -1 if not found. */
    export const indexOf: <T extends number = number>(set: OrderedSet<T>, x: T) => number = Base
        .indexOf as any;
    /** Returns the value in `set` at index `i`. */
    export const getAt: <T extends number = number>(set: OrderedSet<T>, i: number) => T = Base
        .getAt as any;
    export const isEmpty: <T extends number = number>(set: OrderedSet<T>) => boolean = Base
        .isEmpty as any;
    ...
    export const hashCode: <T extends number = number>(set: OrderedSet<T>) => number = Base
        .hashCode as any;

    export const areEqual: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => boolean = Base.areEqual as any;
    export const areIntersecting: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => boolean = Base.areIntersecting as any;
    /** Check if the 2nd argument is a subset of the 1st */
    export const isSubset: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => boolean = Base.isSubset as any;

    export const union: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => OrderedSet<T> = Base.union as any;
    export const intersect: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => OrderedSet<T> = Base.intersect as any;
    export const indexedIntersect: <T extends number = number, S extends number = number>(
        idxA: OrderedSet<T>,
        a: SortedArray<S>,
        b: SortedArray<S>,
    ) => OrderedSet<T> = Base.indexedIntersect as any;
    /** Returns elements of `a` that are not in `b`, i.e `a` - `b` */
    export const subtract: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => OrderedSet<T> = Base.subtract as any;
    ...
    export const findPredecessorIndex: <T extends number = number>(
        set: OrderedSet<T>,
        x: number,
    ) => number = Base.findPredecessorIndex as any;
    export const findPredecessorIndexInInterval: <T extends number = number>(
        set: OrderedSet<T>,
        x: T,
        range: Interval,
    ) => number = Base.findPredecessorIndexInInterval as any;
    export const findRange: <T extends number = number>(
        set: OrderedSet<T>,
        min: T,
        max: T,
    ) => Interval = Base.findRange as any;
    export const intersectionSize: <T extends number = number>(
        a: OrderedSet<T>,
        b: OrderedSet<T>,
    ) => number = Base.intersectionSize as any;

    export function forEach<T extends number, Ctx>(
        set: OrderedSet<T>,
    ...
    export function forEachSegment<T extends number, S extends number, Ctx>(
        set: OrderedSet<T>,
        segment: (v: T) => S,
    ...
    export function isSortedArray<T extends number = number>(
        set: OrderedSet<T>,
    ): set is SortedArray<T> {
        return !Interval.is(set);
    }
