...
export function invertCantorPairing(
  out: [number, number],
  z: number,
): [number, number] {
 ...
 */
export function hashFnv32a(array: ArrayLike<number>): number {
  let hval = 0x811c9dc5;
...
const Fnv256Base = new Uint32Array([
  0x6c62272e, 0x07bb0142, 0x62b82175, 0x6295c58d, 0x16d67530, 0xdd7121e3,
  0xb3174000, 0x00000100,
]);
...
export function murmurHash3_128(key: string, seed: number): string {
  ...
  switch (remainder) {
    case 15:
      k4 ^= key.charCodeAt(bytes + 14) << 16;
    case 14:
      k4 ^= key.charCodeAt(bytes + 13) << 8;
    case 13:
    ...
    case 12:
      k3 ^= key.charCodeAt(bytes + 11) << 24;
    case 11:
      k3 ^= key.charCodeAt(bytes + 10) << 16;
    case 10:
      k3 ^= key.charCodeAt(bytes + 9) << 8;
    case 9:
    ...
    case 8:
      k2 ^= key.charCodeAt(bytes + 7) << 24;
    case 7:
      k2 ^= key.charCodeAt(bytes + 6) << 16;
    case 6:
      k2 ^= key.charCodeAt(bytes + 5) << 8;
    case 5:
    ...
    case 4:
      k1 ^= key.charCodeAt(bytes + 3) << 24;
    case 3:
      k1 ^= key.charCodeAt(bytes + 2) << 16;
    case 2:
      k1 ^= key.charCodeAt(bytes + 1) << 8;
    case 1:
