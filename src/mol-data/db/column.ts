...
namespace Column {
  ...
  export function Undefined<T extends Schema>(
    rowCount: number,
    schema: T,
  ): Column<T["T"]> {
  ...
  export function ofLambda<T extends Schema>(
    spec: LambdaSpec<T>,
  ): Column<T["T"]> {
  ...
  export function ofArray<T extends Column.Schema>(
    spec: Column.ArraySpec<T>,
  ): Column<T["T"]> {
  ...
  export function ofStringAliasArray<T extends string>(
    array: ArrayLike<T>,
  ): Column<T> {
    return arrayColumn<Schema.Aliased<T>>({
      array,
      schema: Schema.Aliased(Schema.str),
    });
  }

  export function ofStringListArray<T extends string>(
    array: ArrayLike<T[]>,
    separator = ",",
  ): Column<T[]> {
  ...
  export function ofIntTokens(tokens: Tokens): Column<number> {
    ...
    return lambdaColumn({
      value: (row: number) =>
        fastParseInt(data, indices[2 * row], indices[2 * row + 1]) || 0,
      rowCount: count,
      schema: Schema.int,
    });
  }
  ...
  export function createIndexer<T, R extends number = number>(
    column: Column<T>,
  ): (e: T) => R {
  ...
  export function asArrayColumn<T>(
    c: Column<T>,
    array?: ArrayCtor<T>,
  ): Column<T> {
    if (c.__array) return c;
    if (!c.isDefined)
      return Undefined(c.rowCount, c.schema) as any as Column<T>;
    return arrayColumn({
...
function lambdaColumn<T extends Column.Schema>({
  ...
  rowCount,
  schema,
}: Column.LambdaSpec<T>): Column<T["T"]> {
  return {
    schema: schema,
    __array: void 0,
    isDefined: true,
  ...
  return {
    ...
    toArray:
      schema.valueType === "str"
        ? (schema as Column.Schema.Str).transform === "lowercase"
          ? (params) => {
              const { start, end } = ColumnHelpers.getArrayBounds(
                rowCount,
                params,
              );
        ...
        : isTyped
          ? (params) =>
              ColumnHelpers.typedArrayWindow(
                array,
                params,
              ) as any as ReadonlyArray<T>
          : (params) => {
              const { start, end } = ColumnHelpers.getArrayBounds(
                rowCount,
                params,
              ...
              if (start === 0 && end === array.length)
                return array as ReadonlyArray<T["T"]>;
              const ret = new (params && typeof params.array !== "undefined"
                ? params.array
                : (array as any).constructor)(end - start) as any;
              for (let i = 0, _i = end - start; i < _i; i++)
...
function windowFull<T>(c: Column<T>, start: number, end: number): Column<T> {
  ...
  return {
    ...
    areValuesEqual:
      start === 0 ? ave : (rowA, rowB) => ave(rowA + start, rowB + start),
  };
}
