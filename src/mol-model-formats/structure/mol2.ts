...
import { Mol2Crysin, Mol2File } from '../../mol-io/reader/mol2/schema.ts';
    ...
    for (let i = 0, il = mol2.structures.length; i < il; ++i) {
        ...
        if (_models.frameCount > 0) {
            const indexA = Column.ofIntArray(
                Column.mapToArray(bonds.origin_atom_id, (x) => x - 1, Int32Array),
            );
            const indexB = Column.ofIntArray(
                Column.mapToArray(bonds.target_atom_id, (x) => x - 1, Int32Array),
            );
