...
function getGraph(
    indexA: ArrayLike<ElementIndex>,
    indexB: ArrayLike<ElementIndex>,
    props: Partial<IndexPairsProps>,
    count: number,
): IndexPairs {
    ...
    for (let i = 0, _i = builder.edgeCount; i < _i; i++) {
        ...
        builder.assignDirectedProperty(
            operatorA,
            props.operatorA ? props.operatorA[i] : -1,
            operatorB,
            props.operatorB ? props.operatorB[i] : -1,
        );
...
export namespace IndexPairBonds {
    ...
    export function fromData(data: Data, props: Partial<Props> = {}): IndexPairBonds {
        ...
        const bonds = getGraph(
            indexA,
            indexB,
            { key, operatorA, operatorB, order, distance, flag },
            count,
        );
    ...
    export function getEdgeIndexForOperators(
        bonds: IndexPairs,
        i: ElementIndex,
        j: ElementIndex,
        opI: number,
        ...
        for (let t = bonds.offset[a], _t = bonds.offset[a + 1]; t < _t; t++) {
            if (
                bonds.b[t] === b && bonds.edgeProps.operatorA[t] === opA &&
                bonds.edgeProps.operatorB[t] === opB
            ) return t;
        }
        return -1;
    }
