...
import type { GroAtoms, GroFile } from '../../mol-io/reader/gro/schema.ts';
...
function getBasic(atoms: GroAtoms, modelNum: number): BasicData {
    ...
    for (let i = 0, il = atoms.count; i < il; ++i) {
        ...
        if (residueNumber !== prevResidueNumber) {
            ...
            currentEntityId = entityBuilder.getEntityId(
                compId,
                moleculeType,
                currentAsymId,
            );
    ...
    const atom_site = Table.ofPartialColumns(
        BasicSchema.atom_site,
        {
            ...
            Cartn_x: Column.ofFloatArray(
                Column.mapToArray(atoms.x, (x) => x * 10, Float32Array),
            ),
            Cartn_y: Column.ofFloatArray(
                Column.mapToArray(atoms.y, (y) => y * 10, Float32Array),
            ),
            Cartn_z: Column.ofFloatArray(
                Column.mapToArray(atoms.z, (z) => z * 10, Float32Array),
            ),
            ...
            pdbx_PDB_model_num: Column.ofConst(
                modelNum,
                atoms.count,
                Column.Schema.int,
            ),
