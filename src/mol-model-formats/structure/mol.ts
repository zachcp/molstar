...
import { formalChargeMapper, MolFile } from '../../mol-io/reader/mol/parser.ts';
...
export async function getMolModels(
    mol: MolFile,
    format: ModelFormat<any> | undefined,
    ctx: RuntimeContext,
) {
    ...
    if (formalCharges.atomIdx.rowCount > 0) {
        for (let i = 0; i < formalCharges.atomIdx.rowCount; i++) {
            computedFormalCharges[formalCharges.atomIdx.value(i) - 1] = formalCharges.charge.value(
                i,
            );
        }
    } else {
    ...
    if (models.frameCount > 0) {
        const indexA = Column.ofIntArray(
            Column.mapToArray(bonds.atomIdxA, (x) => x - 1, Int32Array),
        );
        const indexB = Column.ofIntArray(
            Column.mapToArray(bonds.atomIdxB, (x) => x - 1, Int32Array),
        );
