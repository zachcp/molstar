...
import { Model, Symmetry } from '../../mol-model/structure/model.ts';
...
import { CifCore_Database } from '../../mol-io/reader/cif/schema/cif-core.ts';
...
function getSymmetry(db: CifCore_Database): Symmetry {
    ...
    const spaceCell = SpacegroupCell.create(
        ...
        Vec3.scale(
            Vec3(),
            Vec3.create(
                cell.angle_alpha.value(0),
                cell.angle_beta.value(0),
                cell.angle_gamma.value(0),
            ),
            Math.PI / 180,
        ),
    );
