...
function findHierarchyOffsets(atom_site: AtomSite) {
    ...
    const residues = [start as ElementIndex],
        chains = [start as ElementIndex];
    ...
    for (let i = (start + 1) as ElementIndex; i < end; i++) {
        const newChain = !label_entity_id.areValuesEqual(i - 1, i) ||
            !label_asym_id.areValuesEqual(i - 1, i);
...
function createHierarchyData(
    ...
    offsets: { residues: ArrayLike<number>; chains: ArrayLike<number> },
): AtomicData {
    ...
    const residueSourceIndex = getResidueSourceIndex(
        sourceIndex,
        offsets.residues.length,
        offsets.residues,
    );
...
function getResidueSourceIndex(
    ...
    const sortedResidues = range(nResidues).sort(
        (ia, ib) => residueSourceIndex[ia] - residueSourceIndex[ib],
    );
...
function getConformation(atom_site: AtomSite): AtomicConformation {
    return {
        id: UUID.create22(),
        atomId: atom_site.id,
        occupancy: atom_site.occupancy.isDefined
            ? atom_site.occupancy
            : Column.ofConst(1, atom_site._rowCount, Column.Schema.float),
        B_iso_or_equiv: atom_site.B_iso_or_equiv,
        xyzDefined: atom_site.Cartn_x.isDefined &&
            atom_site.Cartn_y.isDefined &&
            atom_site.Cartn_z.isDefined,
...
function isHierarchyDataEqual(a: AtomicData, b: AtomicData) {
    return (
        Table.areEqual(a.chains, b.chains) &&
        Table.areEqual(a.residues, b.residues) &&
        Table.areEqual(a.atoms, b.atoms)
    );
}

function createChainOperatorMappingAndSubstituteNames(
    hierarchy: AtomicData,
    format: ModelFormat,
) {
    const mapping = new Map<ChainIndex, SymmetryOperator>();
    if (!MmcifFormat.is(format)) return mapping;

    const { molstar_atom_site_operator_mapping: entries } = toDatabase(
        AtomSiteOperatorMappingSchema,
        format.data.frame,
    );
    ...
    const labelMap = new Map<
        string,
        { name: string; operator: SymmetryOperator }
    >();
    const authMap = new Map<string, string>();

    for (let i = 0; i < entries._rowCount; i++) {
        const assembly: SymmetryOperator['assembly'] =
            entries.assembly_operator_id.valueKind(i) === Column.ValueKinds.Present
                ? {
        ...
        const operator = SymmetryOperator.create(
            entries.operator_name.value(i),
            Mat4.identity(),
            {
                assembly,
                spgrOp: entries.symmetry_operator_index.valueKind(i) ===
                        Column.ValueKinds.Present
                    ? entries.symmetry_operator_index.value(i)
                    : void 0,
    ...
    for (let i = 0 as ChainIndex; i < label_asym_id.rowCount; i++) {
        const label = label_asym_id.value(i),
...
function getAtomicHierarchy(
    ...
    const chainOperatorMapping = createChainOperatorMappingAndSubstituteNames(
        hierarchyData,
        format,
    );
    ...
    if (
        previous &&
        isHierarchyDataEqual(previous.atomicHierarchy, hierarchyData)
    ) {
        return {
            sameAsPrevious: true,
    ...
    const hierarchySegments: AtomicSegments = {
        residueAtomSegments: Segmentation.ofOffsets(
            hierarchyOffsets.residues,
            Interval.ofBounds(0, atom_site._rowCount),
        ),
        chainAtomSegments: Segmentation.ofOffsets(
            hierarchyOffsets.chains,
            Interval.ofBounds(0, atom_site._rowCount),
        ),
    };

    const index = getAtomicIndex(hierarchyData, entities, hierarchySegments);
    const derived = getAtomicDerivedData(
        hierarchyData,
        hierarchySegments,
        index,
...
export function getAtomicHierarchyAndConformation(
    atom_site: AtomSite,
    sourceIndex: Column<number>,
    entities: Entities,
    chemicalComponentMap: Model['properties']['chemicalComponentMap'],
    ...
    const { sameAsPrevious, hierarchy, chainOperatorMapping } = getAtomicHierarchy(
        atom_site,
        sourceIndex,
        entities,
        chemicalComponentMap,
