...
import { ChainIndex, ElementIndex } from '../../../mol-model/structure.ts';
...
import { AtomSite } from './schema.ts';
...
function findHierarchyOffsets(atom_site: AtomSite) {
    ...
    const { label_entity_id, label_asym_id, label_seq_id, auth_seq_id, pdbx_PDB_ins_code } =
        atom_site;
...
function getConformation(atom_site: AtomSite): AtomicConformation {
    return {
        ...
        xyzDefined: atom_site.Cartn_x.isDefined && atom_site.Cartn_y.isDefined &&
...
function createChainOperatorMappingAndSubstituteNames(hierarchy: AtomicData, format: ModelFormat) {
    ...
    for (let i = 0; i < entries._rowCount; i++) {
        ...
        const operator = SymmetryOperator.create(entries.operator_name.value(i), Mat4.identity(), {
            assembly,
            spgrOp: entries.symmetry_operator_index.valueKind(i) === Column.ValueKinds.Present
