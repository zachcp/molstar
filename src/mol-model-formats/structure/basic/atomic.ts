...
function findHierarchyOffsets(atom_site: AtomSite) {
  ...
  for (let i = (start + 1) as ElementIndex; i < end; i++) {
    const newChain =
      !label_entity_id.areValuesEqual(i - 1, i) ||
...
function getResidueSourceIndex(
  ...
  for (let iRes = 0; iRes < nResidues; iRes++) {
    ...
    const toAtom =
      iRes + 1 < nResidues
        ? residueOffsets[iRes + 1]
        : atomSourceIndex.rowCount;
  ...
  for (let i = 0; i < entries._rowCount; i++) {
    ...
    const operator = SymmetryOperator.create(
      ...
      Mat4.identity(),
      {
        assembly,
        spgrOp:
          entries.symmetry_operator_index.valueKind(i) ===
...
export function getAtomicHierarchyAndConformation(
  ...
  const { sameAsPrevious, hierarchy, chainOperatorMapping } =
    getAtomicHierarchy(
      atom_site,
      sourceIndex,
      entities,
