...
import { DensityServer_Data_Database } from '../../mol-io/reader/cif/schema/density-server.ts';
...
export function volumeFromDensityServerData(
    ...
    return Task.create<Volume>('Create Volume', async (ctx) => {
        ...
        const cell = SpacegroupCell.create(
            ...
            Vec3.scale(
                Vec3.zero(),
                Vec3.ofArray(info.spacegroup_cell_angles.value(0)),
                Math.PI / 180,
            ),
        );
