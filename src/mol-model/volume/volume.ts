...
export namespace Volume {
    ...
    export namespace IsoValue {
        export type Relative = Readonly<{
            kind: 'relative';
            relativeValue: number;
        }>;
        export type Absolute = Readonly<{
            kind: 'absolute';
    ...
    export function adjustedIsoValue(
        volume: Volume,
        value: number,
        ...
        return PD.Conditioned(
            def,
            {
                absolute: PD.Converted(
                    ...
                    PD.Numeric(
                        mean,
                        { min, max, step: toPrecision(sigma / 100, 2) },
            ...
            (v: Volume.IsoValue, c: 'absolute' | 'relative') =>
                c === 'absolute' ? Volume.IsoValue.toAbsolute(v, sts) : Volume.IsoValue.toRelative(v, sts),
            { isEssential: true },
        );
    }
    ...
    export function areEquivalent(volA: Volume, volB: Volume): boolean {
        return (
            Grid.areEquivalent(volA.grid, volB.grid) &&
            areInstanceTransformsEqual(volA, volB)
        );
    }

    export function areInstanceTransformsEqual(
        volA: Volume,
        volB: Volume,
    ): boolean {
        ...
        for (let i = 0, il = volA.instances.length; i < il; ++i) {
            if (
                !Mat4.areEqual(
                    volA.instances[i].transform,
                    volB.instances[i].transform,
                    EPSILON,
                )
            ) {
                return false;
    ...
    export function areLociEqual(a: Loci, b: Loci): boolean {
        return (
            a.volume === b.volume && OrderedSet.areEqual(a.instances, b.instances)
        );
    }
    ...
    export namespace Isosurface {
        ...
        export function getBoundingSphere(
            ...
            const value = Volume.IsoValue.toAbsolute(
                isoValue,
                volume.grid.stats,
            ).absoluteValue;
            const neg = value < 0;
