...
import type {
  ...
  AtomicConformation,
  AtomicRanges,
} from "./properties/atomic.ts";
import type {
  CoarseHierarchy,
  CoarseConformation,
} from "./properties/coarse.ts";
import type {
  Entities,
...
export namespace Model {
  ...
  export function isCoarseGrained(model: Model): boolean {
    ...
    if (coarseGrained === undefined) {
      ...
      const { polymerType, directionToElementIndex } =
        model.atomicHierarchy.derived.residue;
      ...
      let hasBB = false,
        hasSC1 = false;
      const { label_atom_id, _rowCount: atomCount } =
