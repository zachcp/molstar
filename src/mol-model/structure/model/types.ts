...
import type { EntitySubtype, ChemicalComponent } from "./properties/common.ts";
...
export function getElementFromAtomicNumber(n: number) {
  if (_elementByAtomicNumber.has(n as any))
    return _elementByAtomicNumber.get(n as any)!;
...
export const PolymerTypeAtomRoleId: {
  [k in PolymerType]: { [k in AtomRole]: Set<string> };
} = {
  [PolymerType.NA]: {
    trace: new Set(),
...
export const IonComponentTypeNames = new Set<ChemCompType>(["ion"]);

/** Chemical component type names for lipid (extension to mmcif) */
export const LipidComponentTypeNames = new Set<ChemCompType>(["lipid"]);
...
export const AminoAcidNames = SetUtils.unionMany(
  AminoAcidNamesL,
  AminoAcidNamesD,
);
...
export const PurineBaseNames = new Set([
  "A",
  "G",
  "I",
  "DA",
...
export const PyrimidineBaseNames = new Set([
  "C",
  "T",
  "U",
  "DC",
...
export const BaseNames = SetUtils.unionMany(
  RnaBaseNames,
  DnaBaseNames,
  PeptideBaseNames,
);

export const isPurineBase = (compId: string) =>
  PurineBaseNames.has(compId.toUpperCase());
export const isPyrimidineBase = (compId: string) =>
  PyrimidineBaseNames.has(compId.toUpperCase());
...
export function getPolymerType(
  compType: ChemCompType,
...
export function getEntitySubtype(
  compId: string,
  compType: ChemCompType,
): EntitySubtype {
  compId = compId.toUpperCase();
...
export namespace SecondaryStructureType {
  export const is: (ss: SecondaryStructureType, f: Flag) => boolean =
    BitFlags.has;
...
export namespace BondType {
  ...
  export function isCovalent(flags: BondType.Flag): boolean {
    return (flags & BondType.Flag.Covalent) !== 0;
  }

  export function isAll(flags: BondType.Flag): boolean {
...
export const DefaultResidueHydrophobicity = [0.0, 0.0, 0.0];
