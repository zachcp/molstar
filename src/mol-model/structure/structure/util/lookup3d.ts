...
import { GridLookup3D, Lookup3D, Result } from '../../../../mol-math/geometry.ts';
...
export class StructureLookup3D {
        ...
        for (let t = 0, _t = closeUnits.count; t < _t; t++) {
            ...
            for (let j = 0, _j = groupResult.count; j < _j; j++) {
                StructureResult.add(
                    ctx.result,
                    unit,
                    groupResult.indices[j],
                    groupResult.squaredDistances[j],
                );
            }
        }
        return ctx.result;
    ...
    _nearest(
        ...
        for (let t = 0, _t = closeUnits.count; t < _t; t++) {
            ...
            maxDistResult = Math.max(
                maxDistResult,
                groupResult.squaredDistances[groupResult.count - 1],
            );
            for (let j = 0, _j = groupResult.count; j < _j; j++) {
                heap.insert(groupResult.squaredDistances[j], {
                    index: groupResult.indices[j],
                    unit: unit,
                });
            }
        }
