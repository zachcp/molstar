...
import { ElementIndex, Unit } from '../../../structure.ts';
...
import { isNucleic, isProtein } from '../../model/types.ts';
...
export function getAtomicPolymerElements(unit: Unit.Atomic) {
    ...
    const polymerIt = SortedRanges.transientSegments(
        unit.model.atomicRanges.polymerRanges,
        elements,
    );
    ...
    while (polymerIt.hasNext) {
        ...
        while (residueIt.hasNext) {
            ...
            if (
                OrderedSet.areIntersecting(
                    Interval.ofRange(elements[start], elements[end - 1]),
                    elements,
                )
            ) {
                const elementIndex = traceElementIndex[index];
                indices.push(
                    elementIndex === -1 ? residueAtomSegments.offsets[index] : elementIndex,
                );
            }
        }
    }
...
export function getAtomicGapElements(unit: Unit.Atomic) {
    ...
    while (gapIt.hasNext) {
        ...
        indices.push(
            elementIndexStart === -1 ? residueAtomSegments.offsets[indexStart] : elementIndexStart,
        );
        indices.push(
            elementIndexEnd === -1 ? residueAtomSegments.offsets[indexEnd] : elementIndexEnd,
        );
    }
    return SortedArray.ofSortedArray<ElementIndex>(indices);
}
