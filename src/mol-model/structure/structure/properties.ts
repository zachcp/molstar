...
function p<T>(p: StructureElement.Property<T>) {
    return p;
}
...
const atom = {
    ...
    id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicConformation.atomId.value(l.element)
    ),
    occupancy: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicConformation.occupancy.value(l.element)
    ),
    B_iso_or_equiv: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicConformation.B_iso_or_equiv.value(l.element)
    ),
    sourceIndex: p((l) =>
        Unit.isAtomic(l.unit)
    ...
    type_symbol: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.type_symbol.value(l.element)
    ),
    label_atom_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.label_atom_id.value(l.element)
    ),
    auth_atom_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.auth_atom_id.value(l.element)
    ),
    label_alt_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.label_alt_id.value(l.element)
    ),
    label_comp_id: p(compId),
    auth_comp_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.auth_comp_id.value(l.element)
    ),
    pdbx_formal_charge: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.atoms.pdbx_formal_charge.value(l.element)
    ),
    ...
    vdw_radius: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : VdwRadius(l.unit.model.atomicHierarchy.atoms.type_symbol.value(l.element))
    ),
};
...
function seqId(l: StructureElement.Location) {
    return !Unit.isAtomic(l.unit)
        ? notAtomic()
        : l.unit.model.atomicHierarchy.residues.label_seq_id.value(l.unit.residueIndex[l.element]);
}
...
const residue = {
    ...
    group_PDB: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.residues.group_PDB.value(l.unit.residueIndex[l.element])
    ),
    label_seq_id: p(seqId),
    auth_seq_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.residues.auth_seq_id.value(
                l.unit.residueIndex[l.element],
            )
    ),
    pdbx_PDB_ins_code: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.residues.pdbx_PDB_ins_code.value(
                l.unit.residueIndex[l.element],
            )
    ),
    ...
    isNonStandard: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : microheterogeneityCompIds(l).some((c) =>
                l.unit.model.properties.chemicalComponentMap.get(c)!.mon_nstd_flag[0] === 'n'
            )
    ),
    ...
    secondary_structure_type: p((l) => {
        ...
        return secStruc
            ? secStruc.type[secStruc.getIndex(l.unit.residueIndex[l.element])]
    ...
    chem_comp_type: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.properties.chemicalComponentMap.get(compId(l))!.type
    ),
};
...
const chain = {
    ...
    label_asym_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? l.unit.coarseElements.asym_id.value(l.element)
            : l.unit.model.atomicHierarchy.chains.label_asym_id.value(l.unit.chainIndex[l.element])
    ),
    auth_asym_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? notAtomic()
            : l.unit.model.atomicHierarchy.chains.auth_asym_id.value(l.unit.chainIndex[l.element])
    ),
    label_entity_id: p((l) =>
        !Unit.isAtomic(l.unit)
            ? l.unit.coarseElements.entity_id.value(l.element)
            : l.unit.model.atomicHierarchy.chains.label_entity_id.value(
                l.unit.chainIndex[l.element],
            )
    ),
};
...
const coarse = {
    key: atom.key,
    entityKey: p((l) =>
        !Unit.isCoarse(l.unit) ? notCoarse() : l.unit.coarseElements.entityKey[l.element]
    ),
    ...
    asym_id: p((l) =>
        !Unit.isCoarse(l.unit) ? notCoarse() : l.unit.coarseElements.asym_id.value(l.element)
    ),
    entity_id: p((l) =>
        !Unit.isCoarse(l.unit) ? notCoarse() : l.unit.coarseElements.entity_id.value(l.element)
    ),
    seq_id_begin: p((l) =>
        !Unit.isCoarse(l.unit) ? notCoarse() : l.unit.coarseElements.seq_id_begin.value(l.element)
    ),
    seq_id_end: p((l) =>
        !Unit.isCoarse(l.unit) ? notCoarse() : l.unit.coarseElements.seq_id_end.value(l.element)
    ),

    sphere_radius: p((l) =>
        !Unit.isSpheres(l.unit) ? notCoarse('spheres') : l.unit.coarseConformation.radius[l.element]
    ),
    sphere_rmsf: p((l) =>
        !Unit.isSpheres(l.unit) ? notCoarse('spheres') : l.unit.coarseConformation.rmsf[l.element]
    ),

    gaussian_weight: p((l) =>
        !Unit.isGaussians(l.unit)
            ? notCoarse('gaussians')
            : l.unit.coarseConformation.weight[l.element]
    ),
    gaussian_covariance_matrix: p((l) =>
        !Unit.isGaussians(l.unit)
            ? notCoarse('gaussians')
            : l.unit.coarseConformation.covariance_matrix[l.element]
    ),
};
...
function eK(l: StructureElement.Location) {
    switch (l.unit.kind) {
        case Unit.Kind.Atomic:
            return l.unit.model.atomicHierarchy.index.getEntityFromChain(
                l.unit.chainIndex[l.element],
            );
...
const entity = {
    ...
    pdbx_number_of_molecules: p((l) =>
        l.unit.model.entities.data.pdbx_number_of_molecules.value(eK(l))
    ),
...
const unit = {
    ...
    pdbx_struct_assembly_id: p((l) =>
        l.unit.conformation.operator.assembly?.id || SymmetryOperator.DefaultName
    ),
    pdbx_struct_oper_list_ids: p((l) =>
        l.unit.conformation.operator.assembly?.operList || _emptyList
    ),
