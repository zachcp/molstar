...
export namespace Bundle {
  ...
  export function fromSubStructure(
    parent: Structure,
    structure: Structure,
  ): Bundle {
    return fromLoci(
  ...
  export function fromSelection(selection: StructureSelection): Bundle {
  ...
  export function fromLoci(loci: Loci): Bundle {
    ...
    elementGroups.forEach((e) => {
      ...
      e.groupedUnits.forEach((g) =>
        groupedUnits.push(SortedArray.ofUnsortedArray(g)),
      );
  ...
  function elementToExpression(e: BundleElement): Expression {
    return MS.internal.generator.bundleElement({
      groupedUnits: MS.core.type.list(
        e.groupedUnits.map((u) => MS.core.type.list(u)),
      ),
  ...
  export function areEqual(a: Bundle, b: Bundle): boolean {
    ...
    for (let i = 0, il = a.elements.length; i < il; ++i) {
      ...
      for (let j = 0, jl = elementB.groupedUnits.length; j < jl; ++j) {
        if (
          !SortedArray.areEqual(
            elementA.groupedUnits[j],
            elementB.groupedUnits[j],
          )
        )
          return false;
      }
