...
import type { UnitIndex } from "./element.ts";
...
import type { Box3D, Sphere3D } from "../../../../mol-math/geometry.ts";
import { compile } from "../../../../mol-script/runtime/query/base.ts";
import { QueryContext, type QueryFn, StructureSelection } from "../../query.ts";
...
export function Loci(
  ...
  return {
    kind: "element-loci",
    structure,
    elements: elements as Loci["elements"],
  };
}
...
export namespace Loci {
  ...
  export function isWholeStructure(loci: Loci): boolean {
  ...
  export function fromExpression(
    ...
    const selection = compile(expr)(
      queryContext ?? new QueryContext(structure),
    );
  ...
  export function getFirstLocation(
    ...
    const element =
      unit.elements[OrderedSet.getAt(loci.elements[0].indices, 0)];
    if (e) {
    ...
    return Loci(loci.structure, [
      {
        ...
        indices: OrderedSet.ofSingleton(
          OrderedSet.start(loci.elements[0].indices),
        ),
      },
    ]);
  }
   ...
   * the value around.
   */
  export function forEachLocation(
    loci: Loci,
    f: (loc: Location) => void,
    location?: Location,
  ) {
  ...
  export function remap(loci: Loci, structure: Structure): Loci {
    ...
    loci.elements.forEach((e) => {
      ...
      const indices = OrderedSet.indexedIntersect(
        e.indices,
        e.unit.elements,
        unit.elements,
      );
  ...
  function makeIndexSet(
    newIndices: ArrayLike<UnitIndex>,
  ): OrderedSet<UnitIndex> {
  ...
  export function extendToWholeResidues(
    loci: Loci,
    ...
    for (const lociElement of loci.elements) {
      ...
      if (lociElement.unit.kind === Unit.Kind.Atomic) {
        ...
        const { index: residueIndex, offsets: residueOffsets } =
        ...
        while (i < len) {
          ...
          for (
            let j = residueOffsets[rI], _j = residueOffsets[rI + 1];
            j < _j;
            j++
          ) {
  ...
  function extendGroupToWholeChains(
    ...
    for (let lI = start; lI < end; lI++) {
      ...
      for (let i = 0, _i = OrderedSet.size(indices); i < _i; i++) {
        chainIndices.add(
          chainIndex[unitElements[OrderedSet.getAt(indices, i)]],
        );
      }
    }
  ...
  export function extendToWholeChains(loci: Loci): Loci {
    ...
    for (let i = 0, len = loci.elements.length; i < len; i++) {
      ...
      if (Unit.Traits.is(e.unit.traits, Unit.Trait.Partitioned)) {
        ...
        while (
          i < len &&
          Unit.areSameChainOperatorGroup(loci.elements[i].unit, e.unit)
        ) {
          i++;
        }
        const end = i;
  ...
  export function extendToWholeEntities(loci: Loci): Loci {
    ...
    for (let i = 0, il = units.length; i < il; ++i) {
      ...
      if (entities.has(entityModelKey(l))) {
        const indices = OrderedSet.ofBounds(
          0,
          unit.elements.length,
        ) as OrderedSet<UnitIndex>;
  ...
  function getUnitIndices(
    ...
    if (
      SortedArray.isRange(elements) &&
      SortedArray.areEqual(elements, indices)
    ) {
      return Interval.ofLength(elements.length);
    }
    return makeIndexSet(
      SortedArray.indicesOf<ElementIndex, UnitIndex>(elements, indices),
    );
  }
  ...
  export function extendToAllInstances(loci: Loci): Loci {
    ...
    for (let i = 0, len = loci.elements.length; i < len; i++) {
      ...
      if (byModel.has(model)) {
        byModel.set(
          model,
          SortedArray.union(elementIndices, byModel.get(model)!),
        );
      } else {
  ...
  export function getBoundary(
    ...
    for (const e of loci.elements) {
      ...
      for (let i = 0, _i = OrderedSet.size(indices); i < _i; i++) {
        ...
        boundaryHelper.includePositionRadius(
          tempPosBoundary,
          conformation.r(eI),
        );
      }
    ...
    for (const e of loci.elements) {
      ...
      for (let i = 0, _i = OrderedSet.size(indices); i < _i; i++) {
        ...
        if (transform)
          Vec3.transformMat4(tempPosBoundary, tempPosBoundary, transform);
        boundaryHelper.radiusPositionRadius(
          tempPosBoundary,
          conformation.r(eI),
        );
      }
  ...
  const tempPos = Vec3();
  export function toPositionsArray(
    loci: Loci,
  ...
  export function getPrincipalAxes(loci: Loci): PrincipalAxes {
    ...
    const positions = toPositionsArray(
      loci,
      new Float32Array(3 * elementCount),
    );
        ...
        opQueries.push(
          MS.struct.generator.atomGroups({
            ...
            "entity-test": MS.core.logic.and([
              ...
              MS.core.rel.eq([
                MS.struct.atomProperty.core.modelIndex(),
                modelIndex,
              ]),
            ]),
          }),
      ...
      opQueries.length === 1
        ...
        : // Need to union before merge for fast performance
          MS.struct.combinator.merge(
            opQueries.map((q) => MS.struct.modifier.union([q])),
          ),
    ]);
  }
