...
import { ChainIndex, ElementIndex, ResidueIndex } from '../../model.ts';
...
export function atomicProperty<T>(p: (location: Location<Unit.Atomic>) => T) {
    return property((l) =>
        Unit.isAtomic(l.unit) ? p(l as Location<Unit.Atomic>) : _wrongUnitKind('atomic')
    );
}

export function coarseProperty<T>(p: (location: Location<Unit.Spheres | Unit.Gaussians>) => T) {
    return property((l) =>
        Unit.isCoarse(l.unit)
            ? p(l as Location<Unit.Spheres | Unit.Gaussians>)
            : _wrongUnitKind('coarse')
    );
}
