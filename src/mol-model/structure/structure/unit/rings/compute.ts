  ...
  while (head < size) {
    ...
    for (let i = start; i < end; i++) {
      ...
      if (
        b < startVertex ||
        b >= endVertex ||
        !BondType.isCovalent(bondFlags[i])
      )
        continue;
...
export function getFingerprint(elements: string[]) {
  ...
  for (let i = 0; i < len; i++) {
    const u = elements[(i + rotNormal) % len],
...
export function createIndex(
  ...
  const elementRingIndices: Map<StructureElement.UnitIndex, RingIndex[]> =
    new Map();
  const elementAromaticRingIndices: Map<
  ...
  const ringComponentIndex =
    components.componentIndex as any as RingComponentIndex[];
