...
import { BondType, type ElementSymbol } from '../../model/types.ts';
...
class UnitRings {
    ...
    private _byFingerprint?: ReadonlyMap<
        UnitRing.Fingerprint,
        ReadonlyArray<UnitRings.Index>
    >;
    private _index?: {
        readonly elementRingIndices: ReadonlyMap<
    ...
    get elementRingIndices(): ReadonlyMap<
        StructureElement.UnitIndex,
        UnitRings.Index[]
    > {
    ...
    get elementAromaticRingIndices(): ReadonlyMap<
        StructureElement.UnitIndex,
        UnitRings.Index[]
    > {
    ...
    constructor(
        all: ReadonlyArray<UnitRing>,
        public unit: Unit.Atomic,
    ) {
        this.all = all;
    }
}
...
namespace UnitRing {
    ...
    export function elementFingerprint(
        elements: ArrayLike<ElementSymbol>,
    ): Fingerprint {
        return getFingerprint(
            elements as ArrayLike<string> as string[],
        ) as Fingerprint;
    }
    ...
    export function isAromatic(unit: Unit.Atomic, ring: UnitRing): boolean {
        ...
        for (let i = 0, il = ring.length; i < il; ++i) {
            const aI = ring[i];
            if (
                !hasAromaticRingElement &&
                AromaticRingElements.has(type_symbol.value(elements[aI]))
            ) {
    ...
    export function getAltId(unit: Unit.Atomic, ring: UnitRing): string {
