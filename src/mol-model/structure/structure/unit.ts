...
import { computeIntraUnitBonds, type IntraUnitBonds } from './unit/bonds.ts';
import type {
    CoarseElements,
    CoarseGaussianConformation,
    CoarseSphereConformation,
} from '../model/properties/coarse.ts';
import { BitFlags } from '../../../mol-util/index.ts';
import { UnitRings } from './unit/rings.ts';
import type { StructureElement } from './element.ts';
import type { ChainIndex, ElementIndex, ResidueIndex } from '../model/indexing.ts';
import { IntMap, Segmentation, SortedArray } from '../../../mol-data/int.ts';
import { hash2, hashFnv32a } from '../../../mol-data/util.ts';
import {
    getAtomicGapElements,
    getAtomicPolymerElements,
    getCoarseGapElements,
    getCoarsePolymerElements,
    getNucleotideElements,
    getProteinElements,
...
import { type Boundary, getBoundary, getFastBoundary } from '../../../mol-math/geometry/boundary.ts';
...
import { getResonance, type UnitResonance } from './unit/resonance.ts';
import type { UUID } from '../../../mol-util/uuid.ts';
import type { AtomicConformation, AtomicHierarchy } from '../model/properties/atomic.ts';
import type { CoarseHierarchy } from '../model/properties/coarse.ts';
...
namespace Unit {
    ...
    export enum Kinds {
        Atomic = Kind.Atomic,
        Spheres = Kind.Spheres,
        Gaussians = Kind.Gaussians,
    }
    ...
    export function create<K extends Kind>(
        id: number,
        invariantId: number,
        chainGroupId: number,
        traits: Traits,
        ...
        props?: K extends Kind.Atomic ? AtomicProperties : CoarseProperties,
    ): Unit {
        switch (kind) {
            case Kind.Atomic:
                return new Atomic(
            ...
            case Kind.Spheres:
                return createCoarse(
                    ...
                    Kind.Spheres,
                    elements,
                    SymmetryOperator.createMapping(
                        operator,
                        model.coarseConformation.spheres,
                        getSphereRadiusFunc(model),
                    ),
                    props ?? CoarseProperties(),
                );
            case Kind.Gaussians:
                return createCoarse(
                    id,
    ...
    export namespace SymmetryGroup {
        export function areInvariantElementsEqual(
            a: SymmetryGroup,
            b: SymmetryGroup,
        ): boolean {
        ...
        export function getUnitSymmetryGroupsIndexMap(
            symmetryGroups: ReadonlyArray<Unit.SymmetryGroup>,
        ): IntMap<number> {
    ...
    export function conformationId(unit: Unit): UUID {
    ...
    export class Atomic implements Base {
        ...
        get transientCache(): Map<any, any> {
            if (this._transientCache === void 0) {
                this._transientCache = new Map<any, any>();
            }
        ...
        remapModel(
            ...
            if (
                operator.spgrOp !== -1 &&
                symmetry &&
                symmetry !== ModelSymmetry.Provider.get(this.model)
            ) {
                const [i, j, k] = operator.hkl;
                const { toFractional } = symmetry.spacegroup.cell;
                const ref = Vec3.transformMat4(
                    Vec3(),
                    Model.getCenter(model),
                    toFractional,
                );
                operator = Spacegroup.getSymmetryOperatorRef(
                    symmetry.spacegroup,
                    operator.spgrOp,
                    i,
                    j,
            ...
            const conformation = this.model.atomicConformation !== model.atomicConformation ||
                    operator !== this.conformation.operator
                ? SymmetryOperator.createMapping<ElementIndex>(
                    operator,
        ...
        get residueCount(): number {
            ...
            const residueIt = Segmentation.transientSegments(
                this.model.atomicHierarchy.residueAtomSegments,
                this.elements,
            );
        ...
        getResidueIndex(elementIndex: StructureElement.UnitIndex): number {
    ...
    class Coarse<
        K extends Kind.Gaussians | Kind.Spheres,
        C extends CoarseSphereConformation | CoarseGaussianConformation,
    > implements Base {
        readonly kind: K;
        ...
        remapModel(
            ...
            if (!props) {
                props = { ...this.props };
                if (
                    !Unit.isSameConformation(this as Unit.Spheres | Unit.Gaussians, model)
                ) {
            ...
            const conformation = coarseConformation !== modelCoarseConformation
                ? SymmetryOperator.createMapping(
                    this.conformation.operator,
                    modelCoarseConformation,
                    this.kind === Unit.Kind.Spheres ? getSphereRadiusFunc(model) : getGaussianRadiusFunc(model),
                )
                : this.conformation;
            return new Coarse(
                this.id,
                ...
                conformation,
                props,
            ) as Unit.Spheres | Unit.Gaussians; // TODO get rid of casting
        }
        ...
        get principalAxes(): PrincipalAxes {
            if (this.props.principalAxes) return this.props.principalAxes;
            this.props.principalAxes = getPrincipalAxes(
                this as Unit.Spheres | Unit.Gaussians,
            ); // TODO get rid of casting
            return this.props.principalAxes;
        }
        ...
        get polymerElements(): SortedArray<ElementIndex> {
            if (this.props.polymerElements) return this.props.polymerElements;
            this.props.polymerElements = getCoarsePolymerElements(
                this as Unit.Spheres | Unit.Gaussians,
            ); // TODO get rid of casting
            return this.props.polymerElements;
        }
        ...
        get gapElements(): SortedArray<ElementIndex> {
            if (this.props.gapElements) return this.props.gapElements;
            this.props.gapElements = getCoarseGapElements(
                this as Unit.Spheres | Unit.Gaussians,
            ); // TODO get rid of casting
            return this.props.gapElements;
        }
        ...
        constructor(
            ...
            this.elements = elements;
            this.conformation = conformation;
            this.coarseElements = kind === Kind.Spheres
                ? model.coarseHierarchy.spheres
                : model.coarseHierarchy.gaussians;
            this.coarseConformation = (
                kind === Kind.Spheres ? model.coarseConformation.spheres : model.coarseConformation.gaussians
            ) as C;
            this.props = props;
        }
    }
    ...
    export class Gaussians extends Coarse<
        Kind.Gaussians,
        CoarseGaussianConformation
    > {}

    function createCoarse<K extends Kind.Gaussians | Kind.Spheres>(
        id: number,
        invariantId: number,
        chainGroupId: number,
        traits: Traits,
        ...
        conformation: SymmetryOperator.ArrayMapping<ElementIndex>,
        props: CoarseProperties,
    ): K extends Kind.Spheres ? Spheres : Gaussians {
        return new Coarse(
            id,
            invariantId,
            chainGroupId,
            ...
            conformation,
            props,
        ) as any; // lets call this an ugly temporary hack
    }

    export function areSameChainOperatorGroup(a: Unit, b: Unit): boolean {
        return (
            a.chainGroupId === b.chainGroupId &&
            a.conformation.operator.name === b.conformation.operator.name
        );
    }

    export function areOperatorsEqual(a: Unit, b: Unit): boolean {
        return Mat4.areEqual(
            a.conformation.operator.matrix,
            b.conformation.operator.matrix,
            1e-6,
        );
    }
    ...
    function tryRemapBonds(
        a: Atomic,
        old: IntraUnitBonds | undefined,
        model: Model,
        dynamicBonds: boolean,
    ) {
    ...
    export function getModelConformationOfKind(
        kind: Unit.Kind,
        model: Model,
    ):
        | AtomicConformation
    ...
    export function getConformation(
        u: Unit,
    ):
        | AtomicConformation
        | CoarseSphereConformation
    ...
    export function getModelHierarchyOfKind(
        kind: Unit.Kind,
        model: Model,
    ): AtomicHierarchy | CoarseElements {
        return kind === Kind.Atomic
    ...
    export function getHierarchy(u: Unit): AtomicHierarchy | CoarseElements {
