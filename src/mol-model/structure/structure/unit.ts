...
import {
  ...
  getBoundary,
  getFastBoundary,
} from "../../../mol-math/geometry/boundary.ts";
...
import type {
  AtomicConformation,
  AtomicHierarchy,
} from "../model/properties/atomic.ts";
...
namespace Unit {
  ...
  export function conformationId(unit: Unit): UUID {
    return Unit.isAtomic(unit)
      ? unit.model.atomicConformation.id
      : unit.model.coarseConformation.id;
  }
  ...
  export class Atomic implements Base {
    ...
    applyOperator(
      ...
      const op = dontCompose
        ? operator
        : SymmetryOperator.compose(this.conformation.operator, operator);
      return new Atomic(
        id,
        this.invariantId,
        this.chainGroupId,
    ...
    remapModel(
      ...
      if (!props) {
        props = {
          ...this.props,
          bonds:
            dynamicBonds && !this.props.bonds?.props?.canRemap
              ? undefined
      ...
      const conformation =
        this.model.atomicConformation !== model.atomicConformation ||
  ...
  > implements Base
  {
    ...
    remapModel(
      ...
      const conformation =
        coarseConformation !== modelCoarseConformation
          ? SymmetryOperator.createMapping(
              this.conformation.operator,
              modelCoarseConformation,
              this.kind === Unit.Kind.Spheres
                ? getSphereRadiusFunc(model)
                : getGaussianRadiusFunc(model),
            )
          : this.conformation;
