...
import type { CoarseElements } from "../model/properties/coarse.ts";
...
import type { Carbohydrates } from "./carbohydrates/data.ts";
...
import type { ElementSymbol } from "../model/types.ts";
...
class Structure {
  ...
  get bondCount(): number {
    if (this.state.bondCount === -1) {
      this.state.bondCount =
        this.interUnitBonds.edgeCount / 2 + Bond.getIntraUnitBondCount(this);
    }
  ...
  get interUnitBonds(): InterUnitBonds {
    ...
    if (
      this.parent &&
      this.state.dynamicBonds === this.parent.state.dynamicBonds &&
      this.parent.state.interUnitBonds &&
      this.parent.state.interUnitBonds.edgeCount === 0
    ) {
  ...
  get unitSymmetryGroups(): ReadonlyArray<Unit.SymmetryGroup> {
    ...
    this.state.unitSymmetryGroups =
      StructureSymmetry.computeTransformGroups(this);
  ...
  get unitSymmetryGroupsIndexMap(): IntMap<number> {
    ...
    this.state.unitSymmetryGroupsIndexMap =
      Unit.SymmetryGroup.getUnitSymmetryGroupsIndexMap(this.unitSymmetryGroups);
    return this.state.unitSymmetryGroupsIndexMap;
  }
  ...
  > {
    return (
      ...
      (this.state.uniqueAtomicResidueIndices =
        getUniqueAtomicResidueIndices(this))
    );
  }
  ...
  asParent(): Structure {
    ...
    if (this.parent) {
      const p = this.parent.coordinateSystem.isIdentity
        ...
        : Structure.transform(
            this.parent,
            this.parent.coordinateSystem.inverse,
          );
      const s = this.coordinateSystem.isIdentity
        ? p
        : Structure.transform(p, this.coordinateSystem.matrix);
      this._proxy = new Structure(
        s.units,
        s.unitMap,
        s.unitIndexMap,
        { ...s.state, dynamicBonds: this.dynamicBonds },
...
namespace Structure {
  ...
  export function toStructureElementLoci(
    structure: Structure,
  ...
  export async function ofTrajectory(
    ...
    for (let i = 0, il = trajectory.frameCount; i < il; ++i) {
      const frame = await Task.resolveInContext(
        trajectory.getFrameAtIndex(i),
        ctx,
      );
  ...
  export function ofModel(model: Model, props: Props = {}): Structure {
    ...
    for (let c = 0 as ChainIndex; c < chains.count; c++) {
      const operator =
        atomicChainOperatorMappinng.get(c) || SymmetryOperator.Default;
      ...
      if (isWaterChain(model, c)) {
        ...
        while (
          c + 1 < chains.count &&
          isWaterChain(model, (c + 1) as ChainIndex)
        ) {
  ...
  export function transform(s: Structure, transform: Mat4): Structure {
    ...
    if (
      !Mat4.isRotationAndTranslation(
        transform,
        SymmetryOperator.RotationTranslationEpsilon,
      )
    )
    ...
    const newCS = SymmetryOperator.compose(
      SymmetryOperator.create(cs.name, transform, cs),
      cs,
    );
  ...
  export class StructureBuilder {
    ...
    addUnit(
      ...
      const chainGroupId = this.inChainGroup
        ? this.chainGroupId
        : ++this.chainGroupId;
    ...
    copyUnit(
      unit: Unit,
      options?: Unit.GetCopyOptions & { invariantId?: number },
    ): Unit {
    ...
    addWithOperator(
      ...
      return this.add(
        unit.applyOperator(this.units.length, operator, dontCompose),
      );
    }
  ...
  export class ElementLocationIterator
    implements Iterator<StructureElement.Location>
  {
  ...
  export function minDistanceToPoint(
    s: Structure,
    point: Vec3,
    radius: number,
  ) {
    ...
    for (const unit of structure.units) {
      ...
      while (chainsIt.hasNext) {
        ...
        while (residuesIt.hasNext) {
          ...
          for (
            let j = residueSegment.start, _j = residueSegment.end;
            j < _j;
            j++
          ) {
  ...
  function getPolymerSymmetryGroups(structure: Structure) {
    return structure.unitSymmetryGroups.filter(
      (ug) => ug.units[0].polymerElements.length > 0,
    );
  }
  ...
  function isFiberLike(structure: Structure, thresholds: SizeThresholds) {
    ...
    return (
      ...
      polymerSymmetryGroups[0].units[0].polymerElements.length <
        thresholds.fiberResidueCount
    );
  }
  ...
  export function getSize(
    ...
    if (
      ...
      t.largeResidueCount * residueCountFactor
    ) {
      ...
      structure.polymerResidueCount <
      t.smallResidueCount * residueCountFactor
    ) {
      return Size.Small;
    } else if (
      structure.polymerResidueCount <
      t.mediumResidueCount * residueCountFactor
    ) {
      return Size.Medium;
    } else {
      return Size.Large;
