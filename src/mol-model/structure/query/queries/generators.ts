...
import { StructureElement, StructureProperties as P, Unit } from '../../structure.ts';
import { Structure } from '../../structure/structure.ts';
import { UnitRing } from '../../structure/unit/rings.ts';
import { QueryContextView, QueryFn, QueryPredicate } from '../context.ts';
...
export const all: StructureQuery = (ctx) =>
    StructureSelection.Singletons(ctx.inputStructure, ctx.inputStructure);
...
export function atoms(params?: Partial<AtomsQueryParams>): StructureQuery {
    if (
        !params ||
        (!params.atomTest && !params.residueTest && !params.chainTest && !params.entityTest &&
            !params.unitTest && !params.groupBy)
    ) return all;
    if (
        !!params.atomTest && !params.residueTest && !params.chainTest && !params.entityTest &&
        !params.unitTest && !params.groupBy
...
function atomGroupsGrouped(
    ...
    return function query_atomGroupsGrouped(ctx) {
        ...
        for (const unit of units) {
            ...
            if (unit.kind === Unit.Kind.Atomic) {
                const chainsIt = Segmentation.transientSegments(
                    model.atomicHierarchy.chainAtomSegments,
                    elements,
                );
                const residuesIt = Segmentation.transientSegments(
...
export function rings(
    fingerprints?: ArrayLike<UnitRing.Fingerprint>,
    onlyAromatic?: boolean,
): StructureQuery {
