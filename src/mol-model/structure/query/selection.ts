...
import { Structure, StructureElement, type Unit } from "../structure.ts";
import { structureUnion } from "./utils/structure-set.ts";
...
type StructureSelection =
  | StructureSelection.Singletons
  | StructureSelection.Sequence;
...
namespace StructureSelection {
  ...
  export function isEmpty(s: StructureSelection): boolean {
    return isSingleton(s)
      ? s.structure.units.length === 0
      : s.structures.length === 0;
  }

  export function structureCount(sel: StructureSelection): number {
  ...
  export function toLociWithCurrentUnits(
  ...
  export function toLociWithSourceUnits(
    sel: StructureSelection,
  ): StructureElement.Loci {
    const elements: {
      unit: Unit;
