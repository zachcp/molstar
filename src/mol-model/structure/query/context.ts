...
export class QueryContext implements QueryContextView {
  ...
  readonly element: StructureElement.Location =
    StructureElement.Location.create(void 0);
  ...
  pushCurrentElement(): StructureElement.Location {
    ...
    (this.element as StructureElement.Location) =
      StructureElement.Location.create(void 0);
    return this.element;
  }
  ...
  pushCurrentBond(): QueryContextBondInfo<Unit.Atomic> {
    ...
    (this.atomicBond as QueryContextBondInfo<Unit.Atomic>) =
      new QueryContextBondInfo();
  ...
  popCurrentBond() {
    if (this.currentAtomicBondStack.length > 0) {
      (this.atomicBond as QueryContextBondInfo<Unit.Atomic>) =
        this.currentAtomicBondStack.pop()!;
    } else {
  ...
  throwIfTimedOut() {
    ...
    if (now() - this.timeCreated > this.timeoutMs) {
      throw new Error(
        `The query took too long to execute (> ${this.timeoutMs / 1000}s).`,
      );
    }
  }
