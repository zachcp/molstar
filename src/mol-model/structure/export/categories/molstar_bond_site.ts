...
import { StructureElement, StructureProperties, Unit } from '../../structure.ts';
import type { CifExportCategoryInfo, CifExportContext } from '../mmcif.ts';
...
const Fields = CifWriter.fields<number, Entry[], keyof MolstarBondSiteSchema['molstar_bond_site']>()
    ...
    .str('value_order', (i, xs) => xs[i].value_order ?? '', {
        valueKind: (i, xs) =>
            xs[i].value_order === undefined ? Column.ValueKinds.NotPresent : Column.ValueKinds.Present,
    })
