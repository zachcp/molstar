...
import { Structure, StructureElement, StructureProperties as P } from '../../structure.ts';
    ...
    fields
        ...
        .int('label_seq_id', P.residue.label_seq_id, {
            ...
            valueKind: (k, d) => {
                ...
                return m.atomicHierarchy.residues.label_seq_id.valueKind(
                    m.atomicHierarchy.residueAtomSegments.index[k.element],
                );
            },
        })
        ...
        .int('pdbx_formal_charge', P.atom.pdbx_formal_charge, {
            encoder: E.deltaRLE,
            valueKind: (k, d) =>
                k.unit.model.atomicHierarchy.atoms.pdbx_formal_charge.valueKind(k.element),
        })
        ...
        .int('pdbx_label_index', atom_site_pdbx_label_index.value, {
            shouldInclude: atom_site_pdbx_label_index.shouldInclude,
        })
        // SIFTS
        .str('pdbx_sifts_xref_db_name', SIFTS.pdbx_sifts_xref_db_name.value, {
            shouldInclude: SIFTS.shouldInclude,
            valueKind: SIFTS.pdbx_sifts_xref_db_name.valueKind,
        })
        .str('pdbx_sifts_xref_db_acc', SIFTS.pdbx_sifts_xref_db_acc.value, {
            shouldInclude: SIFTS.shouldInclude,
            valueKind: SIFTS.pdbx_sifts_xref_db_acc.valueKind,
        })
        .str('pdbx_sifts_xref_db_num', SIFTS.pdbx_sifts_xref_db_num.value, {
            shouldInclude: SIFTS.shouldInclude,
            valueKind: SIFTS.pdbx_sifts_xref_db_num.valueKind,
        })
        .str('pdbx_sifts_xref_db_res', SIFTS.pdbx_sifts_xref_db_res.value, {
            shouldInclude: SIFTS.shouldInclude,
            valueKind: SIFTS.pdbx_sifts_xref_db_res.valueKind,
        });
        ...
        return {
            ...
            source: structures.map((s) => ({
                data: {
                    ...
                    atom_site: MmcifFormat.is(s.model.sourceData)
                        ? s.model.sourceData.data.db.atom_site
                        : void 0,
                } as AtomSiteData,
...
function mappedProp<K, D>(
    loc: (key: K, data: D) => StructureElement.Location,
    prop: (e: StructureElement.Location) => any,
) {
    return (k: K, d: D) => prop(loc(k, d));
}

function addModelNum<K, D>(
    fields: CifWriter.Field.Builder<K, D>,
    getLocation: (key: K, data: D) => StructureElement.Location,
    options?: IdFieldsOptions,
) {
...
export function residueIdFields<K, D>(
    getLocation: (key: K, data: D) => StructureElement.Location,
    options?: IdFieldsOptions,
): CifField<K, D>[] {
    ...
    const ret = CifWriter.fields<K, D>()
        ...
        .int(
            prepostfixed(prefix, `label_seq_id`),
            mappedProp(getLocation, P.residue.label_seq_id),
            {
                encoder: E.deltaRLE,
                valueKind: (k, d) => {
                    ...
                    return m.atomicHierarchy.residues.label_seq_id.valueKind(
                        m.atomicHierarchy.residueAtomSegments.index[e.element],
                    );
                },
            },
        )
        ...
        .str(
            prepostfixed(prefix, `label_entity_id`),
            mappedProp(getLocation, P.chain.label_entity_id),
        )
...
export function chainIdFields<K, D>(
    getLocation: (key: K, data: D) => StructureElement.Location,
    options?: IdFieldsOptions,
): CifField<K, D>[] {
    const prefix = options && options.prefix;
...
export function entityIdFields<K, D>(
    getLocation: (key: K, data: D) => StructureElement.Location,
    options?: IdFieldsOptions,
): CifField<K, D>[] {
    const prefix = options && options.prefix;
...
export function atomIdFields<K, D>(
    getLocation: (key: K, data: D) => StructureElement.Location,
    options?: IdFieldsOptions,
): CifField<K, D>[] {
    const prefix = options && options.prefix;
