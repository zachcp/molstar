...
export function atom_site_operator_mapping(
    ctx: CifExportContext,
): CifExportCategoryInfo | undefined {
...
const asmValueKind = (i: number, xs: Entry[]) =>
    typeof xs[i].operator.assembly === 'undefined'
...
>()
    ...
    .int('assembly_operator_id', (i, xs) => xs[i].operator.assembly?.operId || 0, {
        valueKind: asmValueKind,
    })
    // symmetry
    .int('symmetry_operator_index', (i, xs) => xs[i].operator.spgrOp, {
