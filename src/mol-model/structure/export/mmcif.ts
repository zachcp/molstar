...
function isWithoutOperator(structure: Structure) {
    return isWithoutSymmetry(structure) &&
        structure.units.every((u) =>
            !u.conformation.operator.assembly && !u.conformation.operator.suffix
        );
}
...
function getCustomPropCategories(
    ...
    for (const cat of cats) {
        ...
        if (cat.name.indexOf(prefix) !== 0) {
            throw new Error(
                `Custom category '${cat.name}' name must start with prefix '${prefix}.'`,
            );
        }
