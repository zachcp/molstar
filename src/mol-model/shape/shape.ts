...
import { createTransform as _createTransform, type TransformData } from '../../mol-geo/geometry/transform-data.ts';
...
export namespace Shape {
    ...
    export function createTransform(
        ...
        const transformArray = transformData &&
                transformData.aTransform.ref.value.length >= transforms.length * 16
    ...
    export function createRenderObject<G extends Geometry>(
    ...
    ): ReturnType<typeof _createRenderObject<G['kind']>> {
        props;
        ...
        const values = utils.createValues(
            shape.geometry,
            transform,
            locationIt,
            theme,
