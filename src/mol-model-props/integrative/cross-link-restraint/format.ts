...
import { ElementIndex } from '../../../mol-model/structure/model/indexing.ts';
    ...
    export function fromTable(
        ...
        function add(row: number, ps: typeof p1) {
            ...
            if (table.model_granularity.value(row) === 'by-atom') {
                ...
                const sphereElement = model.coarseHierarchy.spheres.findSequenceKey(
                    entityId,
                    asymId,
                    seqId,
                );
                if (sphereElement >= 0) {
                    ...
                    const gaussianElement = model.coarseHierarchy.gaussians.findSequenceKey(
                        entityId,
                        asymId,
                        seqId,
                    );
