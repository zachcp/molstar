    ...
    export function create<T>(builder: Builder<T>): CustomElementProperty<T> {
        ...
        return {
            ...
            colorThemeProvider: builder.coloring?.getColor &&
                createColorThemeProvider(
                    modelProperty,
                    builder.coloring.getColor,
                    builder.coloring.defaultColor,
                ),
        ...
        return {
            label: (loci: Loci) => {
                if (loci.kind === 'element-loci') {
                    ...
                    if (
                        !e || !e.unit.model.customProperties.hasReference(modelProperty.descriptor)
                    ) return;
