...
namespace CustomStructureProperty {
  ...
  export function createProvider<Params extends PD.Params, Value>(
    ...
    const propertyDataName =
      builder.type === "root" ? "inheritedPropertyData" : "currentPropertyData";
    ...
    return {
      ...
      attach: async (
        ...
        if (
          property.data.value &&
          PD.areEqual(builder.defaultParams, property.props, p)
        )
          return;
  ...
  export function createSimple<T>(
    ...
    type: "root" | "local",
    defaultValue?: T,
  ): CustomProperty.Provider<Structure, any, any> {
