...
namespace PropertyWrapper {
    ...
    export function defaultInfoCategory<Ctx>(
        name: string,
        getter: (ctx: Ctx) => Info | undefined,
    ): CifWriter.Category<Ctx> {
        return {
            name,
            instance(ctx) {
    ...
    export function tryGetInfoFromCif(categoryName: string, model: Model): Info | undefined {
        ...
        const timestampField = model.sourceData.data.frame.categories[categoryName].getField(
