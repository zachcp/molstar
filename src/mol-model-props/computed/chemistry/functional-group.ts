...
import { BondType, ElementSymbol } from '../../../mol-model/structure/model/types.ts';
 ...
 */
export function isQuaternaryAmine(
    structure: Structure,
    unit: Unit.Atomic,
    index: StructureElement.UnitIndex,
) {
...
export function isImide(
    ...
    if (
        ...
        (bondCount(structure, unit, index) -
                bondToElementCount(structure, unit, index, Elements.H)) === 2
    ) {
 ...
 */
export function isSulfonicAcid(
    structure: Structure,
    unit: Unit.Atomic,
    index: StructureElement.UnitIndex,
) {
...
export function isCarbonyl(
    ...
    if (typeSymbol(unit, index) === Elements.C) {
        ...
        for (let i = offset[index], il = offset[index + 1]; i < il; ++i) {
            if (
                edgeProps.order[i] === 2 &&
                typeSymbol(unit, b[i] as StructureElement.UnitIndex) === Elements.O
            ) {
                flag = true;
                break;
            }
        ...
        eachBondedAtom(
            ...
            (unit: Unit.Atomic, index: StructureElement.UnitIndex) => {
                if (
                    ...
                    bondCount(structure, unit, index) -
                                bondToElementCount(structure, unit, index, Elements.H) === 1
                ) {
...
const PolarElements = new Set<ElementSymbol>(
    ['N', 'O', 'S', 'F', 'CL', 'BR', 'I'] as ElementSymbol[],
);
export function isPolar(element: ElementSymbol) {
    return PolarElements.has(element);
}

export function hasPolarNeighbour(
    structure: Structure,
    unit: Unit.Atomic,
    index: StructureElement.UnitIndex,
) {
...
export function hasAromaticNeighbour(
    structure: Structure,
    unit: Unit.Atomic,
    index: StructureElement.UnitIndex,
) {
