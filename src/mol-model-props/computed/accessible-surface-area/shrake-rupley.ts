...
import { DefaultMaxAsa, MaxAsa, ShrakeRupleyContext, VdWLookup } from './shrake-rupley/common.ts';
...
export const ShrakeRupleyComputationParams = {
    ...
    probeSize: PD.Numeric(1.4, { min: 0.1, max: 4, step: 0.01 }, {
        description:
            'Corresponds to the size of a water molecule: 1.4 (original paper), 1.5 (occassionally used)',
    }),
...
namespace AccessibleSurfaceArea {
    ...
    export function compute(
        structure: Structure,
        props: Partial<ShrakeRupleyComputationProps> = {},
    ) {
    ...
    export function getNormalizedValue(
        ...
        return value === -1
            ? -1
            : normalize(StructureProperties.atom.label_comp_id(location), value);
    }
