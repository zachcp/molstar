...
import { Elements, isHalogen, isTransitionMetal } from '../../../mol-model/structure/model/properties/atomic/types.ts';
...
export const MetalCoordinationParams: PD.Params = {
...
function addMetal(
    ...
    for (
        let i = 0 as StructureElement.UnitIndex, il = elements.length;
        i < il;
        ++i
    ) {
...
function isMetalCoordination(ti: FeatureType, tj: FeatureType) {
    if (ti === FeatureType.TransitionMetal) {
        return (
            tj === FeatureType.DativeBondPartner || tj === FeatureType.TransitionMetal
        );
...
export const MetalProvider = Features.Provider(
    [FeatureType.IonicTypeMetal, FeatureType.TransitionMetal],
    addMetal,
);
export const MetalBindingProvider = Features.Provider(
    [FeatureType.IonicTypePartner, FeatureType.DativeBondPartner],
    addMetalBinding,
);
