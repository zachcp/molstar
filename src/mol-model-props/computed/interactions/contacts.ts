...
export const ContactsParams: PD.Params = {
...
function checkLineOfSight(
  ...
  for (let r = 0; r < count; ++r) {
    ...
    if (
      invalidAltLoc(unit, i, infoA.unit, indexA) ||
      invalidAltLoc(unit, i, infoB.unit, indexB)
    )
      continue;
    ...
    if (
      Vec3.squaredDistance(tmpVec, tmpVecA) < 1 ||
      Vec3.squaredDistance(tmpVec, tmpVecB) < 1
    )
      continue;
...
function _addUnitContacts(
  ...
  const { lookup3d, indices: subsetIndices } = features.subset(
    tester.requiredFeatures,
  );
