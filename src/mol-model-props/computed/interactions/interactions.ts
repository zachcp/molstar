...
namespace Interactions {
  ...
  export interface Location
    extends DataLocation<StructureInteractions, Element> {}
  ...
  export function Loci(
    ...
    return DataLoci(
      ...
      elements,
      (boundingSphere) =>
        getBoundingSphere(interactions, elements, boundingSphere),
  ...
  export function getBoundingSphere(
    ...
    return CentroidHelper.fromPairProvider(
      elements.length,
      (i, pA, pB) => {
        const e = elements[i];
        Features.setPosition(
          ...
          e.unitA,
          e.indexA,
          unitsFeatures.get(e.unitA.id),
        );
        Features.setPosition(
          pB,
          e.unitB,
          e.indexB,
          unitsFeatures.get(e.unitB.id),
        );
      },
      boundingSphere,
    );
...
export async function computeInteractions(
  ...
  contactTesters.forEach((l) =>
    SetUtils.add(requiredFeatures, l.requiredFeatures),
  );
  ...
  for (let i = 0, il = structure.unitSymmetryGroups.length; i < il; ++i) {
    ...
    const features = findUnitFeatures(
      structure,
      group.units[0],
      featureProviders,
      cacheKey,
