...
const GeometryParams = {
    ...
    ignoreHydrogens: PD.Boolean(false, {
        description: 'Ignore explicit hydrogens in geometric constraints',
    }),
    donAngleDevMax: PD.Numeric(45, { min: 0, max: 180, step: 1 }, {
 ...
 * Weak hydrogen donor.
 */
function addUnitWeakHydrogenDonors(
    structure: Structure,
    unit: Unit.Atomic,
 ...
 */
function addUnitHydrogenAcceptors(
    structure: Structure,
    unit: Unit.Atomic,
    builder: FeaturesBuilder,
) {
...
function isWater(unit: Unit.Atomic, index: StructureElement.UnitIndex) {
    return unit.model.atomicHierarchy.derived.residue
        .moleculeType[unit.residueIndex[unit.elements[index]]] === MoleculeType.Water;
}
...
function isHistidineNitrogen(unit: Unit.Atomic, index: StructureElement.UnitIndex) {
    return compId(unit, index) === 'HIS' && typeSymbol(unit, index) === Elements.N &&
        isRing(unit, index);
}

function isBackboneHydrogenBond(
    unitA: Unit.Atomic,
...
function checkGeometry(
    ...
    const [donAngles, donHAngles] = calcAngles(
        structure,
        don.unit,
        donIndex,
        acc.unit,
        accIndex,
        opts.ignoreHydrogens,
    );
    ...
    const [accAngles, accHAngles] = calcAngles(
        structure,
        acc.unit,
        accIndex,
        don.unit,
...
function testHydrogenBond(
    structure: Structure,
    infoA: Features.Info,
    infoB: Features.Info,
    distanceSq: number,
    opts: HydrogenBondsOptions,
    ...
    const maxDistSq = typeSymbol(don.unit, donIndex) === Elements.S ||
            typeSymbol(acc.unit, accIndex) === Elements.S
...
export const HydrogenDonorProvider = Features.Provider(
    [FeatureType.HydrogenDonor],
    addUnitHydrogenDonors,
);
export const WeakHydrogenDonorProvider = Features.Provider(
    [FeatureType.WeakHydrogenDonor],
    addUnitWeakHydrogenDonors,
);
export const HydrogenAcceptorProvider = Features.Provider(
    [FeatureType.HydrogenAcceptor],
    ...
    createTester: (props: HydrogenBondsProps) => {
        ...
        return {
            ...
            getType: (structure, infoA, infoB, distanceSq) =>
                testHydrogenBond(structure, infoA, infoB, distanceSq, opts),
        };
    },
};
...
export const WeakHydrogenBondsProvider: ContactProvider<WeakHydrogenBondsParams> = {
    ...
    createTester: (props: WeakHydrogenBondsProps) => {
        ...
        return {
            ...
            getType: (structure, infoA, infoB, distanceSq) =>
                testWeakHydrogenBond(structure, infoA, infoB, distanceSq, opts),
        };
    },
};
