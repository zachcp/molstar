...
const HalogenBondsParams: PD.Params = {
...
function testHalogenBond(
  ...
  const [acceptorAngles] = calcAngles(
    structure,
    acc.unit,
    accIndex,
    don.unit,
  ...
  if (
    acceptorAngles.some(
      (acceptorAngle) => OptimalAcceptorAngle - acceptorAngle > opts.angleMax,
    )
  )
    return;
...
export const HalogenBondsProvider: ContactProvider<HalogenBondsParams> = {
  ...
  createTester: (props: HalogenBondsProps) => {
    ...
    return {
      ...
      getType: (structure, infoA, infoB) =>
        testHalogenBond(structure, infoA, infoB, opts),
    };
  },
};
