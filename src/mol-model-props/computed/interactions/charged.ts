...
import { BaseNames, PolymerNames, ProteinBackboneAtoms } from '../../../mol-model/structure/model/types.ts';
...
const CationPiParams: PD.Params = {
...
function addUnitPositiveCharges(
    structure: Structure,
    unit: Unit.Atomic,
...
function addUnitNegativeCharges(
    structure: Structure,
    unit: Unit.Atomic,
    builder: FeaturesBuilder,
) {
...
function areFeaturesWithinDistanceSq(
    ...
    for (let i = offsetsA[featureA], il = offsetsA[featureA + 1]; i < il; ++i) {
        ...
        for (let j = offsetsB[featureB], jl = offsetsB[featureB + 1]; j < jl; ++j) {
            ...
            infoB.unit.conformation.position(
                infoB.unit.elements[elementB],
                tmpPointB,
            );
...
function testPiStacking(
    ...
    if (isPiStacking(typeA, typeB)) {
        ...
        if (offset <= opts.offsetMax) {
            if (
                angle <= opts.angleDevMax ||
                angle >= deg180InRad - opts.angleDevMax
            ) {
                return InteractionType.PiStacking; // parallel
            } else if (
                angle <= opts.angleDevMax + deg90InRad &&
                angle >= deg90InRad - opts.angleDevMax
            ) {
