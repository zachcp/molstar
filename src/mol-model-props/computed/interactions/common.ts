...
interface InteractionsIntraContacts extends
    IntAdjacencyGraph<
        Features.FeatureIndex,
        InteractionsIntraContacts.Props
    > {
...
namespace InteractionsIntraContacts {
    ...
    export function createElementsIndex(
        ...
        for (let i = 0, il = contacts.edgeCount * 2; i < il; ++i) {
            ...
            for (
                let j = featureOffsets[aI], jl = featureOffsets[aI + 1];
                j < jl;
                ++j
            ) {
            ...
            for (
                let j = featureOffsets[bI], jl = featureOffsets[bI + 1];
                j < jl;
                ++j
            ) {
                ++bucketSizes[members[j]];
