...
export class SuperpositionControls extends PurePluginUIComponent<{}, SuperpositionControlsState> {
        ...
        this.subscribe(this.plugin.managers.structure.hierarchy.behaviors.selection, (sel) => {
            this.setState({
                canUseDb: sel.structures.every((s) =>
                    !!s.cell.obj?.data && s.cell.obj.data.models.some((m) => SIFTSMapping.Provider.isApplicable(m))
                ),
            });
        });
    }
    ...
    async transform(
        ...
        const b = o ? this.plugin.state.data.build().to(o).update(params) : this.plugin.state.data.build().to(s)
    ...
    superposeAtoms = async () => {
        ...
        for (let i = 1, il = atomLocis.length; i < il; ++i) {
            ...
            this.plugin.log.info(
                `Superposed ${count} ${count === 1 ? 'atom' : 'atoms'} of [${labelA}] and [${labelB}] with RMSD ${
                    rmsd.toFixed(2)
                }.`,
            );
        }
    ...
    superposeDb = async () => {
        ...
        const formatPairs = (pairs: [number, number][]) => {
            return `[${
                pairs.map(([i, j]) => `(${structures[i].models[0].entryId}, ${structures[j].models[0].entryId})`).join(
                    ', ',
                )
            }]`;
        };
                ...
                {entries.length < 2 && (
                    <div className='msp-control-offset msp-help-text'>
                        <div className='msp-help-description'>
                            <Icon svg={HelpOutlineSvg} inline />Add 2 or more selections{this.toggleHint()}{' '}
                            from separate structures. Selections must be limited to single polymer chains or residues
                            therein.
                        </div>
                    </div>
                )}
        ...
        return (
            <>
                ...
                {entries.length < 2 && (
                    <div className='msp-control-offset msp-help-text'>
                        <div className='msp-help-description'>
                            <Icon svg={HelpOutlineSvg} inline />Add 1 or more selections{this.toggleHint()}{' '}
