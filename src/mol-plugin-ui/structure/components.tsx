...
import { CollapsableControls, CollapsableState, PurePluginUIComponent } from '../base.tsx';
...
export class StructureComponentControls
    extends CollapsableControls<{}, StructureComponentControlState> {
...
class ComponentEditorControls extends PurePluginUIComponent<{}, ComponentEditorControlsState> {
    ...
    componentDidMount() {
        this.subscribe(
            ...
            (c) =>
                this.setState({
                    action: this.state.action !== 'options' || c.structures.length === 0
                        ? void 0
                        : 'options',
        ...
        this.subscribe(this.plugin.behaviors.state.isBusy, (v) => {
            this.setState({
                isBusy: v,
                action: this.state.action !== 'options' ? void 0 : 'options',
            });
        });
...
export class AddComponentControls
    extends PurePluginUIComponent<AddComponentControlsProps, AddComponentControlsState> {
    ...
    apply = () => {
        const structures = this.props.forSelection
...
class ComponentOptionsControls extends PurePluginUIComponent<{ isDisabled: boolean }> {
    componentDidMount() {
        this.subscribe(
            this.plugin.managers.structure.component.events.optionsUpdated,
            () => this.forceUpdate(),
        );
    }
        ...
        return (
            <div style={{ marginTop: '6px' }}>
                {componentGroups.map((g) => (
                    <StructureComponentGroup key={g[0].cell.transform.ref} group={g} />
                ))}
            </div>
        );
    }
...
class StructureComponentGroup extends PurePluginUIComponent<
    { group: StructureComponentRef[] },
    { action?: StructureComponentEntryActions }
> {
    ...
    get colorByActions() {
        ...
        return ActionMenu.createItemsFromSelectOptions(themes, {
            value: (o) => () =>
                mng.updateRepresentationsTheme(this.props.group, { color: o[0] as any }),
            selected: (o) => o[0] === name,
        }) as ActionMenu.Item[];
    }
    ...
    get actions(): ActionMenu.Items {
        ...
        const ret: ActionMenu.Items = [
            [
                ...
                ...StructureComponentManager.getRepresentationTypes(
                    this.plugin,
                    this.props.group[0],
                )
                    .map((t) =>
                        ActionMenu.Item(t[1], () => mng.addRepresentation(this.props.group, t[0]))
                    ),
            ],
        ];
        ...
        if (mng.canBeModified(this.props.group[0])) {
            ret.push([
                ...
                ActionMenu.Item(
                    'Include',
                    () => mng.modifyByCurrentSelection(this.props.group, 'union'),
                    { icon: UnionSvg },
                ),
                ActionMenu.Item(
                    'Subtract',
                    () => mng.modifyByCurrentSelection(this.props.group, 'subtract'),
                    { icon: SubtractSvg },
                ),
                ActionMenu.Item(
                    'Intersect',
                    () => mng.modifyByCurrentSelection(this.props.group, 'intersect'),
                    { icon: IntersectSvg },
                ),
            ]);
        }
        ...
        ret.push(
            ActionMenu.Item('Select This', () => mng.selectThis(this.props.group), {
                icon: SetSvg,
            }),
        );
    ...
    toggleAction = () =>
        this.setState({ action: this.state.action === 'action' ? void 0 : 'action' });
...
class StructureRepresentationEntry extends PurePluginUIComponent<
    { group: StructureComponentRef[]; representation: StructureRepresentationRef }
> {
    remove = () =>
        this.plugin.managers.structure.component.removeRepresentations(
            this.props.group,
    ...
    toggleVisible = (e: React.MouseEvent<HTMLElement>) => {
        e.preventDefault();
        e.currentTarget.blur();
        this.plugin.managers.structure.component.toggleVisibility(
            this.props.group,
            this.props.representation,
        );
    };
