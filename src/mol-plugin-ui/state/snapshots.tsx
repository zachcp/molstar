    ...
    return (
      <>
        ...
        <div className="msp-help-text" style={{ padding: "10px" }}>
          <Icon svg={WarningSvg} /> This is an experimental feature and stored
          states/sessions might not be openable in a future version.
        </div>
      </>
    );
  }
...
> {
  ...
  updateParams = (params: PD.Values<typeof LocalStateSnapshots.Params>) =>
...
function invalidateSnapshotKey(
  plugin: PluginContext,
  key: string | undefined,
  currentId?: string,
) {
  if (!key) return false;
  return plugin.managers.snapshot.state.entries.some(
    (e) => (!currentId || e.snapshot.id !== currentId) && e.key === key,
  );
}

function AddSnapshot({ parent }: { parent: LocalStateSnapshots }) {
  const [state, setState] =
    React.useState<PluginStateSnapshotManager.EntryParams>({
      key: "",
      name: "",
      description: "",
    });
  ...
  return (
    <>
      ...
      <div className="msp-flex-row">
        <IconButton
          onClick={parent.clear}
          svg={DeleteOutlinedSvg}
          title="Remove All"
        />
        <Button
          onClick={add}
          icon={keyExists ? undefined : AddSvg}
          style={{ textAlign: "right" }}
          commit={keyExists ? "off" : "on"}
          disabled={keyExists}
        >
          {keyExists ? "Key must be unique" : "Add"}
        </Button>
      </div>
    </>
  );
...
> {
  ...
  override render() {
    ...
    this.plugin.managers.snapshot.state.entries.forEach((e) => {
      items.push(
        <li key={e!.snapshot.id} className="msp-flex-row">
          <Button
      ...
      if (image) {
        items.push(
          <li key={`${e!.snapshot.id}-image`} className="msp-state-image-row">
...
function EditSnapshotParams({
  state,
  setState,
  apply,
}: {
  state: PluginStateSnapshotManager.EntryParams;
  setState: (s: PluginStateSnapshotManager.EntryParams) => any;
  apply: (s: PluginStateSnapshotManager.EntryParams) => any;
}) {
  ...
  return (
    <>
      ...
      <ControlRow
        label={
          <>
            ...
            <ToggleParamHelpButton
              show={showKeyHelp}
              toggle={() => setShowKeyHelp((prev) => !prev)}
            />
          </>
        }
        control={
      ...
      <div
        className="msp-flex-row msp-text-area-wrapper"
  ...
  return (
    <>
      ...
      <div className="msp-flex-row" style={{ marginBottom: 1 }}>
        <Button
          onClick={apply}
          icon={keyExists ? undefined : CheckSvg}
          style={{ textAlign: "right" }}
          commit={keyExists ? "off" : "on"}
          disabled={keyExists}
        >
          {keyExists ? "Key must be unique" : "Apply"}
        </Button>
      </div>
    </>
  );
...
> {
  Params = {
    ...
    options: PD.Group({
      ...
      serverUrl: PD.Text(
        this.plugin.config.get(PluginConfig.State.CurrentServer),
      ),
    }),
  };

  override state = {
    params: PD.getDefaultValues(this.Params),
    entries: OrderedMap<string, RemoteEntry>(),
    isBusy: false,
  };
  ListOnlyParams = {
  ...
  refresh = async () => {
    try {
      ...
      this.plugin.config.set(
        ...
        this.state.params.options.serverUrl,
      );

      const json =
        (await this.plugin.runTask<RemoteEntry[]>(
          this.plugin.fetch({ url: this.serverUrl("list"), type: "json" }),
        )) || [];
  ...
  fetch = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    this.setState({ isBusy: true });
    try {
      await PluginCommands.State.Snapshots.Fetch(this.plugin, {
        url: entry.url,
      });
    } finally {
        ...
        {!this.props.listOnly && (
          <>
            <ParameterControls
              ...
              onChange={(p) => {
                this.setState({
                  params: { ...this.state.params, [p.name]: p.value },
                } as any);
              }}
...
> {
  open = async (e: React.MouseEvent<HTMLElement>) => {
    ...
    globalThis.open(
      `${url}?snapshot-url=${encodeURIComponent(entry.url)}`,
      "_blank",
    );
  };
    ...
    return (
      ...
      >
        {this.props.entries.valueSeq().map((e) => (
          <li key={e!.id} className="msp-flex-row">
            <Button
              data-id={e!.id}
