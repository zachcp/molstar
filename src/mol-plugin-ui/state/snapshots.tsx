...
import { ParameterControls, ParamHelp, ToggleParamHelpButton } from '../controls/parameters.tsx';
        ...
        return (
            <>
                ...
                <div className='msp-help-text' style={{ padding: '10px' }}>
                    <Icon svg={WarningSvg} />{' '}
                    This is an experimental feature and stored states/sessions might not be openable in a future
                    version.
                </div>
            </>
        );
...
function AddSnapshot({ parent }: { parent: LocalStateSnapshots }) {
    const [state, setState] = React.useState<PluginStateSnapshotManager.EntryParams>({
        key: '',
        name: '',
        description: '',
    });
...
> {
    ...
    refresh = async () => {
        try {
            ...
            const json = (await this.plugin.runTask<RemoteEntry[]>(
