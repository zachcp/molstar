        ...
        return (
            <>
                ...
                <div className='msp-help-text' style={{ padding: '10px' }}>
                    <Icon svg={WarningSvg} />{' '}
                    This is an experimental feature and stored states/sessions might not be openable
                    in a future version.
                </div>
            </>
        );
...
function invalidateSnapshotKey(plugin: PluginContext, key: string | undefined, currentId?: string) {
    if (!key) return false;
    return plugin.managers.snapshot.state.entries.some((e) =>
        (!currentId || e.snapshot.id !== currentId) && e.key === key
    );
}
            ...
            items.push(
                <li key={e!.snapshot.id} className='msp-flex-row'>
                    ...
                    >
                        ...
                        <small>
                            {`${
                                e!.snapshot.durationInMs
                                    ? formatTimespan(e!.snapshot.durationInMs, false)
                                    : ''
                            }`}
                        </small>
                    </Button>
                ...
                {!this.props.listOnly && (
                    <>
                        <ParameterControls
                            ...
                            onChange={(p) => {
                                this.setState(
                                    { params: { ...this.state.params, [p.name]: p.value } } as any,
                                );
                            }}
