...
export { TransformUpdaterControl, UpdateTransformControl };
...
class UpdateTransformControl
    extends TransformControlBase<UpdateTransformControl.Props, UpdateTransformControl.ComponentState> {
    applyAction() {
    ...
    canAutoApply(newParams: any) {
        ...
        return autoUpdate(
            { a: parentCell.obj!, b: cell.obj!, oldParams: this.getInfo().initialValues, newParams },
            this.plugin,
        );
    }
    ...
    override componentDidMount() {
        ...
        this.subscribe(this.plugin.state.events.object.updated, ({ ref, state }) => {
            ...
            if (this.state.params !== this.props.transform.params) {
                this._getInfo = memoizeLatest((t: StateTransform) =>
                    StateTransformParameters.infoFromTransform(this.plugin, this.props.state, t)
                );
    ...
    private _getInfo = memoizeLatest((t: StateTransform) =>
        StateTransformParameters.infoFromTransform(this.plugin, this.props.state, t)
    );
    ...
    override componentDidUpdate(prevProps: UpdateTransformControl.Props) {
...
class TransformUpdaterControl extends PluginUIComponent<
    { nodeRef: string; initiallyCollapsed?: boolean; header?: StateTransformer.Definition['display'] }
> {
    override componentDidMount() {
        this.subscribe(this.plugin.state.events.object.updated, ({ ref, state }) => {
