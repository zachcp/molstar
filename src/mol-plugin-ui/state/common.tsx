...
namespace StateTransformParameters {
  ...
  export function infoFromAction(
    ...
    const params = action.definition.params
      ? action.definition.params(source, plugin)
      : {};
...
abstract class TransformControlBase<
  P,
  S extends TransformControlBase.ComponentState,
> extends PurePluginUIComponent<P & TransformControlBase.CommonProps, S> {
  ...
  renderApply() {
    ...
    if (
      this.props.autoHideApply &&
      (!canApply || this.canAutoApply(this.state.params))
    )
      return null;
  ...
  renderDefault() {
    ...
    const ParamEditor: StateTransformParameters.Class =
      this.plugin.customParamEditors.has(tId)
    ...
    const ctrl = (
      <div
        className={wrapClass}
        style={{ marginBottom: this.props.noMargin ? 0 : void 0 }}
      >
          ...
          <div className="msp-transform-header">
            <Button onClick={this.toggleExpanded} title={display.description}>
              {!isEmpty && (
                <Icon
                  svg={
                    this.state.isCollapsed ? ArrowRightSvg : ArrowDropDownSvg
                  }
                />
              )}
              {display.name}
