...
import { _Props, _State, PluginUIComponent } from '../base.tsx';
        ...
        if (this.state.showActions) {
            return (
                <div style={{ margin: '10px', cursor: 'default' }}>
                    ...
                    <p>
                        Structures and Volumes can be loaded from the <Icon svg={HomeOutlinedSvg} />
                        {' '}
                        tab.
                    </p>
                </div>
...
> {
    ...
    private static hasDecorator(cell: StateObjectCell) {
        ...
        return !!child &&
            !!cell.parent?.tree.transforms.get(child).transformer.definition.isDecorator;
    }
...
class StateTreeNodeLabel
    extends PluginUIComponent<{ cell: StateObjectCell; depth: number }, StateTreeNodeLabelState> {
    is(e: State.ObjectEvent) {
        return e.ref === this.ref && e.state === this.props.cell.parent;
    }
    ...
    componentDidMount() {
        this.subscribe(
            this.plugin.state.events.cell.stateUpdated.pipe(
                filter((e) => this.is(e)),
                debounceTime(33),
            ),
            (e) => {
        ...
        this.subscribe(this.props.cell.parent!.behaviors.currentObject, (e) => {
            if (!this.is(e)) {
                if (this.state.isCurrent && e.state.transforms.has(this.ref)) {
                    this._setCurrent(
                        this.props.cell.parent!.current === this.ref,
            ...
            if (e.state.transforms.has(this.ref)) {
                this._setCurrent(
                    this.props.cell.parent!.current === this.ref,
                    !!this.props.cell.state.isCollapsed,
                );
            }
        });
    }
    ...
    get actions() {
        ...
        return [
            ...
            ...actions.map((a) =>
                ActionMenu.Item(
                    a.definition.display.name,
                    () => this.setState({ action: 'apply', currentAction: a }),
                )
            ),
        ];
    }
    ...
    render() {
        ...
        if (cell.status === 'error' || !cell.obj) {
            const name = cell.status === 'error'
                ? cell.errorText
                : n.transformer.definition.display.name;
            label = (
                <Button
            ...
            label = (
                ...
                >
                    <span>{obj.label}</span>{' '}
                    {obj.description ? <small>{obj.description}</small> : void 0}
                </Button>
            );
        }
