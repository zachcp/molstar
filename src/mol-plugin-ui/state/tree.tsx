...
import { type _Props, type _State, PluginUIComponent } from '../base.tsx';
        ...
        if (this.state.showActions) {
            return (
                <div style={{ margin: '10px', cursor: 'default' }}>
                    ...
                    <p>
                        Structures and Volumes can be loaded from the <Icon svg={HomeOutlinedSvg} /> tab.
                    </p>
                </div>
            );
        }
...
class StateTreeNode extends PluginUIComponent<
    ...
    { isCollapsed: boolean; isNull: boolean; showLabel: boolean }
> {
    ...
    override componentDidMount() {
        this.subscribe(this.plugin.state.events.cell.stateUpdated, (e) => {
            if (this.props.cell === e.cell && this.is(e) && e.state.cells.has(this.ref)) {
                if (
                    this.state.isCollapsed !== !!e.cell.state.isCollapsed ||
    ...
    static getDerivedStateFromProps(
        props: _Props<StateTreeNode>,
        state: _State<StateTreeNode>,
    ): _State<StateTreeNode> | null {
        const isNull = StateTreeNode.isNull(props.cell);
        const showLabel = StateTreeNode.showLabel(props.cell);
        if (
            !!props.cell.state.isCollapsed === state.isCollapsed && state.isNull === isNull &&
    ...
    private static isNull(cell?: StateObjectCell) {
        return !cell || !cell.parent || cell.obj === StateObject.Null ||
            !cell.parent.tree.transforms.has(cell.transform.ref);
    }

    private static showLabel(cell: StateObjectCell) {
        return (cell.transform.ref !== StateTransform.RootRef) &&
            (cell.status !== 'ok' || (!cell.state.isGhost && !StateTreeNode.hasDecorator(cell)));
    }
    ...
    override render() {
        ...
        if (!this.state.showLabel) {
            ...
            return (
                <>
                    {children.map((c) => (
                        <StateTreeNode cell={cell.parent!.cells.get(c!)!} key={c} depth={this.props.depth} />
                    ))}
                </>
            );
        }
            ...
            <>
                ...
                {children.size === 0 ? void 0 : (
                    <div style={{ display: this.state.isCollapsed ? 'none' : 'block' }}>
                        {children.map((c) => (
                            <StateTreeNode cell={cell.parent!.cells.get(c!)!} key={c} depth={newDepth} />
                        ))}
                    </div>
                )}
            </>
...
class StateTreeNodeLabel extends PluginUIComponent<{ cell: StateObjectCell; depth: number }, StateTreeNodeLabelState> {
    ...
    override componentDidMount() {
        this.subscribe(
            this.plugin.state.events.cell.stateUpdated.pipe(filter((e) => this.is(e)), debounceTime(33)),
            (e) => {
                this.forceUpdate();
            },
        );
    ...
    override state: StateTreeNodeLabelState = {
    ...
    static getDerivedStateFromProps(
        props: _Props<StateTreeNodeLabel>,
        state: _State<StateTreeNodeLabel>,
    ): _State<StateTreeNodeLabel> | null {
    ...
    setCurrentRoot = (e?: React.MouseEvent<HTMLElement>) => {
        ...
        PluginCommands.State.SetCurrentObject(this.plugin, {
    ...
    get actions() {
        ...
        actions.sort((a, b) =>
            a.definition.display.name < b.definition.display.name
                ? -1
                : a.definition.display.name === b.definition.display.name
                ? 0
                : 1
        ...
        return [
            ...
            ...actions.map((a) =>
                ActionMenu.Item(a.definition.display.name, () => this.setState({ action: 'apply', currentAction: a }))
            ),
        ];
    }
        ...
        for (let i = decoratorChain.length - 1; i >= 0; i--) {
            ...
            decorators!.push(
                <UpdateTransformControl
                    key={`${d.transform.transformer.id}-${i}`}
                    state={cell.parent!}
    ...
    override render() {
        ...
        const expand = (
            <IconButton
                ...
                style={{ visibility: children.size > 0 ? 'visible' : 'hidden' }}
            />
        );
        const remove = !cell.state.isLocked
            ? <IconButton svg={DeleteOutlinedSvg} onClick={this.remove} disabled={disabled} small toggleState={false} />
            : void 0;
        const visibility = (
            <IconButton
        ...
        if (this.state.action === 'options') {
            ...
            return (
                <div style={{ marginBottom: '1px' }}>
                    ...
                    {updates}
                    {actions && (
                        <div style={{ marginLeft: `${this.props.depth * 8 + 21}px`, marginTop: '-1px' }}>
