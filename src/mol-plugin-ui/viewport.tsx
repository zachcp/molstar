...
export class ViewportControls
    extends PluginUIComponent<ViewportControlsProps, ViewportControlsState> {
    ...
    toggleControls = () => {
        PluginCommands.Layout.Update(this.plugin, {
            state: { showControls: !this.plugin.layout.state.showControls },
        });
    };

    toggleExpanded = () => {
        PluginCommands.Layout.Update(this.plugin, {
            state: { isExpanded: !this.plugin.layout.state.isExpanded },
        });
    };
    ...
    componentDidMount() {
        ...
        if (this.plugin.canvas3d) {
            this.subscribe(
                ...
                (snapshot) =>
                    this.enableCameraReset(snapshot.radius !== 0 && snapshot.radiusMax !== 0),
            );
        }
    }
        ...
        return (
            <div className='msp-viewport-controls'>
                <div className='msp-viewport-controls-buttons'>
                            ...
                            <div className='msp-flex-column'>
                                <div className='msp-flex-row'>
                                    <Button
                                        ...
                                        disabled={!this.state.isCameraResetEnabled}
                                        title='Set camera zoom to fit the visible scene into view'
                                    >
                                        Reset Zoom
                                    </Button>
                                </div>
                                <div className='msp-flex-row'>
                                    <Button
                                        onClick={() =>
                                            PluginCommands.Camera.OrientAxes(this.plugin)}
                                        disabled={!this.state.isCameraResetEnabled}
                                        title='Align principal component axes of the loaded structures to the screen axes (“lay flat”)'
                                    >
                                        Orient Axes
                                    </Button>
                                </div>
                                <div className='msp-flex-row'>
                                    <Button
                                        onClick={() => PluginCommands.Camera.ResetAxes(this.plugin)}
                                        disabled={!this.state.isCameraResetEnabled}
                                        title='Align Cartesian axes to the screen axes'
                                    >
                                        Reset Axes
                                    </Button>
                                </div>
                    ...
                    {this.plugin.config.get(PluginConfig.Viewport.ShowScreenshotControls) && (
                        <div>
                            <div className='msp-semi-transparent-background' />
                            {this.icon(
                                CameraOutlinedSvg,
                    ...
                    <div>
                        <div className='msp-semi-transparent-background' />
                        {this.plugin.config.get(PluginConfig.Viewport.ShowControls) &&
                            this.icon(
                                BuildOutlinedSvg,
                                this.toggleControls,
                        ...
                        {this.plugin.config.get(PluginConfig.Viewport.ShowSettings) &&
                            this.icon(
                                TuneSvg,
...
export const Logo = () => <a className='msp-logo' href='https://molstar.org' target='_blank' />;
