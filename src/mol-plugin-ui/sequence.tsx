...
import { HelpOutlineSvg, Icon } from './controls/icons.tsx';
...
const SequenceViewModeParam = PD.Select<SequenceViewMode>('single', [['single', 'Chain'], [
    'polymers',
    'Polymers',
], ['all', 'Everything']]);
...
export class SequenceView
    extends PluginUIComponent<{ defaultMode?: SequenceViewMode }, SequenceViewState> {
    ...
    componentDidMount() {
        ...
        this.subscribe(this.plugin.state.events.object.updated, ({ ref, obj }) => {
            if (
                ref === this.state.structureRef && obj &&
                obj.type === PSO.Molecule.Structure.type && obj.data !== this.state.structure
            ) {
                this.sync();
            }
        });
    ...
    private getSequenceWrapper(params: SequenceView['params']) {
        return {
            ...
            label: `${PD.optionLabel(params.chain, this.state.chainGroupId)} | ${
    ...
    private getSequenceWrappers(params: SequenceView['params']) {
        ...
        for (
            const [modelEntityId, eLabel] of getModelEntityOptions(
                structure,
            ...
            for (const [chainGroupId, cLabel] of getChainOptions(structure, modelEntityId)) {
                for (
                    const [operatorKey] of getOperatorOptions(
                        structure,
                        modelEntityId,
                        chainGroupId,
                    )
                ...
                <div className='msp-sequence-select'>
                    ...
                    {params.operator.options.length > 1 && (
                        <>
                            <PureSelectControl
                                title={`[Instance] ${
                                    PD.optionLabel(params.operator, values.operator)
                                }`}
...
function NonEmptySequenceWrapper({ children }: { children: React.ReactNode }) {
    return (
        <div className='msp-sequence-wrapper-non-empty'>
            {children}
        </div>
    );
}
