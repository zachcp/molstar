...
export class DownloadScreenshotControls
    extends PluginUIComponent<{ close: () => void }, ImageControlsState> {
        ...
        return (
            <div>
                ...
                <div className='msp-flex-row'>
                    ...
                    {this.state.imageData && (
                        <Button
                            onClick={() => this.setState({ imageData: void 0 })}
                            disabled={this.state.isDisabled}
                        >
                            Clear
                        </Button>
                ...
                {this.state.imageData && (
                    <div className='msp-row msp-copy-image-wrapper'>
...
function CameraInfoSection({ title, children }: { title: string; children: any }): JSX.Element {
    return (
        <div className='msp-control-row'>
            ...
            <div
                className='msp-control-row-text'
                style={{ fontSize: '0.85rem', overflow: 'hidden', whiteSpace: 'nowrap' }}
            >
                {children}
            </div>
        </div>
...
function CameraInfo({ plugin }: { plugin: PluginContext }) {
    ...
    const direction = Vec3.sub(
        Vec3(),
        state?.target ?? Vec3.origin,
        state?.position ?? Vec3.origin,
    );
    ...
    return (
        <div>
            ...
            <CameraInfoSection title='Distance'>
                {round(
                    Vec3.distance(state?.position ?? Vec3.origin, state?.target ?? Vec3.origin),
                    2,
                )}
            </CameraInfoSection>
    ...
    up: [${state?.up.map((v) => round(v, 2)).join(', ')}],
                ...
                style={{ marginTop: 1 }}
                title='Copy JSON usable in MolViewSpec, uses FoV Normalized Position'
            >
                Copy MVS JSON
            </Button>
        </div>
...
function ScreenshotParams({ plugin, isDisabled }: { plugin: PluginContext; isDisabled: boolean }) {
    ...
    return (
        <ParameterControls
            ...
            values={values}
            onChangeValues={(v) => helper.behaviors.values.next(v)}
...
function CropControls({ plugin }: { plugin: PluginContext }) {
    ...
    return (
        <div style={{ width: '100%', height: '24px', marginTop: '8px' }}>
            ...
            {!cropParams.auto && !helper.isFullFrame && (
                <Button
                    icon={CropFreeSvg}
                    title='Reset Crop'
                    style={{
