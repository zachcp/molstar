...
import { type State, StateSelection, type StateTransformer } from '../../mol-state/index.ts';
                    ...
                    return !Binding.isEmpty(binding)
                        ? (
                            <div key={name} style={{ marginBottom: '6px' }}>
                                <b>{binding.action}</b>
                                <br />
                                <span dangerouslySetInnerHTML={{ __html: Binding.format(binding, name) }} />
                            </div>
                        )
                        : null;
                })}
...
export class HelpGroup extends React.PureComponent<
    ...
    { isExpanded: boolean }
> {
    override state = { header: this.props.header, isExpanded: !!this.props.initiallyExpanded };
        ...
        return (
            <div>
                ...
                <HelpGroup header='Inline Help'>
                    <HelpText>
                        Many user interface elements show a little questionmark icon when hovered over. Clicking the
                        icon toggles the display of an inline help text.
                    </HelpText>
                    <HelpText>
                        Tooltips may provide additional information on a user interface element and are shown when
                        hovering over it with the mouse.
                    </HelpText>
                </HelpGroup>
                <HelpGroup header='Selections'>
                    <HelpText>
                        The viewer allows changing colors and representations for selections of atoms, residues or
                        chains. Selections can be created by
                        <ul style={{ paddingLeft: '20px' }}>
                            <li>
                                picking elements on the 3D canvas or the sequence view using the mouse, e.g. toggle
                                selection using {selectToggleTriggers} (for more see help section on{' '}
                                <i>Mouse Controls</i>)
                            </li>
                            <li>
                                using the <i>Add</i>, <i>Remove</i> and <i>Only</i> dropdown buttons in the{' '}
                                <i>Manage Selection</i>{' '}
                ...
                <HelpGroup header='Coloring'>
                    <HelpText>
                        There are two ways to color structures. Every representation (e.g. cartoon or spacefill) has a
                        color theme which can be changed using the dropdown for each representation in the{' '}
                        <i>Structure Settings</i>{' '}
                        panel. Additionally any selection atoms, residues or chains can by given a custom color. For
                        that, first select the parts of the structure to be colored (see help section on{' '}
                        <i>Selections</i>) and, second, choose a color from the color dropdown botton in the{' '}
                        <i>Selection</i> row of the <i>Change Representation</i>{' '}
                        panel. The theme color can be seen as a base color that is overpainted by the custom color.
                        Custom colors can be removed for a selection with the 'Clear' option in the color dropdown.
                    </HelpText>
                </HelpGroup>
                <HelpGroup header='Representations'>
                    <HelpText>
                        Structures can be shown with many different representations (e.g. cartoon or spacefill). The
                        {' '}
                        <i>Change Representation</i>{' '}
                        panel offers a collection of predefined styles which can be applied using the <i>Preset</i>{' '}
                        dropdown button. Additionally any selection atoms, residues or chains can by shown with a custom
                        representation. For that, first select the parts of the structure to be mofified (see help
                        section on <i>Selections</i>) and, second, choose a representation to hide or show from the{' '}
                        <i>Show</i> and <i>Hide</i> dropdown bottons in the <i>Selection</i> row of the{' '}
                        <i>Change Representation</i> panel. The <i>Everything</i>{' '}
                        row applies the action to the whole structure instead of the current selection.
                    </HelpText>
                </HelpGroup>
                <HelpGroup header='Surroundings'>
                    <HelpText>
                        To show the surroundings of a residue or ligand, click it in the 3D scene or in the sequence
                        widget using {focusTriggers}.
                    </HelpText>
                </HelpGroup>
                ...
                <HelpGroup header='Create an Image'>
                    <HelpText>
                        <p>
                            Use the <Icon svg={CameraSvg} /> icon in the viewport to bring up the screenshot controls.
                        </p>
                        <p>
