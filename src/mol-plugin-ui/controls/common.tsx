...
import { AddSvg, ArrowDropDownSvg, ArrowRightSvg, Icon, RemoveSvg } from './icons.tsx';
...
export class ControlGroup extends React.Component<{
    ...
    render() {
        let groupClassName = this.props.hideOffset
            ? 'msp-control-group-children'
            : 'msp-control-group-children msp-control-offset';
        ...
        return (
            <div
                ...
                style={{ position: 'relative', marginTop: this.props.noTopMargin ? 0 : void 0 }}
            >
                <div
                    className='msp-control-group-header'
                    style={{ marginLeft: this.props.headerLeftMargin }}
                    title={this.props.title}
                >
                    <Button onClick={this.headerClicked}>
                        {!this.props.hideExpander && (
                            <Icon svg={this.state.isExpanded ? ArrowRightSvg : ArrowDropDownSvg} />
                        )}
...
export class TextInput<T = string> extends React.PureComponent<TextInputProps<T>, TextInputState> {
    ...
    onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const isInvalid = (this.props.isValid && !this.props.isValid(value)) ||
            (this.props.numeric && Number.isNaN(+value));
        if (isInvalid) {
            this.clearTimeout();
            this.setState({ value });
            return;
    ...
    render() {
        return (
            <input
                ...
                onKeyPress={this.props.onEnter || this.props.blurOnEnter || this.props.blurOnEscape
                    ? this.onKeyPress
                    : void 0}
                ...
                >
                    {this.props.colorStripe && (
                        <div
                            ...
                            style={{ backgroundColor: Color.toStyle(this.props.colorStripe) }}
                        />
                    )}
                </ControlRow>
                {this.state.isExpanded && (
...
export function SectionHeader(
    props: { icon?: React.FC; title: string | JSX.Element; desc?: string; accent?: ColorAccent },
) {
    return (
        <div
            className={`msp-section-header${
                props.accent ? ' msp-transform-header-brand-' + props.accent : ''
            }`}
        >
...
export function IconButton(props: {
    ...
    let className = `msp-btn msp-btn-icon${props.small ? '-small' : ''}${
...
export class ToggleButton extends React.PureComponent<ToggleButtonProps> {
    ...
    render() {
        ...
        const className = props.isSelected
            ? `${props.className || ''} msp-control-current`
            : props.className;
        return (
            <Button
    ...
    render() {
        return (
            <>
                ...
                {this.state.isExpanded &&
                    (this.props.noOffset ? this.props.children : (
                        <div
                            className={this.props.accent
                                ? 'msp-accent-offset'
                                : 'msp-control-offset'}
                        >
                            {this.props.children}
