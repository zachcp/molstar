                ...
                >
                    ...
                    {snapshots.state.entries.valueSeq().map((e, i) => (
                        <option key={e!.snapshot.id} value={e!.snapshot.id}>
                            {`[${i! + 1}/${count}]`} {e!.name || new Date(e!.timestamp).toLocaleString()}
                        </option>
                    ))}
                </select>
                <IconButton
                    ...
                    <>
                        <div className='msp-state-snapshot-animation-slider msp-form-control'>
                            <Slider
                                ...
                                max={PluginState.getStateTransitionDuration(entry?.snapshot) ??
                                    1000}
                                onChange={() => {}}
...
export function ViewportSnapshotDescription() {
    ...
    return (
        <div id='snapinfo' className='msp-snapshot-description-wrapper'>
            {(e.descriptionFormat === 'plaintext' && e.description) || <Markdown>{e.description}</Markdown>}
        </div>
    );
}
            ...
            <div className='msp-animation-viewport-controls'>
                <div>
                    ...
                    <IconButton
                        ...
                        disabled={isAnimating || isPlaying ? false : this.state.isBusy ||
