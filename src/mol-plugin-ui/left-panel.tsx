...
export class LeftPanelControls extends PluginUIComponent<{}, { tab: LeftPanelTabName }> {
        ...
        this.subscribe(this.plugin.behaviors.layout.leftPanelTabName, (tab) => {
            ...
            if (tab === 'none' && this.plugin.layout.state.regionState.left !== 'collapsed') {
                PluginCommands.Layout.Update(this.plugin, {
                    state: {
                        regionState: { ...this.plugin.layout.state.regionState, left: 'collapsed' },
                    },
                });
            }
        });
    ...
    set = (tab: LeftPanelTabName) => {
        ...
        if (this.plugin.layout.state.regionState.left !== 'full') {
            PluginCommands.Layout.Update(this.plugin, {
                state: { regionState: { ...this.plugin.layout.state.regionState, left: 'full' } },
            });
        }
    };
...
class DataIcon
    extends PluginUIComponent<{ set: (tab: LeftPanelTabName) => void }, { changed: boolean }> {
    state = { changed: false };
    ...
    render() {
        return (
            <IconButton
                ...
                extraContent={this.state.changed
