...
import { createRenderable, type GraphicsRenderObject } from './render-object.ts';
...
namespace Scene {
    ...
    export function create(
        ctx: WebGLContext,
        transparency: Transparency = 'blended',
        globals: GlobalDefines = { dColorMarker: true, dLightCount: 1 },
    ): Scene {
        ...
        function calculateOpacityAverage() {
            ...
            for (let i = 0, il = primitives.length; i < il; ++i) {
                ...
                const xray =
                    (p.values.dXrayShaded?.ref.value === 'on' || p.values.dXrayShaded?.ref.value === 'inverted')
                        ? 0.5
                        : 1;
