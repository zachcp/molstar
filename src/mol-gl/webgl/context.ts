...
import { checkFramebufferStatus, createNullFramebuffer, Framebuffer } from './framebuffer.ts';
import { Scheduler } from '../../mol-task/index.ts';
import { isDebugMode } from '../../mol-util/debug.ts';
import { createExtensions, resetExtensions, WebGLExtensions } from './extensions.ts';
import { createState, WebGLState } from './state.ts';
import { createResources, WebGLResources } from './resources.ts';
import { createRenderTarget, RenderTarget } from './render-target.ts';
import { Subject } from 'rxjs';
import { now } from '../../mol-util/now.ts';
import { createNullTexture, Texture, TextureFilter } from './texture.ts';
import { ComputeRenderable } from '../renderable.ts';
import { createTimer, WebGLTimer } from './timer.ts';

export function getGLContext(
    canvas: HTMLCanvasElement,
    attribs?: WebGLContextAttributes & { preferWebGl1?: boolean },
    ...
    const gl = (attribs?.preferWebGl1 ? null : get('webgl2')) || get('webgl') ||
...
export function checkError(gl: GLRenderingContext, message?: string) {
    ...
    if (error !== gl.NO_ERROR) {
        throw new Error(
            `WebGL error: '${getErrorDescription(gl, error)}'${message ? ` (${message})` : ''}`,
        );
    }
}
...
function getShaderPrecisionFormat(
    ...
    const glPrecisionType = gl[
        `${precision.toUpperCase()}_${type.toUpperCase()}` as
            | 'LOW_FLOAT'
            | 'MEDIUM_FLOAT'
            | 'HIGH_FLOAT'
            | 'LOW_INT'
...
export function createContext(
    gl: GLRenderingContext,
    props: Partial<{ pixelScale: number }> = {},
): WebGLContext {
    ...
    const parameters = {
        ...
        maxDrawBuffers: extensions.drawBuffers
            ? gl.getParameter(extensions.drawBuffers.MAX_DRAW_BUFFERS) as number
            : 0,
