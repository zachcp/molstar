...
import { createState, type WebGLState } from "./state.ts";
import { createResources, type WebGLResources } from "./resources.ts";
import { createRenderTarget, type RenderTarget } from "./render-target.ts";
...
export function getGLContext(
  ...
  const gl =
    (attribs?.preferWebGl1 ? null : get("webgl2")) ||
    get("webgl") ||
        ...
        try {
          ...
          xr.layer = new XRWebGLLayer(xr.session, gl, {
            ...
            framebufferScaleFactor:
              pixelScale * (options?.resolutionScale ?? 1),
          });
