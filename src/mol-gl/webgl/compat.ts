...
export function getInstancedArrays(
  gl: GLRenderingContext,
): COMPAT_instanced_arrays | null {
  if (isWebGL2(gl)) {
    return {
...
export function getStandardDerivatives(
  gl: GLRenderingContext,
): COMPAT_standard_derivatives | null {
  if (isWebGL2(gl)) {
    return {
...
export function getElementIndexUint(
  gl: GLRenderingContext,
): COMPAT_element_index_uint | null {
...
export function getVertexArrayObject(
  gl: GLRenderingContext,
): COMPAT_vertex_array_object | null {
  if (isWebGL2(gl)) {
    return {
    ...
    return {
      ...
      isVertexArray: (value: any): value is WebGLVertexArrayObject =>
        ext.isVertexArrayOES(value) === true,
    };
  }
}
...
export function getTextureFloat(
  gl: GLRenderingContext,
): COMPAT_texture_float | null {
...
export function getTextureFloatLinear(
  gl: GLRenderingContext,
): COMPAT_texture_float_linear | null {
...
export function getTextureHalfFloat(
  gl: GLRenderingContext,
): COMPAT_texture_half_float | null {
  if (isWebGL2(gl)) {
...
export function getBlendMinMax(
  gl: GLRenderingContext,
): COMPAT_blend_minmax | null {
  if (isWebGL2(gl)) {
...
export function getColorBufferFloat(
  gl: GLRenderingContext,
): COMPAT_color_buffer_float | null {
  if (isWebGL2(gl)) {
...
export function getDrawBuffers(
  gl: GLRenderingContext,
): COMPAT_draw_buffers | null {
  if (isWebGL2(gl)) {
    return {
...
export function getDrawBuffersIndexed(
  gl: GLRenderingContext,
): COMPAT_draw_buffers_indexed | null {
...
export function getShaderTextureLod(
  gl: GLRenderingContext,
): COMPAT_shader_texture_lod | null {
...
export function getDisjointTimerQuery(
  gl: GLRenderingContext,
): COMPAT_disjoint_timer_query | null {
  if (isWebGL2(gl)) {
    ...
    return {
      ...
      queryCounter: ext.queryCounterEXT.bind(ext),
      getQuery: (target: number, pname: number) =>
        ext.getQueryEXT(target, pname) ?? 0,
  ...
  if (ext) {
    return {
      drawArraysInstancedBaseInstance:
        ext.drawArraysInstancedBaseInstanceWEBGL.bind(ext),
      drawElementsInstancedBaseVertexBaseInstance:
        ext.drawElementsInstancedBaseVertexBaseInstanceWEBGL.bind(ext),
    };
  } else {
    return null;
  }
  ...
  if (ext) {
    return {
      multiDrawArraysInstancedBaseInstance:
        ext.multiDrawArraysInstancedBaseInstanceWEBGL.bind(ext),
      multiDrawElementsInstancedBaseVertexBaseInstance:
        ext.multiDrawElementsInstancedBaseVertexBaseInstanceWEBGL.bind(ext),
    };
  } else {
    return null;
  }
...
export function getFboRenderMipmap(
  gl: GLRenderingContext,
): COMPAT_fboRenderMipmap | null {
...
export function getProvokingVertex(
  gl: GLRenderingContext,
): COMPAT_provoking_vertex | null {
  if (isWebGL2(gl)) {
...
export function getClipCullDistance(
  gl: GLRenderingContext,
): COMPAT_clip_cull_distance | null {
  if (isWebGL2(gl)) {
...
export function getConservativeDepth(
  gl: GLRenderingContext,
): COMPAT_conservative_depth | null {
  if (isWebGL2(gl)) {
...
export function getStencilTexturing(
  gl: GLRenderingContext,
): COMPAT_stencil_texturing | null {
  if (isWebGL2(gl)) {
...
export function getClipControl(
  gl: GLRenderingContext,
): COMPAT_clip_control | null {
...
export function getRenderSnorm(
  gl: GLRenderingContext,
): COMPAT_render_snorm | null {
  if (isWebGL2(gl)) {
...
export function getTextureNorm16(
  gl: GLRenderingContext,
): COMPAT_texture_norm16 | null {
...
export function getDepthClamp(
  gl: GLRenderingContext,
): COMPAT_depth_clamp | null {
...
export function getMultiview2(
  gl: GLRenderingContext,
): COMPAT_multiview2 | null {
  if (isWebGL2(gl)) {
    ...
    if (ext) {
      return {
        ...
        FRAMEBUFFER_INCOMPLETE_VIEW_TARGETS:
          ext.FRAMEBUFFER_INCOMPLETE_VIEW_TARGETS_OVR,

        framebufferTextureMultiview:
          ext.framebufferTextureMultiviewOVR.bind(ext),
      };
    }
  }
  return null;
...
export function getNoNonInstancedActiveAttribs(
  gl: GLRenderingContext,
): boolean {
...
export function testColorBuffer(gl: GLRenderingContext, type: number) {
  // setup shaders
  const vertShader = getShader(gl, {
    type: "vert",
    source: TextureTestVertShader,
  });
  const fragShader = getShader(gl, {
    type: "frag",
  ...
  if (type === gl.FLOAT) {
    ...
    if (error) {
      if (isDebugMode) {
        console.log(
          `error reading float pixels: '${getErrorDescription(gl, error)}'`,
        );
      }
      return false;
    }
