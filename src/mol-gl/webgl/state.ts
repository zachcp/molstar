...
export function createState(gl: GLRenderingContext, e: WebGLExtensions): WebGLState {
    ...
    return {
        ...
        colorMask: (red: boolean, green: boolean, blue: boolean, alpha: boolean) => {
            if (
                red !== currentColorMask[0] || green !== currentColorMask[1] ||
                blue !== currentColorMask[2] || alpha !== currentColorMask[3]
            ) {
        ...
        clearColor: (red: number, green: number, blue: number, alpha: number) => {
            if (
                red !== currentClearColor[0] || green !== currentClearColor[1] ||
                blue !== currentClearColor[2] || alpha !== currentClearColor[3]
            ) {
        ...
        blendFunc: (src: number, dst: number) => {
            if (
                src !== currentBlendSrcRGB || dst !== currentBlendDstRGB ||
                src !== currentBlendSrcAlpha || dst !== currentBlendDstAlpha
            ) {
        ...
        blendFuncSeparate: (srcRGB: number, dstRGB: number, srcAlpha: number, dstAlpha: number) => {
            if (
                srcRGB !== currentBlendSrcRGB || dstRGB !== currentBlendDstRGB ||
                srcAlpha !== currentBlendSrcAlpha || dstAlpha !== currentBlendDstAlpha
            ) {
        ...
        blendColor: (red: number, green: number, blue: number, alpha: number) => {
            if (
                red !== currentBlendColor[0] || green !== currentBlendColor[1] ||
                blue !== currentBlendColor[2] || alpha !== currentBlendColor[3]
            ) {
        ...
        stencilFunc: (func: number, ref: number, mask: number) => {
            if (
                func !== currentStencilFunc || ref !== currentStencilRef ||
                mask !== currentStencilValueMask || func !== currentStencilBackFunc ||
                ref !== currentStencilBackRef || mask !== currentStencilBackValueMask
            ) {
        ...
        stencilOp: (fail: number, zfail: number, zpass: number) => {
            if (
                ...
                zfail !== currentStencilBackPassDepthFail ||
        ...
        viewport: (x: number, y: number, width: number, height: number) => {
            if (
                x !== currentViewport[0] || y !== currentViewport[1] ||
                width !== currentViewport[2] || height !== currentViewport[3]
            ) {
        ...
        reset: () => {
            ...
            currentClipDepthMode = e.clipControl
                ? gl.getParameter(e.clipControl.CLIP_DEPTH_MODE)
                : -1;
        },
    };
}
