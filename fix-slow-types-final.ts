        ...
        if (pathMatch) {
          ...
          for (let j = i + 2; j < Math.min(i + 10, lines.length); j++) {
            ...
            if (codeMatch) {
              ...
              const nameMatch = context.match(/(?:export\s+)?(?:get|set|async|function)?\s*(\w+)\s*\(/);
              if (nameMatch) {
...
function inferReturnType(lines: string[], targetLineIndex: number): string | null {
  ...
  for (let i = targetLineIndex; i < lines.length && i < targetLineIndex + 100; i++) {
    const line = lines[i];
  ...
  if (firstReturn === 'undefined' || firstReturn === '') return 'void';
  ...
  if (firstReturn.match(/^Quat\./)) return 'Quat';
...
async function addReturnType(
  ...
  try {
    ...
    // Pattern 4: export function
    else if (targetLine.match(/^\s*export\s+function\s+\w+\s*\([^)]*\)\s*\{/)) {
      newLine = targetLine.replace(/(\([^)]*\))\s*\{/, `$1: ${returnType} {`);
      modified = true;
    }
...
async function main() {
  ...
  for (const [file, fileErrors] of grouped) {
    ...
    for (const error of fileErrors) {
      if (fixed + skipped >= maxFixes) break;

      const contextLines = lines.slice(
        Math.max(0, error.line - 15),
        Math.min(lines.length, error.line + 5)
      );
      const targetLineIndex = Math.min(14, error.line - 1 - Math.max(0, error.line - 15));
